...
`v8::Local<v8::Value>`, then V8 will always pass the receiver (the `this` value
of the JavaScript function call) in the first position, meaning that the actual
arguments will be shifted one position to the right:
...
The majority of function bindings in the Node.js API do not need to access the
receiver, as they are intended to be called as a standalone function, not as a
method on an object or prototype.

However, this still leads to an important caveat if your fast callback takes an
argument of type `v8::Local<v8::Value>` in the first position. Since V8 will
always interpret this argument as the receiver (even if the receiver is `null`
...
### Appending an `options` argument

Fast callbacks may add an optional final function argument of type
`const v8::FastApiCallbackOptions&`. This is required if the callback interacts
with the isolate in any way: see
...
```cpp
void FastThrowExample(const int32_t n,
                      const v8::FastApiCallbackOptions& options) {
  if (IsEvilNumber(n)) {
...
```cpp
int32_t FastFuncWithoutArg() {
  return -1;
}
int32_t FastFuncWithArg(const v8::FastOneByteString& s) {
  return s.length;
}
...
final argument of type `const v8::FastApiCallbackOptions&`. V8 will pass the
isolate pointer in `options.isolate`.
...
in a JavaScript function, then use V8 natives to force the call to that wrapper
