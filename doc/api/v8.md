...
### Class: `v8.MemoryProfileWatchdog`

<!-- YAML
added: REPLACEME
-->

#### `new MemoryProfileWatchdog([options])`

* `interval` {number} How often(milliseconds) the watchdog polls.
* `maxRss` {number} Node.js will take snapshot when rss is
  greater than or equal to `maxRss` bytes.
* `maxUsedHeapSize` {number} Node.js will take snapshot when used heap size is
  greater than or equal to `maxUsedHeapSize` bytes.
* `filename` {string} Node.js will write heap snapshot to `filename`.

Creates a new `MemoryProfileWatchdog` object, `maxRss` or `maxUsedHeapSize` must
be setted. `MemoryProfileWatchdog` will create a thread and poll the memory
info periodically, when the rss or used heap size is greater than or equal to
the value you set, Node.js will write a heap snapshot to `filename` then stop
the watchdog.

Writing a heap snapshot is a blocking operation therefore this may have
significant impact to application responsivness. You should configured with
enough memory left on the machine, otherwise the watchdog might trigger OOM.

#### `memoryProfileWatchdog.stop()`
