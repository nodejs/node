...
This behavior can be observed by opening a resource then
...
therefore not safe to use it as a key in a `WeakMap` or add properties to it.
...
Using `executionAsyncResource()` in the top-level execution context
returns an empty object as there is no handle or request object to use,
...
const server = net.createServer((conn) => {
  ...
  // the callback itself exists because the server's .listen() method
  // was called. So the value returned would be the ID of the server.
  async_hooks.triggerAsyncId();
});
```
