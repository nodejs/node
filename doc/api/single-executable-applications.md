...
> Stability: 0 - Deprecated. Use `--build-sea` instead.
...
For example, with [postject][], users can inject the blob using the following arguments:

 * `hello` / `hello.exe`: The name of a copy of the `node` executable as the output.
 * `NODE_SEA_BLOB`: The name of the resource / note / section in the binary
   where the contents of the blob will be stored.
 * Resource path: `sea-prep.blob` - The name of the blob created in step 1.
...
This essentially translates to these commands:
