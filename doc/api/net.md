...
This function is asynchronous. Stops the server from accepting new connections
and keeps existing connections. The server closes when all connections are
ended. Then, the server emits the [`'close'`][] event.
The `'close'` event does not occur if the server was not open.
