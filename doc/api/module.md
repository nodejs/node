...
1. `module.registerHooks(options)`: takes synchronous hook functions that are
   run directly on the thread where the modules are loaded.
2. `module.register(specifier[, parentURL][, options])`: takes specifier to a
...
Alternatively,  `registerHooks()` can be called from the entry point. If the
entry point itself needs to load ESM and needs the hooks to be effective for
the loading process, that ESM should be loaded dynamically using `import()` after
...
// after the hooks are registered.
import('./my-app.mjs');
require('./my-app-2.cjs');
userRequire('./my-app-3.cjs');
```
...
node's default ← `hook1.resolve` ← `hook2.resolve`
...
These caveats do not apply to synchronous customization hooks. If they are too
complex to deal with, consider using the synchronous customization hooks.
