...
if (!isMainThread)
  skip('Worker bootstrapping works differently -> different timing');
...
setImmediate(mustCall(onfirstImmediate));
...
function onfirstImmediate() {
  ...
  strictEqual(as1.length, 1);
  // hook2 was not enabled yet .. it is enabled after hook3's "before" completed
  strictEqual(as2.length, 0);
  strictEqual(as3.length, 1);

  // Check that hook1 and hook3 captured the same Immediate and that it is valid
  const firstImmediate = as1[0];
  strictEqual(as3[0].uid, as1[0].uid);
  strictEqual(firstImmediate.type, 'Immediate');
  strictEqual(typeof firstImmediate.uid, 'number');
  strictEqual(typeof firstImmediate.triggerAsyncId, 'number');
  ...
  setImmediate(mustCall(onsecondImmediate));
}
...
function onsecondImmediate() {
  ...
  const as2 = hook2.activitiesOfTypes(types);
  const as3 = hook3.activitiesOfTypes(types);
  strictEqual(as1.length, 2);
  strictEqual(as2.length, 2);
  strictEqual(as3.length, 2);
  ...
  strictEqual(hook2Second.uid, hook3Second.uid);
  strictEqual(hook1Second.uid, hook3Second.uid);
  strictEqual(secondImmediate.type, 'Immediate');
  strictEqual(typeof secondImmediate.uid, 'number');
