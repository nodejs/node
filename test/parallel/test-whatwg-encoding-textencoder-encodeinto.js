...
function assertAllDestinationSizes(input) {
  const encoded = encoder.encode(input);
  const boundaries = [{ read: 0, written: 0 }];

  for (const character of input) {
    const previous = boundaries[boundaries.length - 1];
    boundaries.push({
      read: previous.read + character.length,
      written: previous.written + encoder.encode(character).length,
    });
  }

  let boundary = 0;
  for (let capacity = 0; capacity <= encoded.length + 1; capacity++) {
    while (boundary + 1 < boundaries.length &&
           boundaries[boundary + 1].written <= capacity) {
      boundary++;
    }
    ...
    assert.deepStrictEqual(
      ...
      boundaries[boundary]);
    assert.deepStrictEqual(
      destination.subarray(0, boundaries[boundary].written),
      encoded.subarray(0, boundaries[boundary].written));
  }
}
...
// Adapt the bounded-conversion coverage from simdutf#1035 to TextEncoder's
// observable read/written result and output bytes.
{
  let input = '';
  for (let i = 0; i < 1024; i++)
    input += String.fromCharCode(i & 0xff);
  assertAllDestinationSizes(input);
}

assertAllDestinationSizes(
  `${'A'.repeat(32)}\u00e9\u20ac\ud83d\ude00\ud800B\udc00C${
    'Z'.repeat(32)}`);

// 33 code units bypass the small-string path and exercise an exact-fit SIMD
// chunk containing only three-byte characters.
assertAllDestinationSizes('\u20ac'.repeat(33));
