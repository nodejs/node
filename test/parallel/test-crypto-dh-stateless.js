...
if (hasOpenSSL(4, 0)) {
  keyTypeMismatchCode =
    /^ERR_(CRYPTO_INCOMPATIBLE_KEY|OSSL_EVP_(OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE|INTERNAL_ERROR))$/;
} else if (hasOpenSSL(3)) {
  keyTypeMismatchCode =
    /^ERR_(CRYPTO_INCOMPATIBLE_KEY|OSSL_EVP_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE)$/;
} else {
  keyTypeMismatchCode =
    /^ERR_(CRYPTO_INCOMPATIBLE_KEY|OSSL_EVP_DIFFERENT_KEY_TYPES)$/;
}
...
function isOpenSSLCodeExpectation(value) {
  return String(value).includes('OSSL_');
}
...
function testDHError(options, expected) {
  ...
  const callback = common.mustCall((err) => {
    ...
    for (const [key, value] of Object.entries(expected)) {
      if (key === 'code' &&
          err.code === undefined &&
          isOpenSSLCodeExpectation(value)) {
