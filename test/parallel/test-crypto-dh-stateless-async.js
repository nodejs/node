...
if (isBoringSSL) {
  ...
  for (const [params1, params2] of list) {
    crypto.diffieHellman({
      ...
      assert.match(err.message, hasOpenSSL(3) ?
        /mismatching domain parameters/ : /different[ _]parameters/i);
      assert.match(err.code,
                   /^ERR_OSSL_.*(?:MISMATCHING_DOMAIN_PARAMETERS|DIFFERENT_PARAMETERS)$/);
    }));
  }
...

{
  ...
  crypto.diffieHellman({ publicKey, privateKey }, common.mustCall((err) => {
    ...
    assert.strictEqual(err.name, 'Error');
    if (isBoringSSL) {
      assert.strictEqual(err.code, 'ERR_OSSL_EVP_INVALID_PEER_KEY');
    } else {
      assert.match(err.message, hasOpenSSL(3) ?
        /failed during derivation/ : /Deriving bits failed/);
      assert.strictEqual(err.code, hasOpenSSL(3) ?
        'ERR_OSSL_FAILED_DURING_DERIVATION' : 'ERR_CRYPTO_OPERATION_FAILED');
    }
  }));
}
...
for (const { privateKey: alicePriv, publicKey: bobPub } of [
  ...
  for (const options of encodings) {
    crypto.diffieHellman(options, common.mustSucceed((buf) => {
      assert.deepStrictEqual(buf, expected);
    }));
  }
}

// Test C++ error conditions (delivered via the callback)
{
  ...
  crypto.diffieHellman({
    ...
    assert.strictEqual(err.code,
                       hasOpenSSL(3) ? 'ERR_OSSL_MISMATCHING_DOMAIN_PARAMETERS' :
