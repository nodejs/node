...
for (const [params1, params2] of list) {
  crypto.diffieHellman({
    ...
    assert.match(err.message, hasOpenSSL(3) ? /mismatching domain parameters/ : /different parameters/);
...

{
  ...
  crypto.diffieHellman({ publicKey, privateKey }, common.mustCall((err) => {
    assert.ok(err);
    assert.strictEqual(err.name, 'Error');
    assert.match(err.message, hasOpenSSL(3) ? /failed during derivation/ : /Deriving bits failed/);
    assert.strictEqual(err.code, hasOpenSSL(3) ? 'ERR_OSSL_FAILED_DURING_DERIVATION' : 'ERR_CRYPTO_OPERATION_FAILED');
  }));
}
