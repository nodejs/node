...
async function testDiffieHellman() {
  // Test cryptoPromises.diffieHellman
  await assert.rejects(() => diffieHellman(), {
    name: 'TypeError',
    code: 'ERR_INVALID_ARG_TYPE',
    message: 'The "options" argument must be of type object. Received undefined'
  });

  await assert.rejects(() => diffieHellman(null), {
    name: 'TypeError',
    code: 'ERR_INVALID_ARG_TYPE',
  ...
  async function test(
    { publicKey: alicePublicKey, privateKey: alicePrivateKey },
    { publicKey: bobPublicKey, privateKey: bobPrivateKey },
    expectedValue) {
    const buf1 = await diffieHellman({
      privateKey: alicePrivateKey,
      publicKey: bobPublicKey
    });
    const buf2 = await diffieHellman({
  ...
  await assert.rejects(() => diffieHellman({ privateKey: alicePrivateKey }), {
    name: 'TypeError',
    code: 'ERR_INVALID_ARG_TYPE',
    message: 'The "options.publicKey" property must be an instance of ' +
             'KeyObject or CryptoKey. Received undefined'
  });

  await assert.rejects(() => diffieHellman({ publicKey: alicePublicKey }), {
    name: 'TypeError',
    code: 'ERR_INVALID_ARG_TYPE',
    message: 'The "options.privateKey" property must be an instance of ' +
             'KeyObject or CryptoKey. Received undefined'
  });

  const privateKey = Buffer.from(
    '487CD880159D835FD0A8DBA9848898317283DB07E822741B344AD397BA84CDDD3920A515' +
    '88B891B03B3EBEF3C9F767D921FAC1294D4B5E09CABB6D1DE3EB4527989754FEB64D007E' +
    'BBDA2E6C8CE7A17EF41DE3C2DFE7CEAAF963199F55D5DBD9A415E77552FE69B7A41D8788' +
    '8B7D166BC569A3957B60EEA6A4ABEB1CDB7FFCF238DF961790791CD54E597B3082981D52' +
    'C0B2CA0B3DF212B2FD78DE4C6CC95285889D6DFDB746203177A726B912D26EB0A25F1187' +
    '1C7CD401A010B355EC41D9AA', 'hex');
  const publicKey = Buffer.from(
    '8b6ea8abccff18d4819b7ce280db7b480edc02b5016d3c4835af622d85a9e9bc6bbc22b0' +
    '0d0c0848ddfafd0530f275007bc691c8cb74a189fecbabd63f0e4e94ef932eb51e94c545' +
    '6800c4ce8628987d335466f4b16e1a04df21682d266eb3edf50b21802be3af58443c49da' +
    '40529f8f335a25134c1457787f60e7f0c481e32bb5c690354f68b4252936e2f4b61f9e63' +
    'c76e3185462d7e14f62c980a26f9da3837b2ff1b58e0aaa5d7464a7f8dcbc3a81d402dc6' +
    'f28a42f4ec55c6df68351ed9', 'hex');

  const group = getDiffieHellman('modp5');
  const dh = createDiffieHellman(group.getPrime(), group.getGenerator());
  ...
  await test(
    generateKeyPairSync('dh', { group: 'modp5' }),
    generateKeyPairSync('dh', { group: 'modp5' }));

  await test(
    generateKeyPairSync('dh', { group: 'modp5' }),
    generateKeyPairSync('dh', { prime: group.getPrime() }));

  // TODO(@jasnell): instead of rejecting this resolves, the underlying
  // implementation resolves an empty buffer.
  ...
  //     return test(
  //       generateKeyPairSync('dh', params1),
  //       generateKeyPairSync('dh', params2));
  //   }, {
  //     name: 'Error',
...

  {
    ...
    const secret = await diffieHellman({ publicKey, privateKey });
    assert.strictEqual(
      secret.toString('hex'),
  ...
  // TODO(@jasnell): this rejects with an openssl error instead of the
  // node `ERR_OSSL_EVP_DIFFERENT_PARAMETERS` code one.
  // const not256k1 = getCurves().find((c) => /^sec.*(224|384|512)/.test(c));
  // await assert.rejects(() => {
  //   return test(
  //     generateKeyPairSync('ec', { namedCurve: 'secp256k1' }),
  //     generateKeyPairSync('ec', { namedCurve: not256k1 }));
  // }, {
  //   name: 'Error',
  //   code: 'ERR_OSSL_EVP_DIFFERENT_PARAMETERS'
  // });

  // Test w/ x25519 and x448 keys.
...
testDiffieHellman().then(common.mustCall());
