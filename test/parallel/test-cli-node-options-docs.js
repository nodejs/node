...
// Helper Functions
const optionRegEx = (envVar, depth = 3) => new RegExp(`${'#'.repeat(depth)}.*\`${envVar}[[=\\s\\b\`]`)
const isOptionDocumentedInCli = (envVar) => optionRegEx(envVar).test(cliText);
const isOptionDocumentedInInternalApi = (envVar) => optionRegEx(envVar, 4).test(internalApiText);
const isOptionDocumentedInV8 = (envVar) => optionRegEx(envVar).test(v8OptionsText);
...
const validateOption = (envVar, config) => {
  ...
  if (envVar.startsWith('[') || deprecatedOptions.has(envVar) ||
      isNoOp || isOptionDocumentedInInternalApi(envVar)) {
    return;
  }

  const noOption = `--no${envVar.slice(1)}`;

  if (!isV8Option) {
    if (!hasTrueAsDefault && !isOptionDocumentedInCli(envVar)) {
      assert.fail(`Should have option ${envVar} documented`);
    }
    if (hasTrueAsDefault && !isOptionDocumentedInCli(noOption)) {
      assert.fail(`Should have option ${noOption} documented`);
    }
  }

  if (!hasTrueAsDefault && isOptionDocumentedInCli(noOption)) {
    assert.fail(`Should not have option ${noOption} documented`);
  }

  if (isInNodeOptions) {
    const nodeOption = hasTrueAsDefault ? noOption : envVar;
    if (!isOptionInNodeOptions(nodeOption)) {
      assert.fail(`Should have option ${nodeOption} in NODE_OPTIONS documented`);
    }
  }

  if (isV8Option) {
    if (!isOptionDocumentedInV8(envVar)) {
      assert.fail(`Should have option ${envVar} in V8 options documented`);
    }
  } else if (!isOptionDocumentedInV8(envVar)) {
    assert(manPageText.includes(`Fl ${manpageEntry}`), `Should have option -${manpageEntry} in node.1`);
  }
};
