...
[
  ...
  zlib[method[0]](expect, opts, function(err, result) {
    zlib[method[1]](result, opts, function(err, result) {
      assert.equal(
        result,
        expect,
        `Should get original string after ${method[0]}/${method[1]} with ` +
          'options.'
      );
      hadRun++;
    });
  ...
  zlib[method[0]](expect, function(err, result) {
    zlib[method[1]](result, function(err, result) {
      assert.equal(
        result,
        expect,
        `Should get original string after ${method[0]}/${method[1]} without ` +
          'options.'
      );
      hadRun++;
    });
  ...
  assert.equal(
    result,
    expect,
    `Should get original string after ${method[0]}/${method[1]} with options.`
  );
  hadRun++;
