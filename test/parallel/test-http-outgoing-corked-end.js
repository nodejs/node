  ...
  return new Promise((resolve, reject) => {
    ...
    function onRequest(req, res) {
      if (res.socket.encrypted) {
        tlsHandles.add(res.socket._handle);
      }
      let body = '';
      ...
      req.on('end', common.mustCall(() => {
        ...
        res.socket[kInternalWritev] = common.mustCall(function(...args) {
          assert.notStrictEqual(this.writableCorked, 0);
        ...
        try {
          ...
          res.socket[kInternalWritev] = originalWritev;
        }
    ...
    server.listen(0, common.localhostIPv4, common.mustCall(() => {
      ...
      const req = transport.request({
        ...
        res.on('end', common.mustCall(() => {
          ...
          assert.strictEqual(res.trailers['x-test'], 'é');
...
function verifyWriteWrapOwnership() {
  hook.disable();
  let tlsWriteWraps = 0;
  for (const writeWrap of writeWraps) {
    if (tlsHandles.has(writeWrap.handle)) {
      tlsWriteWraps++;
      assert.strictEqual(Object.hasOwn(writeWrap, '_chunks'), false);
    }
  }
  if (tlsHandles.size !== 0) {
    assert.notStrictEqual(tlsWriteWraps, 0);
  }
}

main()
  .then(verifyWriteWrapOwnership)
