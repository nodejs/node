...
// Server creates an uni stream and send data to the client
...
const serverEndpoint = await listen(mustCall(async (ss) => {
  ...
  onheaders: mustCall(async function(headers) {
    try {
      ...
      serverSessionStream.onwtsessionclose = mustCall((code, reason) => {
    ...
    const serverUnidiStream = await this.session.createUnidirectionalStream({
      incremental: true,
    ...
    const w = serverUnidiStream.writer;
    for (let i = 0; i < numChunks; i++) {
      const chunk = buildChunk(i);
...
wtSessionStream = await clientSession.createBidirectionalStream({
  body: '',
});
