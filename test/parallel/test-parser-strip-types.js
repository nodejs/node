...
const { stripTypeScriptTypes } = require('node:parser');
...
// Enforce that the parser module is not exposed without a schema.
test('parser required without schema', () => {
  assert.throws(() => require('parser'), { code: 'MODULE_NOT_FOUND' });
});

test('stripTypeScriptTypes explicit', () => {
  const source = 'const x: number = 1;';
  const result = stripTypeScriptTypes(source, { mode: 'strip-only' });
  assert.strictEqual(result, 'const x         = 1;');
});
...
test('stripTypeScriptTypes sourceMap throws when mode is strip-only', () => {
  const source = 'const x: number = 1;';
