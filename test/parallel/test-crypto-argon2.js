...
const defaults = { password, salt, keylen: 32, algorithm: 'argon2id', iterations: 3, parallelism: 4, memory: 64 << 10 };
...
const bad = [
  [{ iterations: 0 }, 'iterations'], // iterations < 2
  [{ iterations: 2 ** 32 }, 'iterations'], // Iterations > 2^(32)-1
...
for (const [options, param] of bad) {
  ...
  assert.throws(() => crypto.argon2(password, salt, 32, optionsWithDefaults, () => {}),
                expected);
  assert.throws(() => crypto.argon2Sync(password, salt, 32, optionsWithDefaults),
                expected);
}
