...
// path of the archive file plus a cleanup that unmounts the VFS. mount() picks
// the mount point itself, inside a reserved namespace.
function mountArchive(archive) {
  ...
  const mountPoint = memfs.mount();
  return { vpath: `${mountPoint}/archive.zip`, cleanup: () => memfs.unmount() };
}
