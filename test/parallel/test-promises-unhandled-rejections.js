...
asyncTest(
  'Catching a promise rejection after setImmediate is not soon enough to ' +
...
asyncTest(
  'When re-throwing new errors in a promise catch, only the re-thrown error ' +
    'should hit unhandledRejection',
  function(done) {
    var e = new Error();
    var e2 = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
...
asyncTest(
  'When re-throwing new errors in a promise catch, only the re-thrown error ' +
    'should hit unhandledRejection: original promise rejected async with ' +
    'setTimeout(,1)',
  function(done) {
    var e = new Error();
    var e2 = new Error();
...
asyncTest(
  'When re-throwing new errors in a promise catch, only the re-thrown error ' +
    'should hit unhandledRejection: promise catch attached a ' +
    'process.nextTick after rejection',
  function(done) {
    var e = new Error();
    var e2 = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
...
asyncTest(
  'Attaching a promise catch in a process.nextTick is soon enough to' +
...
asyncTest(
  'While inside setImmediate, catching a rejected promise derived ' +
...
asyncTest(
  'catching a promise which is asynchronously rejected (via ' +
...
asyncTest(
  'Catching a rejected promise derived from throwing in a' +
    ' fulfillment handler prevents unhandledRejection',
  function(done) {
    var e = new Error();
    onUnhandledFail(done);
    Promise.resolve().then(function() {
...
asyncTest(
  'Catching a rejected promise derived from returning a' +
...
asyncTest(
  'A rejected promise derived from returning an' +
...
asyncTest(
  'Catching the Promise.all() of a collection that includes a' +
    'rejected promise prevents unhandledRejection',
  function(done) {
    var e = new Error();
    onUnhandledFail(done);
...
asyncTest(
  'Failing to catch the Promise.all() of a collection that includes' +
    ...
    ' promise, not the passed promise',
  function(done) {
    var e = new Error();
    onUnhandledSucceed(done, function(reason, promise) {
      assert.strictEqual(e, reason);
...
asyncTest(
  'Waiting setTimeout(, 10) to catch a promise causes an' +
    ' unhandledRejection + rejectionHandled pair',
  function(done) {
    clean();
...
asyncTest(
  'Waiting for some combination of process.nextTick + promise' +
...
asyncTest(
  'Waiting for some combination of process.nextTick + promise' +
    ' microtasks to attach a catch handler is still soon enough to ' +
    'prevent unhandledRejection: inside setImmediate',
  function(done) {
    var e = new Error();
    onUnhandledFail(done);
...
asyncTest(
  'Waiting for some combination of process.nextTick + promise ' +
    'microtasks to attach a catch handler is still soon enough to ' +
    'prevent unhandledRejection: inside setTimeout',
  function(done) {
    var e = new Error();
    onUnhandledFail(done);
...
asyncTest(
  'Waiting for some combination of promise microtasks +' +
    ' process.nextTick to attach a catch handler is still soon enough' +
    ' to prevent unhandledRejection: inside setTimeout',
  function(done) {
    var e = new Error();
    onUnhandledFail(done);
...
asyncTest(
  'setImmediate + promise microtasks is too late to attach a catch' +
...
asyncTest(
  'Throwing an error inside a rejectionHandled handler goes to' +
