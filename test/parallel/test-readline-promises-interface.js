...
[ true, false ].forEach(function(terminal) {
  ...
  // \t does not become part of the input when there is a completer function
  {
    ...
    const completer = (line) => {
      return new Promise(common.mustCall((resolve) => {
        resolve([[], line]);
      }));
    };
    const rli = new readline.Interface({
  ...
  if (terminal) {
    ...
    // question
    {
      ...
      const expectedLines = ['foo'];
      rli.question(expectedLines[0]).catch(common.mustNotCall());
    ...
    // deleteLineLeft
    {
      ...
      // Delete from current to start of line
              ...
              { ctrl: true, shift: true, name: 'backspace' });
      cursorPos = rli._getCursorPos();
      assert.strictEqual(cursorPos.rows, 0);
      assert.strictEqual(cursorPos.cols, 0);
      rli.on('line', common.mustCall((line) => {
  ...
  // Can create a new readline Interface with a null output argument
  {
    ...
    rli.question('What do you think of node.js? ')
