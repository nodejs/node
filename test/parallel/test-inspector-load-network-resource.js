...
async function test(
  ...
  const child = new NodeInstance(['--inspect-brk=0', '--experimental-inspector-network-resource'],
                                 '',
  ...
  let result = await session.send({ method: 'Network.loadNetworkResource',
                                    params: {
                                      url: getMapUrl ?
                                        getMapUrl(server) :
  ...
  await session.send({ method: 'IO.close',
                       params: {
                         handle: streamId
                       } });

  result = await session.send({ method: 'Network.loadNetworkResource',
                                params: {
  ...
  result = await session.send({ method: 'IO.read',
                                params: {
                                  handle: streamId
                                } });
...
const mapText = fs.readFileSync(
  fixtures.path('inspector-network-resource/app.js.map'), 'utf8'
);

const main = {
  serverCb: (req, res) => {
    const filename = req.url.split('/').pop();
    const data = fs.readFileSync(fixtures.path('inspector-network-resource/' + filename));
    res.writeHead(200, {
  ...
  assertReadTsResult: (result) => {
    assert.ok(result.data);
    assert.ok(result.data.length > 0);
    assert.ok(result.data.includes('function add(a: number, b: number): number {'));
  }
...
const smallChunkRead = {
  ...
  readMap: async (session, stream) => {
    let result = await session.send({ method: 'IO.read', params: { handle: stream, size: 10 } });
