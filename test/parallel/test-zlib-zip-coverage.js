...
test('using a ZipFile after close() is rejected cleanly', async () => {
  ...
  await zip.close();
  // A read through a retained entry is rejected by the shared closed-state,
  // never by falling through to the (possibly reused) raw descriptor.
  await assert.rejects(entry.content(), { code: 'ERR_INVALID_STATE' });
  // Further mutations are rejected the same way...
  await assert.rejects(
    zip.addEntry(await zlib.ZipEntry.create('b.txt', Buffer.from('x'))),
    { code: 'ERR_INVALID_STATE' });
  // ...and a second close() is an idempotent no-op, not a double-close.
  await zip.close();

  const zipSync = zlib.ZipFile.openSync(filePath, { writable: true });
  const entrySync = zipSync.getSync('a.txt');
  zipSync.closeSync();
  assert.throws(() => entrySync.contentSync(), { code: 'ERR_INVALID_STATE' });
  assert.throws(
