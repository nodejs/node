...
// failing syscall, accept every ArrayBufferView, write the data as it was at
// the call even if the buffer is resized or detached right after, and write
// buffers larger than one write() call in full.
...
async function check(write) {
  ...
  if (!common.isWindows && isMainThread) {
    const file = next();
  ...
  {
    const file = next();
    const resizable = new ArrayBuffer(64 * 1024, { maxByteLength: 128 * 1024 });
    const view = new Uint8Array(resizable).fill(97);
    const done = write(file, view);
    resizable.resize(0);
    await done;
    assert.strictEqual(fs.readFileSync(file, 'utf8'), 'a'.repeat(64 * 1024));
  }
  {
    const file = next();
    const detached = new ArrayBuffer(1024);
    const done = write(file, new Uint8Array(detached).fill(98));
    detached.transfer(8);
    await done;
    assert.strictEqual(fs.readFileSync(file, 'utf8'), 'b'.repeat(1024));
  }
  {
    const file = next();
