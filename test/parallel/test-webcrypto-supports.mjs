...
function supportsRawSecret(alg) {
  if (typeof alg === 'string') {
    alg = alg.toLowerCase();
    if (alg.startsWith('aes')) return true;
    if (alg === 'chacha20-poly1305') return true;
    if (alg === 'pbkdf2') return true;
    if (alg === 'hkdf') return true;
    if (alg.startsWith('argon2')) return true;
    if (alg.startsWith('kmac')) return true;
    if (alg === 'hmac') return true;
    return false;
  } else if (typeof alg?.name === 'string') {
