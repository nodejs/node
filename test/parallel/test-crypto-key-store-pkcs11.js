...
// it, and the PIN that unlocks it are all provided by the environment.
...
// A SoftHSM fixture can provide a directory holding the token and its
// configuration. SoftHSM opens its token read-write, so run from a writable
// copy of that directory; the configuration names the token relative to the
// working directory. A real HSM sets no directory and is used as it stands.
