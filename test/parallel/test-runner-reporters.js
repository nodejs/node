...
describe('node:test reporters', { concurrency: true }, () => {
  ...
  ['js', 'cjs', 'mjs'].forEach((ext) => {
    it(`should support a '${ext}' file as a custom reporter`, async () => {
      ...
      assert.match(stdout, /{"test:enqueue":5,"test:dequeue":5,"test:file":1,"test:start":4,"test:pass":2,"test:fail":2,"test:plan":2,"test:diagnostic":\d+}$/);
  ...
  it('should support a custom reporter from node_modules', async () => {
    ...
    assert.match(
      ...
      /^package: reporter-cjs{"test:enqueue":5,"test:dequeue":5,"test:file":1,"test:start":4,"test:pass":2,"test:fail":2,"test:plan":2,"test:diagnostic":\d+}$/,
    );
  });
  ...
  it('should support a custom ESM reporter from node_modules', async () => {
    ...
    assert.match(
      child.stdout.toString(),
      /^package: reporter-esm{"test:enqueue":5,"test:dequeue":5,"test:file":1,"test:start":4,"test:pass":2,"test:fail":2,"test:plan":2,"test:diagnostic":\d+}$/,
    );
  });
