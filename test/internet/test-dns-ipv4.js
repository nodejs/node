...
TEST(function test_resolve4(done) {
  const req = dns.resolve4('www.google.com', function(err, ips) {
    if (err) throw err;
...
TEST(function test_reverse_ipv4(done) {
  const req = dns.reverse('8.8.8.8', function(err, domains) {
    if (err) throw err;
...
TEST(function test_lookup_ipv4_explicit(done) {
  const req = dns.lookup('www.google.com', 4, function(err, ip, family) {
    if (err) throw err;
...
TEST(function test_lookup_ipv4_implicit(done) {
  const req = dns.lookup('www.google.com', function(err, ip, family) {
    if (err) throw err;
    assert.ok(net.isIPv4(ip));
    assert.strictEqual(family, 4);
...
TEST(function test_lookup_ip_ipv4(done) {
  const req = dns.lookup('127.0.0.1', function(err, ip, family) {
    if (err) throw err;
...
TEST(function test_lookup_localhost_ipv4(done) {
  const req = dns.lookup('localhost', 4, function(err, ip, family) {
    if (err) throw err;
    assert.strictEqual(ip, '127.0.0.1');
    assert.strictEqual(family, 4);
...
TEST(function test_lookupservice_ip_ipv4(done) {
  const req = dns.lookupService('127.0.0.1', 80, function(err, host, service) {
    if (err) throw err;
