...
if (isInvokedAsChild) {
  ...
  }, false /* abort */, true /* launchSecondary */, +process.env.QUEUE_SIZE);
  ...
  function testUnref(queueSize) {
    return new Promise((resolve, reject) => {
      const child = spawnTestSync([], {
        env: { ...process.env, QUEUE_SIZE: String(queueSize) },
      });
      if (child.status === 0) {
        const output = child.stdout ? child.stdout.toString() : '';
        resolve(output.match(/\S+/g));
      } else {
        reject(new Error('Child process died with code ' + child.status));
      }
    })
