...
void* SetImmediate(napi_env env, T&& cb) {
  ...
  check->data = ptr;
  NAPI_ASSERT(
      env, napi_get_uv_event_loop(env, &loop) == napi_ok, "can get event loop");
...
napi_value SetImmediateBinding(napi_env env, napi_callback_info info) {
  ...
  NAPI_ASSERT(
      env, t == napi_function, "Wrong first argument, function expected.");

  napi_ref cbref;
  NAPI_CALL(env, napi_create_reference(env, argv[0], 1, &cbref));
  ...
  SetImmediate(env, [=]() -> char* {
    ...
    NAPI_CALL(
        env, napi_call_function(env, undefined, callback, 0, nullptr, nullptr));
