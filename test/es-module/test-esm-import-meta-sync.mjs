...
describe('import.meta.sync', () => {
  ...
  it('should import JSON with import attributes', () => {
    const jsonPath = fixtures.path('es-modules', 'import-attrs', 'data.json');
    const jsonNs = import.meta.sync(jsonPath, { with: { type: 'json' } });
    assert.deepStrictEqual(jsonNs.default, { value: 42 });
  });

  it('should throw ERR_IMPORT_ATTRIBUTE_MISSING when importing JSON without type attribute', () => {
    const jsonPath = fixtures.path('es-modules', 'import-attrs', 'data.json');
    assert.throws(() => {
      import.meta.sync(jsonPath);
    }, { code: 'ERR_IMPORT_ATTRIBUTE_MISSING' });
  });

  it('should throw ERR_IMPORT_ATTRIBUTE_UNSUPPORTED for unknown attribute keys', () => {
    const jsonPath = fixtures.path('es-modules', 'import-attrs', 'data.json');
    assert.throws(() => {
      import.meta.sync(jsonPath, { with: { bogus: 'value' } });
    }, { code: 'ERR_IMPORT_ATTRIBUTE_UNSUPPORTED' });
  });

  it('should ignore explicit type: "javascript" for ESM (no throw)', () => {
    const esmUrl = fixtures.fileURL('es-modules', 'test-esm-import-meta-sync-simple.mjs').href;
    const ns = import.meta.sync(esmUrl, { with: { type: 'javascript' } });
    assert.strictEqual(ns.value, 42);
  });

  it('should select the correct default export in package exports', async () => {
    const fixtureCwd = fixtures.path('es-modules', 'cond-exports');
    const code = [
      "import assert from 'node:assert';",
      "import { createRequire } from 'node:module';",
      'const requireFn = createRequire(import.meta.url);',
      "const syncNs = import.meta.sync('cond-pkg');",
      "assert.strictEqual(syncNs.which, 'default');",
      "const ns = await import('cond-pkg');",
      "assert.strictEqual(ns.which, 'default');",
      "const req = requireFn('cond-pkg');",
      "assert.strictEqual(req.which, 'require');",
    ].join('');
