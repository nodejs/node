...
test('execute CommonJS TypeScript file from node_modules with require-module and default-type module', async () => {
  const result = await spawnPromisified(process.execPath, [
    '--experimental-strip-types',
    '--experimental-default-type=module',
    fixtures.path('typescript/ts/test-import-ts-node-modules.ts'),
...
test('execute a TypeScript file with Partial Union Type', async () => {
  const result = await spawnPromisified(process.execPath, [
    '--experimental-strip-types',
    '--no-warnings',
    fixtures.path('typescript/ts/test-partial-union-type.ts'),
  ]);
...
test('execute a TypeScript file with Extract Union Type', async () => {
  const result = await spawnPromisified(process.execPath, [
    '--experimental-strip-types',
    '--no-warnings',
    fixtures.path('typescript/ts/test-extract-union-type.ts'),
  ]);
