...
function ReadStream(path, options) {
  ...
  if (options.fd == null) {
    ...
    // Allow 'flag' (singular) for consistency with fs.readFile
    if (options.flag !== undefined && options.flags === undefined) {
...
function WriteStream(path, options) {
  ...
  if (options.fd == null) {
    ...
    // Path will be ignored when fd is specified, so it can be falsy
    this.path = toPathIfFileURL(path);
    // Allow 'flag' (singular) for consistency with fs.writeFile
    if (options.flag !== undefined && options.flags === undefined) {
      options.flags = options.flag;
    }
