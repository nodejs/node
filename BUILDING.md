...
* [Installing Node.js on z/OS](#installing-nodejs-on-zos)
...
Hardware and Software Requirements for installing Node.js on z/OS
* Hardware
  * IBM zEnterprise 196 (z196) or newer
* Software
  * z/OS Unix System Services enabled
   * z/OS V2R4 with PTFs:UI64830,UI64837,UI64839,UI64940,UI65567
   * z/OS V2R3 with PTFs:UI61308,UI61375,UI61747
  * Integrated Cryptographic Service Facility (ICSF) must be enabled on systems.
  * Python 3.8.2 or later 3.x versions, required to compile native add-ons.
    It must also be defined in your PATH environment variable.
  * Make 4.1 or later, required to compile native add-ons.
   It must also be defined in your PATH environment variable.

### Installing

* Download the PAX archive installable file to a z/OS machine from
  [here](https://www.ibm.com/products/sdk-nodejs-compiler-zos)
* Extract the downloaded PAX file
```console
$ pax -p p -r -f <downloaded_pax_file>
```
* Run setup.sh to validate system prerequisites and setup environment variables
```console
$ cd <nodejs_dir>
$ ./setup.sh
```
* Run the .nodeenv script generated by setup.sh to set the required environment
  variables in order to work Node.js properly.
```console
$ . <nodejs_dir>/.nodeenv
```
