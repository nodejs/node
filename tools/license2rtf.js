...
class ParagraphParser extends Stream {
  constructor() {
    const self = this;
    let block_is_license_block = false;
    let block_has_c_style_comment;
    let paragraph_line_indent;
    let paragraph;
    ...
    this.write = function(paragraph) {
      ...
      for (i = 0; i < lines.length - 1; i++) {
        ...
        if (line.length < 50) {
          ...
          const next_first_word_length = lines[i + 1].replace(/\s.*$/, '').length;
