...
def LoadBuildFileIncludesIntoDict(subdict, subdict_path, data, aux_data,
  ...
  for k, v in six.iteritems(subdict):
    if type(v) is dict:
...
def CallLoadTargetBuildFile(global_flags,
  ...
  try:
    ...
    for key, value in six.iteritems(global_flags):
      globals()[key] = value
...
def ExpandVariables(input, phase, variables, build_file):
  ...
  if type(output) is list:
    for index in six.moves.xrange(0, len(output)):
...
def LoadVariablesFromVariablesDict(variables, the_dict, the_dict_key):
  ...
  for key, value in six.iteritems(the_dict.get('variables', {})):
...
def ProcessVariablesAndConditionsInDict(the_dict, phase, variables_in,
  ...
  if 'variables' in the_dict:
    ...
    # in ExpandVariables.
    for key, value in six.iteritems(the_dict['variables']):
      variables[key] = value
...
def QualifyDependencies(targets):
  ...
  for target, target_dict in six.iteritems(targets):
    target_build_file = gyp.common.BuildFile(target)
    toolset = target_dict['toolset']
    for dependency_key in all_dependency_sections:
      dependencies = target_dict.get(dependency_key, [])
      for index in six.moves.xrange(0, len(dependencies)):
...
def RemoveDuplicateDependencies(targets):
  ...
  lists."""
  for target_name, target_dict in six.iteritems(targets):
    for dependency_key in dependency_sections:
      dependencies = target_dict.get(dependency_key, [])
      if dependencies:
...
def BuildDependencyList(targets):
  ...
  # access.
  dependency_nodes = {}
  for target, spec in six.iteritems(targets):
...
def VerifyNoGYPFileCircularDependencies(targets):
  ...
  root_node = DependencyGraphNode(None)
  for build_file_node in six.itervalues(dependency_nodes):
...
def SetUpConfigurations(target, target_dict):
  ...
  if not 'default_configuration' in target_dict:
    concrete = [i for (i, config) in six.iteritems(target_dict['configurations'])
                if not config.get('abstract')]
    target_dict['default_configuration'] = sorted(concrete)[0]

  merged_configurations = {}
  configs = target_dict['configurations']
  for (configuration, old_configuration_dict) in six.iteritems(configs):
    ...
    for (key, target_val) in six.iteritems(target_dict):
      key_ext = key[-1:]
  ...
  for list_key in lists:
    ...
    if exclude_key in the_dict:
      for exclude_item in the_dict[exclude_key]:
        for index in six.moves.xrange(0, len(the_list)):
    ...
    for index in six.moves.xrange(len(list_actions) - 1, -1, -1):
...
def ValidateSourcesInTarget(target, target_dict, build_file,
  ...
  error = ''
  for basename, files in six.iteritems(basenames):
    if len(files) > 1:
      error += '  %s: %s\n' % (basename, ' '.join(files))
