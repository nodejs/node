...
coverage-build: all
	...
	if [ ! -d gcovr ]; then timeout 20s \
		git clone -b 3.4 --depth=1 --single-branch git://github.com/gcovr/gcovr.git \
		|| \
		git clone -b 3.4 --depth=1 --single-branch https://github.com/gcovr/gcovr.git; fi
