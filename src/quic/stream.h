...
//
// The flow direction and origin of the stream are important in determining the write and read
// state (Open or Closed). Specifically:
//
...
// +------------+--------------+--------------------+---------------------+
//
// All data sent via the Stream is buffered internally until either receipt is acknowledged from
// the peer or attempts to send are abandoned. The fact that data is buffered in memory makes it
// essential that the flow control for the session and the stream are properly handled. For now,
// we are largely relying on ngtcp2's default flow control mechanisms which generally should be
// doing the right thing but we may need to switch to a more manual management process if too much
// data ends up being buffered for too long.
//
// A Stream may be in a fully closed state (No longer readable nor writable) state but still have
// unacknowledged data in it's outbound queue.
// TODO(@jasnell): We should likely evaluate just how long a streams is allowed to be in such a
// state. In theory, ngtcp2 should be able to handle this but we need to test to make sure it's not
// possible to orphan and leak defunct streams.
//
// A Stream is gracefully closed when (a) both Read and Write states are Closed, (b) all queued
// data has been acknowledged.
//
// The Stream may be forcefully closed immediately using destroy(err). This causes all queued data
// and pending JavaScript writes to be abandoned, and causes the Stream to be immediately closed at
// the ngtcp2 level without waiting for any outstanding acknowledgements. Keep in mind, however,
// that the peer is not notified that the stream is destroyed and may attempt to continue sending
...
#undef V
  ...
  static BaseObjectPtr<Stream> Create(Environment* env, Session* session, stream_id id);
  ...
  // Acknowledge is called when ngtcp2 has received an acknowledgement for one or more stream
  // frames for this Stream.
  void Acknowledge(uint64_t offset, size_t datalen);

  // Returns false if the header cannot be added. This will typically only happen if a maximimum
  // number of headers, or the maximum total header length is received.
  ...
  // ResetStream will cause ngtcp2 to queue a RESET_STREAM and STOP_SENDING frame, as appropriate,
  // for the given stream_id. For a locally-initiated unidirectional stream, only a RESET_STREAM
  // frame will be scheduled and the stream will be immediately closed. For a bidirectional stream,
  // a STOP_SENDING frame will be sent.
  ...
  // Sends headers to the QUIC Application. If headers are not supported, false will be returned.
  ...
  inline Direction direction() const {
    return id_ & 0b10 ? Direction::UNIDIRECTIONAL : Direction::BIDIRECTIONAL;
  }
  ...
  // Set the final size for the Stream. This only works the first time it is called. Subsequent
  // calls will be ignored unless the subsequent size is greater than the prior set size, in which
  // case we have a bug and we'll assert.
  void set_final_size(uint64_t final_size);

  // The final size is the maximum amount of data that has been acknowleged to have been received
  // for a Stream.
 ...
 private:
  ...
  // The outbound_source_ provides the data that is to be sent by this Stream. After the source is
  // read the writable side of the Stream will be closed by sending a fin data frame.
  Buffer::Source* outbound_source_ = nullptr;
  BaseObjectPtr<BaseObject> outbound_source_strong_ptr_;

  // The inbound_ buffer contains the data that has been received by this Stream. The received data
  // will be buffered in inbound_ until an inbound_consumer_ is attached. Only a single
  // inbound_consumer_ may be attached at a time.
  Buffer inbound_;
 ...
 public:
  // The Queue/Schedule/Unschedule here are part of the mechanism used to determine which streams
  // have data to send on the session. When a stream potentially has data available, it will be
  // scheduled in the Queue. Then, when the Session::Application starts sending pending data, it
  // will check the queue to see if there are streams waiting. If there are, it will grab one and
  // check to see if there is data to send. When a stream does not have data to send (such as when
  // it is initially created or is using an async source that is still waiting for data to be
