...
// The bootstrap object is used only while bootstrapping the Node.js
// process then it is allowed to be gc'd. No permanent references to
// the bootstrap object should be held after bootstrap.
void SetupBootstrapObject(Environment* env, Local<Object> bootstrap) {
#define BOOTSTRAP_METHOD(name, fn) env->SetMethod(bootstrap, #name, fn)
  ...
  CHECK(bootstrap->Set(env->context(),
