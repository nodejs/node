...
class WalkerState {
 ...
 private:
  ...
  const std::unordered_set<uint32_t>* SeenBucket(
      const std::string& path) const {
    auto it = seen_.find(path);
    return it == seen_.end() ? nullptr : &it->second;
  }

  bool SeenIn(const std::unordered_set<uint32_t>* bucket,
              const PatternState& pattern,
              size_t index) const {
  ...
  struct SerialCtx {
    ...
    const void* SeenTokenFor(const std::string& path) const {
      return w->SeenBucket(path);
    }
    bool SeenAt(const void* token,
                const PatternState& pattern,
                size_t index) const {
      return w->SeenIn(static_cast<const std::unordered_set<uint32_t>*>(token),
                       pattern,
                       index);
    }
  ...
  struct FreeCtx {
    ...
    const void* SeenTokenFor(const std::string&) const { return nullptr; }
    bool SeenAt(const void*, const PatternState&, size_t) const {
      return false;
    }
  ...
  void VisitDirectory(Ctx& ctx,
    ...
    for (const DirEntry& entry : *children) {
      ...
      const void* seen_token = ctx.SeenTokenFor(entry_path);
      ...
      for (size_t index : pattern.indexes.values()) {
        ...
        // entry
        if (ctx.SeenAt(seen_token, pattern, index) ||
            ctx.SeenAt(seen_token, pattern, index + 1)) {
          return;
        }
