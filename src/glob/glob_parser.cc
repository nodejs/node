...
void Utf8ToUtf16(std::string_view utf8, PatternString* out) {
  ...
  simdutf::result status = simdutf::convert_utf8_to_utf16le_with_errors(
  ...
  while (read < utf8.size()) {
    ...
    status = simdutf::convert_utf8_to_utf16le_with_errors(
...
std::string Utf16ToUtf8(PatternView utf16) {
  ...
  if (simdutf::validate_utf16le_with_errors(utf16.data(), utf16.size()).error !=
      simdutf::SUCCESS) {
    well_formed.resize(utf16.size());
    simdutf::to_well_formed_utf16le(
        utf16.data(), utf16.size(), well_formed.data());
    source = well_formed;
  }
  std::string out;
  out.resize(simdutf::utf8_length_from_utf16le(source.data(), source.size()));
