...
env_reader::read_result env_reader::read_pair(std::istream& file, const env_pair* pair)
{
    ...
    if (!pair->key->has_own_buffer())
    {
        ...
        tmp_str->replace(0, pair->key->key_index, *pair->key->key, 0, pair->key->key_index);
    ...
    case success:
        if (!pair->value->has_own_buffer())
        {
            ...
            tmp_str->replace(0, pair->value->value_index, *pair->value->value, 0, pair->value->value_index);
...
int env_reader::read_pairs(std::istream& file, std::map<std::string, env_pair*>* mapped_pairs)
{
    int count = 0;
    auto buffer = std::string();
...
void env_reader::close_variable(env_value* value)
{
    ...
    interpolation->variable_str->replace(0, variable_len, *value->value, interpolation->variable_start, variable_len);
                                                       ...
                                                       std::map<std::string, env_pair*>* mapped_pairs)
{
    ...
    for (auto i = size - 1; i >= 0; i--)
    {
        ...
        const auto other_pair = (*mapped_pairs)[interpolation->variable_str->c_str()];
        const auto interpolation_length = (interpolation->end_brace - interpolation->dollar_sign) + 1;
        if (other_pair)
        {
    ...
    for (auto i = size - 1; i >= 0; i--)
    {
        ...
        if (!other_pair)
        {
            ...
            buffer_index -= count;
            buffer->replace(buffer_index, count, *pair->value->value, interpolation->dollar_sign, count);
            continue;
        }
        if (count > 0)
        {
            buffer->replace(buffer_index, count, *pair->value->value, interpolation->end_brace, count);
        ...
        buffer->replace(buffer_index, variable_value_count, *other_pair->value->value );
