...
static WindowsNamespacedPathType ClassifyWindowsNamespacedPath(
  ...
  if (path.size() >= 8 && ToLower(path[4]) == 'u' && ToLower(path[5]) == 'n' &&
      ToLower(path[6]) == 'c' && path[7] == '\\') {
    size_t i = 8;
...
static void StripExtendedPathPrefixForPathResolve(std::string& path) {
  // PathResolve() is an internal filesystem identity resolver, not the public
  // path.win32.resolve() API. For valid namespaced filesystem paths, the
  // namespace prefix does not identify a different file or directory:
  //   \\?\C:\foo                 -> C:\foo
  //   \\?\UNC\server\share\foo   -> \\server\share\foo
  // Keep all other namespaced forms intact. For example,
  // \\?\PHYSICALDRIVE0 is not a drive-absolute filesystem path, and
  // \\?\C:foo is drive-relative rather than drive-absolute.
