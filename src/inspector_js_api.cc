  ...
  class JSBindingsSessionDelegate : public InspectorSessionDelegate {
   public:
    JSBindingsSessionDelegate(Environment* env,
        ...
        : env_(env), connection_(connection) {}
...
void CallAndPauseOnStart(const FunctionCallbackInfo<v8::Value>& args) {
  ...
  v8::MaybeLocal<v8::Value> retval = args[0].As<v8::Function>()->Call(
      env->context(), args[1], call_args.size(), call_args.data());
  if (!retval.IsEmpty()) {
...
void InspectorConsoleCall(const FunctionCallbackInfo<Value>& info) {
  ...
  if (InspectorEnabled(env)) {
    ...
    if (!config_object->Has(context, in_call_key).FromMaybe(false)) {
      CHECK(config_object->Set(context, in_call_key, v8::True(isolate))
                .FromJust());
      CHECK(!inspector_method.As<Function>()
                 ->Call(
...
static void RegisterAsyncHookWrapper(const FunctionCallbackInfo<Value>& args) {
  ...
  env->inspector_agent()->RegisterAsyncHook(
      env->isolate(), enable_function, disable_function);
}
...
void Open(const FunctionCallbackInfo<Value>& args) {
  ...
  agent->StartIoThread();
  if (wait_for_connect) agent->WaitForConnect();
}
