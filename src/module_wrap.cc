...
static Local<Array> createModuleRequestsContainer(
  ...
  Local<Array> requests = Array::New(isolate, raw_requests->Length());
  for (int i = 0; i < raw_requests->Length(); i++) {
    ...
    requests->Set(realm->context(), i, request).ToChecked();
  }
...
void ModuleWrap::New(const FunctionCallbackInfo<Value>& args) {
  ...
  if (!that->Set(context,
                 realm->isolate_data()->module_requests_string(),
...
void ModuleWrap::Link(const FunctionCallbackInfo<Value>& args) {
  ...
  for (uint32_t i = 0; i < specifiers->Length(); i++) {
    Local<String> specifier_str =
        specifiers->Get(context, i).ToLocalChecked().As<String>();
    Local<Object> module_object =
        modules->Get(context, i).ToLocalChecked().As<Object>();
