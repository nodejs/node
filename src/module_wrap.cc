...
void ModuleWrap::Link(const FunctionCallbackInfo<Value>& args) {
  ...
  Local<Array> promises =
      Array::New(isolate, module->GetModuleRequestsLength());
...
enum CheckFileOptions { LEAVE_OPEN_AFTER_CHECK, CLOSE_AFTER_CHECK };
...
const PackageConfig& GetPackageConfig(Environment* env,
  ...
  if (check.IsNothing()) {
    auto entry = env->package_json_cache.emplace(
        path, PackageConfig{Exists::No, IsValid::Yes, HasMain::No, ""});
  ...
  if (!JSON::Parse(env->context(), src).ToLocal(&pkg_json_v) ||
    ...
    auto entry = env->package_json_cache.emplace(
        path, PackageConfig{Exists::Yes, IsValid::No, HasMain::No, ""});
    return entry.first->second;
  }
  ...
  auto entry = env->package_json_cache.emplace(
      path, PackageConfig{Exists::Yes, IsValid::Yes, has_main, main_std});
  return entry.first->second;
}

enum ResolveExtensionsOptions { TRY_EXACT_NAME, ONLY_VIA_EXTENSIONS };
...
static MaybeLocal<Promise> ImportModuleDynamically(
  ...
  Local<Value> import_args[] = {referrer->GetResourceName(),
                                Local<Value>(specifier)};
  MaybeLocal<Value> maybe_result =
      import_callback->Call(context, v8::Undefined(iso), 2, import_args);
...
void ModuleWrap::HostInitializeImportMetaObjectCallback(Local<Context> context,
  ...
  Local<Function> callback = env->host_initialize_import_meta_object_callback();
...
void ModuleWrap::Initialize(Local<Object> target,
  ...
  env->SetProtoMethodNoSideEffect(
      tpl, "getStaticDependencySpecifiers", GetStaticDependencySpecifiers);
