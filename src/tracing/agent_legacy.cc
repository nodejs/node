...
class LegacyTracingAgent::ScopedSuspendTracing {
 public:
  ScopedSuspendTracing(TracingController* controller,
...
void NodeTraceStateObserver::OnTraceEnabled() {
  ...
  ProcessMeta trace_process;
  TRACE_EVENT_METADATA1(
      "__metadata", "node", "process", CastTracedValue(trace_process));
  // This only runs the first time tracing is enabled
  controller_->RemoveTraceStateObserver(this);
}

LegacyTracingAgent::LegacyTracingAgent()
    : tracing_controller_(new TracingController()) {
  tracing_controller_->Initialize(nullptr);

  trace_state_observer_ =
      std::make_unique<NodeTraceStateObserver>(tracing_controller_.get());
  tracing_controller_->AddTraceStateObserver(trace_state_observer_.get());
  tracing_file_writer_ = AgentWriterHandle(this, kDefaultHandleId);

  CHECK_EQ(uv_loop_init(&tracing_loop_), 0);
  CHECK_EQ(uv_async_init(&tracing_loop_,
                         &initialize_writer_async_,
                         [](uv_async_t* async) {
                           LegacyTracingAgent* agent = ContainerOf(
                               &LegacyTracingAgent::initialize_writer_async_,
                               async);
...
void LegacyTracingAgent::InitializeWritersOnThread() {
  ...
  CHECK_EQ(0,
           uv_thread_create(
               ...
               [](void* arg) {
                 ...
                 LegacyTracingAgent* agent =
                     static_cast<LegacyTracingAgent*>(arg);
...
AgentWriterHandle LegacyTracingAgent::AddClient(
...
void LegacyTracingAgent::Disconnect(int client) {
...
void LegacyTracingAgent::Enable(int id,
                                const std::set<std::string>& categories) {
  if (categories.empty()) return;

  ScopedSuspendTracing suspend(
      tracing_controller_.get(), this, id != kDefaultHandleId);
  categories_[id].insert(categories.begin(), categories.end());
}

void LegacyTracingAgent::Disable(int id,
                                 const std::set<std::string>& categories) {
  ScopedSuspendTracing suspend(
      tracing_controller_.get(), this, id != kDefaultHandleId);
...
TraceConfig* LegacyTracingAgent::CreateTraceConfig() const {
...
std::string LegacyTracingAgent::GetEnabledCategories() const {
...
void LegacyTracingAgent::AppendTraceEvent(TraceObject* trace_event) {
  for (const auto& id_writer : writers_)
    id_writer.second->AppendTraceEvent(trace_event);
}

void LegacyTracingAgent::AddMetadataEvent(std::unique_ptr<TraceObject> event) {
  Mutex::ScopedLock lock(metadata_events_mutex_);
  metadata_events_.push_back(std::move(event));
}

void LegacyTracingAgent::Flush(bool blocking) {
  {
    Mutex::ScopedLock lock(metadata_events_mutex_);
    for (const auto& event : metadata_events_) AppendTraceEvent(event.get());
  }

  for (const auto& id_writer : writers_) id_writer.second->Flush(blocking);
}

void LegacyTracingAgent::StartTracing(const std::string& categories) {
  ...
  tracing_file_writer_ =
      AddClient(categories_set,
                std::unique_ptr<AsyncTraceWriter>(new NodeTraceWriter(
                    per_process::cli_options->trace_event_file_pattern)),
                kUseDefaultCategories);
}

void LegacyTracingAgent::StopTracing() {
  tracing_file_writer_.reset();
  tracing_file_writer_ = AgentWriterHandle(this, kDefaultHandleId);
}

AgentWriterHandle* LegacyTracingAgent::GetDefaultWriterHandle() {
...
void TracingController::AddMetadataEvent(
  ...
  LegacyTracingAgent* node_agent =
      static_cast<LegacyTracingAgent*>(Agent::GetInstance());
