...
// Example --experimental-transform metadata: { type: kBoolean, help_text: "..." }
...
void GetEnvOptionsInputType(const FunctionCallbackInfo<Value>& args) {
  ...
  for (const auto& item : _ppop_instance.options_) {
    if (!item.first.empty() && !item.first.starts_with('[') &&
      ...
      if (!AddOptionTypeToObject(
              isolate, context, options_metadata, item.first, mapping_details)) {
        return;
      }
    }
  }
...
// - Values are objects mapping option names to their metadata (type and help text)
