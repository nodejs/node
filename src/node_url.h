...
class URL {
 public:
  ...
  URL(const char* input, const size_t len, const URL* base) {
    if (base != nullptr)
      Parse(
          input, len, kUnknownState, &context_, false, &(base->context_), true);
    else
      Parse(input, len, kUnknownState, &context_, false, nullptr, false);
  }
  ...
  URL(const char* input,
    ...
    if (base != nullptr && baselen > 0) {
      ...
      Parse(
          input, len, kUnknownState, &context_, false, &(_base.context_), true);
    } else {
      Parse(input, len, kUnknownState, &context_, false, nullptr, false);
    }
  }

  explicit URL(std::string input) : URL(input.c_str(), input.length()) {}
  ...
  int32_t flags() { return context_.flags; }

  int port() { return context_.port; }

  const std::string& protocol() const { return context_.scheme; }

  const std::string& username() const { return context_.username; }

  const std::string& password() const { return context_.password; }

  const std::string& host() const { return context_.host; }

  const std::string& query() const { return context_.query; }

  const std::string& fragment() const { return context_.fragment; }
