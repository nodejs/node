...
MaybeLocal<Object> BuildResultObject(Environment* env, sqlite3* connection, bool use_big_ints) {
  ...
  if (use_big_ints) {
    ...
    last_insert_rowid_val = Number::New(env->isolate(), static_cast<double>(last_insert_rowid));
...
void Statement::Run(const FunctionCallbackInfo<Value>& args) {
  ...
  if (!stmt->db_->async()) {
    ...
    CHECK_ERROR_OR_THROW(env->isolate(), stmt->db_.get(), r, SQLITE_OK, void());
    Local<Object> result;
    if(!BuildResultObject(env, stmt->db_->Connection(), stmt->use_big_ints_).ToLocal(&result)) {
      return;
    }
...
static void Initialize(Local<Object> target,
 ...
 SetConstructorFunction(context,
                         target,
                         "Statement",
                         StatementGetAsyncConstructorTemplate(env));
