...
static void Dirname(std::string_view& ret_view) {
  std::cout << ret_view << "\n";
  #ifdef _WIN32
...
static void ImportMetaPathHelpersLazyGetter(
  ...
  Local<Value> ret;
  if (receiver->HasPrivate(context, env->module_import_meta_path()).FromMaybe(false)) {
    auto maybe_value = receiver->GetPrivate(context, env->module_import_meta_path());
    if (!maybe_value.ToLocal(&ret)) return;

    if (plain_name == "dirname") {
      node::Utf8Value utf8name(isolate, ret.As<String>());
      auto ret_view = utf8name.ToStringView();
      Dirname(ret_view);
    ...
    receiver->DeletePrivate(context, env->module_import_meta_path());
    return;
  }
  ...
  receiver->SetPrivate(context, env->module_import_meta_path(), ret);
  if (plain_name == "dirname") {
    Dirname(ret_view);
  }
  if (!ToV8Value(context, ret_view, isolate).ToLocal(&ret)) {
...
void InitImportMetaPathHelpers(const FunctionCallbackInfo<Value>& args) {
  ...
  target
      ->SetLazyDataProperty(
          context, env->dirname_string(), ImportMetaPathHelpersLazyGetter, args[1])
      .Check();
  target
      ->SetLazyDataProperty(
          context, env->filename_string(), ImportMetaPathHelpersLazyGetter, args[1])
      .Check();
}
