...
bool BindingData::ParsePackageJSON(Realm* realm,
                                   PackageConfig* package_config,
                                   ErrorContext* error_context) {
  auto binding_data = realm->GetBindingData<BindingData>();
  const std::string_view path = package_config->file_path;
...
const BindingData::PackageConfig* BindingData::GetPackageJSON(
  ...
  if (!ParsePackageJSON(realm, &package_config, error_context)) {
    return nullptr;
  }
...
void BindingData::ParsePackageJSONBinding(
    const FunctionCallbackInfo<Value>& args) {
  // parsePackageJSON(source, jsonPath, isESM, base, specifier)
  CHECK_GE(args.Length(), 2);
  CHECK(args[0]->IsString() || args[0]->IsArrayBufferView());  // source
  CHECK(args[1]->IsString());                                  // jsonPath

  Realm* realm = Realm::GetCurrent(args);
  auto isolate = realm->isolate();

  BufferValue json_path(isolate, args[1]);
  bool is_esm = args.Length() >= 3 && args[2]->IsTrue();
  auto error_context = ErrorContext();
  if (is_esm) {
    CHECK(args[3]->IsUndefined() || args[3]->IsString());  // base
    CHECK(args[4]->IsString());                            // specifier

    if (args[3]->IsString()) {
      Utf8Value base_value(isolate, args[3]);
      error_context.base = base_value.ToString();
    }
    Utf8Value specifier(isolate, args[4]);
    error_context.specifier = specifier.ToString();
  }

  PackageConfig package_config{};
  package_config.file_path = json_path.ToStringView();
  if (args[0]->IsString()) {
    Utf8Value source(isolate, args[0]);
    package_config.raw_json = source.ToString();
  } else {
    ArrayBufferViewContents<char> view(args[0]);
    package_config.raw_json = std::string(view.data(), view.length());
  }

  if (!ParsePackageJSON(
          realm, &package_config, is_esm ? &error_context : nullptr)) {
    return;  // ParsePackageJSON already threw
  }

  args.GetReturnValue().Set(package_config.Serialize(realm));
}
...
// Deserializes cached compiled code for the given wire bytes. Returns an empty
// MaybeLocal if V8 rejects the cached code, in which case the module must be
// compiled from the wire bytes as usual.
MaybeLocal<WasmModuleObject> DeserializeWasmModule(
    Isolate* isolate,
  ...
  compilation.Finish(
      ...
      [state,
       isolate](std::variant<Local<WasmModuleObject>, Local<Value>> result) {
        // When the cached code is accepted, V8 resolves synchronously within
        // Finish(). Otherwise V8 falls back to compiling the wire bytes
        // asynchronously; that result arrives later and is unused.
...
// Reads `{ builtins, importedStringConstants }` as accepted by the
// WebAssembly.Module constructor. `constants_module` provides storage for the
// string referenced by the returned options.
bool ParseWasmCompileOptions(Environment* env,
                             Local<Object> options,
                             std::string* constants_module,
                             WasmModuleObject::CompileOptions* result) {
  Isolate* isolate = env->isolate();
  Local<Context> context = env->context();
  Local<Value> builtins;
  if (!options->Get(context, FIXED_ONE_BYTE_STRING(isolate, "builtins"))
           .ToLocal(&builtins)) {
    return false;
  }
  if (builtins->IsArray()) {
    Local<Array> array = builtins.As<Array>();
    for (uint32_t i = 0; i < array->Length(); i++) {
      Local<Value> builtin;
      if (!array->Get(context, i).ToLocal(&builtin)) return false;
      if (builtin->IsString() &&
          builtin.As<String>()->StringEquals(
              FIXED_ONE_BYTE_STRING(isolate, "js-string"))) {
        result->builtins |=
  ...
  if (!options
           ->Get(context,
                 FIXED_ONE_BYTE_STRING(isolate, "importedStringConstants"))
           .ToLocal(&constants)) {
    return false;
  }
  if (constants->IsString()) {
    *constants_module = Utf8Value(isolate, constants).ToString();
    result->imported_string_constants_module = constants_module->c_str();
  }
  return true;
}

// Returns undefined when the compile cache is disabled or not applicable,
// otherwise an object with an `external` handle to the cache entry, and, when
// the cached compiled code was accepted by V8, the deserialized `module`.
void GetWasmCompileCacheEntry(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  ...
  ArrayBufferViewContents<uint8_t> source(args[0]);
  Local<String> filename = args[1].As<String>();
  auto* cache_entry = env->compile_cache_handler()->GetOrInsert(
      source.data(), source.length(), filename, CachedCodeType::kWasm);
  if (cache_entry == nullptr) {
    return;
  }
  ...
  if (cache_entry->cache != nullptr) {
    ...
    if (!ParseWasmCompileOptions(
            env, args[2].As<Object>(), &constants_module, &options)) {
      return;
    }
    Debug(env,
          DebugCategory::COMPILE_CACHE,
          "[compile cache] deserializing %s %s...",
          cache_entry->type_name(),
          cache_entry->source_filename);
    Local<WasmModuleObject> module;
    if (DeserializeWasmModule(isolate,
                              {source.data(), source.length()},
                              {cache_entry->cache->data,
                               static_cast<size_t>(cache_entry->cache->length)},
                              options)
            .ToLocal(&module)) {
      Debug(env, DebugCategory::COMPILE_CACHE, "success\n");
      names.push_back(FIXED_ONE_BYTE_STRING(isolate, "module"));
      values.push_back(module);
    } else {
      Debug(env, DebugCategory::COMPILE_CACHE, "rejected\n");
    }
  } else {
    Debug(env,
          DebugCategory::COMPILE_CACHE,
          "[compile cache] no cache for %s %s\n",
          cache_entry->type_name(),
          cache_entry->source_filename);
  }
  args.GetReturnValue().Set(Object::New(
...
void SaveWasmCompileCacheEntry(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  Local<Context> context = isolate->GetCurrentContext();
  Environment* env = Environment::GetCurrent(context);
  DCHECK(env->use_compile_cache());
  CHECK(args[0]->IsExternal());
  CHECK(args[1]->IsWasmModuleObject());
  auto* cache_entry = static_cast<CompileCacheEntry*>(
      args[0].As<External>()->Value(v8::kExternalPointerTypeTagDefault));
  // Only called when the module was compiled from the wire bytes, so any
  // existing cache was rejected.
  env->compile_cache_handler()->MaybeSave(cache_entry,
                                          args[1].As<WasmModuleObject>(),
                                          cache_entry->cache != nullptr);
}
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data,
  ...
  SetMethod(isolate, target, "parsePackageJSON", ParsePackageJSONBinding);
  SetMethod(isolate,
            target,
  ...
  SetMethod(
      isolate, target, "getWasmCompileCacheEntry", GetWasmCompileCacheEntry);
  SetMethod(
      isolate, target, "saveWasmCompileCacheEntry", SaveWasmCompileCacheEntry);
}
...
void BindingData::RegisterExternalReferences(
  ...
  registry->Register(ParsePackageJSONBinding);
  ...
  registry->Register(GetWasmCompileCacheEntry);
  registry->Register(SaveWasmCompileCacheEntry);
}
