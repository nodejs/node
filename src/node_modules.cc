...
void InitImportMeta(const FunctionCallbackInfo<Value>& args) {
  ...
  if (args[2]->IsTrue()) {
    target->SetLazyDataProperty(context, FIXED_ONE_BYTE_STRING(isolate, "dirname"), InitImportMetaLazyGetter, args[1]).Check();
    target->SetLazyDataProperty(context, FIXED_ONE_BYTE_STRING(isolate, "filename"), InitImportMetaLazyGetter, args[1]).Check();
  }

  if (args[3]->IsFunction()) {
    target->Set(context, FIXED_ONE_BYTE_STRING(isolate, "resolve"), args[3]).Check();
  }
