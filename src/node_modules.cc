...
struct Reader {
  ...
  bool Str(std::string* s) {
    uint32_t n;
    if (!U32(&n) || static_cast<size_t>(end - p) < n) return false;
    s->assign(p, n);
    p += n;
    return true;
  }
...
void BindingData::ResolveCacheInit(const FunctionCallbackInfo<Value>& args) {
...
void BindingData::ResolveCacheLookup(const FunctionCallbackInfo<Value>& args) {
  Realm* realm = Realm::GetCurrent(args);
  auto binding_data = realm->GetBindingData<BindingData>();
...
void BindingData::ResolveCacheStore(const FunctionCallbackInfo<Value>& args) {
  Realm* realm = Realm::GetCurrent(args);
  auto binding_data = realm->GetBindingData<BindingData>();
  if (binding_data->resolve_path_.empty()) return;
  Utf8Value key(realm->isolate(), args[0]);
...
void BindingData::FlushResolveCache(const FunctionCallbackInfo<Value>& args) {
  Realm* realm = Realm::GetCurrent(args);
  auto binding_data = realm->GetBindingData<BindingData>();
  ...
  AppendU32(&out, static_cast<uint32_t>(binding_data->resolve_cache_.size()));
