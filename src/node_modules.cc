...
static void InitImportMetaLazyGetter(
  ...
  std::string_view url_view = url.ToStringView();
  std::string ret_string;
  std::string_view ret_view;
  // We can expect that 1 URL <=> 1 module, and extract the cached value right
  // away. If two modules share the same URL, it means they would have to
  // recompute the URL->path conversion if both module access both
  // import.meta.filename and import.meta.dirname.
  auto cached_value = cache.extract(url_view);
  if (cached_value.empty()) {
    auto file_url = ada::parse(url_view);
    CHECK(file_url);
    auto file_path = url::FileURLToPath(env, *file_url);
    CHECK(file_path.has_value());
    ret_string = file_path.value();
    cache.insert({url_view, ret_string});
    ret_view = ret_string;
  } else {
    ret_view = cached_value.mapped();
  }
...
void InitImportMeta(const FunctionCallbackInfo<Value>& args) {
  // target, url, shouldSetDirnameAndFilename, resolve
  CHECK_GE(args.Length(), 4);
  CHECK(args[0]->IsObject());
  CHECK(args[1]->IsString());
  CHECK(args[2]->IsBoolean());
  CHECK(args[3]->IsFalse() || args[3]->IsFunction());
  ...
  if (args[3]->IsFunction()) {
    target->Set(context, env->resolve_string(), args[3]).Check();
  }
...
void BindingData::CreatePerIsolateProperties(IsolateData* isolate_data,
  ...
  SetMethod(isolate, target, "initializeImportMeta", InitImportMeta);
}
...
void BindingData::RegisterExternalReferences(
  ...
  registry->Register(InitImportMeta);
}
