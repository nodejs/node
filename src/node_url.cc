...
#define V(key, _, name)                                                        \
  if (scheme == name) return env->url_special_##key##_string();
  SPECIALS(V)
#undef V
  UNREACHABLE();
}
...
std::vector<std::string> FromJSStringArray(Environment* env,
  ...
  if (array->Length() > 0) vec.reserve(array->Length());
...
url_data HarvestBase(Environment* env, Local<Object> base_obj) {
  ...
  if (port->IsInt32()) base.port = port->Int32Value(context).FromJust();
...
std::string URL::SerializeURL(const url_data& url, bool exclude = false) {
  std::string output;
  output.reserve(
  ...
  if (url.flags & URL_FLAGS_CANNOT_BE_BASE) {
    ...
    if (!(url.flags & URL_FLAGS_HAS_HOST) && url.path.size() > 1 &&
        url.path[0].empty()) {
      output += "/.";
    }
