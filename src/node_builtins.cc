...
void BuiltinLoader::Initialize(Local<Object> target,
  ...
  target
      ->SetAccessor(env->context(),
                    FIXED_ONE_BYTE_STRING(env->isolate(), "builtinCategories"),
  ...
  env->SetMethod(target, "getCacheUsage", BuiltinLoader::GetCacheUsage);
  env->SetMethod(
      target, "compileFunction", BuiltinLoader::CompileFunction);
