...
static void Umask(const FunctionCallbackInfo<Value>& args) {
  ...
  } else if (env->is_worker_instance()) {
...
void SetupProcessObject(Environment* env,
  ...
  process->SetAccessor(env->title_string(),
                       ProcessTitleGetter,
                       env->is_main_instance() ? ProcessTitleSetter : nullptr,
                       env->as_external());

  // process.isMainInstance
  READONLY_PROPERTY(process,
                    "isMainInstance",
                    Boolean::New(env->isolate(), env->is_main_instance()));
  ...
  process_env_template->SetNamedPropertyHandler(
    EnvGetter,
    env->is_main_instance() ? EnvSetter : nullptr,
    EnvQuery,
    env->is_main_instance() ? EnvDeleter : nullptr,
    EnvEnumerator,
    env->as_external());
  ...
  process->SetAccessor(env->debug_port_string(),
                       DebugPortGetter,
                       env->is_main_instance() ? DebugPortSetter : nullptr,
                       env->as_external());
...
void InitializeEnvironment(Environment* env,
  ...
  if (env->is_main_instance() && v8_is_profiling)
    StartProfilerIdleNotifier(env);
...
static int RunMainInstance(int argc,
                           const char** argv,
                           int exec_argc,
                           const char** exec_argv) {
...
int Start(int argc, char** argv) {
  ...
  int exit_code = RunMainInstance(argc,
