...
#if NODE_HAVE_USDT
void BindingData::SetupProbeSemaphore(Isolate* isolate, Local<Object> target) {
  ...
  // The real semaphore is a static symbol outside the sandbox and cannot
  // back a JS-visible ArrayBuffer. Use an always-enabled, V8-allocated
  // semaphore instead: JS then always calls emitPublishProbe(), which
  // checks NODE_DC_PUBLISH_ENABLED() and returns early when no tracer is
  // attached (the same semantics as the macOS tier).
  auto backing = ArrayBuffer::NewBackingStore(isolate, sizeof(uint16_t));
  *static_cast<unsigned short*>(backing->Data()) = 1;  // NOLINT(runtime/int)
#else
  ...
  auto fire_usdt_probe = [&]() {
    if (NODE_DC_PUBLISH_ENABLED()) {
      NODE_DC_PUBLISH_PROBE(
          name_.c_str(),
          message->IsObject() ? static_cast<const void*>(*message.As<Object>())
                              : nullptr);
    }
  };
