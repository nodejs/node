...
const debug = I18N.debuglog('tls-legacy');
...
try {
  ...
  throw I18N.Error(I18N.NO_OPENSSL);
}
...
function debug_label(self) {
  return self === self.pair.cleartext ? 'cleartext' : 'encrypted';
}
...
function onCryptoStreamFinish() {
  ...
  debug(I18N.TLS_LEGACY_DEBUG_ONFINISH, debug_label(this));
...
function onCryptoStreamEnd() {
  this._ended = true;
  debug(I18N.TLS_LEGACY_DEBUG_ONEND, debug_label(this));
}
...
CryptoStream.prototype._write = function write(data, encoding, cb) {
  ...
  if (!this._resumingSession &&
    ...
    var written;
    debug(I18N.TLS_LEGACY_DEBUG_WRITE, label, data.length);
    if (this === this.pair.cleartext)
    ...
    if (written === data.length) {
      debug(I18N.TLS_LEGACY_DEBUG_WRITE_SUCCEED, label, written);
    ...
    debug(I18N.TLS_LEGACY_DEBUG_CLEARTEXT_WRITE_QUEUE_FULL);
  ...
  debug(I18N.TLS_LEGACY_DEBUG_WRITE_QUEUE_BYTES, label, data.length);
};
...
CryptoStream.prototype._read = function read(size) {
  ...
  debug(I18N.TLS_LEGACY_DEBUG_READ_CALLED, label, size);
  if (this === this.pair.cleartext)
  ...
  debug(I18N.TLS_LEGACY_DEBUG_READ_SUCCEED, label, bytesRead);
  ...
  if (this._halfRead !== halfRead) {
    ...
    if (!halfRead) {
      debug(I18N.TLS_LEGACY_DEBUG_OUTEND, label);
      this.emit('sslOutEnd');
    }
  }
};
...
CryptoStream.prototype.end = function(chunk, encoding) {
  debug(I18N.TLS_LEGACY_DEBUG_END, debug_label(this));
...
CryptoStream.prototype.destroySoon = function(err) {
  debug(I18N.TLS_LEGACY_DEBUG_DESTROYSOON, debug_label(this));
...
CryptoStream.prototype.destroy = function(err) {
  ...
  // Destroy both ends
  debug(I18N.TLS_LEGACY_DEBUG_DESTROY, debug_label(this));
...
function onhandshakestart() {
  debug(I18N.TLS_LEGACY_DEBUG_ONHANDSHAKESTART);
...
function onhandshakedone() {
  // for future use
  debug(I18N.TLS_LEGACY_DEBUG_ONHANDSHAKEONE);
}
...
SecurePair.prototype.maybeInitFinished = function() {
  if (this.ssl && !this._secureEstablished && this.ssl.isInitFinished()) {
    ...
    debug(I18N.TLS_LEGACY_DEBUG_SECURE_ESTABLISHED);
    this.emit('secure');
  }
};
...
SecurePair.prototype.destroy = function() {
  ...
  if (!this._doneFlag) {
    debug(I18N.TLS_LEGACY_DEBUG_SECUREPAIR_DESTROY);
...
SecurePair.prototype.error = function(returnOnly) {
  ...
  if (!this._secureEstablished) {
    ...
    if (!err || err.message === 'ZERO_RETURN') {
      var connReset = I18N.Error(I18N.SOCKET_HANG_UP);
