...
SlabBuffer.prototype.use = function use(context, fn, size) {
  ...
  const actualSize = this.remaining;
...
CryptoStream.prototype._read = function read(size) {
  ...
  do {
    ...
    const read = this._buffer.use(this.pair.ssl, out, size - bytesRead);
    if (read > 0) {
      bytesRead += read;
    }
...
exports.createSecurePair = function(context,
                                    ...
                                    rejectUnauthorized,
                                    options) {
  const pair = new SecurePair(context,
                            isServer,
                            requestCert,
                            rejectUnauthorized,
                            options);
