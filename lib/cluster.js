...
RoundRobinHandle.prototype.distribute = function(err, handle) {
  var numOfWorkers = this.free.length;
  var counter = 0;
  var worker;
  this.handles.push(handle);
  if (cluster.skipInactiveWorkers) {
    worker = this.free.shift();
    while (worker && worker.state === 'inactive' && counter++ < numOfWorkers) {
...
function masterInit() {
  ...
  function queryServer(worker, message) {
    ...
    if (cluster.settings.shouldExposeSchedulingHandles) {
      cluster.handles = cluster.handles || handles;
      cluster.handles[key] = handle;
    }
  }
