...
function RoundRobinHandle(key, address, port, addressType, fd) {
  ...
  this.server.once('listening', listeningCallback);
}
...
RoundRobinHandle.prototype.add = function(worker, send) {
  ...
  this.server.once('error', function errorCodeTranslation(err) {
...
function masterInit() {
  ...
  cluster.setupMaster = function setupMaster(options) {
    var settings = {
    ...
    var hasDebugArg = process.execArgv.some(function hasDebugTest(argv) {
    ...
    process.on('internalMessage', function internalMessageCallback(message) {
      ...
      for (key in cluster.workers) {
        ...
        if (worker.state === 'online' || worker.state === 'listening') {
          ...
          worker.once('online', function onlineCallback() {
  ...
  cluster.fork = function(env) {
    ...
    worker.on('message', function messageCallback(message, handle) {
      cluster.emit('message', this, message, handle);
    });

    worker.process.once('exit', function exitCallback(exitCode, signalCode) {
      /*
    ...
    worker.process.once('disconnect', function disconnectCallback() {
      /*
  ...
  cluster.disconnect = function disconnect(cb) {
    ...
    if (workers.length === 0) {
      const nextTickCallback = () => intercom.emit('disconnect');
  ...
  Worker.prototype.destroy = function(signo) {
    ...
    if (this.isConnected()) {
      const disconnectKill = () => proc.kill(signo);
      this.once('disconnect', disconnectKill);
      this.disconnect();
      return;
    }
    proc.kill(signo);
  ...
  function queryServer(worker, message) {
    ...
    handle.add(worker, function workerCallback(errno, reply, handle) {
      reply = util._extend({
        errno: errno,
        key: key,
        ack: message.seq,
...
function workerInit() {
  ...
  cluster._setupWorker = function _setupWorker() {
    ...
    cluster.worker = worker;
    process.once('disconnect', function disconnectCallback() {
      worker.emit('disconnect');
      if (!worker.exitedAfterDisconnect) {
  ...
  cluster._getServer = function _getServer(obj, options, cb) {
    ...
    send(message, function messageCallback(reply, handle) {
    ...
    obj.once('listening', function listeningCallback() {
  ...
  Worker.prototype.destroy = function() {
    ...
    if (!this.isConnected()) {
      ...
      const processDisconnect = () => process.disconnect();
      const processExit = () => process.exit(0);
      send({ act: 'exitedAfterDisconnect' }, processDisconnect);
      process.once('disconnect', processExit);
    }
  };
...
function internal(worker, cb) {
  return function internalMessageListener(message, handle) {
