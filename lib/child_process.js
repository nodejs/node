...
exports.fork = function(modulePath /*, args, options*/) {
  ...
  if (Array.isArray(arguments[1])) {
    ...
    throw I18N.TypeError(I18N.CHILD_PROCESS_ARG_OPTION);
  } else {
    args = [];
...
exports.execFile = function(file /*, args, options, callback*/) {
  ...
  function exithandler(code, signal) {
    ...
    if (!ex) {
      ex = I18N.Error(I18N.CHILD_PROCESS_COMMAND_FAILED, cmd, stderr);
  ...
  if (child.stdout) {
    ...
    child.stdout.addListener('data', function(chunk) {
      ...
      if (stdoutLen > options.maxBuffer) {
        ex = I18N.Error(I18N.CHILD_PROCESS_MAXBUFFER_EXCEEDED, 'stdout');
        kill();
      } else {
        if (!encoding)
          _stdout.push(chunk);
  ...
  if (child.stderr) {
    ...
    child.stderr.addListener('data', function(chunk) {
      ...
      if (stderrLen > options.maxBuffer) {
        ex = I18N.Error(I18N.CHILD_PROCESS_MAXBUFFER_EXCEEDED, 'stderr');
        kill();
      } else {
        if (!encoding)
          _stderr.push(chunk);
...
function normalizeSpawnArguments(file /*, args, options*/) {
  ...
  if (options === undefined)
    ...
    throw I18N.TypeError(I18N.INVALID_ARG_TYPE, 'options', 'object');
...
function lookupSignal(signal) {
  ...
  if (!(signal in constants))
    throw I18N.Error(I18N.UNKNOWN_SIGNAL, signal);
  ...
  for (i = 0; i < options.stdio.length; i++) {
    ...
    if (input != null) {
      ...
      if (Buffer.isBuffer(input))
       ...
       throw I18N.TypeError(I18N.CHILD_PROCESS_INVALID_STDIO, i, typeof input);
    }
  }
...
function checkExecSyncError(ret) {
  if (ret.error || ret.status !== 0) {
    ...
    if (!err) {
      err = I18N.Error(I18N.CHILD_PROCESS_COMMAND_FAILED,
