...
exports.execFile = function(file, args, options, callback) {
  ...
  function exithandler(code, signal) {
    ...
    var stderr;
    if (!encoding) {
      stdout = Buffer.concat(_stdout, stdoutLen);
      stderr = Buffer.concat(_stderr, stderrLen);
    } else {
      stdout = _stdout;
      stderr = _stderr;
    }
...
function spawnSync(file, args, options) {
  ...
  if (options.input) {
    const stdin = options.stdio[0] = Object.assign({}, options.stdio[0]);
  ...
  for (i = 0; i < options.stdio.length; i++) {
    ...
    if (input != null) {
      const pipe = options.stdio[i] = Object.assign({}, options.stdio[i]);
