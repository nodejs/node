...
const debug = I18N.debuglog('child_process');
...
exports.fork = function(modulePath /*, args, options*/) {
  ...
  if (Array.isArray(arguments[1])) {
    ...
    throw new I18N.TypeError(I18N.CHILD_PROCESS_ARG_OPTION);
  } else {
    args = [];
...
exports.execFile = function(file /*, args, options, callback*/) {
  ...
  function exithandler(code, signal) {
    ...
    if (!ex) {
      ex = new I18N.Error(I18N.CHILD_PROCESS_COMMAND_FAILED, cmd, stderr);
  ...
  if (child.stdout) {
    ...
    child.stdout.addListener('data', function(chunk) {
      ...
      if (stdoutLen > options.maxBuffer) {
        ex = new I18N.Error(I18N.CHILD_PROCESS_MAXBUFFER_EXCEEDED, 'stdout');
        kill();
      } else {
        if (!encoding)
          _stdout.push(chunk);
  ...
  if (child.stderr) {
    ...
    child.stderr.addListener('data', function(chunk) {
      ...
      if (stderrLen > options.maxBuffer) {
        ex = new I18N.Error(I18N.CHILD_PROCESS_MAXBUFFER_EXCEEDED, 'stderr');
        kill();
      } else {
        if (!encoding)
          _stderr.push(chunk);
...
}, I18N(I18N.CHILD_PROCESS_CUSTOMFDS_DEPRECATE));
...
function normalizeSpawnArguments(file /*, args, options*/) {
  ...
  if (options === undefined)
    ...
    throw new I18N.TypeError(I18N.INVALID_ARG_TYPE, 'options', 'object');
...
var spawn = exports.spawn = function(/*file, args, options*/) {
  ...
  debug(I18N.CHILD_PROCESS_DEBUG_SPAWN, opts.args, options);
...
function lookupSignal(signal) {
  ...
  if (!(signal in constants))
    throw new I18N.Error(I18N.UNKNOWN_SIGNAL, signal);
...
function spawnSync(/*file, args, options*/) {
  ...
  debug(I18N.CHILD_PROCESS_DEBUG_SPAWNSYNC, opts.args, options);
  ...
  for (i = 0; i < options.stdio.length; i++) {
    ...
    if (input != null) {
      ...
      if (Buffer.isBuffer(input))
       ...
       throw new I18N.TypeError(I18N.CHILD_PROCESS_INVALID_STDIO, i,
                                typeof input);
    }
  }
  ...
  if (result.error) {
    result.error = errnoException(result.error,
                                  I18N(I18N.CHILD_PROCESS_SPAWNSYNC,
                                       opts.file));
...
function checkExecSyncError(ret) {
  if (ret.error || ret.status !== 0) {
    ...
    if (!err) {
      err = new I18N.Error(I18N.CHILD_PROCESS_COMMAND_FAILED,
