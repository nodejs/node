...
const _deprecatedCustomFds = internalUtil.deprecate(function(options) {
...
const spawn = exports.spawn = function(/*file, args, options*/) {
...
function spawnSync(/*file, args, options*/) {
  ...
  if (options.input) {
    const stdin = options.stdio[0] = util._extend({}, options.stdio[0]);
  ...
  for (i = 0; i < options.stdio.length; i++) {
    ...
    if (input != null) {
      const pipe = options.stdio[i] = util._extend({}, options.stdio[i]);
...
function execFileSync(/*command, args, options*/) {
  ...
  const err = checkExecSyncError(ret);
...
function execSync(command /*, options*/) {
  const opts = normalizeExecArgs.apply(null, arguments);
  const inheritStderr = opts.options ? !opts.options.stdio : true;
