...
// Target API:
//
// const s = net.connect({port: 80, host: 'google.com'}, function() {
//   ...
// });
//
...
function Socket(options) {
  ...
  if (options.handle) {
    ...
    if ((options.fd == 1 || options.fd == 2) &&
      ...
      const err = this._handle.setBlocking(true);
      if (err)
...
Socket.prototype.__defineGetter__('bytesWritten', function() {
  const bytes = this._bytesDispatched;
...
function afterWrite(status, handle, req, err) {
  ...
  if (status < 0) {
    const ex = errnoException(status, 'write', req.error);
...
function lookupAndConnect(self, options) {
  ...
  const addressType = exports.isIP(host);
  if (addressType) {
...
Server.prototype._listen2 = function(address, port, addressType, backlog, fd) {
  ...
  const err = _listen(this._handle, backlog);
...
Server.prototype.listen = function() {
  ...
  const port = toNumber(arguments[0]);

  // The third optional argument is the backlog size.
  // When the ip is omitted it can be the second argument.
  const backlog = toNumber(arguments[1]) || toNumber(arguments[2]);
...
Server.prototype.getConnections = function(cb) {
  ...
  // Poll slaves
  const left = this._slaves.length;
  const total = this._connections;
