...
function newHandle(type) {
  ...
  if (type == 'unix_dgram')
    throw I18N.Error(I18N.DGRAM_UNIX_DGRAM_NOT_SUPPORTED);

  throw I18N.Error(I18N.DGRAM_BAD_SOCKET_TYPE);
}
...
Socket.prototype.bind = function(port /*, address, callback*/) {
  ...
  if (this._bindState != BIND_STATE_UNBOUND)
    throw I18N.Error(I18N.SOCKET_BOUND);
...
Socket.prototype.sendto = function(buffer,
  ...
  if (typeof offset !== 'number' || typeof length !== 'number')
    throw I18N.Error(I18N.DGRAM_SEND_BAD_ARGUMENTS);

  if (typeof address !== 'string')
    throw I18N.Error(I18N.DGRAM_MUST_SEND_TO_ADDRESS_PORT, this.type);
...
Socket.prototype.send = function(buffer,
  ...
  offset = offset | 0;
  if (offset < 0)
    throw I18N.RangeError(I18N.DGRAM_OFFSET_TOO_LOW);

  if ((length == 0 && offset > buffer.length) ||
      (length > 0 && offset >= buffer.length))
    throw I18N.RangeError(I18N.DGRAM_OFFSET_TOO_HIGH);

  // Sending a zero-length datagram is kind of pointless but it _is_
  // allowed, hence check that length >= 0 rather than > 0.
  length = length | 0;
  if (length < 0)
    throw I18N.RangeError(I18N.DGRAM_LENGTH_TOO_LOW);

  if (offset + length > buffer.length)
    throw I18N.RangeError(I18N.DGRAM_OFFSET_LENGTH_OVER);

  port = port | 0;
  if (port <= 0 || port > 65535)
    throw I18N.RangeError(I18N.PORT_OUT_OF_RANGE);
...
Socket.prototype.setTTL = function(arg) {
  if (typeof arg !== 'number')
    throw I18N.TypeError(I18N.INVALID_ARG_TYPE, 'arg', 'number');
...
Socket.prototype.addMembership = function(multicastAddress,
  ...
  if (!multicastAddress)
    throw I18N.Error(I18N.REQUIRED_ARGUMENT, 'multicastAddress');
...
Socket.prototype._healthCheck = function() {
  if (!this._handle)
    throw I18N.Error(I18N.NOT_RUNNING); // error message from dgram_legacy.js
};
