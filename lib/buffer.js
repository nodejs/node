...
const poolSize, poolOffset, allocPool;
...
function allocate(size) {
  ...
  if (size < (Buffer.poolSize >>> 1)) {
    if (size > (poolSize - poolOffset))
      createPool();
    const b = allocPool.slice(poolOffset, poolOffset + size);
    poolOffset += size;
    alignPool();
    return b;
  } else {
...
function fromString(string, encoding) {
  ...
  if (length > (poolSize - poolOffset))
    createPool();
  const actual = allocPool.write(string, poolOffset, encoding);
  const b = allocPool.slice(poolOffset, poolOffset + actual);
  poolOffset += actual;
  alignPool();
  return b;
}
...
Buffer.prototype.readUIntBE = function(offset, byteLength, noAssert) {
  ...
  const val = this[offset + --byteLength];
  const mul = 1;
