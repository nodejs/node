...
Buffer.prototype.swap16 = function swap16() {
  ...
  if (len % 2 !== 0) {
    ...
    throw new errors.RangeError('BUFFERSWAP16');
  }
  if (len < 512) {
...
Buffer.prototype.swap32 = function swap32() {
  ...
  if (len % 4 !== 0) {
    const errors = require('internal/errors');
    throw new errors.RangeError('BUFFERSWAP32');
  }
  if (len < 1024) {
...
Buffer.from = function(value, encodingOrOffset, length) {
  if (typeof value === 'number') {
    const errors = require('internal/errors');
    throw new errors.TypeError('ARGNOTNUM', 'value');
  }
...
function assertSize(size) {
  if (typeof size !== 'number') {
    const errors = require('internal/errors');
    const err = new errors.TypeError('INVALIDARG', 'size', 'number');
...
function fromString(string, encoding) {
  ...
  if (!Buffer.isEncoding(encoding)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('UNKNOWNENCODING', encoding);
  }
...
function fromObject(obj) {
  ...
  const errors = require('internal/errors');
  throw new errors.TypeError('INVALIDARG', 1,
...
Buffer.compare = function compare(a, b) {
  if (!(a instanceof Buffer)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 'a', 'Buffer');
  }
  if (!(b instanceof Buffer)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 'b', 'Buffer');
  }
...
Buffer.concat = function(list, length) {
  ...
  if (!Array.isArray(list)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 'list', 'Array of Buffers');
  }
...
Buffer.prototype.equals = function equals(b) {
  if (!(b instanceof Buffer)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 1, 'Buffer');
  }
...
Buffer.prototype.compare = function compare(target,
  ...
  if (!(target instanceof Buffer)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 'target', 'Buffer');
  }
  ...
  if (start < 0 ||
      ...
      thisEnd > this.length) {
    const errors = require('internal/errors');
    throw new errors.RangeError('INDEXOUTOFRANGE');
  }
...
function bidirectionalIndexOf(buffer, val, byteOffset, encoding, dir) {
  ...
  const errors = require('internal/errors');
  throw new errors.TypeError('INVALIDARG', 'val',
...
Buffer.prototype.fill = function fill(val, start, end, encoding) {
  ...
  if (typeof val === 'string') {
    ...
    if (encoding !== undefined && typeof encoding !== 'string') {
      const errors = require('internal/errors');
      throw new errors.TypeError('INVALIDARG', 'encoding', 'string');
    }
...
Buffer.prototype.write = function(string, offset, length, encoding) {
  ...
  if (string.length > 0 && (length < 0 || offset < 0)) {
    const errors = require('internal/errors');
    throw new errors.RangeError('WRITEOUTOFBOUNDS');
  }
...
function checkInt(buffer, value, offset, ext, max, min) {
  if (!(buffer instanceof Buffer)) {
    const errors = require('internal/errors');
    throw new errors.TypeError('INVALIDARG', 'buffer', 'Buffer');
  }
  if (value > max || value < min) {
    const errors = require('internal/errors');
    throw new errors.TypeError('OUTOFBOUNDSARG', 'value');
  }
