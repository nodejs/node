...
// This returns true if Hmac's digest matches the provided digest, comparing the
// two using timingSafeEqual().
//
...
Hmac.prototype.validate = function(inputBuffer) {
  ...
  return timingSafeEqual(this.digest(), inputBuffer);
};
...
function timingSafeEqual(a, b) {
  ...
  // The test for === is just in case of the vanishingly small chance of
  // a collision. It only fires if the digest comparison passes and so doesn't
  // leak timing information.
  return ah.equals(bh) && a.toString() === b.toString();
}
