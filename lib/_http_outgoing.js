...
OutgoingMessage.prototype._storeHeader = function(firstLine, headers) {
  ...
  if ((statusCode === 204 || statusCode === 304) &&
    ...
    debug(I18N.HTTP_OUTGOING_DEBUG_NOT_USE_CHUNKED, statusCode);
    this.chunkedEncoding = false;
  ...
  if (state.sentContentLengthHeader === false &&
    ...
    if (!this._hasBody) {
      ...
      if (!state.sentTrailer &&
        ...
        debug(I18N.HTTP_OUTGOING_DEBUG_BOTH_REMOVED);
      }
    }
  }
...
function storeHeader(self, state, field, value) {
  if (!common._checkIsHttpToken(field)) {
    throw new I18N.TypeError(I18N.HTTP_INVALID_TOKEN, 'field');
  }
...
OutgoingMessage.prototype.setHeader = function(name, value) {
  if (!common._checkIsHttpToken(name))
    throw new I18N.TypeError(I18N.HTTP_INVALID_TOKEN, 'name');
  if (typeof name !== 'string')
    throw new I18N.TypeError(I18N.INVALID_ARG_TYPE, 'name', 'string');
  if (value === undefined)
    throw new I18N.Error(I18N.REQUIRED_ARG, 'value');
  if (this._header)
    throw new I18N.Error(I18N.HTTP_OUTGOING_SET_AFTER_SEND);
...
OutgoingMessage.prototype.getHeader = function(name) {
  if (arguments.length < 1)
    throw new I18N.Error(I18N.REQUIRED_ARG, 'name');
...
OutgoingMessage.prototype.removeHeader = function(name) {
  ...
  if (this._header)
    throw new I18N.Error(I18N.HTTP_OUTGOING_REMOVE_AFTER_SEND);
...
OutgoingMessage.prototype._renderHeaders = function() {
  if (this._header)
    throw I18N(I18N.HTTP_OUTGOING_RENDER_AFTER_SEND);
...
OutgoingMessage.prototype.write = function(chunk, encoding, callback) {
  if (this.finished) {
    process.nextTick(writeAfterEndNT, this,
                     new I18N.Error(I18N.WRITE_AFTER_END),
                     callback);
    return true;
  }
  ...
  if (!this._hasBody) {
    debug(I18N.HTTP_OUTGOING_DEBUG_IGNORING_WRITE);
    return true;
  }

  if (typeof chunk !== 'string' && !(chunk instanceof Buffer))
    throw new I18N.TypeError(I18N.FIRST_ARGUMENT_STRING_OR_BUFFER);
  ...
  debug(I18N.HTTP_OUTGOING_DEBUG_WRITE_RET, ret);
  return ret;
};
...
OutgoingMessage.prototype.end = function(data, encoding, callback) {
  ...
  if (data && !this._hasBody) {
    debug(I18N.HTTP_OUTGOING_DEBUG_IGNORING_END_DATA);
    data = null;
  }
  ...
  debug(I18N.HTTP_OUTGOING_MESSAGE_END);
...
OutgoingMessage.prototype.flush = internalUtil.deprecate(function() {
  this.flushHeaders();
}, I18N(I18N.HTTP_OUTGOING_FLUSH_DEPRECATE));
