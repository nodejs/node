...
exports.connect = function(/* [port, host], options, cb */) {
  ...
  // these two environment variables cause node to accept untrusted certificates
  // by default (that is, unless the options explictly specificy otherwise)
  var rejectUnauthorized = true;
  if ('1' === process.env.NODE_TLS_INSECURELY_ACCEPT_UNTRUSTED_CERTIFICATES) {
    rejectUnauthorized = false;
  } else if ('0' === process.env.NODE_TLS_REJECT_UNAUTHORIZED) {
    warned = internalUtil.printDeprecationWarning(
      'NODE_TLS_REJECT_UNAUTHORZED is deprecated and may make your program ' +
      'insecure against man-in-the-middle attacks. Are you sure? If so, ' +
      'please use NODE_TLS_INSECURELY_ACCEPT_UNTRUSTED_CERTIFICATES=1',
      warned);
    rejectUnauthorized = false;
  }
