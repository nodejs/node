...
function loadSession(self, hello, cb) {
  ...
  function onSession(err, session) {
    ...
    const ret = self._handle.loadSession(session);
...
TLSSocket.prototype.renegotiate = function(options, callback) {
  const requestCert = this._requestCert;
  const rejectUnauthorized = this._rejectUnauthorized;
...
// Target API:
//
//  const s = tls.connect({port: 8000, host: "google.com"}, function() {
//    if (!s.authorized) {
//      s.destroy();
//      return;
//    }
...
function normalizeConnectArgs(listArgs) {
  const args = net._normalizeConnectArgs(listArgs);
...
exports.connect = function(/* [port, host], options, cb */) {
  ...
  socket.on('secure', function() {
    ...
    if (ekeyinfo.type === 'DH' && ekeyinfo.size < options.minDHSize) {
      const err = new Error('DH parameter size ' + ekeyinfo.size +
