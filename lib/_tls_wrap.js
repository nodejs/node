...
function onhandshakestart() {
  ...
  if (++ssl.handshakes > tls.CLIENT_RENEG_LIMIT) {
    ...
    setImmediate(function() {
      var err = I18N.Error(I18N.TLS_RENEGOTIATION_ATTACK);
...
function loadSession(self, hello, cb) {
  ...
  function onSession(err, session) {
    if (once)
      return cb(I18N.Error(I18N.TLS_CALLBACK_CALLED_TWICE));
    once = true;
    ...
    if (!self._handle)
      return cb(I18N.Error(I18N.SOCKET_CLOSED));
...
function oncertcb(info) {
  ...
  loadSNI(self, servername, function(err, ctx) {
    ...
    requestOCSP(self, info, ctx, function(err) {
      ...
      if (!self._handle)
        return self.destroy(I18N.Error(I18N.SOCKET_CLOSED));
...
TLSSocket.prototype.renegotiate = function(options, callback) {
  ...
  if (!this._handle.renegotiate()) {
    if (callback) {
      process.nextTick(callback, I18N.Error(I18N.TLS_FAILED_TO_RENEGOTIATE));
    }
    return false;
  }
...
TLSSocket.prototype._handleTimeout = function() {
  this._emitTLSError(I18N.Error(I18N.TLS_HANDSHAKE_TIMEOUT));
};
...
function Server(/* [options], listener */) {
  ...
  if (typeof timeout !== 'number') {
    throw I18N.TypeError(I18N.INVALID_OPTION_TYPE,
  ...
  net.Server.call(this, function(raw_socket) {
    ...
    socket.on('close', function(err) {
      ...
      if (!socket._controlReleased && !errorEmitted) {
        errorEmitted = true;
        var connReset = I18N.Error(I18N.NET_SOCKET_HANGUP);
...
Server.prototype.addContext = function(servername, context) {
  if (!servername)
    throw I18N.Error(I18N.REQUIRED_ARG, 'servername');
...
exports.connect = function(/* [port, host], options, cb */) {
  ...
  socket.on('secure', function() {
    ...
    if (ekeyinfo.type === 'DH' && ekeyinfo.size < options.minDHSize) {
      var err = I18N.Error(I18N.TLS_DH_PARAMETER_SIZE_SMALL,
