...
function createStaticServer(options = kEmptyObject) {
  const {
    ...
    port,
    host = 'localhost',
    mimeOverrides = kEmptyObject,
    filter = filterDotFiles,
  } = options;
  const CWD_URL = isURLInstance(directory) ? directory : pathToFileURL(directory + sep);
  ...
  const server = new Server(async (req, res) => {
    ...
    try {
      try {
        await pipeline(createReadStream(url, { emitClose: false }), res);
      } catch (err) {
        if (err?.code === 'EISDIR') {
          if (StringPrototypeEndsWith(req.url, '/') ||
              RegExpPrototypeExec(/^[^?]+\/\?/, req.url) !== null) {
            await pipeline(
              createReadStream(new URL('./index.html', url), { emitClose: false }),
              res
            );
          } else {
            res.statusCode = 307;
