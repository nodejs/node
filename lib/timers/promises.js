...
function cancelListenerHandler(clear, reject, signal) {
  if (!this._destroyed) {
    clear(this);
    reject(new AbortError(undefined, { __proto__: null, cause: signal?.reason }));
  }
}
...
function setTimeout(after, value, options = kEmptyObject) {
  ...
  if (signal?.aborted) {
    return PromiseReject(new AbortError(undefined, { __proto__: null, cause: signal.reason }));
  }
  let oncancel;
...
class Scheduler {
  ...
  wait(delay, options) {
    if (!this[kScheduler])
      throw new ERR_INVALID_THIS('Scheduler');
    return setTimeout(delay, undefined, { __proto__: null, signal: options?.signal });
  }
}
