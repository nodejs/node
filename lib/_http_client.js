...
function ClientRequest(options, cb) {
  ...
  if (options.path && ! /^[\x00-\x09\x0E-\x1F\x21-\x7F]*$/.test(options.path)) {
    // The actual regex is more like /[^A-Za-z0-9\-._~!$&'()*+,;=/:@]/
    // with an additional rule for ignoring percentage-escaped characters
    // but that's a) hard to capture in a regular expression that performs
    // well, and b) possibly too restrictive for real-world usage.
    //
    // This regex rejects paths with whitespace (U+000A, U+000B, U+000C, U+000D
    // and U+0020) as those can have special meaning in the HTTP protocol, as
    // well as paths with non-ASCII characters, (to deprecate previous buggy
    // behaviour).
