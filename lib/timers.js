...
const not_called_symbol = Symbol('notCalled');
...
function insert(item, unrefed) {
  ...
  if (!item[async_id_symbol]) {
    item[async_id_symbol] = ++async_uid_fields[kAsyncUidCntr];
    item[trigger_id_symbol] = initTriggerId();
    // NOTE: While all "item" passed to insert() are emitted with type
    // 'Timeout', the actual object can be created outside of the Timeout
    // constructor. Even so, they'll all be emitted as 'Timeout'.
...
function timerEmitBefore(id, triggerId) {
...
function ontimeout(timer) {
  ...
  timerEmitBefore(timer[async_id_symbol], timer[trigger_id_symbol]);
  // TODO(trevnorris): Like with nextTick, this is a bit of a hack to get
  // around needing to store the id and wait to check for kDestroy until after
  // the callback has run. This is to account for the callback possibly
  // throwing an exception.
  if (async_hook_fields[kDestroy] > 0 && timer[not_called_symbol])
    emitDestroy(timer[async_id_symbol]);
  timer[not_called_symbol] = false;
  if (!args)
    callback.call(timer);
  else {
    switch (args.length) {
  ...
  if (timer._repeat)
    rearm(timer);
  timerEmitAfter(timer[async_id_symbol]);
}
...
Timeout.prototype.close = function() {
  ...
  if (async_hook_fields[kDestroy] && this[not_called_symbol]) {
    emitDestroy(this[async_id_symbol]);
  }
  this[not_called_symbol] = false;
  return this;
};
...
function tryOnImmediate(immediate, oldTail) {
  ...
  immediate[not_called_symbol] = false;
  try {
...
exports.clearImmediate = function(immediate) {
  ...
  if (immediate[async_id_symbol] > 0 && immediate[not_called_symbol]) {
