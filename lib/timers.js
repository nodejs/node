...
const debug = I18N.debuglog('timer');
...
function listOnTimeout() {
  ...
  debug(I18N.TIMERS_DEBUG_CALLBACK, msecs);

  var now = Timer.now();
  debug(I18N.TIMERS_DEBUG_NOW, now);
  ...
  while (first = L.peek(list)) {
    ...
    if (diff < msecs) {
      ...
      debug(I18N.TIMERS_DEBUG_LIST_WAIT, msecs, diff);
      return;
    } else {
      L.remove(first);
  ...
  debug(I18N.TIMERS_DEBUG_LIST_EMPTY, msecs);
...
function reuse(item) {
  ...
  if (list && L.isEmpty(list)) {
    debug(I18N.TIMERS_DEBUG_REUSE_HIT);
    list.stop();
...
const unenroll = exports.unenroll = function(item) {
  var list = reuse(item);
  if (list) {
    debug(I18N.TIMERS_DEBUG_UNENROLL);
    list.close();
  }
...
exports.enroll = function(item, msecs) {
  if (typeof msecs !== 'number')
    throw new I18N.TypeError(I18N.INVALID_ARG_TYPE, 'msecs', 'number');

  if (msecs < 0 || !isFinite(msecs))
    throw new I18N.RangeError(I18N.TIMERS_NON_NEGATIVE_FINITE, 'msecs');
...
function _makeTimerTimeout(timer) {
  ...
  debug(I18N.TIMERS_DEBUG_UNREFTIMER_TIMEOUT);
  timer._called = true;
  _runOnTimeout(timer);
...
function unrefTimeout() {
  ...
  debug(I18N.TIMERS_DEBUG_UNREFTIMER_FIRED);
  ...
  if (minNextTimeoutTime !== TIMEOUT_MAX) {
    ...
    debug(I18N.TIMERS_DEBUG_UNREFTIMER_RESCHED);
  } else if (L.isEmpty(unrefList)) {
    debug(I18N.TIMERS_DEBUG_UNREFLIST_EMPTY);
  }
}
...
exports._unrefActive = function(item) {
  ...
  if (!unrefList) {
    debug(I18N.TIMERS_DEBUG_UNREFLIST_INIT);
    unrefList = {};
    L.init(unrefList);

    debug(I18N.TIMERS_DEBUG_UNREFTIMER_INIT);
  ...
  if (unrefTimer.when === -1 || unrefTimer.when > when) {
    ...
    debug(I18N.TIMERS_DEBUG_UNREFTIMER_SCHED);
  }

  debug(I18N.TIMERS_DEBUG_UNREFLIST_APPEND);
