...
StringDecoder.prototype.write = function(buffer) {
  ...
  const charReceived = this.charReceived;
  const surrogateSize = this.surrogateSize;
  const encoding = this.encoding;
  const charCode;
  // if our last write ended with an incomplete multibyte character
  while (charLength) {
    // determine how many remaining bytes this buffer has to offer for this char
    const diff = charLength - charReceived;
    const available = (buflen >= diff) ? diff : buflen;
...
StringDecoder.prototype.detectIncompleteChar = function(buffer) {
  ...
  const i = (buflen >= 3) ? 3 : buflen;
  const newlen = false;

  // Figure out if one of the last i bytes of our buffer announces an
  // incomplete char.
  for (; i > 0; i--) {
    const c = buffer[buflen - i];
...
function utf16DetectIncompleteChar(buffer) {
  const charReceived = this.charReceived = buffer.length % 2;
  this.charLength = charReceived ? 2 : 0;
  return true;
}

function base64DetectIncompleteChar(buffer) {
  const charReceived = this.charReceived = buffer.length % 3;
