...
class SyncHeapProfileHandle {
  ...
  getAllocationProfile() {
    ...
    return _getAllocationProfile(this.#sessionGeneration);
  }
  ...
  stop() {
    ...
    try {
      return _stopHeapProfile(this.#sessionGeneration);
    } finally {
      this.#stopped = true;
    }
  };
...
function startHeapProfile(options) {
  ...
  if (labels) {
    ensureHeapProfileLabelsALS();
  }
  const generation = _startHeapProfile(sampleInterval, stackDepth, flags, labels);
  return new SyncHeapProfileHandle(generation);
}
...
// Requires async-context-frame (on by default in Node.js 27).
// Lazily initialized on first use so processes that never use heap profiling
// do not create an ALS instance or require async_hooks.
...
function ensureHeapProfileLabelsALS() {
  if (_heapProfileLabelsALS === undefined) {
    ...
    // Without async-context-frame, CPED is not a JSMap, so every sample
    // gets an empty labels object with no error. Warn once at first use.
    if (!getOptionValue('--async-context-frame')) {
      process.emitWarning(
        'Heap profile labels require async-context-frame, which is ' +
          'disabled. Remove --no-async-context-frame; all labels will ' +
          'be empty.',
        { code: 'NODE_HEAP_PROFILE_LABELS_NO_ASYNC_CONTEXT' },
      );
    }
 ...
 * Flattening happens once here at label-set time rather than per allocation.
 * The flat array is stored in AsyncLocalStorage; V8 interns it at allocation
 * time, and getAllocationProfile() resolves the interned value to a labels
 * object at read time.
...
function withHeapProfileLabels(labels, fn) {
  ...
  // Store the flat [key1, val1, key2, val2, ...] array in ALS. V8 interns
  // this value per allocation; pre-flattening avoids V8 Object property
  // access on the allocation path.
