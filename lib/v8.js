...
class MemoryProfileWatchdog {
  ...
  constructor(options) {
    ...
    validateInteger(options.interval, 'interval', 1);

    const filename = getValidatedPath(options.filename);

    if (filename.length === 0) {
      throw new ERR_INVALID_ARG_VALUE('filename', options.filename);
    }

    if (!options.maxRss && !options.maxUsedHeapSize) {
      throw new ERR_INVALID_ARG_VALUE('maxRss and maxUsedHeapSize', options,
                                      'must be greater than 0');
    }

    if (options.maxRss) {
      validateInteger(options.maxRss, 'maxRss', 1);
    }

    if (options.maxUsedHeapSize) {
      validateInteger(options.maxUsedHeapSize, 'maxUsedHeapSize', 1);
    }
