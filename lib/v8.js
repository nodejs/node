...
function getObjectStatistics(typeIndex) {
  if (typeof typeIndex !== 'number' || typeIndex === null) {
    throw new ERR_INVALID_ARG_TYPE('typeIndex', 'Number', typeIndex);
  }

  if (typeIndex >= kTrackedHeapObjectTypes || typeIndex < 0) {
    throw new ERR_INVALID_ARG_VALUE(
      'typeIndex',
      typeIndex,
      `must be between 0 and ${kTrackedHeapObjectTypes}`
    );
  }

  const buffer = heapObjectStatisticsBuffer;
  updateHeapObjectStatisticsBuffer(typeIndex);
  return {
    'object_count': buffer[kObjectCountIndex],
    'object_size': buffer[kObjectSizeIndex]
  };
}
