...
// Lazy: pull async_hooks (and the cpu_profiler binding) only on first use of
// startCpuProfile, since they're not free to load.
let lazyCpuProfilerBits;
function getCpuProfilerBits() {
  if (lazyCpuProfilerBits === undefined) {
...
class SyncCPUProfileHandle {
  ...
  constructor(binding, als) {
    this.#binding = binding;
    this.#als = als;
  }

  /**
   * Stop the profiler and return the V8 JSON profile string.
  ...
  /**
   * Stop the profiler and return a structured profile object tree:
   *   { startTime, endTime, droppedContexts,
   *     topDownRoot: { functionName, scriptName, lineNumber, columnNumber,
   *                    hitCount, contexts?, children } }
  ...
  stopAndCapture() {
    ...
    this.#stopped = true;
    return this.#binding.stopAndCapture();
  }

  /**
   * Capture a profile of samples since the last start/snapshot, and continue
   * profiling with a fresh internal session. Returns the same shape as
   * stopAndCapture(). Throws if the profiler has been stopped.
   * @returns {object}
   */
  snapshot() {
    if (this.#stopped) {
      throw new Error('CPU profile has been stopped');
    }
    return this.#binding.snapshot();
  }

  /**
   * Run `fn(...args)` with `value` recorded as the context for any samples
   * captured during its synchronous and propagated-async execution. Requires
   * { withContext: true } at start time.
   */
  runWithContext(value, fn, ...args) {
    if (this.#als === null) {
      throw new Error(
        'runWithContext requires { withContext: true } on startCpuProfile');
    }
    if (this.#stopped) {
      throw new Error('CPU profile has been stopped');
    }
    const bits = getCpuProfilerBits();
    bits.validateFunction(fn, 'fn');
    const holder = this.#binding.createContextHolder(value);
    return this.#als.run(holder, fn, ...args);
  }

  /**
   * Enter a scope in which `value` is the current sample context, for the
   * rest of the active AsyncLocalStorage scope. Mirrors the naming of
   * AsyncLocalStorage.enterWith. Requires { withContext: true } at start
   * time.
   */
  enterWithContext(value) {
    if (this.#als === null) {
      throw new Error(
        'enterWithContext requires { withContext: true } on startCpuProfile');
    }
    if (this.#stopped) {
      throw new Error('CPU profile has been stopped');
    }
    const holder = this.#binding.createContextHolder(value);
    this.#als.enterWith(holder);
  }
 ...
 * @param {number} [options.sampleInterval] V8's per-profile sampling interval
 *   in milliseconds. Default is V8's internal default.
 * @param {number} [options.maxBufferSize] Maximum number of samples V8 will
 *   buffer for this profile. Default unlimited.
 * @param {boolean} [options.withContext] When true, the handle exposes
 *   runWithContext() / enterWithContext() for associating arbitrary JS values
 *   with the samples captured during their execution scope. The values are
 *   surfaced on each sample of the structured profile returned by
 *   stopAndCapture() or snapshot(). When false (default), no per-sample
 *   context tracking is performed and there is no extractor overhead.
 * @param {number} [options.contextBufferSize] Maximum number of samples that
 *   can carry a context value during a single profile session. Once
 *   exceeded, further samples are recorded with no context and the
 *   `droppedContexts` counter on the result is incremented. Default 60000
 *   which is enough for 60 seconds of sampling at the default interval of
 *   10ms. Only meaningful with withContext.
...
function startCpuProfile(options = {}) {
  ...
  const contextBufferSize = options.contextBufferSize ?? 60000;

  const als = withContext ? new bits.AsyncLocalStorage() : null;
  const binding = new bits.SampledCpuProfiler(
    als ?? {}, contextBufferSize, samplingIntervalMicros, maxSamples,
    withContext);
  binding.start();
  return new SyncCPUProfileHandle(binding, als);
}
