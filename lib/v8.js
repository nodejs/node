...
const serdesBinding = internalBinding('serdes');
const { Serializer, Deserializer } = serdesBinding;
const {
...
const heapUtilsBinding = internalBinding('heap_utils');
const {
  HeapSnapshotStream,
...
function writeHeapSnapshot(filename, options) {
  ...
  return heapUtilsBinding.triggerHeapSnapshot(filename, optionArray);
}
...
function getHeapSnapshot(options) {
  const optionArray = getHeapSnapshotOptions(options);
  const handle = heapUtilsBinding.createHeapSnapshotStream(optionArray);
  assert(handle);
...
class DefaultDeserializer extends Deserializer {
  ...
  _readHostObject() {
    ...
    bufferBinding.copy(this.buffer, buffer_copy, 0, byteOffset, byteOffset + byteLength);
