...
const USAGE = `${I18N(I18N.NODE_DEBUGGER_USAGE)}:
...
exports.start = function(argv, stdin, stdout) {
  ...
  process.on('uncaughtException', function(e) {
    ...
    internalUtil.error(I18N(I18N.NODE_DEBUGGER_ERROR));
...
Protocol.prototype.execute = function(d) {
  ...
  switch (this.state) {
    ...
    default:
      throw I18N.Error(I18N.NODE_DEBUGGER_UNKNOWN_STATE);
  }
};
...
Client.prototype.mirrorObject = function(handle, depth, cb) {
  ...
  if (handle.type === 'object') {
    ...
    this.reqLookup(propertyRefs, function(err, res) {
      if (err) {
        internalUtil.error(I18N(I18N.NODE_DEBUGGER_PROBLEM_REQLOOKUP));
        cb(null, handle);
        return;
      }
...
Client.prototype.fullTrace = function(cb) {
  ...
  this.reqBacktrace(function(err, trace) {
    ...
    if (trace.totalFrames <= 0)
      return cb(I18N.Error(I18N.NODE_DEBUGGER_NO_FRAMES));
...
var helpMessage = `${I18N(I18N.NODE_DEBUGGER_COMMANDS)}: ${cmdList}`;
...
function SourceInfo(body) {
  var result = body.exception ?
    I18N(I18N.NODE_DEBUGGER_EXCEPTION_IN) :
    I18N(I18N.NODE_DEBUGGER_BREAK_IN);
...
Interface.prototype.requireConnection = function() {
  if (!this.client) {
    this.error(I18N(I18N.NODE_DEBUGGER_APP_NOT_RUNNING));
    return false;
  }
  return true;
};
...
Interface.prototype.run = function() {
  ...
  if (this.child) {
    this.error(I18N(I18N.NODE_DEBUGGER_APP_RUNNING));
...
Interface.prototype.list = function(delta) {
  ...
  client.reqSource(from, to, function(err, res) {
    if (err || !res) {
      self.error(I18N(I18N.NODE_DEBUGGER_CANNOT_LIST_SOURCE));
      self.resume();
      return;
    }
...
Interface.prototype.backtrace = function() {
  ...
  client.fullTrace(function(err, bt) {
    if (err) {
      self.error(I18N(I18N.NODE_DEBUGGER_CANNOT_REQUEST_BACKTRACE));
      self.resume();
      return;
    }

    if (bt.totalFrames == 0) {
      self.print(I18N(I18N.NODE_DEBUGGER_EMPTY_STACK));
    } else {
      var trace = [],
...
Interface.prototype.watchers = function() {
  ...
  function wait() {
    if (--waiting === 0) {
      if (verbose) self.print(`${I18N(I18N.NODE_DEBUGGER_WATCHERS)}: `);
...
Interface.prototype.setBreakpoint = function(script, line,
  ...
  if (script === undefined) {
    this.print(I18N(I18N.NODE_DEBUGGER_CANNOT_DETERMINE_SCRIPT));
    return;
  }
  ...
  if (/\(\)$/.test(script)) {
    ...
    if (ambiguous)
      return this.error(I18N(I18N.NODE_DEBUGGER_SCRIPT_NAME_AMBIGUOUS));
    if (line <= 0)
      return this.error(I18N(I18N.NODE_DEBUGGER_LINE_POSITIVE));
    ...
    if (scriptId) {
      ...
      this.print(I18N(I18N.NODE_DEBUGGER_SCRIPT_NOT_LOADED, script));
...
Interface.prototype.clearBreakpoint = function(script, line) {
  ...
  if (scriptId === undefined)
    return this.error(I18N(I18N.NODE_DEBUGGER_SCRIPT_NOT_FOUND, script));
...
Interface.prototype.repl = function() {
  ...
  self.print(I18N(I18N.NODE_DEBUGGER_REPL_EXIT));
...
Interface.prototype.trySpawn = function(cb) {
  ...
  if (this.args.length === 2) {
    ...
    if (this.args[1] === '-p' && /^\d+$/.test(this.args[2])) {
      ...
      try {
        ...
        if (e.code === 'ESRCH') {
          internalUtil.error(I18N(I18N.NODE_DEBUGGER_TARGET_PROCESS, pid));
          process.exit(1);
        }
        throw e;
      }
  ...
  client.once('ready', function() {
    self.stdout.write(` ${I18N(I18N.NODE_DEBUGGER_READY)}\n`);

    // Restore breakpoints
    breakpoints.forEach(function(bp) {
      self.print(`${I18N(I18N.NODE_DEBUGGER_RESTORING_BREAKPOINT)} ` +
    ...
    client.on('close', function() {
      self.pause();
      self.print(I18N(I18N.NODE_DEBUGGER_PROGRAM_TERMINATED));
      self.resume();
      self.client = null;
      self.killChild();
    });
  ...
  client.on('unhandledResponse', function(res) {
    self.pause();
    self.print(`\n${I18N(I18N.NODE_DEBUGGER_UNHANDLED_RESPONSE)}: ` +
               `${JSON.stringify(res)}`);
    self.resume();
  });
  ...
  function connectError() {
    ...
    if (connectionAttempts >= 10) {
      internalUtil.error(I18N(I18N.NODE_DEBUGGER_CONNECTION_FAILED));
      process.exit(1);
    }
  ...
  if (isRemote) {
    self.print(`${I18N(I18N.NODE_DEBUGGER_CONNECTING)} ${host}:${port} ..`,
               true);
    attemptConnect();
  } else {
