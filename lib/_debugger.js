  ...
  switch (this.state) {
    ...
    case 'body':
      ...
      if (resRawByteLength - this.bodyStartByteIndex >= this.contentLength) {
        const buf = Buffer.allocUnsafe(resRawByteLength);
...
function Client() {
  ...
  const protocol = this.protocol = new Protocol(this);
  this._reqCallbacks = [];
  const socket = this;
...
Client.prototype._onResponse = function(res) {
  ...
  if (cb) {
    ...
    const err = res.success === false && (res.message || true) ||
...
Client.prototype.reqScopes = function(cb) {
  ...
  this.req(req, function(err, res) {
    if (err) return cb(err);
    const refs = res.scopes.map(function(scope) {
    ...
    self.reqLookup(refs, function(err, res) {
      ...
      const globals = Object.keys(res).map(function(key) {
...
Client.prototype.reqEval = function(expression, cb) {
  ...
  this.reqBacktrace(function(err, bt) {
    ...
    const frame = bt.frames[self.currentFrame];

    const evalFrames = frame.scopes.map(function(s) {
      if (!s) return;
      const x = bt.frames[s.index];
      if (!x) return;
      return x.index;
    });
...
Client.prototype.mirrorObject = function(handle, depth, cb) {
  ...
  if (handle.type === 'object') {
    ...
    const propertyRefs = handle.properties.map(function(p) {
      return p.ref;
    });
...
const helpMessage = 'Commands: ' + commands.map(function(group) {
  return group.join(', ');
}).join(',\n');
...
function SourceUnderline(sourceText, position, repl) {
  ...
  const tail = sourceText.slice(position);
...
function SourceInfo(body) {
  const result = body.exception ? 'exception in ' : 'break in ';

  if (body.script) {
    if (body.script.name) {
      const name = body.script.name;
...
function Interface(stdin, stdout, args) {
  ...
  function defineProperty(key, protoKey) {
    // Check arity
    const fn = proto[protoKey].bind(self);
...
Interface.prototype.debugEval = function(code, context, filename, callback) {
  ...
  const frame = client.currentFrame === NO_FRAME ? frame : undefined;
  ...
  client.reqSource(from, to, function(err, res) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (lineno == 1) {
        ...
        // it.
        const wrapper = Module.wrapper[0];
...
Interface.prototype.watchers = function() {
  const self = this;
  const verbose = arguments[0] || false;
  const callback = arguments[1] || function() {};
  const waiting = this._watchers.length;
  const values = [];
...
Interface.prototype.setBreakpoint = function(script, line,
  ...
  if (script.endsWith('()')) {
    ...
    if (script != +script && !this.client.scripts[script]) {
      const scripts = this.client.scripts;
    ...
    if (scriptId) {
      ...
      const scriptPathRegex = '^(.*[\\/\\\\])?' + escapedPath + '$';
      req = {
        type: 'scriptRegExp',
...
Interface.prototype.repl = function() {
  ...
  const listeners = this.repl.rli.listeners('SIGINT').slice(0);
