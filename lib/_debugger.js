...
Client.prototype.reqFrame = function(frame, cb) {
  const req = {
		command: 'frame',
		arguments: {
			number: frame
...
Interface.prototype.frame = function(frame) {
  if (!this.requireConnection()) return;

	this.pause();
	this.client.reqFrame(frame, (err, r) => {
		if (err) {
			this.error(err);
		} else {
			// Save execution context's data
			this.client.currentSourceLine = r.line;
			this.client.currentSourceLineText = r.sourceLineText;
			this.client.currentSourceColumn = r.column;
			this.client.currentFrame = frame;

			// Print break data
			this.print('Frame selected: ' + frame);
		}
		this.resume();
	});
};
