...
exports.start = function(argv, stdin, stdout) {
  ...
  process.on('uncaughtException', function(e) {
    console.error(prefix + pid + "There was an internal error in Node's debugger. " +
...
Client.prototype.mirrorObject = function(handle, depth, cb) {
  ...
  if (handle.type === 'object') {
    ...
    this.reqLookup(propertyRefs, function(err, res) {
      if (err) {
        console.error(prefix + pid + 'problem with reqLookup');
        cb(null, handle);
        return;
      }
...
Interface.prototype.trySpawn = function(cb) {
  ...
  function connectError() {
    ...
    if (connectionAttempts >= 10) {
      console.error(prefix + pid + ' failed, please retry');
      process.exit(1);
    }
