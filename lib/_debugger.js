...
Client.prototype._onResponse = function(res) {
  ...
  this._reqCallbacks.some(function someReqCallbacks(fn, i) {
...
Client.prototype.reqVersion = function(cb) {
  ...
  this.req({ command: 'version' }, function reqWithVersion(err, body, res) {
...
Client.prototype.reqLookup = function(refs, cb) {
  ...
  cb = cb || function empty() {};
  this.req(req, function reqWithLookup(err, res) {
    if (err) return cb(err);
    for (var ref in res) {
...
Client.prototype.reqScopes = function(cb) {
  ...
  cb = cb || function empty() {};
  this.req(req, function reqWithScopes(err, res) {
    if (err) return cb(err);
    var refs = res.scopes.map(function mapScopes(scope) {
      return scope.object.ref;
    });

    self.reqLookup(refs, function reqWithSelfLookup(err, res) {
      if (err) return cb(err);

      var globals = Object.keys(res).map(function mapKeys(key) {
        return res[key].properties.map(function mapProps(prop) {
          return prop.name;
        });
      });
...
Client.prototype.reqEval = function(expression, cb) {
  ...
  this.reqBacktrace(function reqWithBacktrace(err, bt) {
    if (err || !bt.frames) {
      // ??
      return cb(null, {});
    }
    ...
    var evalFrames = frame.scopes.map(function mapScopes(s) {
      if (!s) return;
...
Client.prototype._reqFramesEval = function(expression, evalFrames, cb) {
  ...
  cb = cb || function empty() {};
  this.reqFrameEval(expression, i, function doEval(err, res) {
...
Client.prototype.reqFrameEval = function(expression, frame, cb) {
  ...
  cb = cb || function empty() {};
  this.req(req, function reqAddHandle(err, res) {
...
Client.prototype.reqScripts = function(cb) {
  ...
  this.req({ command: 'scripts' }, function reqWithScripts(err, res) {
    if (err) return cb(err);
...
Client.prototype.mirrorObject = function(handle, depth, cb) {
  ...
  if (handle.type === 'object') {
    ...
    var propertyRefs = handle.properties.map(function mapProps(p) {
      return p.ref;
    });

    cb = cb || function empty() {};
    this.reqLookup(propertyRefs, function reqLookupWithProps(err, res) {
      if (err) {
      ...
      var keyValues = [];
      handle.properties.forEach(function handleProps(prop, i) {
        ...
        if (value && value.handle && depth > 0) {
          waiting++;
          self.mirrorObject(value, depth - 1, function mirror(err, result) {
      ...
      function waitForOthers() {
        if (--waiting === 0) {
          keyValues.forEach(function mirrorKeyValues(kv) {
...
Client.prototype.fullTrace = function(cb) {
  ...
  cb = cb || function empty() {};
  this.reqBacktrace(function backtrace(err, trace) {
    if (err) return cb(err);
...
var helpMessage = 'Commands: ' + commands.map(function mapCommands(group) {
  return group.join(', ');
}).join(',\n');
...
function Interface(stdin, stdout, args) {
  ...
  if (parseInt(process.env['NODE_NO_READLINE'], 10)) {
    ...
    if (!this.stdout.isTTY) {
      process.on('SIGINT', function emitSigInt() {
  ...
  this.repl.on('exit', function onExit() {
    process.exit(0);
  });
...
Interface.prototype.childPrint = function(text) {
  this.print(text.toString().split(/\r\n|\r|\n/g).filter(function fcnk(chunk) {
    return chunk;
  }).map(function mapChunck(chunk) {
    return '< ' + chunk;
  }).join('\n'));
...
Interface.prototype.handleBreak = function(r) {
  ...
  this.watchers(true, function showValues(err) {
...
Interface.prototype.debugEval = function(code, context, filename, callback) {
  ...
  client.reqFrameEval(code, frame, function requestRemoteEval(err, res) {
    if (err) {
      callback(err);
      self.resume(true);
      return;
    ...
    client.mirrorObject(res, 3, function requestByHandles(err, mirror) {
...
Interface.prototype.restart = function() {
  ...
  setTimeout(function selfTimeout() {
    self.trySpawn();
    self.resume();
  }, 1000);
};
...
Interface.prototype.version = function() {
  ...
  this.pause();
  this.client.reqVersion(function reqVersionWithClient(err, v) {
    if (err) {
      self.error(err);
    } else {
      self.print(v);
...
Interface.prototype.list = function(delta) {
  ...
  self.pause();
  client.reqSource(from, to, function reqSourceWithClient(err, res) {
    if (err || !res) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      const breakpoint = client.breakpoints.some(function someBreakpoints(bp) {
...
Interface.prototype.backtrace = function() {
  ...
  self.pause();
  client.fullTrace(function clientFulltrace(err, bt) {
    if (err) {
...
Interface.prototype.cont = function() {
  ...
  var self = this;
  this.client.reqContinue(function clientContinue(err) {
...
Interface.stepGenerator = function(type, count) {
  return function setpCommands() {
    if (!this.requireConnection()) return;

    var self = this;

    self.pause();
    self.client.step(type, count, function clientStep(err, res) {
      if (err) self.error(err);
      self.resume();
    });
  };
...
Interface.prototype.watchers = function() {
  ...
  var callback = arguments[1] || function empty() {};
  ...
  this._watchers.forEach(function watchThis(watcher, i) {
    self.debugEval(watcher, null, null, function selfDebugEval(err, value) {
  ...
  function wait() {
    if (--waiting === 0) {
      ...
      self._watchers.forEach(function watchSelf(watcher, i) {
...
Interface.prototype.breakOnException = function breakOnException() {
  ...
  // Break on exceptions
  this.pause();
  this.client.reqSetExceptionBreak('all', function breakAll(err, res) {
    self.resume();
  });
};
...
Interface.prototype.setBreakpoint = function(script, line,
  ...
  self.pause();
  self.client.setBreakpoint(req, function setClientBp(err, res) {
    if (err) {
      if (!silent) {
        self.error(err);
      }
...
Interface.prototype.clearBreakpoint = function(script, line) {
  ...
  this.client.breakpoints.some(function someClientBp(bp, i) {
  ...
  self.pause();
  self.client.clearBreakpoint(req, function clearClientBp(err, res) {
    if (err) {
      self.error(err);
    } else {
...
Interface.prototype.breakpoints = function() {
  ...
  this.pause();
  var self = this;
  this.client.listbreakpoints(function listClientBp(err, res) {
    if (err) {
      self.error(err);
    } else {
      self.print(res);
...
Interface.prototype.pause_ = function() {
  ...
  this.pause();
  this.client.reqFrameEval(cmd, NO_FRAME, function doEval(err, res) {
    if (err) {
      self.error(err);
    } else {
      self.resume();
...
Interface.prototype.repl = function() {
  ...
  function exitDebugRepl() {
    ...
    process.nextTick(function tick() {
      listeners.forEach(function listenOnSigInt(listener) {
...
Interface.prototype.trySpawn = function(cb) {
  ...
  client.once('ready', function onReady() {
    ...
    // Restore breakpoints
    breakpoints.forEach(function restoreBp(bp) {
  ...
  client.on('unhandledResponse', function onUnhandledResponse(res) {
    self.pause();
    self.print('\nunhandled res:' + JSON.stringify(res));
    self.resume();
  });

  client.on('break', function onBreak(res) {
    self.handleBreak(res.body);
  });

  client.on('exception', function onException(res) {
    self.handleBreak(res.body);
  });
  ...
  if (isRemote) {
    ...
    this.child.stderr.once('data', function onData() {
      self.print('connecting to ' + host + ':' + port + ' ..', true);
