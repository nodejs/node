...
Client.prototype.reqScopes = function(cb) {
  ...
  this.req(req, (err, res) => {
    if (err) return cb(err);
    var refs = res.scopes.map(scope => scope.object.ref);

    this.reqLookup(refs, (err, res) => {
      if (err) return cb(err);

      var globals = Object.keys(res).map((key) => {
        return res[key].properties.map(prop => prop.name);
      });
...
Client.prototype.reqEval = function(expression, cb) {
  ...
  this.reqBacktrace((err, bt) => {
    ...
    var evalFrames = frame.scopes.map((s) => {
      if (!s) return;
    ...
    this.reqLookup(propertyRefs, (err, res) => {
      ...
      handle.properties.forEach((prop, i) => {
        ...
        if (value && value.handle && depth > 0) {
          waiting++;
          this.mirrorObject(value, depth - 1, (err, result) => {
...
const helpMessage = 'Commands: ' +
                    commands.map(group => group.join(', ')).join(',\n');
...
Interface.prototype.list = function(delta) {
  ...
  client.reqSource(from, to, (err, res) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      var line = current ?
          SourceUnderline(lines[i],
                          client.currentSourceColumn,
                          this.repl) : lines[i];
      var prefixChar = current ? '>' : breakpoint ? '*' : ' ';
...
Interface.prototype.backtrace = function() {
  ...
  client.fullTrace((err, bt) => {
    ...
    if (bt.totalFrames == 0) {
      ...
      for (let i = 0; i < bt.frames.length; i++) {
...
Interface.prototype.scripts = function() {
  ...
  this.pause();
  for (const id in client.scripts) {
...
Interface.prototype.watchers = function() {
  ...
  this._watchers.forEach((watcher, i) => {
    this.debugEval(watcher, null, null, (err, value) => {
    ...
    if (--waiting === 0) {
      ...
      self._watchers.forEach((watcher, i) => {
        self.print(leftPad(i, ' ', self._watchers.length - 1) +
                   `: ${watcher} = ${JSON.stringify(values[i])}`);
      });
...
Interface.prototype.clearBreakpoint = function(script, line) {
  ...
  this.client.breakpoints.some((bp, i) => {
...
Interface.prototype.pause_ = function() {
  ...
  this.pause();
  this.client.reqFrameEval('process._debugPause();', NO_FRAME, (err, res) => {
    if (err) {
      this.error(err);
    } else {
      this.resume();
...
Interface.prototype.repl = function() {
  ...
  function exitDebugRepl(self) {
    ...
    self.repl.rli.removeListener('SIGINT', () => exitDebugRepl(this));
    self.repl.removeListener('exit', () => exitDebugRepl(this));
  }

  // Exit debug repl on SIGINT
  this.repl.rli.on('SIGINT', () => exitDebugRepl(this));

  // Exit debug repl on repl exit
  this.repl.on('exit', () => exitDebugRepl(this));
...
Interface.prototype.trySpawn = function(cb) {
  ...
  client.on('error', () => {
    ...
    setTimeout(() => {attemptConnect(this);}, 500);
  });
  ...
  if (isRemote) {
    ...
    this.child.stderr.once('data', () => {
      this.print(`connecting to ${host}:${port} ..`, true);
      setImmediate(() => {attemptConnect(this);});
    });
  }
};
