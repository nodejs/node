...
ServerResponse.prototype.writeHead = function(statusCode, reason, obj) {
  ...
  const statusLine = 'HTTP/1.1 ' + statusCode.toString() + ' ' +
...
function connectionListener(socket) {
  ...
  socket.on('timeout', function() {
    ...
    const serverTimeout = self.emit('timeout', socket);
  ...
  function socketOnDrain() {
    const needPause = outgoingData > socket._writableState.highWaterMark;
  ...
  function parserOnIncoming(req, shouldKeepAlive) {
    ...
    if (!socket._paused) {
      const needPause = socket._writableState.needDrain ||
