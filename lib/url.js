...
Url.prototype.format = function() {
  var auth = '';
  if (this.username) {
    auth = encodeField(this.username);
    if (this.password)
      auth += ':' + encodeField(this.password);
    auth += '@';
  } else if (this.auth) {
  ...
  if (isRelAbs) {
    ...
    if (psychotic) {
      ...
      if (result.host && result.host.indexOf('@') > 0) {
        ...
        result.parseAuth(authInHost.shift());
...
//this adheres to RFC 3986 and conforms to the browsers
function encodeField(input) {
  const stripped = input.replace(/[\r\n\t]/g, '');
  //Ensure the field is URI encoded.
  //Technically, the username and password field should be already encoded.
  //Realistically, they may not always be. But often they can still be parsed
  //successfully. So decode before encode to avoid double encoding.
  const decoded = decodeURIComponent(stripped);
  return encodeURIComponent(decoded).replace(/[!'()*]/g, function(chr) {
    return '%' + chr.charCodeAt(0).toString(16);
  });
}

Url.prototype.parseAuth = function(auth) {
  this.auth = decodeURIComponent(auth);

  var colon = auth.indexOf(':');
  if (colon !== -1) {
    this.username = encodeField(auth.slice(0, colon));
    this.password = encodeField(auth.slice(colon + 1));
  } else {
    this.username = encodeField(auth);
  }
};
