...
const noSerializePattern = /^(?:_protocol|_port|_href|_query|_prependSlash)$/;
...
Url.prototype.parse = function(str, parseQueryString, hostDenotesSlash) {
  ...
  if (!this.pathname && this.hostname && _slashProtocols[this._protocol])
    this.pathname = '/';
...
Url.prototype._parseHost = function(str,
  ...
  if (str.charCodeAt(start) === 0x5B /*'['*/) {
    for (var i = start + 1; i <= end; ++i) {
      ...
      if (ch === 0x5D /*']'*/) {
        ...
        this.host = this._hasValidPort() ?
            '[' + hostname + ']:' + this._port : '[' + hostname + ']';
        this.pathname = '/';
  ...
  if (hostNameEnd + 1 !== start &&
    ...
    this.hostname = hostname;
    this.host = this._hasValidPort() ? hostname + ':' + this._port : hostname;
  }
...
Url.prototype._parseQuery = function(str, start, end) {
  ...
  var query = escape ?
      this._getComponentEscaped(str, queryStart, queryEnd, true) :
...
Object.defineProperty(Url.prototype, 'query', {
  ...
  set: function(v) {
    if (typeof v === 'string') {
      ...
      this.search = string !== '' ? '?' + querystring.stringify(v) : '';
      this._query = v;
    } else {
      this._query = this.search = null;
    }
  },
  enumerable: true,
  configurable: true
...
Object.defineProperty(Url.prototype, 'path', {
  ...
  set: function(path) {
    if (path == null) {
      this.pathname = this.search = this._query = null;
      return;
    }
    ...
    if (this._prependSlash && !/^\//.test(pathname))
      pathname = '/' + pathname;
