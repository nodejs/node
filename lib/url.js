...
Url.prototype.resolveObject = function(relative) {
  ...
  let srcPath = !this._isEmpty(result.pathname) &&
      result.pathname.split('/') || [];
  const relPath = !this._isEmpty(relative.pathname) &&
      relative.pathname.split('/') || [];
  const psychotic = !this._isEmpty(result.protocol) &&
  ...
  if (isRelAbs) {
    ...
    if (psychotic) {
      ...
      const authInHost = !this._isEmpty(result.host) &&
...
Url.prototype._parseHost = function(str,
  ...
  if (hostnameEnd + 1 !== start &&
    ...
    let hostname = str.slice(hostNameStart, hostnameEnd + 1);
    if (needsLowerCasing)
