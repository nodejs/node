...
EventEmitter.prototype._addListener = function _addListener(type, fn, once) {
  var m;
  var events;
  var existing;
  ...
  if (!existing) {
    ...
    if (once)
      existing = events[type] = { once: fn, fired: false };
    else
      existing = events[type] = fn;
    ++this._eventsCount;
  } else {
    ...
    if (typeof existing === 'function' || existingOnce) {
      ...
      if (once) {
        ...
        existing = events[type] = [existing, fn];
        existing.onceCount = (existingOnce ? 1 : 0);
      }
    } else {
      // If we've already got an array, just append.
      if (once) {
        existing.push({ once: fn, fired: false });
        ++existing.onceCount;
      } else
        existing.push(fn);
    }
...
EventEmitter.prototype.on = function on(type, fn) {
  if (typeof this._addListener !== 'function')
    return EventEmitter.prototype._addListener.call(this, type, fn, false);
  return this._addListener(type, fn, false);
};
...
EventEmitter.prototype.once = function once(type, fn) {
  if (typeof this._addListener !== 'function')
    return EventEmitter.prototype._addListener.call(this, type, fn, true);
  return this._addListener(type, fn, true);
};
...
function listenerCount(type) {
  ...
  if (events) {
    ...
    if (typeof evlistener === 'function' ||
        (typeof evlistener === 'object' &&
         evlistener !== null &&
         typeof evlistener.once === 'function')) {
      return 1;
    } else if (evlistener) {
