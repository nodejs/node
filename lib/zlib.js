...
function Zlib(opts, mode) {
  ...
  const level = exports.Z_DEFAULT_COMPRESSION;
  if (typeof opts.level === 'number') level = opts.level;

  const strategy = exports.Z_DEFAULT_STRATEGY;
...
Zlib.prototype._processChunk = function(chunk, flushFlag, cb) {
  const availInBefore = chunk && chunk.length;
  ...
  if (!async) {
    ...
    assert(!this._closed, 'zlib binding closed');
    do {
      const res = this._handle.writeSync(flushFlag,
                                       chunk, // in
                                       inOff, // in_off
  ...
  assert(!this._closed, 'zlib binding closed');
  const req = this._handle.write(flushFlag,
                               chunk, // in
                               inOff, // in_off
                               availInBefore, // in_len
                               this._buffer, // out
  ...
  function callback(availInAfter, availOutAfter) {
    ...
    if (availOutAfter === 0) {
      ...
      const newReq = self._handle.write(flushFlag,
                                      chunk,
                                      inOff,
                                      availInBefore,
                                      self._buffer,
