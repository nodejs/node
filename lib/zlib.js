...
function zlibBuffer(engine, buffer, callback) {
  ...
  function onEnd() {
    ...
    if (nread >= kMaxLength) {
      err = new errors.RangeError('BUFFERTOOLARGE', kMaxLength);
    } else {
...
function zlibBufferSync(engine, buffer) {
  ...
  if (!(buffer instanceof Buffer))
    throw new errors.TypeError('INVALIDARG', 'buffer', ['Buffer', 'string']);
...
Zlib.prototype.params = function(level, strategy, callback) {
  if (level < exports.Z_MIN_LEVEL ||
    ...
    throw new errors.RangeError('INVALIDARGVALUE', 'level', level);
  }
  if (strategy != exports.Z_FILTERED &&
    ...
    throw new errors.TypeError('INVALIDARGVALUE', 'strategy', strategy);
  }
...
Zlib.prototype._processChunk = function(chunk, flushFlag, cb) {
  ...
  if (!async) {
    ...
    if (nread >= kMaxLength) {
      _close(this);
      throw new errors.RangeError('BUFFERTOOLARGE', kMaxLength);
    }
