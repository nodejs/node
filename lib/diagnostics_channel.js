...
function incRefIfNew(channel, isNew) {
  if (isNew) {
    channels.incRef(channel.name);
    subscriberCounts[channel._index]++;
  }
}

function decRefAndMaybeInactive(channel) {
  channels.decRef(channel.name);
  subscriberCounts[channel._index]--;
  maybeMarkInactive(channel);
}
