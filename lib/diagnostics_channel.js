...
class WindowChannelScope {
  ...
  [SymbolDispose]() {
    ...
    // Clear scope first to make idempotent
    const scope = this.#scope;
    this.#scope = undefined;

    // Copy all enumerable own properties back to context
    for (const key of ObjectKeys(this)) {
...
class TracingChannel {
  ...
  tracePromise(fn, context = {}, thisArg, ...args) {
    ...
    function reject(err) {
      context.error = err;
      error.publish(context);
      // Just publish asyncStart/asyncEnd without running stores
      continuationWindow.start.publish(context);
      // TODO: Is there a way to have asyncEnd _after_ the continuation?
      continuationWindow.end.publish(context);
