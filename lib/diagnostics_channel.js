...
class Channel {
  ...
  subscribe(subscription) {
    ...
    this.#ensureActive();
    this.#subscribers = ArrayPrototypeSlice(this.#subscribers);
    ArrayPrototypePush(this.#subscribers, subscription);
  ...
  unsubscribe(subscription) {
    ...
    const index = ArrayPrototypeIndexOf(this.#subscribers, subscription);
    if (index === -1) return false;

    const before = ArrayPrototypeSlice(this.#subscribers, 0, index);
    const after = ArrayPrototypeSlice(this.#subscribers, index + 1);
    this.#subscribers = before;
    ArrayPrototypePushApply(this.#subscribers, after);
  ...
  bindStore(store, transform) {
    this.#ensureActive();
    const replacing = this.#stores.has(store);
    if (!replacing) channels.incRef(this.name);
    this.#stores.set(store, transform);
  }

  unbindStore(store) {
    if (!this.#active || !this.#stores.has(store)) {
      return false;
    }
  ...
  runStores(data, fn, thisArg, ...args) {
    ...
    for (const { 0: store, 1: transform } of this.#stores) {
  ...
  static [SymbolHasInstance](instance) {
    return ObjectGetPrototypeOf(instance) === Channel.prototype;
  }
}
