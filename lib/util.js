...
function formatValue(ctx, value, recurseTimes) {
  ...
  if (ctx.customInspect &&
    ...
    const ret = value.inspect(recurseTimes, ctx);
  ...
  const base = '', empty = false, braces;
  const formatter = formatObject;
  ...
  if (Array.isArray(value)) {
    ...
    const promiseInternals = inspectPromise(value);
    if (promiseInternals) {
      braces = ['{', '}'];
...
function formatSet(ctx, value, recurseTimes, visibleKeys, keys) {
  ...
  value.forEach(function(v) {
    ...
    const str = formatValue(ctx, v, nextRecurseTimes);
    output.push(str);
  });
  keys.forEach(function(key) {
    output.push(formatProperty(ctx, value, recurseTimes, visibleKeys,
...
function formatMap(ctx, value, recurseTimes, visibleKeys, keys) {
  ...
  value.forEach(function(v, k) {
    const nextRecurseTimes = recurseTimes === null ? null : recurseTimes - 1;
    const str = formatValue(ctx, k, nextRecurseTimes);
    str += ' => ';
...
function formatPromise(ctx, value, recurseTimes, visibleKeys, keys) {
  const output = [];
  const internals = inspectPromise(value);
  if (internals.status === 'pending') {
    output.push('<pending>');
  } else {
    const nextRecurseTimes = recurseTimes === null ? null : recurseTimes - 1;
    const str = formatValue(ctx, internals.value, nextRecurseTimes);
