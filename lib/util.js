...
function formatValue(ctx, value, recurseTimes) {
  ...
  if (ctx.showProxy && typeof value === 'object') {
    const inProxyCache = ctx.proxyCache.has(value);
    var proxy = inProxyCache ? ctx.proxyCache.get(value) : undefined;
    if (inProxyCache && proxy) {
      return 'Proxy ' + formatValue(ctx, proxy, recurseTimes);
    } else if (!inProxyCache) {
      if (binding.isProxy(value)) {
        proxy = binding.getProxyDetails(value);
        ctx.proxyCache.set(value, proxy);
        return 'Proxy ' + formatValue(ctx, proxy, recurseTimes);
      } else {
        ctx.proxyCache.set(value, undefined);
      }
    }
  }
