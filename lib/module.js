...
Module._debug = I18N.debuglog('module');
    ...
    if (filename) {
      ...
      if (request === '.' && i > 0) {
        warned = internalUtil.printDeprecationMessage(
          I18N(I18N.MODULE_REQUIRE_OUTSIDE_WARNING), warned);
      }
...
Module._resolveLookupPaths = function(request, parent) {
  ...
  debug(I18N.MODULE_DEBUG_RELATIVE_REQUESTED, request, id, parent.id);
...
Module._load = function(request, parent, isMain) {
  if (parent) {
    debug(I18N.MODULE_DEBUG_LOAD_REQUEST, request, parent.id);
  }
  ...
  if (NativeModule.nonInternalExists(filename)) {
    debug(I18N.MODULE_DEBUG_LOAD_NATIVE, request);
...
Module._resolveFilename = function(request, parent) {
  ...
  debug(I18N.MODULE_DEBUG_LOOKING_FOR,
        JSON.stringify(id), JSON.stringify(paths));

  var filename = Module._findPath(request, paths);
  if (!filename) {
    var err = new I18N.Error(I18N.MODULE_NOT_FOUND, request);
...
Module.prototype.load = function(filename) {
  debug(I18N.MODULE_DEBUG_LOAD, filename, this.id);
...
Module.prototype.require = function(path) {
  assert(path, I18N(I18N.MODULE_ASSERT_MISSING_PATH));
  assert(typeof path === 'string', I18N(I18N.MODULE_ASSERT_PATH_STRING));
...
}, I18N(I18N.MODULE_REQUIREREPL_DEPRECATED));
