...
Module._nodeModulePaths = function(from) {
  ...
  for (var tip = parts.length - 1; tip >= 0; tip--) {
    ...
    const dir = parts.slice(0, tip + 1).concat('node_modules').join(path.sep);
    paths.push(dir);
  }
...
Module._resolveLookupPaths = function(request, parent) {
  ...
  if (!parent || !parent.id || !parent.filename) {
    ...
    const mainPaths = ['.'].concat(modulePaths);
  ...
  const isIndex = isIndexRe.test(path.basename(parent.filename));
...
Module._initPaths = function() {
  ...
  const paths = [path.resolve(process.execPath, '..', '..', 'lib', 'node')];
