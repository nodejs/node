...
function Interface(input, output, completer, terminal, saveHistory) {
  ...
  if (!this.terminal) {
    ...
    this.history = saveHistory ?
        new ReadlineFileHistory({ maxEntries: kHistorySize }) :
        new ReadlineHistory({ maxEntries: kHistorySize });
...
Interface.prototype._addHistory = function() {
  ...
  this.history.addEntry(this.line);
  return this.line;
};
...
Interface.prototype._historyNext = function() {
  var hline = this.history.nextEntry();
  if (this.line != hline) {
    this.line = hline;
...
Interface.prototype._historyPrev = function() {
  var hline = this.history.prevEntry();
  if (this.line != hline) {
    this.line = hline;
    this.cursor = this.line.length; // set cursor to end of line.
    this._refreshLine();
...
function ReadlineHistory(options) {
  this._maxEntries = options.maxEntries;
  this._index = 0;
  this._entries = [];
}

ReadlineHistory.prototype.addEntry = function(line) {
  var added = false;
  if (line && this._entries[this._entries.length - 1] !== line) {
    if (this._maxEntries == this._entries.length) {
      this._entries.shift();
    }
...
ReadlineHistory.prototype.nextEntry = function() {
  this._index += 1;
...
ReadlineHistory.prototype.prevEntry = function() {
  this._index -= 1;
...
function ReadlineFileHistory(options) {
  ...
  ReadlineHistory.call(this, options);

  this._load();
}

inherits(ReadlineFileHistory, ReadlineHistory);

ReadlineFileHistory.prototype._load = function() {
  try {
    var lines = fs.readFileSync(this._filePath, {encoding: this._fileEncoding})
      .split('\n');
    lines.pop();//last line always empty
    ...
    //ignore errors, just init empty history
    this._entries = [];
    this._index = 0;
  }
};

ReadlineFileHistory.prototype.addEntry = function(line) {
  if (ReadlineHistory.prototype.addEntry.call(this, line)) {
