...
const processMethodsBinding = internalBinding('process_methods');
...
class Session extends EventEmitter {
  ...
  connect() {
    ...
    this.#connection = new inspectorBinding.Connection((message) => this.#onMessage(message));
  }
  ...
  connectToMainThread() {
    ...
    if (this.#connection)
      throw new ERR_INSPECTOR_ALREADY_CONNECTED('The inspector session');
    this.#connection =
      new inspectorBinding.MainThreadConnection(
...
function inspectorOpen(port, host, wait) {
  ...
  inspectorBinding.open(port, host);
  if (wait)
    inspectorBinding.waitForDebugger();

  return { __proto__: null, [SymbolDispose]() { processMethodsBinding._debugEnd(); } };
}
...
function inspectorWaitForDebugger() {
  if (!inspectorBinding.waitForDebugger())
...
function broadcastToFrontend(eventName, params) {
  ...
  inspectorBinding.emitProtocolEvent(eventName, JSONStringify(params ?? {}));
}
...
module.exports = {
  open: inspectorOpen,
  close: processMethodsBinding._debugEnd,
  url: inspectorBinding.url,
  waitForDebugger: inspectorWaitForDebugger,
  console: inspectorBinding.console,
  Session,
  Network,
};
