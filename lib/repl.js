...
function REPLServer(prompt,
  ...
  function defaultEval(code, context, file, cb) {
    let err, result, retry = false, wrappedErr, script;
    const input = code;
    ...
    while (true) {
      try {
        ...
        if (this.replMode === exports.REPL_MODE_MAGIC &&
            ...
            !retry || this.wrappedCmd) {
          if (this.wrappedCmd) {
            this.wrappedCmd = false;
        ...
        if (isRecoverableError(error, this))
    ...
    if (!err) {
      ...
      if (this.breakEvalOnSigint) {
        // Start the SIGINT watchdog before entering raw mode so that a very
        // quick Ctrl+C doesn’t lead to aborting the process completely.
        utilBinding.startSigintWatchdog();
        previouslyInRawMode = this._setRawMode(false);
      }
      ...
      try {
        try {
          const scriptOptions = {
            displayErrors: false,
            breakOnSigint: this.breakEvalOnSigint
          };
          ...
          if (this.breakEvalOnSigint) {
            ...
            this._setRawMode(previouslyInRawMode);
  ...
  this.on('line', (cmd) => {
    ...
    const finish = (function(_repl) {
      return function finish(e, ret) {
        debug('finish', e, ret);
        _repl.memory(cmd);

        _repl.wrappedCmd = false;
        if (e && !_repl.bufferedCommand && cmd.trim().startsWith('npm ')) {
          _repl.outputStream.write('npm should be run outside of the ' +
                                  'node repl, in your normal shell.\n' +
                                  '(Press Control-D to exit.)\n');
          _repl.lineParser.reset();
          _repl.bufferedCommand = '';
          _repl.displayPrompt();
          return;
        }
        ...
        if (e) {
          if (e instanceof Recoverable && !_repl.lineParser.shouldFail) {
            // Start buffering data like that:
            // {
            // ...  x: 1
            // ... }
            _repl.bufferedCommand += cmd + '\n';
            _repl.displayPrompt();
            return;
          } else {
            _repl._domain.emit('error', e.err || e);
          }
        }
        ...
        if (!e &&
            ...
            arguments.length === 2 &&
            (!_repl.ignoreUndefined || ret !== undefined)) {
          if (!_repl.underscoreAssigned) {
            _repl.last = ret;
          }
          _repl.outputStream.write(_repl.writer(ret) + '\n');
        }
    ...
    let evalCmd = this.bufferedCommand + cmd;
  ...
  this.on('SIGCONT', () => this.displayPrompt(true));
  this.displayPrompt();
}
...
REPLServer.prototype.complete = function(line, callback) {
  ...
  if (this.bufferedCommand !== undefined && this.bufferedCommand.length) {
    ...
    const lineArray = this.lines.slice();
    ...
    flat.run(lineArray);                        // eval the flattened code
  ...
  let completeOn, i, group, c;

  // REPL commands (e.g. ".break").
  let match = null, filter;
  ...
  if (match) {
    ...
    const paths = module.paths.concat(require('module').globalPaths);
    let dir, files, f, name, base, ext, abs, subfiles, s;
    ...
    if (depth) {
      (function workIt(_repl) {
        if (depth > 0) {
          ...
          _repl.lines.level.push({
            line: _repl.lines.length - 1,
            depth: depth,
            isFunction: /\s*function\s*/.test(cmd)
          });
        } else if (depth < 0) {
          // going... up.
          var curr = _repl.lines.level.pop();
          if (curr) {
...
REPLServer.prototype.convertToContext = function(cmd) {
  ...
  if (matches && matches.length === 3) {
    return 'this.context.' + matches[1] + matches[2];
  }
...
function isRecoverableError(e, _repl) {
  if (e && e.name === 'SyntaxError') {
    var message = e.message;
    if (message === 'Unterminated template literal' ||
        message === 'Missing } in template expression') {
      _repl._inTemplateLiteral = true;
      return true;
    }
    ...
    if (message === 'Invalid or unexpected token')
      return !bailOnIllegalToken(_repl.lineParser);
  }
  return false;
}
