...
function REPLServer(prompt,
  ...
  const options, input, output, dom;
  ...
  function defaultEval(code, context, file, cb) {
    const err, result, retry = false, input = code, wrappedErr;
...
REPLServer.prototype.complete = function(line, callback) {
  ...
  if (match) {
    ...
    const dir, files, f, name, base, ext, abs, subfiles, s;
    group = [];
    ...
    if (line.length === 0 || match) {
      ...
      if (!expr) {
        ...
        if (this.useGlobal || vm.isContext(this.context)) {
          const contextProto = this.context;
...
REPLServer.prototype.memory = function memory(cmd) {
  ...
  if (cmd) {
    ...
    // going up is } and )
    const dw = cmd.match(/{|\(/g);
    const up = cmd.match(/}|\)/g);
    up = up ? up.length : 0;
    dw = dw ? dw.length : 0;
    const depth = dw - up;
...
/**
 * Converts commands that use const and function <name>() to use the
...
REPLServer.prototype.convertToContext = function(cmd) {
  ...
  // Replaces: const foo = "bar";  with: self.context.foo = bar;
