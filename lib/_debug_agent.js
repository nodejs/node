...
exports.start = function start() {
  ...
  agent.listen(process._debugAPI.port, function() {
    ...
    process._rawDebug(I18N(I18N.NODE_DEBUGAGENT_LISTENING, addr.port));
...
function Agent() {
  ...
  this.clients = [];
  assert(this.binding, I18N(I18N.NODE_DEBUGAGENT_NO_BINDINGS));
}
...
Client.prototype._transform = function _transform(data, enc, cb) {
  ...
  while (true) {
    if (this.state === 'headers') {
      ...
      if (!match)
        return this.destroy(I18N(I18N.NODE_DEBUGAGENT_EXPECTED_HEADER));
      ...
      if (len === undefined)
        return this.destroy(I18N(I18N.NODE_DEBUGAGENT_EXPECTED_CONTENT_LENGTH));
