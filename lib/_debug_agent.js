...
exports.start = function start() {
  ...
  agent.listen(process._debugAPI.port, () => {
    process._rawDebug(`Debugger listening on port ${agent.address().port}`);
...
const matchHeaderValue = /^([^:\s\r\n]+)\s*:\s*([^\s\r\n]+)\r\n/;
const matchNewLine = /\r\n/;
const matchStartNewLine = /^\r\n/;
...
Client.prototype._transform = function _transform(data, enc, cb) {
  ...
  while (true) {
    if (this.state === 'headers') {
      // Not enough data
      if (!matchNewLine.test(this.buffer))
        break;

      if (matchStartNewLine.test(this.buffer)) {
      ...
      var match = this.buffer.match(matchHeaderValue);
      if (!match)
