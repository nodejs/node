...
const debug = I18N.debuglog('stream_wrap');
...
function StreamWrap(stream) {
  ...
  handle.close = function(cb) {
    debug(I18N.STREAM_WRAP_DEBUG_CLOSE);
    self.doClose(cb);
  };
  ...
  this.stream.on('data', function ondata(chunk) {
    if (!(chunk instanceof Buffer)) {
      ...
      self.emit('error', new I18N.Error(I18N.STREAM_WRAP_HAS_STRINGDECODER));
      return;
    }

    debug(I18N.STREAM_WRAP_DEBUG_DATA, chunk.length);
    if (self._handle)
      self._handle.readBuffer(chunk);
  });
  this.stream.once('end', function onend() {
    debug(I18N.STREAM_WRAP_DEBUG_END);
    if (self._handle)
