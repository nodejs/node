...
exports.checkServerIdentity = function checkServerIdentity(host, cert) {
  ...
  var dnsNames = [],
      ...
      matchCN = true,
      valid = false,
      reason = I18N(I18N.TLS_UNKNOWN_REASON);
  ...
  if (net.isIP(host)) {
    ...
    if (!valid)
      reason = I18N(I18N.TLS_HOST_NOT_IN_CERT_LIST, host, ips.join(', '));
    ...
    if (!valid) {
      if (cert.subjectaltname) {
        reason = I18N(I18N.TLS_HOST_NOT_IN_CERT_ALTNAMES,
                      host, cert.subjectaltname);
      } else {
        reason = I18N(I18N.TLS_HOST_NOT_IN_CERT_CN, host, cert.subject.CN);
      }
    }
  } else {
    reason = I18N(I18N.TLS_CERT_IS_EMPTY);
  }

  if (!valid) {
    var err = new I18N.Error(I18N.TLS_HOSTNAME_NO_MATCH, reason);
    err.reason = reason;
    err.host = host;
    err.cert = cert;
    return err;
