...
const cachedResultCallback = () => {
  return internalUtil.filterDuplicateStrings(binding.getSSLCiphers(), true);
};
exports.getCiphers = internalUtil.cachedResult(cachedResultCallback);

// Convert protocols array into valid OpenSSL protocols list
// ("\x06spdy/2\x08http/1.1\x08http/1.0")
function convertProtocols(protocols) {
  const lens = new Array(protocols.length);
  const protocolsReduceLambda = (p, c, i) => {
    var len = Buffer.byteLength(c);
    lens[i] = len;
    return p + 1 + len;
  };
  const buff = Buffer.allocUnsafe(protocols.reduce(protocolsReduceLambda, 0));
