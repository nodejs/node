...
function convertProtocols(protocols) {
  const buff = Buffer.allocUnsafe(protocols.reduce(function(p, c) {
    return p + 1 + Buffer.byteLength(c);
  }, 0));

  protocols.reduce(function(offset, c) {
    const clen = Buffer.byteLength(c);
    buff[offset] = clen;
...
exports.checkServerIdentity = function checkServerIdentity(host, cert) {
  ...
  if (cert.subjectaltname) {
    cert.subjectaltname.split(/, /g).forEach(function(altname) {
      const option = altname.match(/^(DNS|IP Address|URI):(.*)$/);
      if (!option)
        return;
      if (option[1] === 'DNS') {
  ...
  if (net.isIP(host)) {
    ...
    // RFC6125
    if (matchCN) {
      const commonNames = cert.subject.CN;
