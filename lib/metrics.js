...
/**
 * Create a stream which converts metrics to a string using a converter.
 * @param {Function} converter The function to convert metrics to strings.
 * @returns {Transform} A readable stream of converted metrics.
 * @private
 */
function metricStreamFactory(converter) {
  return function makeMetricStream() {
    const stream = new Transform({
      // Receives Metric objects
      writableObjectMode: true,

      // Emits statsd-formatted strings
      transform(metric, encoding, callback) {
        if (!this.closed) {
          callback(null, `${converter(metric)}\n`);
        }
      },
    });

    // Track all channel subscriptions to unsubscribe when the stream ends.
    const subscribers = new SafeMap();

    // Subscribe and track subscriptions.
    function sub(name, listener) {
      subscribe(name, listener);
      subscribers.set(name, listener);
    }

    // Subscribe to new metric channel to discover and subscribe to new metrics.
    newMetricChannel.subscribe(({ type, name }) => {
      const key = `metrics:${type}:${name}`;
      sub(key, (result) => stream.write(result));
    });

    // If the stream is ended, unsubscribe all listeners.
    stream.on('finish', () => {
      for (const { 0: name, 1: listener } of subscribers.entries()) {
...
/**
 * Create a stream converting all metrics to a statsd-compatible format.
 * @returns {Transform} A readable stream of statsd-formatted metrics.
 */
const statsdStream = metricStreamFactory((r) => r.toStatsd());

/**
 * Create a stream converting all metrics to a Dogstatsd-compatible format.
 * @returns {Transform} A readable stream of Dogstatsd-formatted metrics.
 */
const dogstatsdStream = metricStreamFactory((r) => r.toDogStatsd());

/**
 * Create a stream converting all metrics to a Prometheus-compatible format.
 * @returns {Transform} A readable stream of Prometheus-formatted metrics.
 */
const prometheusStream = metricStreamFactory((r) => r.toPrometheus());

/**
 * Create a stream converting all metrics to a graphite-compatible format.
 * @returns {Transform} A readable stream of graphite-formatted metrics.
 */
const graphiteStream = metricStreamFactory((r) => r.toGraphite());
 ...
 * @param {object} [meta] Additional metadata to include with the report.
 * @returns {Gauge} The meter metric.
 */
const meter = direct('meter');
...
/**
 * Create a unique set metric.
 * @param {string} name The name of the set.
 * @param {object} [meta] Additional metadata to include with the report.
 * @returns {Set} The set metric.
 */
const uniqueSet = direct('uniqueSet');
