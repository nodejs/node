...
QueryString.unescapeBuffer = function(s, decodeSpaces) {
  ...
  for (var inIndex = 0, outIndex = 0; inIndex <= s.length; inIndex++) {
    const c = inIndex < s.length ? s.charCodeAt(inIndex) : NaN;
    switch (state) {
...
QueryString.stringify = QueryString.encode = function(obj, sep, eq, options) {
  ...
  const encode = QueryString.escape;
...
QueryString.parse = QueryString.decode = function(qs, sep, eq, options) {
  ...
  const pairs = Infinity;
  if (maxKeys > 0)
    pairs = maxKeys;

  const decode = QueryString.unescape;
  ...
  const key = '';
  const value = '';
  const keyEncoded = customDecode;
  const valEncoded = customDecode;
  const encodeCheck = 0;
