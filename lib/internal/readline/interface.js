...
class Interface extends InterfaceConstructor {
  ...
  setLine(line) {
    this.line = line;
    this.isMultiline = StringPrototypeIndexOf(line, '\n') !== -1;
  }
  ...
  [kAddHistory]() {
    ...
    const normalizedLine = this.normalizeHistoryLineEndings(this.line);
  ...
  [kTabCompleter](lastKeypressWasTab, { 0: completions, 1: completeOn }) {
    ...
    if (StringPrototypeStartsWith(prefix, completeOn) &&
      ...
      this.setLine(StringPrototypeSlice(this.line,
                                        0,
  ...
  [kDeleteLineLeft]() {
    ...
    this.setLine(StringPrototypeSlice(this.line, this.cursor));
    this.cursor = 0;
  ...
  [kDeleteLineRight]() {
    ...
    this.setLine(StringPrototypeSlice(this.line, 0, this.cursor));
    this[kPushToKillRing](del);
    this[kRefreshLine]();
  }
  ...
  [kMoveDownOrHistoryNext]() {
    ...
    if (this.isMultiline && rows < splitLine.length - 1) {
      ...
      const amountToMove = (cols > nextLine.length + 1) ?
        currentLine.length - cols + nextLine.length + 3 : // Move to the end of the current line
        // + 3 chars to account for the "| " prefix
  ...
  [kHistoryNext]() {
    if (this.historyIndex >= 0) {
      ...
      if (index === -1) {
        ...
        this.setLine(this.denormalizeHistoryLineEndings(this.history[index]));
      }
  ...
  [kGetDisplayPos](str) {
    ...
    // Determine if this is a prompt string
    const isPrompt = str === this[kPrompt] || !StringPrototypeStartsWith(str, this[kPrompt]);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (i > 0 && !isPrompt && this.isMultiline) {
        // When we're calculating the cursor position, we need special handling
        // for line boundaries to make navigation smooth
        lineOffset += 2; // Account for "| " prefix
      }
      ...
      for (const char of new SafeStringIterator(line)) {
        ...
        if (char === '\t') {
          lineOffset += this.tabSize - (lineOffset % this.tabSize);
          continue;
        }
      ...
      if (i < lines.length - 1) {
        ...
        rows += MathCeil(lineOffset / col) || 1;
        offset = 0;
      } else {
        // For the last line
        offset = lineOffset;
  ...
  [kTtyWrite](s, key) {
    ...
    if (
      ...
      if (this[kSubstringSearch] === null && !this.isMultiline) {
