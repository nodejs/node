...
function Interface(input, output, completer, terminal) {
  ...
  // Check completer arity, 2 - for async, 1 for sync.
  if (typeof opts.completer === 'function') {
    const completer = opts.completer;
    opts.completer = completer.length === 2 ?
      completer :
...
function normalizeInterfaceArguments(input, output, completer, terminal) {
  let historySize;
  let prompt;
  let crlfDelay;
  let removeHistoryDuplicates;
  let escapeCodeTimeout;
  ...
  // Validate terminal. For backwards compatibility, check the isTTY prop of the
  // output stream when `terminal` was not specified.
  if (terminal === undefined && output != null)
    terminal = !!output.isTTY;
  else
    terminal = !!terminal;

  // Validate historySize.
  if (historySize === undefined) {
    historySize = kHistorySize;
  } else if (typeof historySize !== 'number' ||
  ...
  // Validate crlfDelay.
  crlfDelay = crlfDelay ? Math.max(kMincrlfDelay, crlfDelay) : kMincrlfDelay;

  // Validate removeHistoryDuplicates.
  removeHistoryDuplicates = !!removeHistoryDuplicates;

  // Validate escapeCodeTimeout.
  if (escapeCodeTimeout === undefined)
    escapeCodeTimeout = kEscapeCodeTimeout;
  else if (!Number.isFinite(escapeCodeTimeout))
    throw new ERR_INVALID_OPT_VALUE('escapeCodeTimeout', escapeCodeTimeout);
