...
const {
  codes: { ERR_INVALID_ARG_VALUE, ERR_INVALID_ARG_TYPE, ERR_MISSING_ARGS },
...
class PerformanceObserver {
  ...
  observe(options = {}) {
    ...
    if (this[kEntryTypes].size) kObservers.add(this);
...
function enqueue(entry) {
  ...
  if (
    count > kMaxPerformanceEntryBuffers &&
...
function filterBufferMapByNameAndType(name, type) {
  ...
  return ArrayPrototypeFlatMap(bufferList, (buffer) =>
    filterBufferByName(buffer, name)
  );
}
