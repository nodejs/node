...
function extractFunctionName(description) {
  const fnNameMatch = RegExpPrototypeSymbolMatch(
...
const { moduleIds: PUBLIC_BUILTINS } = internalBinding('native_module');
...
function markSourceColumn(sourceText, position, useColors) {
  ...
  if (useColors) {
    tail = RegExpPrototypeSymbolReplace(
      /(.+?)([^\w]|$)/,
      tail,
      '\u001b[32m$1\u001b[39m$2'
    );
...
function createRepl(inspector) {
    ...
    [customInspectSymbol]() {
      return ArrayPrototypeJoin(
        ArrayPrototypeMap(this, (callFrame, idx) => {
          ...
          const frameLocation = `${relativeUrl}:${
            lineNumber + 1
          }:${columnNumber}`;
  ...
  async function formatWatchers(verbose = false) {
    ...
    const values = await PromiseAll(
      new SafeArrayIterator(ArrayPrototypeMap(watchedExpressions, inspectValue))
    );
  ...
  function setBreakpoint(script, line, condition, silent) {
    ...
    const escapedPath = RegExpPrototypeSymbolReplace(
      /([/\\.?*()^${}|[\]])/g,
      script,
      '\\$1'
    );
  ...
  Debugger.on('paused', ({ callFrames, reason /* , hitBreakpoints */ }) => {
    ...
    inspector.suspendReplWhile(() =>
      PromisePrototypeThen(
        PromiseAll(
          new SafeArrayIterator([formatWatchers(true), selectedFrame.list(2)])
        ),
