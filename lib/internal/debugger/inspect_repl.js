...
function createRepl(inspector) {
  ...
  const history = { __proto__: null, control: [], debug: [] };
  ...
  const INSPECT_OPTIONS = { __proto__: null, colors: inspector.stdout.isTTY };
  ...
  async function getSourceSnippet(location, delta = 5) {
    ...
    const { scriptSource } = await Debugger.getScriptSource({ __proto__: null, scriptId });
  ...
  function handleBreakpointResolved({ breakpointId, location }) {
    ...
    if (scriptUrl) {
      ObjectAssign(location, { __proto__: null, scriptUrl });
    }
    const isExisting = ArrayPrototypeSome(knownBreakpoints, (bp) => {
      if (bp.breakpointId === breakpointId) {
        ObjectAssign(bp, { __proto__: null, location });
        return true;
      }
      return false;
    });
    if (!isExisting) {
      ArrayPrototypePush(knownBreakpoints, { __proto__: null, breakpointId, location });
    }
  }
  ...
  function setBreakpoint(script, line, condition, silent) {
    function registerBreakpoint({ breakpointId, actualLocation }) {
      handleBreakpointResolved({ __proto__: null, breakpointId, location: actualLocation });
    ...
    if (script === undefined) {
      return PromisePrototypeThen(
        Debugger.setBreakpoint({ __proto__: null, location: getCurrentLocation(), condition }),
        registerBreakpoint);
    }
    ...
    if (line === undefined && typeof script === 'number') {
      ...
      return PromisePrototypeThen(
        Debugger.setBreakpoint({ __proto__: null, location, condition }),
        registerBreakpoint);
    }
    ...
    if (scriptId !== null) {
      const location = { __proto__: null, scriptId, lineNumber: line - 1 };
      return PromisePrototypeThen(
        Debugger.setBreakpoint({ __proto__: null, location, condition }),
        registerBreakpoint);
    }
  ...
  function clearBreakpoint(url, line) {
    ...
    return PromisePrototypeThen(
      Debugger.removeBreakpoint({ __proto__: null, breakpointId: breakpoint.breakpointId }),
      () => {
  ...
  function setPauseOnExceptions(state) {
    return PromisePrototypeThen(
      Debugger.setPauseOnExceptions({ __proto__: null, state }),
      () => {
  ...
  Debugger.on('scriptParsed', (script) => {
    ...
    if (url) {
      knownScripts[scriptId] = { __proto__: null, isNative: isNativeUrl(url), ...script };
    }
  });

  Profiler.on('consoleProfileFinished', ({ profile }) => {
    Profile.createAndRegister({ __proto__: null, profile });
    print(
    ...
    copyOwnProperties(context, {
      ...
      takeHeapSnapshot(filename = 'node.heapsnapshot') {
        ...
        heapSnapshotPromise = new Promise((resolve, reject) => {
          ...
          PromisePrototypeThen(
            HeapProfiler.takeHeapSnapshot({ __proto__: null, reportProgress: true }),
            onResolve, onReject);
        });
        return heapSnapshotPromise;
      },
  ...
  async function initAfterStart() {
    ...
    await Profiler.setSamplingInterval({ __proto__: null, interval: 100 });
    await Debugger.enable();
    await Debugger.setAsyncCallStackDepth({ __proto__: null, maxDepth: 0 });
    await Debugger.setBlackboxPatterns({ __proto__: null, patterns: [] });
    await Debugger.setPauseOnExceptions({ __proto__: null, state: pauseOnExceptionState });
