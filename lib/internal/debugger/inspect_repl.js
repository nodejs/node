...
function createRepl(inspector) {
  ...
  async function initAfterStart() {
    const waitForInitialBreakRender =
      !!inspector.options?.script &&
      process.env.NODE_INSPECT_RESUME_ON_START !== '1';
    const initialBreak = waitForInitialBreakRender ?
      PromiseWithResolvers() : null;
    initialBreakRender = initialBreak;
    const onClientClose = initialBreak?.resolve;
    if (onClientClose) {
      inspector.client.once('close', onClientClose);
    }
    ...
    try {
      ...
      await initialBreak?.promise;
    } finally {
      if (onClientClose) {
        inspector.client.removeListener('close', onClientClose);
      }
      if (initialBreakRender === initialBreak) {
        initialBreakRender = null;
      }
    }
  }
