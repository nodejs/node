 ...
 */
function asciiToLowerIfNeeded(key) {
...
function processNgHeader(state, key, value) {
  key = asciiToLowerIfNeeded(key);
  const isSingleValueField = kSingleValueFields.has(key);
  const isStrictSingleValueField = state.strictSingleValueFields &&
    isSingleValueField;
  ...
  if (isStrictSingleValueField) {
    if (state.singles === undefined) {
      state.singles = new SafeSet();
    } else if (state.singles.has(key)) {
      throw new ERR_HTTP2_HEADER_SINGLE_VALUE(key);
    }
    state.singles.add(key);
  }
  const flags = state.neverIndex !== null && state.neverIndex.has(key) ?
    kNeverIndexFlag :
    kNoHeaderFlags;
  if (key[0] === ':') {
    const err = state.validatePseudoHeaderValue(key);
    if (err !== undefined)
      throw err;
    state.pseudoHeaders += `${key}\0${value}\0${flags}`;
    state.count++;
    return;
  }
  ...
  if (isArray) {
    for (let j = 0; j < value.length; ++j) {
      ...
      state.headers += `${key}\0${val}\0${flags}`;
    }
    state.count += value.length;
    return;
  }
  state.headers += `${key}\0${value}\0${flags}`;
  state.count++;
}
...
function buildNgHeaderString(arrayOrMap,
  ...
  const state = {
    ...
    count: 0,
    singles: undefined,
    neverIndex: sensitiveHeaders.length === 0 ?
      null : new SafeSet(sensitiveHeaders.map((v) => v.toLowerCase())),
    validatePseudoHeaderValue,
    strictSingleValueFields,
  };
  ...
  if (ArrayIsArray(arrayOrMap)) {
    for (let i = 0; i < arrayOrMap.length; i += 2) {
      ...
      if (value === undefined || key === '')
        continue;
      processNgHeader(state, key, value);
    }
  } else {
  ...
  return [state.pseudoHeaders + state.headers, state.count];
}
