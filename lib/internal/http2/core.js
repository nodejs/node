...
function onSessionHeaders(handle, id, cat, flags, headers, sensitiveHeaders) {
  ...
  if (stream === undefined) {
    ...
    if (type === NGHTTP2_SESSION_SERVER) {
      ...
      stream = new ServerHttp2Stream(session, handle, id, { __proto__: null }, obj);
      if (endOfStream) {
        stream.push(null);
      }
      ...
      // eslint-disable-next-line no-use-before-define
      stream = new ClientHttp2Stream(session, handle, id, { __proto__: null });
      if (endOfStream) {
        stream.push(null);
      }
      stream.end();
...
function onStreamTrailers() {
  ...
  if (!stream.emit('wantTrailers')) {
    ...
    stream.sendTrailers({ __proto__: null });
  }
}
...
function setupHandle(socket, type, options) {
  ...
  const settings = typeof options.settings === 'object' ?
    options.settings : { __proto__: null };
...
class Http2Session extends EventEmitter {
  ...
  get state() {
    return this.connecting || this.destroyed ?
      { __proto__: null } : getSessionState(this[kHandle]);
  }
  ...
  settings(settings, callback) {
    ...
    const settingsFn = FunctionPrototypeBind(submitSettings, this,
                                             { __proto__: null, ...settings }, callback);
    if (this.connecting) {
...
class ClientHttp2Session extends Http2Session {
  ...
  request(headers, options) {
    ...
    // eslint-disable-next-line no-use-before-define
    const stream = new ClientHttp2Stream(this, undefined, undefined, { __proto__: null });
...
function connectionListener(socket) {
  ...
  const options = this[kOptions] || { __proto__: null };
...
function initializeOptions(options) {
  ...
  options.settings = { __proto__: null, ...options.settings };
...
class Http2SecureServer extends TLSServer {
  ...
  updateSettings(settings) {
    assertIsObject(settings, 'settings');
    validateSettings(settings);
    this[kOptions].settings = { __proto__: null, ...this[kOptions].settings, ...settings };
  }
}
  ...
  switch (event) {
    case 'stream': {
      ...
      if (stream.sentHeaders) {
        ...
        stream.respond({ __proto__: null, [HTTP2_HEADER_STATUS]: 500 });
        stream.end();
      }
      break;
    }
...
function connect(authority, options, listener) {
  ...
  if (typeof options.createConnection === 'function') {
    ...
    switch (protocol) {
      case 'http:':
        socket = net.connect({ __proto__: null, port, host, ...options });
        break;
      case 'https:':
...
function getPackedSettings(settings) {
  assertIsObject(settings, 'settings');
  validateSettings(settings);
  updateSettingsBuffer({ __proto__: null, ...settings });
