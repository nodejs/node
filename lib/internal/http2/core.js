...
function onSessionHeaders(handle, id, cat, flags, headers, sensitiveHeaders) {
  ...
  if (stream === undefined) {
    ...
    process._deferTick(emit, session, 'stream', stream, obj, flags, headers);
  } else {
    let event;
    ...
    process._deferTick(emit, stream, event, obj, flags, headers);
  }
  if (endOfStream) {
    stream.push(null);
  }
...
function setupHandle(socket, type, options) {
  ...
  if (this.destroyed) {
    process._deferTick(emit, this, 'connect', this, socket);
    return;
  }
...
function finishSessionClose(session, error) {
  ...
  if (socket && !socket.destroyed) {
    ...
    process._deferTick(emitClose, session, error);
  }
}
...
class Http2Session extends EventEmitter {
  ...
  ping(payload, callback) {
    ...
    if (this.connecting || this.closed) {
      process._deferTick(cb, false, 0.0, payload);
      return;
    }
...
class ServerHttp2Stream extends Http2Stream {
  ...
  pushStream(headers, options, callback) {
    ...
    process._deferTick(callback, null, stream, headers, 0);
  }
