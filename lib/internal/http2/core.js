    ...
    switch (prop) {
      ...
      default:
        ...
        return typeof value === 'function' ?
          FunctionPrototypeBind(value, socket) : value;
    }
  },
...
function closeSession(session, code, error) {
  ...
  if (handle !== undefined) {
    handle.ondone =
      FunctionPrototypeBind(finishSessionClose, null, session, error);
...
class Http2Session extends EventEmitter {
  constructor(type, options, socket) {
    ...
    const setupFn =
      FunctionPrototypeBind(setupHandle, this, socket, type, options);
  ...
  settings(settings, callback) {
    ...
    const settingsFn =
      FunctionPrototypeBind(submitSettings, this, { ...settings }, callback);
    if (this.connecting) {
  ...
  goaway(code = NGHTTP2_NO_ERROR, lastStreamID = 0, opaqueData) {
    ...
    const goawayFn =
      FunctionPrototypeBind(submitGoaway, this, code, lastStreamID, opaqueData);
    if (this.connecting) {
...
class ClientHttp2Session extends Http2Session {
  ...
  request(headers, options) {
    ...
    const onConnect =
      FunctionPrototypeBind(requestOnConnect, stream, headersList, options);
    if (this.connecting) {
...
function shutdownWritable(callback) {
  ...
  if (err === 1)  // synchronous finish
    return FunctionPrototypeCall(afterShutdown, req, 0);
}
...
class Http2Stream extends Duplex {
  ...
  [kWriteGeneric](writev, data, encoding, cb) {
    ...
    if (this.pending) {
      this.once(
        'ready',
        FunctionPrototypeBind(this[kWriteGeneric], this,
  ...
  _final(cb) {
    ...
    FunctionPrototypeCall(shutdownWritable, this, cb);
  }
...
function onFileUnpipe() {
  ...
  if (stream.ownsFd)
    this.source.close().catch(FunctionPrototypeBind(stream.destroy, stream));
  else
...
function doSendFileFD(session, options, fd, headers, streamOptions, err, stat) {
  ...
  // of response so check again for the HEADERS_SENT flag
  if ((typeof options.statCheck === 'function' &&
       FunctionPrototypeCall(options.statCheck, this, stat, headers) === false
  ) ||
       (this[kState].flags & STREAM_FLAGS_HEADERS_SENT)) {
    tryClose(fd);
    return;
...
function afterOpen(session, options, headers, streamOptions, err, fd) {
  ...
  fs.fstat(fd,
           FunctionPrototypeBind(doSendFileFD, this, session, options, fd,
...
class ServerHttp2Stream extends Http2Stream {
  ...
  respondWithFD(fd, headers, options) {
    ...
    if (options.statCheck !== undefined) {
      fs.fstat(fd,
               FunctionPrototypeBind(doSendFD, this, session, options, fd,
                                     headers, streamOptions));
      return;
    }
  ...
  respondWithFile(path, headers, options) {
    ...
    fs.open(path, 'r',
            FunctionPrototypeBind(afterOpen, this, session, options, headers,
                                  streamOptions));
  }
