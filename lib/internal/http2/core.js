...
const { ShutdownWrap, kReadBytesOrError, streamBaseState } =
...
const validateSettings = hideStackFrames((settings) => {
  ...
  if (
    ...
    throw new ERR_HTTP2_INVALID_SETTING_VALUE(
      'enablePush',
      settings.enablePush
    );
  }
...
class Http2Session extends EventEmitter {
  constructor(type, options, socket) {
    ...
    if (socket[kSession] !== undefined) throw new ERR_HTTP2_SOCKET_BOUND();
...
class ServerHttp2Session extends Http2Session {
  ...
  origin(...origins) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (origin === 'null') throw new ERR_HTTP2_INVALID_ORIGIN();
...
class ClientHttp2Session extends Http2Session {
  ...
  request(headers, options) {
    ...
    if (this.closed) throw new ERR_HTTP2_GOAWAY_SESSION();
    ...
    if (!connect || headers[HTTP2_HEADER_PROTOCOL] !== undefined) {
      ...
      if (headers[HTTP2_HEADER_SCHEME] === undefined)
        headers[HTTP2_HEADER_SCHEME] = StringPrototypeSlice(
          this[kProtocol],
          0,
          -1
        );
    ...
    stream[kOrigin] =
      `${headers[HTTP2_HEADER_SCHEME]}://` + `${getAuthority(headers)}`;
...
function finishSendTrailers(stream, headersList) {
  ...
  if (ret < 0) stream.destroy(new NghttpError(ret));
  else stream[kMaybeDestroy]();
}
...
function closeStream(stream, code, rstStreamStatus = kSubmitRstStream) {
  ...
  if (rstStreamStatus !== kNoRstStream) {
    ...
    if (
      ...
      finishFn();
    else stream.once('finish', finishFn);
  }
}
...
class Http2Stream extends Duplex {
  ...
  [kWriteGeneric](writev, data, encoding, cb) {
    ...
    if (writev) req = writevGeneric(this, data, writeCallback);
    else req = writeGeneric(this, data, encoding, writeCallback);
...
function callTimeout(self, kSession) {
  ...
  if (self[kState].writeQueueSize > 0) {
    ...
    const chunksSentSinceLastWrite =
      handle !== undefined ? handle.chunksSentSinceLastWrite : null;
    if (
...
function processHeaders(oldHeaders, options) {
  ...
  if (options.sendDate == null || options.sendDate) {
    if (
      headers[HTTP2_HEADER_DATE] === null ||
...
function doSendFD(session, options, fd, headers, streamOptions, err, stat) {
  ...
  if (
    (typeof options.statCheck === 'function' &&
      ReflectApply(options.statCheck, this, [stat, headers, statOptions]) ===
        false) ||
...
class ServerHttp2Stream extends Http2Stream {
  ...
  respondWithFD(fd, headers, options) {
    ...
    if (fd instanceof fsPromisesInternal.FileHandle) fd = fd.fd;
  ...
  additionalHeaders(headers) {
    ...
    const headersList = mapToHeaders(headers, assertValidPseudoHeaderResponse);
    if (!this[kInfoHeaders]) this[kInfoHeaders] = [headers];
    else ArrayPrototypePush(this[kInfoHeaders], headers);
...
class ClientHttp2Stream extends Http2Stream {
  constructor(session, handle, id, options) {
    super(session, options);
    this[kState].flags |= STREAM_FLAGS_HEADERS_SENT;
    if (id !== undefined) this[kInit](id, handle);
...
function connect(authority, options, listener) {
  ...
  if (authority.hostname) {
    ...
    if (host[0] === '[') host = StringPrototypeSlice(host, 1, -1);
