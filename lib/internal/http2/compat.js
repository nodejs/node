...
class Http2ServerRequest extends Readable {
  constructor(stream, headers, options, rawHeaders) {
    ...
    stream.onEnd(onStreamEnd);
    stream.on('error', onStreamError);
    stream.on('aborted', onStreamAbortedRequest);
    stream.onClose(onStreamCloseRequest);
    stream.on('timeout', onStreamTimeout(kRequest));
    this.onPause(onRequestPause);
    this.onResume(onRequestResume);
  }
  ...
  _read(nread) {
    ...
    if (!state.didRead) {
      state.didRead = true;
      this[kStream].onData(onStreamData);
    } else {
...
class Http2ServerResponse extends Stream {
  constructor(stream, options) {
    ...
    stream.onDrain(onStreamDrain);
    stream.on('aborted', onStreamAbortedResponse);
    stream.onClose(onStreamCloseResponse);
