...
function validateFastIntegerArg(type, value, index) {
  ...
  if (info.kind === 'number') {
    valid = typeof value === 'number' && NumberIsInteger(value) &&
            value >= info.min && value <= info.max;
  } else if (typeof value === 'bigint') {
    valid = value >= info.min && value <= info.max;
  } else {
    valid = typeof value === 'number' && NumberIsSafeInteger(value) &&
            (info.min < 0n || value >= 0);
  }
...
function convertFastIntegerArg(type, value) {
  const info = fastIntegerTypeInfo[type];
  if (info !== undefined &&
      info.kind === 'bigint' &&
      typeof value === 'number') {
...
function convertFastArg(type, value, stringState, index) {
  ...
  value = convertFastIntegerArg(type, value);
...
function wrapWithRawPointerConversions(rawFn, argumentTypes, owner) {
  ...
  if (nargs === 1 && indexes.length === 1 && indexes[0] === 0) {
    ...
    wrapper = { invoke(a0) {
      ...
      let arg = convertFastIntegerArg(t0, a0);
