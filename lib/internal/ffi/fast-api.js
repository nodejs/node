...
// Mirrors THROW_ERR_FFI_LIBRARY_CLOSED in src/node_errors.h. The native
// `closed` checks in InvokeFunction/InvokeFunctionSB cannot run on the fast-call
// path: V8 embeds the trampoline address in optimized code, so a tiered-up call
// site jumps straight into the library that `close()` unloaded.
function throwLibraryClosed() {
  // eslint-disable-next-line no-restricted-syntax
  const err = new Error('Library is closed');
  err.code = 'ERR_FFI_LIBRARY_CLOSED';
  throw err;
}

// Guard-only wrappers for fast signatures that need no argument conversion.
// The flag is a one-byte Uint8Array, so the check is an element load that V8
// cannot fold away across the call.
function wrapWithClosedGuard(rawFn, nargs, closedFlag) {
  function wrapper(...args) {
    if (args.length !== nargs) {
      throwFFIArgCountError(nargs, args.length);
    }
    if (closedFlag[0] !== 0) throwLibraryClosed();
...
function wrapWithRawPointerConversions(rawFn, argumentTypes, owner) {
  ...
  // `kFastArguments` is attached by `CreateFunction` to every function that got
  // a fast-call trampoline, so reaching here means the raw function must never
  // be exposed unguarded: after `close()` the trampoline still points into the
  // unloaded library and only a JavaScript-side check can stop the call.
  // `DynamicLibrary::New` installs the flag on every instance, so a missing one
  // means the native side and this wrapper are out of sync.
  const closedFlag = owner?.[kClosedFlag];
  assert(closedFlag !== undefined,
         'FFI: fast function is missing the owning library closed flag');

  const indexes = getFastArgumentIndexes(argumentTypes);
  if (indexes === null) {
    return wrapWithClosedGuard(rawFn, argumentTypes.length, closedFlag);
  }
