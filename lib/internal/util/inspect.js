...
const ansi = new RegExp(
  ...
  '(?:\\u001B\\][\\s\\S]*?(?:\\u0007|\\u001B\\u005C|\\u009C))' +
  // Control sequences, using the structure ECMA-48 5.4 defines: CSI, then any
  // number of parameter bytes (0x30-0x3F), then any number of intermediate
  // bytes (0x20-0x2F), then a single final byte (0x40-0x7E). The list of final
  ...
  '|(?:\\u001B\\[|\\u009B)[\\u0030-\\u003F]*[\\u0020-\\u002F]*[\\u0040-\\u007E]' +
  // Everything else the reference implementation matched, including sequences
  // whose last byte is a digit, such as `ESC 7`.
  '|[\\u001B\\u009B][[\\]()#;?]*' +
  '(?:\\d{1,4}(?:[;:]\\d{0,4})*)?' +
  '[\\dA-PR-TZcf-nq-uy=><~]', 'g',
);
