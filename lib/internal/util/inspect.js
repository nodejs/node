...
function formatWithOptionsInternal(inspectOptions, args) {
  ...
  if (typeof first === 'string') {
      ...
      if (StringPrototypeCharCodeAt(first, i) === 37) { // '%'
        ...
        if (a + 1 !== args.length) {
          switch (nextChar) {
            ...
            case 99: { // 'c'
              const tempCss = args[++a];
              tempStr = '';
              if (inspectOptions?.colors && typeof tempCss === 'string') {
                cssToAnsi ??= require('internal/util/css_to_ansi').cssToAnsi;
                const sgr = cssToAnsi(tempCss);
                // Each `%c` replaces the styles applied by the previous one.
                if (styled) {
                  tempStr = '\u001b[0m';
                }
                styled = sgr !== '';
    ...
    if (lastPos !== 0) {
      ...
      // Styles applied via `%c` only affect the format string itself.
      if (styled) {
        str += '\u001b[0m';
      }
    }
