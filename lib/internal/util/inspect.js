...
function formatProperty(ctx, value, recurseTimes, key, type, desc, original = value) {
  ...
  if (desc.value !== undefined) {
    ...
    const s = ctx.stylize;
    const sp = 'special';
    // Defensive getter: always catch exceptions thrown by property access
    if (ctx.getters && (ctx.getters === true ||
      ...
      try {
        ...
        // This catch ensures that broken DOM-style getters won't crash inspect!
        const message = `<Inspection threw (${err && err.message ? err.message : err})>`;
