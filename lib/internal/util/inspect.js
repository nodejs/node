  ...
  for (let i = 0; i < a.length - 3; i++) {
    ...
    if (pos !== -1) {
      ...
      if (rest > 3) {
        ...
        if (len > 3) {
          return { __proto__: null, len, offset: i };
        }
      }
    }
  }

  return { __proto__: null, len: 0, offset: 0 };
}
...
function formatNamespaceObject(keys, ctx, value, recurseTimes) {
  ...
  for (let i = 0; i < keys.length; i++) {
    try {
      ...
      const tmp = { __proto__: null, [keys[i]]: '' };
...
function formatProperty(ctx, value, recurseTimes, key, type, desc,
  ...
  desc = desc || ObjectGetOwnPropertyDescriptor(value, key) ||
    { __proto__: null, value: value[key], enumerable: true };
...
function formatWithOptions(inspectOptions, ...args) {
  validateObject(inspectOptions, 'inspectOptions', { __proto__: null, allowArray: true });
