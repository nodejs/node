...
function formatTypedArray(value, length, ctx, ignored, recurseTimes) {
  ...
  if (ctx.showHidden) {
    ...
    for (let i = 0; i < typedArrayKeys.length; i++) {
      const key = typedArrayKeys[i];
...
function formatSet(value, ctx, ignored, recurseTimes) {
  ...
  ctx.indentationLvl += 2;
  let i = 0;
  const setIterator = value.values();
  let setResult = setIterator.next();
  while (!setResult.done && i < maxLength) {
    ArrayPrototypePush(output, formatValue(ctx, setResult.value, recurseTimes));
    i++;
...
function formatMap(value, ctx, ignored, recurseTimes) {
  ...
  ctx.indentationLvl += 2;
  let i = 0;
  const mapIterator = value.entries();
  let mapResult = mapIterator.next();
  while (!mapResult.done && i < maxLength) {
    const k = mapResult.value[0];
    const v = mapResult.value[1];
    ArrayPrototypePush(
      output,
      `${formatValue(ctx, k, recurseTimes)} => ${formatValue(ctx, v, recurseTimes)}`,
    );
    i++;
    mapResult = mapIterator.next();
  }
  if (remaining > 0) {
    ArrayPrototypePush(output, remainingText(remaining));
  }
...
if (internalBinding('config').hasIntl) {
  ...
  getStringWidth = function getStringWidth(str, removeControlChars = true) {
    ...
    const iterator = new SafeStringIterator(str);
