...
function formatRaw(ctx, value, recurseTimes) {
  ...
  if (noIterator) {
    ...
    if (constructor === 'Object') {
      ...
      const prefix = getPrefix(constructor, '', 'Error');
      // Make error with message first say the error.
      base = formatError(value, prefix);
      ...
      if (ctx.indentationLvl !== 0) {
        ...
        base = formatError(value, prefix).replace(/\n/g, `\n${indentation}`);
      }
      if (keys.length === 0)
        return base;
...
function formatError(value, constructorName) {
  if (value.stack) {
    ...
    if (!stack.startsWith(constructorName)) {
      stack = stack.replace(parent, constructorName.trim());
    } else if (parent === null) {
      stack = stack.replace('Error', constructorName.trim());
    }
    return stack;
  }
