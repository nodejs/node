...
function setEquiv(a, b, mode, memo) {
  ...
  const iteratorBValues = [...iteratorB];
  for (let i = 0; i < iteratorBValues.length; ++i) {
    const val = iteratorBValues[i];
    if (!a.has(val)) {
...
function partialObjectMapEquiv(array, a, b, mode, memo) {
  ...
  for (let i = 0; i < aEntries.length; ++i) {
    const { 0: key1, 1: item1 } = aEntries[i];
    aPos++;
...
function mapEquiv(a, b, mode, memo) {
  ...
  for (let i = 0; i < bEntries.length; ++i) {
    const { 0: key2, 1: item2 } = bEntries[i];
