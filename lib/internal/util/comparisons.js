...
//
// To be treated as deep equal, objects should have:
...
function innerDeepEqual(val1, val2, strict, memos) {
  ...
  if (strict && Object.getPrototypeOf(val1) !== Object.getPrototypeOf(val2)) {
    return false;
  }
...
function setEquiv(a, b, strict, memo) {
  ...
  for (const val of a) {
    ...
    if (typeof val === 'object' && val !== null) {
      ...
      // If the specified value doesn't exist in the second set and it's a
      // not-null object, we'll need to go hunting for something thats
      // deep-equal to it. To make this O(n log n) complexity, we have to copy
      // these values in a new set first.
      set.add(val);
    } else if (!b.has(val)) {
      return false;
    }
