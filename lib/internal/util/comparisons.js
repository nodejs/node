...
const {
  ...
  propertyFilter: { ONLY_ENUMERABLE, SKIP_SYMBOLS },
...
function getEnumerables(val, keys) {
  return ArrayPrototypeFilter(keys, (k) =>
    ObjectPrototypePropertyIsEnumerable(val, k)
  );
}
  ...
  if (iterationType === kIsSet) {
    ...
    for (; i < a.length; i++) {
      if (ObjectPrototypeHasOwnProperty(a, i)) {
        if (
          !ObjectPrototypeHasOwnProperty(b, i) ||
        ...
        for (; i < keysA.length; i++) {
          const key = keysA[i];
          if (
            !ObjectPrototypeHasOwnProperty(b, key) ||
