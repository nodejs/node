...
function strictDeepEqual(val1, val2, memos) {
  ...
  if (val1Tag === '[object Array]') {
    ...
    // properties).
    if (val1.length === keys1.length) {
      if (keys1.length === 0 || +(keys1[val1.length - 1]) === val1.length - 1) {
        return keyCheck(val1, val2, kStrict, memos, kIsArray, array);
      }
    } else if (keys1.length > val1.length &&
               +(keys1[val1.length - 1]) === val1.length - 1) {
      const keys = keys1.slice(val1.length);
      return keyCheck(val1, val2, kStrict, memos, kIsArray, keys);
    }
  ...
  if (isDate(val1)) {
    ...
    if (keys1.length === val1.length) {
      return keyCheck(val1, val2, kStrict, memos, kNoIterator, array);
    }
...
function keyCheck(val1, val2, strict, memos, iterator, aKeys) {
  ...
  if (strict) {
    let symbolKeysA = getOwnPropertySymbols(val1);
    let symbolKeysB = getOwnPropertySymbols(val2);
    ...
    if (symbolKeysA.length === 0) {
      if (aKeys.length === 0) {
        if (iterator === kNoIterator) {
          return true;
        }
        if (iterator === kIsArray && val1.length === 0) {
          return true;
        }
      }
