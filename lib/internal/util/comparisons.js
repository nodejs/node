...
function checkBoxedPrimitives(val1, val2) {
  if (isNumberObject(val1)) {
    return isNumberObject(val2) &&
           innerDeepEqual(numberValueOf(val1), numberValueOf(val2), kStrict);
  }
...
function strictDeepEqual(val1, val2, memos) {
  ...
  } else if (isBoxedPrimitive(val1) && !checkBoxedPrimitives(val1, val2)) {
    return false;
  }
  return keyCheck(val1, val2, kStrict, memos, kNoIterator);
}
