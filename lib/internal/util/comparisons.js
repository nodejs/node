...
function strictDeepEqual(val1, val2, memos) {
  ...
  if (val1Tag === '[object Array]') {
    ...
    const keys2 = getOwnNonIndicesProperties(val2);
    return keyCheck(val1, val2, kStrict, memos, kIsArray, keys1, keys2);
  }
  ...
  if (isDate(val1)) {
    ...
    return keyCheck(val1, val2, kStrict, memos, kNoIterator, keys1, keys2);
  } else if (isSet(val1)) {
    ...
    if (val1Value !== val1) {
      ...
      if (typeof val1Value === 'string') {
        const keys1 = Object.keys(val1).slice(val1.length);
        const keys2 = Object.keys(val2).slice(val1.length);
        return keyCheck(val1, val2, kStrict, memos, kNoIterator, keys1, keys2);
      }
    }
  }
...
function enumerable(val, keys) {
  return keys.filter((k) => propertyIsEnumerable.call(val, k));
}

function keyCheck(val1, val2, strict, memos, iterator, aKeys, bKeys) {
  ...
  if (strict) {
    ...
    if (symbolKeysA.length !== 0) {
      symbolKeysA = enumerable(val1, symbolKeysA);
      symbolKeysB = enumerable(val2, symbolKeysB);
      if (symbolKeysA.length !== symbolKeysB.length)
        return false;
    } else if (symbolKeysB.length !== 0 &&
               enumerable(val2, symbolKeysB).length !== 0) {
      return false;
    }
    if (symbolKeysA.length === 0) {
      if (aKeys.length === 0 && iterator === kNoIterator) {
        return true;
      }
  ...
  } else if (aKeys.length === 0 && iterator === kNoIterator) {
    return true;
  }
  ...
  const areEq = objEquiv(val1, val2, strict, aKeys, memos, iterator);
...
function objEquiv(a, b, strict, keys, memos, iterator) {
  ...
  if (iterator === kIsSet) {
  ...
  } else if (iterator === kIsMap) {
    if (!mapEquiv(a, b, strict, memos)) {
      return false;
    }
  } else if (iterator === kIsArray) {
    ...
    for (; i < a.length; i++) {
      if (i in a) {
        ...
        return false;
      // The array is sparse. Special handle this by falling back to
      // Object.keys() if potentially more than 100 further entries exist.
      } else if (a.length - i > 100) {
        const aKeys = Object.keys(a);
        const bKeys = Object.keys(b);
        if (aKeys.length !== bKeys.length) {
          return false;
        }
        // Only match not yet matched entries.
        const lastKey = aKeys.indexOf(last) + 1;
        // Cheap key test:
        const keys = {};
        for (i = lastKey; i < aKeys.length; i++) {
          keys[aKeys[i]] = true;
        }
        for (i = lastKey; i < aKeys.length; i++) {
          if (keys[bKeys[i]] === undefined) {
