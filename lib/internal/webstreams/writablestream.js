...
class WritableStream {
  ...
  [kTransfer]() {
    ...
    const { readable, promise } = lazyTransfer().newCrossRealmReadableStream(
      this,
...
const isWritableStreamDefaultWriter = isBrandCheck(
  'WritableStreamDefaultWriter'
);
const isWritableStreamDefaultController = isBrandCheck(
  'WritableStreamDefaultController'
);
...
function writableStreamHasOperationMarkedInFlight(stream) {
  ...
  if (
    inFlightWriteRequest.promise === undefined &&
...
function writableStreamFinishInFlightWriteWithError(stream, error) {
  ...
  assert(
    stream[kState].state === 'writable' || stream[kState].state === 'erroring'
  );
...
function writableStreamCloseQueuedOrInFlight(stream) {
  if (
    stream[kState].closeRequest.promise === undefined &&
...
function writableStreamDefaultWriterRelease(writer) {
  ...
  const releasedError = new ERR_INVALID_STATE.TypeError(
...
function setupWritableStreamDefaultControllerFromSink(
  ...
  const closeAlgorithm = close
    ? FunctionPrototypeBind(close, sink)
    : nonOpCancel;
  const abortAlgorithm = abort
    ? FunctionPrototypeBind(abort, sink)
    : nonOpCancel;
