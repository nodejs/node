...
class WritableStream {
  ...
  constructor(sink, strategy = kEmptyObject) {
    if (!sink) throw new ERR_INVALID_ARG_VALUE('sink', 'WritableStream sink is required');
    ...
    if (typeof sink.write !== 'function') {
      throw new ERR_INVALID_ARG_VALUE.TypeError(
        'sink',
        `WritableStream sink must implement a write() method. ` +
        `If you want a writable stream that silently consumes and discards data, ` +
        `consider using DummyWritableStream instead.`
      );
    }
...
const x = new DummyWritableStream();
