...
function newWritableStreamFromSink(sink, highWaterMark, sizeMode, sizeFn) {
  ...
  const abortAlgorithm = abort !== undefined ?
    ...
    nonOpCancel;
  return newWritableStream(startAlgorithm, write, closeAlgorithm,
                           abortAlgorithm, highWaterMark, sizeMode, sizeFn,
                           sink);
}
...
// field accessor) here for test use.
function writableStreamState(stream) {
  return kWritableStateNames[writableStreamStateField(stream)];
}
