...
function writableStreamDefaultControllerDrainWriteQueue(controller) {
  const controllerState = controller[kState];
  ...
  for (;;) {
    ...
    if (!controllerState.queue.length)
      return;
    ...
    let result;
    try {
      result = controllerState.writeAlgorithm(value, controller);
    } catch (error) {
      result = PromiseReject(error);
    }
    if (isNonThenable(result)) {
      // pipeTo's shared write tracker uses `promise: null` and has no
      // per-write then-callback that must interleave with the next sink
      // write. Regular writer.write() requests carry a real Promise and
      // must keep the spec's one-completion-per-microtask order.
      ...
      PromisePrototypeThen(
        kResolvedPromise,
        controllerState.writeFulfilled,
        controllerState.writeRejected);
      return;
    }
    thenAlgorithmResult(
      result,
...
function writableStreamDefaultControllerProcessWrite(controller, chunk) {
  ...
  if (controller[kState].writeFulfilled === undefined) {
    ...
    controller[kState].writeFulfilled = () => {
      writableStreamDefaultControllerCompleteWrite(controller);
      // Already in the spec's "upon fulfillment" turn: drain further
      // synchronous writes here instead of one-write-per-microtask.
      writableStreamDefaultControllerDrainWriteQueue(controller);
    };
  ...
  // writer.write() completion is still separated by a microtask.
  let result;
  try {
...
function writableStreamDefaultControllerProcessClose(controller) {
  ...
  dequeueValue(controller);
  assert(!queue.length);
  const sinkClosePromise = promiseFromAlgorithmResult(closeAlgorithm());
  writableStreamDefaultControllerClearAlgorithms(controller);
  PromisePrototypeThen(
    sinkClosePromise,
