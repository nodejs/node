...
// Mirrors the C++ `SizeMode` enum (src/streams/streams_binding.h):
//   0 = kCountOne  (size === 1, no JS crossing)
//   1 = kByteLength (size === chunk.byteLength, computed in C++)
//   2 = kUserFn    (arbitrary user-provided size())
const kSizeModeCount = 0;
const kSizeModeByteLength = 1;
const kSizeModeUserFn = 2;

let lazyQueuingStrategies;

// Classifies a `strategy.size` value into the C++ SizeMode + the size function
// to forward (only needed for kUserFn). Validation order matches
// extractSizeAlgorithm so the WebIDL "size must be a function" throw fires
// before any underlying-source setup, as the spec requires.
function extractSizeMode(size) {
  if (size === undefined)
    return { __proto__: null, mode: kSizeModeCount, size: undefined };
  validateFunction(size, 'strategy.size');
  lazyQueuingStrategies ??= require('internal/webstreams/queuingstrategies');
  if (size === lazyQueuingStrategies.countSizeFunction)
    return { __proto__: null, mode: kSizeModeCount, size: undefined };
  if (size === lazyQueuingStrategies.byteSizeFunction)
    return { __proto__: null, mode: kSizeModeByteLength, size: undefined };
  return { __proto__: null, mode: kSizeModeUserFn, size };
}
