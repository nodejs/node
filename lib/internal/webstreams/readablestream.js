...
// Sentinel handed to (and compared against) readableStreamFastDequeue: returned
// when no buffered chunk could be dequeued synchronously. Module-private, so no
// user chunk can ever equal it.
const kFastDequeueEmpty = Symbol('kFastDequeueEmpty');
...
function readableStreamPipeTo(
  ...
  async function step() {
    ...
    // Fast path: synchronously drain chunks already buffered in the C++ value
    // queue, skipping the per-chunk read promise and { value, done } result
    // allocation (how chunks move through a pipe is unobservable per spec).
    // Backpressure is re-checked before each write; the dequeue itself pulls
    // the source, so a synchronous underlying source keeps the queue fed
    // without ever allocating a read.
    let chunk;
    while ((chunk = readableStreamFastDequeue(source, kFastDequeueEmpty)) !==
           kFastDequeueEmpty) {
      // Per spec, pipeTo writes without awaiting completion; the write promise
      // is tracked so a clean shutdown can wait on the last write.
      state.currentWrite = writer.write(chunk);
