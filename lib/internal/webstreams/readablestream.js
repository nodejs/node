...
const streamDoneChannel = require('diagnostics_channel').channel('stream.web.done');
  ...
  values(options = kEmptyObject) {
    ...
    function nextSteps() {
      ...
      readableStreamDefaultReaderRead(
        reader,
        // eslint-disable-next-line no-use-before-define
        new ReadableStreamAsyncIteratorReadRequest(reader, state, promise, stream),
      );
    ...
    async function returnSteps(value) {
      ...
      if (!preventCancel) {
        ...
        await result;
        if (streamDoneChannel.hasSubscribers) {
          streamDoneChannel.publish({ stream });
        }
        return { done: true, value }; // eslint-disable-line node-core/avoid-prototype-pollution
      }
...
class ReadableStreamAsyncIteratorReadRequest {
  constructor(reader, state, promise, stream) {
    this.reader = reader;
    this.state = state;
  ...
  [kClose]() {
    ...
    if (streamDoneChannel.hasSubscribers) {
      streamDoneChannel.publish({ stream: this.stream });
    }
