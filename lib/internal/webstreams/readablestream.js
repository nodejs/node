  ...
  value: {
    read() {
      ...
      if (result === kReadParked) {
        ...
        readerParkRead(this, (value, done, isErr) => {
...
// BYOB read() fast path: mirrors the default reader's parked-read protocol.
// byobReaderFastRead completes every case in a single crossing — a
// synchronously-filled view comes back raw and the { value, done } result
// plus already-resolved promise are built here in JIT code; rejections and
// the closed-state empty view come back as real promises — except the park,
// where the wrapper creates the read promise and a single settle closure and
// parks it via byobReaderParkRead (the pull-into descriptor was already
// pushed; nothing can run between the two crossings). Settling through the
// closure passes the view even when done (a closing BYOB stream hands back
// the partially-filled view). Defined AFTER the originals capture above so
// the byte tee keeps using the raw native read (its internal reads must use
...
ObjectDefineProperty(ReadableStreamBYOBReaderNative.prototype, 'read', {
  ...
  value: {
    read(view, options = undefined) {
      const result = byobReaderFastRead(this, view, options, kReadParked);
      if (result === kReadParked) {
        const { promise, resolve, reject } = PromiseWithResolvers();
        byobReaderParkRead(this, (value, done, isErr) => {
          if (isErr) reject(value);
          else resolve({ done, value });
        });
        return promise;
...
ObjectDefineProperty(ReadableByteStreamControllerNative.prototype, 'enqueue', {
  ...
  value: {
    enqueue(chunk) {
      ...
      if (pair !== undefined) pair[0](pair[1], false);
    },
  }.enqueue,
});
