...
// Sentinel returned by readerFastRead when the read must park (readable
// stream, default controller, empty queue): the wrapper then creates the read
// promise and its settle closure here in JIT code and parks it via
// readerParkRead: building the result with StoreICs and resolving through
// the (IC'd) builtin is substantially cheaper than the C++ API resolver +
// result-clone path.
const kReadParked = Symbol('kReadParked');

const { FastBuffer } = require('internal/buffer');

// Result-view constructors for the byte-path settle protocol, indexed by the
// C++ ViewType enum (src/streams/readable_stream.h - keep in sync). The
// settle closures construct read-result views in JIT code from a
// (buffer, byteOffset, length, viewTag) tuple instead of materializing them
// through the V8 API. Float16Array is not in primordials; captured at module
// load (when it is not exposed, no view of that type can exist to read
// into).
const kViewCtors = [
  DataView,
  Uint8Array,
...
ObjectDefineProperty(ReadableStreamDefaultReaderNative.prototype, 'read', {
  ...
  value: {
    read() {
      const result = readerFastRead(this, kReadFastEmpty, kReadParked);
      if (result === kReadParked) {
        // Parked read: build the promise + settle closure in JIT code; C++
        // parks it (and calls pull) in the readerParkRead crossing. No user
        // code can run between the two crossings. A single closure settles
        // both ways (isErr) so C++ parks exactly one value per read.
        const { promise, resolve, reject } = PromiseWithResolvers();
        readerParkRead(
          this, (value, done, isErr, byteOffset, length, tag, transfer) => {
            if (isErr) reject(value);
            else if (tag === undefined) resolve({ done, value });
            // Byte-stream descriptor commit (the auto-allocate respond
            // shape): value is the descriptor's held ArrayBuffer; construct
            // the result view here in JIT code, transferring first only when
            // a byobRequest view exposed the buffer.
            else resolve({ done, value: new kViewCtors[tag](
              transfer ?
                ArrayBufferPrototypeTransferToFixedLength(value) : value,
              byteOffset, length) });
          });
        return promise;
      }
      ...
      if (ObjectPrototypeIsPrototypeOf(PromisePrototype, result))
        return result;
      return PromiseResolve({ done: false, value: result });
    },
  }.read,
});

// controller.enqueue() fast path: in the parked steady state every enqueued
// chunk settles a pending read parked by the read() wrapper above. The
// binding hands that read's settle closure back here (only when the
// settle-after-pull-bookkeeping reorder is unobservable; see
// ControllerEnqueueOrSettle) so the { value, done } result and the promise
// resolution run as a JIT call from this frame instead of a C++->JS
// Function::Call per chunk. All other cases (no pending read, native-parked
// resolver reads, throwing states) complete inside the binding, which then
// returns undefined. The `chunk = undefined` default keeps length at 0,
// matching the native operation (Web IDL: optional arguments do not count).
ObjectDefineProperty(ReadableStreamDefaultControllerNative.prototype, 'enqueue', {
  __proto__: null,
  // Matches the native operation's descriptor (and Web IDL: operations are
  // writable, enumerable and configurable; name "enqueue", length 0).
  writable: true,
  enumerable: true,
  configurable: true,
  value: {
    enqueue(chunk = undefined) {
      const settle = controllerEnqueueOrSettle(this, chunk);
      if (settle !== undefined) settle(chunk, false);
    },
  }.enqueue,
});
...
// BYOB read() staged protocol. Crossing 1 (byobReaderReadStage) validates
// and stages the view's geometry, returning a Smi-encoded outcome
// ((viewTag << 1) | syncBit) or a real promise for rejections. The wrapper
// then transfers the view's buffer here in JIT code
// (%ArrayBuffer.prototype.transferToFixedLength% - no BackingStore API
// round trip) and re-enters: byobReaderFillStaged for sync completions (a
// queue fill or the closed-state empty view; the result view is constructed
// here from the returned length) or byobReaderParkRead for parks, where the
// settle closure receives (buffer, done, isErr, byteOffset, length, tag)
// and constructs the result view itself - the descriptor's held buffer is
// transferred and materialized entirely in JIT code. Nothing can run
// between the crossings, so the staged geometry cannot go stale. A done
// settle still passes the buffer (a closing BYOB stream hands back the
// partially-filled view). Defined AFTER the originals capture above so the
// byte tee keeps using the raw native read (its internal reads must use
// unpatchable originals and need null-prototype results).
// `options = undefined` keeps length at 1, matching the native operation.
ObjectDefineProperty(ReadableStreamBYOBReaderNative.prototype, 'read', {
  __proto__: null,
  // Matches the native operation's descriptor (and Web IDL: operations are
  // writable, enumerable and configurable; name "read", length 1).
  writable: true,
  enumerable: true,
  configurable: true,
  value: {
    read(view, options = undefined) {
      const stage = byobReaderReadStage(this, view, options);
      if (typeof stage !== 'number')
        return stage;  // A promise: rejections, or the internal-reader path
      // stage encodes (viewTag << 1) | syncBit; tag 0 is DataView.
      const tag = stage >> 1;
      const byteOffset = tag === 0 ?
        DataViewPrototypeGetByteOffset(view) :
        TypedArrayPrototypeGetByteOffset(view);
      const buffer = ArrayBufferPrototypeTransferToFixedLength(
        tag === 0 ? DataViewPrototypeGetBuffer(view) :
          TypedArrayPrototypeGetBuffer(view));
      let parkBuffer = buffer;
      if (stage & 1) {
        // Sync flavor: a queue fill or the closed-state empty view. The Smi
        // return is the filled length in elements (0 <=> done).
        const len = byobReaderFillStaged(this, buffer, kReadParked);
        if (typeof len === 'number') {
          return PromiseResolve({
            done: len === 0,
            value: new kViewCtors[tag](buffer, byteOffset, len),
          });
        }
        if (len !== kReadParked)
          return len;  // A promise (a closing error)
        // The queue could not satisfy options.min: the descriptor was pushed
        // with its partial fill; park the settle closure.
        parkBuffer = undefined;
      }
      const { promise, resolve, reject } = PromiseWithResolvers();
      byobReaderParkRead(
        this, parkBuffer,
        (value, done, isErr, settleByteOffset, length, settleTag,
         transfer) => {
          if (isErr) reject(value);
          else if (settleTag === undefined) resolve({ done, value });
          // Descriptor commit: value is the held ArrayBuffer; construct the
          // result view here in JIT code, transferring first only when a
          // byobRequest view exposed the buffer.
          else resolve({ done, value: new kViewCtors[settleTag](
            transfer ?
              ArrayBufferPrototypeTransferToFixedLength(value) : value,
            settleByteOffset, length) });
        });
      return promise;
    },
  }.read,
...
// controller.enqueue() fast path for the byte controller - the byte-side
// analog of the default controller's enqueue wrapper above (same
// reorderability gate, evaluated in C++). The value that settles a pending
// read is a C++-materialized transferred view, not the caller's chunk, so an
// armed defer comes back as a [closure, view] pair and the settle runs here
// as a JIT call. All other cases complete inside the binding, which then
// returns undefined. `chunk` is required (Web IDL length 1).
ObjectDefineProperty(ReadableByteStreamControllerNative.prototype, 'enqueue', {
  __proto__: null,
  // Matches the native operation's descriptor (and Web IDL: operations are
  // writable, enumerable and configurable; name "enqueue", length 1).
  writable: true,
  enumerable: true,
  configurable: true,
  value: {
    enqueue(chunk) {
      const pair = byteControllerEnqueueOrSettle(this, chunk);
      if (pair !== undefined) {
        if (pair.length === 2)  // [closure, view]
          pair[0](pair[1], false);
        else  // [closure, buffer, byteOffset, length, viewTag, transfer]
          pair[0](pair[1], false, false, pair[2], pair[3], pair[4], pair[5]);
      }
    },
  }.enqueue,
});

// The BYOBRequest's view, cached on the request as a JS own property under a
// module-private symbol by the byobRequest getter below. Possessing the
// symbol doubles as the brand (only that getter ever sets it, and the symbol
// is unreachable from user code) - matching the pre-flip implementation's
// per-realm symbol-keyed state.
const kViewCache = Symbol('kViewCache');

// controller.byobRequest: the binding returns the existing request, null, or
// a fresh [request, buffer, byteOffset, byteLength] tuple, and the request
// view is constructed here in JIT code over the descriptor's held buffer (no
// API view materialization) and cached on the request.
ObjectDefineProperty(
  ReadableByteStreamControllerNative.prototype, 'byobRequest', {
    __proto__: null,
    // Matches the native accessor's descriptor (and Web IDL: attributes are
    // enumerable and configurable; getter name "get byobRequest").
    enumerable: true,
    configurable: true,
    get: ObjectGetOwnPropertyDescriptor({
      get byobRequest() {
        const staged = byobRequestGetOrCreate(this);
        if (!ArrayIsArray(staged))
          return staged;  // The existing request, or null.
        const req = staged[0];
        req[kViewCache] = new Uint8Array(staged[1], staged[2], staged[3]);
        return req;
      },
    }, 'byobRequest').get,
  });

// byobRequest.view: a plain property load while the request is valid - zero
// binding crossings. A detached cached view needs one cold crossing to
// distinguish invalidation (the getter returns null from then on) from a
// user-detached buffer (the same view object keeps being returned).
ObjectDefineProperty(ReadableStreamBYOBRequestNative.prototype, 'view', {
  __proto__: null,
  // Matches the native accessor's descriptor (and Web IDL: attributes are
  // enumerable and configurable; getter name "get view").
  enumerable: true,
  configurable: true,
  get: ObjectGetOwnPropertyDescriptor({
    get view() {
      const view = this[kViewCache];
      if (view === undefined)
        throw new ERR_INVALID_THIS('ReadableStreamBYOBRequest');
      if (!ArrayBufferPrototypeGetDetached(TypedArrayPrototypeGetBuffer(view)))
        return view;
      return byobRequestIsInvalidated(this) ? null : view;
    },
  }, 'view').get,
});
...
function newReadableStreamFromSource(source, highWaterMark, sizeMode, sizeFn) {
  ...
  const startAlgorithm = start !== undefined ?
    (controller) => FunctionPrototypeCall(start, source, controller) :
    nonOpStart;
...
function values(options = kEmptyObject) {
  ...
  function nextSteps() {
    ...
    return PromisePrototypeThen(
      result === kReadFastEmpty ?
        FunctionPrototypeCall(originalDefaultReaderRead, reader) :
        result,
      onReadFulfilled,
      onReadRejected);
  }
  ...
  async function returnSteps(value) {
    ...
    if (!preventCancel) {
      const result = FunctionPrototypeCall(originalDefaultReaderCancel, reader, value);
      FunctionPrototypeCall(originalDefaultReaderReleaseLock, reader);
      await result;
      return { done: true, value }; // eslint-disable-line node-core/avoid-prototype-pollution
    }
...
function readableStreamPipeTo(
  ...
  async function step() {
    ...
    const result = await FunctionPrototypeCall(originalDefaultReaderRead, reader);
...
function readableStreamDefaultTee(stream, cloneForBranch2) {
  ...
  const branch1 = newReadableStream(
    (c) => { branch1Controller = c; }, pullAlgorithm, cancel1Algorithm,
    1, 0, undefined);
  const branch2 = newReadableStream(
...
function readableByteStreamTee(stream) {
  ...
  function pullWithDefaultReader() {
    if (readerIsByob) {
      FunctionPrototypeCall(originalByobReaderReleaseLock, reader);
  ...
  function pullWithBYOBReader(view, forBranch2) {
    ...
    const otherBranchController = forBranch2 === false ?
      branch2Controller : branch1Controller;
    PromisePrototypeThen(
      FunctionPrototypeCall(originalByobReaderRead, reader, view, { min: 1 }),
      (result) => {
  ...
  const branch1 = newReadableByteStream(
    (c) => { branch1Controller = c; }, pull1Algorithm, cancel1Algorithm, 0, 0);
  const branch2 = newReadableByteStream(
