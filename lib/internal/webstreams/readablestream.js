    ...
    return ObjectSetPrototypeOf({
      ...
      next() {
        ...
        if (!state.done && stream !== undefined &&
          ...
          if (isReadableStreamDefaultController(controller)) {
            if (queueLength(controller) > 0) {
              stream[kState].disturbed = true;
              const chunk = dequeueValue(controller);

              if (controller[kState].closeRequested &&
                  queueLength(controller) === 0) {
                readableStreamDefaultControllerClearAlgorithms(controller);
                readableStreamClose(stream);
              } else {
                readableStreamDefaultControllerPullIfNeededAfterDequeue(controller);
              }
...
function readableStreamPipeTo(
  ...
  async function step() {
    ...
    if (source[kState].state === 'readable' &&
      ...
      while (queueLength(controller) > 0) {
        if (shuttingDown) return true;
...
// ShouldCallPull reduced for a caller that has just dequeued a chunk from a
// readable default controller: the state is known to be readable and the
// queue was non-empty, so no read requests can be parked and only the
// close / started / desired-size arms of the predicate remain.
function readableStreamDefaultControllerPullIfNeededAfterDequeue(controller) {
  const state = controller[kState];
  if (!state.closeRequested &&
      state.started &&
      state.highWaterMark - state.queueTotalSize > 0) {
    readableStreamDefaultControllerPull(controller);
  }
}
...
function readableStreamDefaultControllerPullSteps(controller, readRequest) {
  ...
  if (queueLength(controller) > 0) {
    const chunk = dequeueValue(controller);
    if (controller[kState].closeRequested && queueLength(controller) === 0) {
      readableStreamDefaultControllerClearAlgorithms(controller);
      readableStreamClose(stream);
    } else {
      readableStreamDefaultControllerPullIfNeededAfterDequeue(controller);
