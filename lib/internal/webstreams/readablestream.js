...
class ReadableStream {
  ...
  getReader(options = kEmptyObject) {
    ...
    if (mode === undefined)
      return new classDefinitions.ReadableStreamDefaultReader(this);

    if (`${mode}` !== 'byob')
      throw new ERR_INVALID_ARG_VALUE('options.mode', mode);
    return new classDefinitions.ReadableStreamBYOBReader(this);
  }
  ...
  values(options = kEmptyObject) {
    ...
    const reader = new classDefinitions.ReadableStreamDefaultReader(this);
    let done = false;
    let started = false;
    let current;
  ...
  [kInspect](depth, options) {
    return customInspect(depth, options, this[kType], {
      ...
      supportsBYOB:
        this[kState].controller instanceof classDefinitions.ReadableByteStreamController,
    });
  }
