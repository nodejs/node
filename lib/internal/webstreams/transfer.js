...
class CrossRealmTransformReadableSource {
  constructor(port) {
    ...
    port.onmessageerror = () => {
      const error = new CloneableDOMException(
        ...
        'DataCloneError');
      port.postMessage({ __proto__: null, type: 'error', value: error });
      readableStreamDefaultControllerError(
  ...
  async pull() {
    this[kState].port.postMessage({ __proto__: null, type: 'pull' });
  }

  async cancel(reason) {
    try {
      this[kState].port.postMessage({ __proto__: null, type: 'error', value: reason });
    } catch (error) {
      if (error instanceof DOMException) {
        // eslint-disable-next-line no-ex-assign
        error = new CloneableDOMException(error.message, error.name);
      }
      this[kState].port.postMessage({ __proto__: null, type: 'error', value: error });
      throw error;
    } finally {
...
class CrossRealmTransformWritableSink {
  ...
  async write(chunk) {
    ...
    try {
      this[kState].port.postMessage({ __proto__: null, type: 'chunk', value: chunk });
    } catch (error) {
      if (error instanceof DOMException) {
        // eslint-disable-next-line no-ex-assign
        error = new CloneableDOMException(error.message, error.name);
  ...
  close() {
    this[kState].port.postMessage({ __proto__: null, type: 'close' });
    this[kState].port.close();
  }
