...
// Binds a MessagePort into a cross-realm source/sink that was constructed
// without one (the deserialize path, where the port arrives after the object).
const kBindPort = Symbol('kBindPort');
...
class CrossRealmTransformReadableSource {
  constructor(port, unref) {
    this[kState] = {
      ...
      unref,
      // Resolves once a port has been bound; pull() waits on it so a stream
      // produced by deserialization (port arrives later) does not post before
      // the port exists.
      portReady: undefined,
      resolvePortReady: undefined,
    };
    if (port !== undefined) {
      this[kState].portReady = PromiseResolve();
      this.#wire();
    } else {
      const { promise, resolve } = PromiseWithResolvers();
      this[kState].portReady = promise;
      this[kState].resolvePortReady = resolve;
    }
  }

  // Binds a port that was not available at construction time (deserialization).
  [kBindPort](port) {
    this[kState].port = port;
    this.#wire();
    this[kState].resolvePortReady?.();
    this[kState].resolvePortReady = undefined;
  }

  #wire() {
    const port = this[kState].port;
    port.onmessage = ({ data }) => {
      const {
        controller,
      } = this[kState];
