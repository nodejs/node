...
function newWritableStreamFromStreamWritable(streamWritable) {
  ...
  const strategy =
    streamWritable.writableObjectMode ?
      new CountQueuingStrategy({ __proto__: null, highWaterMark }) :
  ...
  const cleanup = finished(streamWritable, (error) => {
    if (error?.code === 'ERR_STREAM_PREMATURE_CLOSE') {
      const err = new AbortError(undefined, { __proto__: null, cause: error });
      error = err;
    }
...
function newReadableStreamFromStreamReadable(streamReadable, options = kEmptyObject) {
  ...
  const evaluateStrategyOrFallback = (strategy) => {
    ...
    if (objectMode) {
      ...
      return new CountQueuingStrategy({ __proto__: null, highWaterMark });
    }
    ...
    // is unnecessary.
    return { __proto__: null, highWaterMark };
  };
...
function newReadableWritablePairFromDuplex(duplex) {
  ...
  if (isDestroyed(duplex)) {
    ...
    writable.close();
    readable.cancel();
    return { __proto__: null, readable, writable };
  }
  ...
  return { __proto__: null, writable, readable };
}
