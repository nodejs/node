...
function newReadableStreamFromStreamReadable(streamReadable, options = kEmptyObject) {
  ...
  function evaluateStrategyOrFallback(objectMode, strategy) {
    // Fallback to default behavior
    if (!strategy)
      return objectMode ? new CountQueuingStrategy({ highWaterMark }) : { highWaterMark };
    // Is there is a strategy available, use it
    return strategy;
  }

  const strategy = evaluateStrategyOrFallback(objectMode, options?.strategy);
