...
const messageArguments = new SafeMap();
const codes = {};
...
const prepareStackTrace = (globalThis, error, trace) => {
  ...
  if (firstFrame && StringPrototypeStartsWith(firstFrame, nodeInternalPrefix)) {
    ...
    for (; i > 0; i--) {
      ...
      if (fn && StringPrototypeStartsWith(fn, nodeInternalPrefix)) {
        trace = ArrayPrototypeSlice(trace, i + 1);
        break;
      }
    }
    if (i === 0) {
      ArrayPrototypeShift(trace);
    }
  }
  ...
  if (typeof toString === 'function') {
    errorString = FunctionPrototypeCall(toString, error);
    if (typeof errorString !== 'string' || StringPrototypeStartsWith(errorString, '[object ')) {
...
let stackTraceLimitWritable = 0;
function setStackTraceLimit(limit) {
  if (stackTraceLimitWritable === 1) {
    try {
      Error.stackTraceLimit = limit;
    } catch {
      stackTraceLimitWritable = -1;
    }
  } else if (stackTraceLimitWritable === 0) {
...
function E(sym, val, def, ...otherClasses) {
  ...
  if (typeof val === 'string') {
    ...
    while (RegExpPrototypeExec(regex, val) !== null) expectedLength++;
    messageArguments.set(sym, expectedLength);
  } else {
    messageArguments.set(sym, val.length);
  }
...
function getMessage(key, msg, args) {
  ...
  return lazyInternalUtilInspect().format(msg, ...args);
}
...
E('ERR_OUT_OF_RANGE',
  (str, range, input, replaceDefaultBoolean = false) => {
    ...
    const received = lazyInternalUtilInspect().inspect(input, { numericSeparator: true });
