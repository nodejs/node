...
E('ERR_INVALID_SIGNATURE', (fn, signature, args) => {
  const formatted = lazyUtil().inspect(args, {
    depth: 0,
    breakLength: Infinity
  }).replace(/^\[\s*/, '(').replace(/\s*\]$/, ')');
  return 'Function was invoked with an unknown combination of arguments.' +
         `\nExpected ${fn}${signature}\nReceived ${fn}${formatted}`;
});
