...
E('ERR_REQUIRE_ESM', (filename, module = null, pkg = null) => {
  let msg = `Must use import to load ES Module: ${filename}\n`;
  if (module && pkg) {
    const parentPath = module.parent && module.parent.filename;
    const basename = parentPath &&
        lazyPath().basename(filename) === lazyPath().basename(parentPath) ?
      filename : lazyPath().basename(filename);
    msg +=
      ...
      `${lazyPath().resolve(pkg.path, 'package.json')}.\n`;
    return msg;
  }
  return msg;
}, Error);
