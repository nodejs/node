...
class SystemError extends Error {
  constructor(key, context) {
    ...
    if (isErrorStackTraceLimitWritable()) Error.stackTraceLimit = limit + 25;
    super();
...
const uvException = hideStackFrames(function uvException(ctx) {
  ...
  if (isErrorStackTraceLimitWritable()) Error.stackTraceLimit = tmpLimit + 25;
...
E('ERR_OUT_OF_RANGE',
  (str, range, input, replaceDefaultBoolean = false) => {
    ...
    if (NumberIsInteger(input) && MathAbs(input) > 2 ** 32) {
      received = lazyInternalUtilInspect().addNumericSeparator(String(input));
    } else if (typeof input === 'bigint') {
      received = String(input);
      if (input > 2n ** 32n || input < -(2n ** 32n)) {
        received = lazyInternalUtilInspect().addNumericSeparator(received);
      }
      received += 'n';
    } else {
