...
function isOwnedByVFS(vfs, cjsFilename) {
  ...
  try {
    return vfs.shouldHandle(cjsFilename);
  } catch {
    return false;
  }
}
...
function purgeLoaderCachesForVFS(vfs) {
  ...
  // Use ArrayPrototypeForEach (not for-of) so a polluted Array iterator
  // can't hijack the loader's cleanup path. Same defense applies to all
  // the loops in this function.
  const cjsLoader = require('internal/modules/cjs/loader');
  const moduleCache = cjsLoader.Module._cache;
  ArrayPrototypeForEach(ObjectKeys(moduleCache), (filename) => {
  ...
  ArrayPrototypeForEach(ObjectKeys(pathCache), (key) => {
...
function installModuleLoaderOverrides() {
  ...
  setLoaderFsOverrides({
    ...
    legacyMainResolve(pkgPath, main, base) {
      ...
      // Match the C++ binding's message shape (src/node_file.cc:3927-4005):
      // when `main` is a non-empty string, the initial candidate path is
      // pkgPath/main; otherwise the binding sets it to pkgPath/index.js
      // (the first extensionless fallback + ".js"). The third arg
      // `exactUrl` must be undefined - not a string - so the message uses
      // the "package" word and err.url is not overwritten.
  ...
  setLoaderPackageOverrides({
    ...
    getNearestParentPackageJSON(checkPath) {
      for (let i = 0; i < activeVFSList.length; i++) {
        if (activeVFSList[i].shouldHandle(checkPath)) {
    ...
    getPackageScopeConfig(resolved) {
      ...
      for (let i = 0; i < activeVFSList.length; i++) {
        if (activeVFSList[i].shouldHandle(filePath)) {
