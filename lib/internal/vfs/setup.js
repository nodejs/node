...
function formatMount(vfs) {
  return vfs.mountPoint ?? '<unmounted>';
}

function debugRoute(op, filename, vfs, result) {
  debug('%s %s -> %s (mount=%s overlay=%s)',
        op,
        filename,
        result,
        formatMount(vfs),
...
function registerVFS(vfs) {
  ...
  if (ArrayPrototypeIndexOf(activeVFSList, vfs) === -1) {
    ...
    debug('register mount=%s overlay=%s active=%d',
          formatMount(vfs), vfs.overlay, activeVFSList.length);
    if (!hooksInstalled) {
      installHooks();
...
function deregisterVFS(vfs) {
  ...
  if (index !== -1) {
    ...
    debug('deregister mount=%s active=%d', formatMount(vfs), activeVFSList.length);
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    if (vfs.shouldHandle(normalized)) {
      ...
      if (vfs.mounted || result >= 0) {
        debugRoute('stat', normalized, vfs, result);
        return { vfs, result };
      }
      debugRoute('stat', normalized, vfs, 'fallthrough');
    }
  }
  debug('stat %s -> fallthrough', normalized);
  return null;
}
    ...
    if (vfs.shouldHandle(normalized)) {
      ...
      if (vfs.existsSync(normalized)) {
        ...
        if (statResult !== 0) {
          ...
          debugRoute('read', normalized, vfs, 'fallthrough:not-file');
          return null;
        }
        try {
          const content = vfs.readFileSync(normalized, options);
          debugRoute('read', normalized, vfs, 'hit');
          return { vfs, content };
        } catch (e) {
          // If read fails, fall through to default fs
          // unless we're in mounted mode (where we should return the error)
          if (vfs.mounted) {
            debugRoute('read', normalized, vfs, `throw:${e?.code ?? e?.name ?? 'error'}`);
            throw e;
          }
          debugRoute('read', normalized, vfs, 'fallthrough:error');
        }
      } else if (vfs.mounted) {
        // In mounted mode, if path is under mount point but doesn't exist,
        // don't fall through to real fs - throw ENOENT
        debugRoute('read', normalized, vfs, 'throw:ENOENT');
        throw createENOENT('open', filename);
      }
      debugRoute('read', normalized, vfs, 'fallthrough:missing');
    }
  }
  debug('read %s -> fallthrough', normalized);
  return null;
}
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    if (vfs.shouldHandle(normalized)) {
      ...
      if (vfs.mounted || exists) {
        debugRoute('exists', normalized, vfs, exists);
        return { vfs, exists };
      }
      debugRoute('exists', normalized, vfs, 'fallthrough');
    }
  }
  debug('exists %s -> fallthrough', normalized);
  return null;
}
...
function findVFSForPath(filename) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    const vfs = activeVFSList[i];
    if (vfs.shouldHandle(normalized)) {
      debugRoute('path', normalized, vfs, 'hit');
      return { vfs, normalized };
    }
  }
  debug('path %s -> fallthrough', normalized);
  return null;
}
...
function findVFSForWatch(filename) {
  ...
  for (let i = 0; i < activeVFSList.length; i++) {
    ...
    if (vfs.shouldHandle(normalized)) {
      ...
      if (vfs.overlay) {
        if (vfs.existsSync(normalized)) {
          debugRoute('watch', normalized, vfs, 'hit');
          return { vfs };
        }
        // File doesn't exist in VFS, fall through to real fs.watch
        debugRoute('watch', normalized, vfs, 'fallthrough');
        continue;
      }
  ...
  debug('watch %s -> fallthrough', normalized);
  return null;
}
...
function createVfsHandlers() {
  return {
    ...
    readv(fd, buffers, position) {
      const vfd = getVirtualFd(fd);
      if (!vfd) return undefined;
      return PromiseResolve(this.readvSync(fd, buffers, position));
    },
    writev(fd, buffers, position) {
      const vfd = getVirtualFd(fd);
      if (!vfd) return undefined;
      return PromiseResolve(this.writevSync(fd, buffers, position));
    },
...
/**
 * Checks that two paths are handled by the same VFS instance.
 * Throws EXDEV if they belong to different VFS or if one is VFS and the other real.
 * @param {string} srcPath Source path
 * @param {string} destPath Destination path
 * @param {string} syscall The syscall name for error
 * @param {VirtualFileSystem} srcVfs The VFS that handles srcPath
 */
function checkSameVFS(srcPath, destPath, syscall, srcVfs) {
  ...
  // destPath is not in any VFS but srcPath is
  throw createEXDEV(syscall, srcPath);
}
