...
function createVfsHandlers() {
  return {
    ...
    readdirSync(path, options) {
      const pathStr = toPathStr(path);
      if (pathStr === null) return undefined;
      const result = findVFSWith(pathStr, 'scandir', (vfs, n) => vfs.readdirSync(n, options));
    ...
    statSync(path, options) {
      ...
      if (pathStr === null) return undefined;
      try {
        return findVFSWith(pathStr, 'stat', (vfs, n) => vfs.statSync(n, options));
      } catch (err) {
    ...
    realpathSync(path, options) {
      const pathStr = toPathStr(path);
      if (pathStr === null) return undefined;
      const result = findVFSWith(pathStr, 'realpath', (vfs, n) => vfs.realpathSync(n));
    ...
    chmodSync(path, mode) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) { r.vfs.chmodSync(r.normalized, mode); return true; }
      }
      return undefined;
    },
    chownSync(path, uid, gid) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) { r.vfs.chownSync(r.normalized, uid, gid); return true; }
      }
      return undefined;
    },
    ...
    utimesSync(path, atime, mtime) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) { r.vfs.utimesSync(r.normalized, atime, mtime); return true; }
      }
      return undefined;
    },
    lutimesSync(path, atime, mtime) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) { r.vfs.lutimesSync(r.normalized, atime, mtime); return true; }
      }
      return undefined;
    },
    ...
    mkdtempSync(prefix, options) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) {
          const result = r.vfs.mkdtempSync(r.normalized);
          if (options?.encoding === 'buffer') {
            return Buffer.from(result);
          }
          return result;
    ...
    async readdir(path, options) {
      const pathStr = toPathStr(path);
      if (pathStr === null) return undefined;
      const result = await findVFSWithAsync(pathStr, 'scandir', (vfs, n) => vfs.promises.readdir(n, options));
      if (result !== undefined && options?.encoding === 'buffer' && !options?.withFileTypes) {
        for (let i = 0; i < result.length; i++) {
          if (typeof result[i] === 'string') result[i] = Buffer.from(result[i]);
        }
    ...
    async lstat(path, options) {
      ...
      for (let i = 0; i < activeVFSList.length; i++) {
        ...
        if (vfs.shouldHandle(normalized)) {
          try {
            return await vfs.promises.lstat(normalized, options);
          } catch (e) {
            if (vfs.mounted) throw e;
          }
        }
    ...
    async stat(path, options) {
      ...
      if (pathStr === null) return undefined;
      try {
        return await findVFSWithAsync(pathStr, 'stat', (vfs, n) => vfs.promises.stat(n, options));
      } catch (err) {
        if (err?.code === 'ENOENT' && options?.throwIfNoEntry === false) return undefined;
        throw err;
      }
    ...
    async realpath(path, options) {
      const pathStr = toPathStr(path);
      if (pathStr === null) return undefined;
      const result = await findVFSWithAsync(pathStr, 'realpath', (vfs, n) => vfs.promises.realpath(n, options));
      if (result !== undefined && options?.encoding === 'buffer') {
        return Buffer.from(result);
      }
      return result;
    ...
    async access(path, mode) {
      ...
      if (pathStr !== null) {
        ...
        if (r !== null) {
          ...
          await r.vfs.promises.access(r.normalized, mode);
          return true;
        }
      }
      return undefined;
    ...
    async readlink(path, options) {
      ...
      for (let i = 0; i < activeVFSList.length; i++) {
        ...
        if (vfs.shouldHandle(normalized)) {
          try {
            const result = await vfs.promises.readlink(normalized, options);
            if (options?.encoding === 'buffer') {
              return Buffer.from(result);
            }
            return result;
    ...
    async mkdtemp(prefix, options) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) {
          const result = await r.vfs.promises.mkdtemp(r.normalized);
          if (options?.encoding === 'buffer') {
            return Buffer.from(result);
          }
          return result;
