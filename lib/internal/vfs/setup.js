...
function findVFSPackageJSON(vfs, startPath, normalizedStart) {
  ...
  while (currentDir !== lastDir) {
    ...
    if (vfsStat(vfs, pjsonPath) === 0) {
      let content;
      try {
        content = vfs.readFileSync(pjsonPath, 'utf8');
      } catch {
        // File vanished or is unreadable after stat said it existed.
        // Keep walking - matches the "no package.json here" case in
        // native GetPackageJSON when ReadFileSync fails.
        content = null;
      }
      if (content !== null) {
        // parsePackageJSON validates types and throws
        // ERR_INVALID_PACKAGE_CONFIG on any error; propagates to
        // abort the walk exactly like native GetPackageJSON.
...
function installModuleLoaderOverrides() {
  ...
  setLoaderPackageOverrides({
    readPackageJSON(jsonPath, isESM, base, specifier) {
      ...
      // Native parsePackageJSON throws ERR_INVALID_PACKAGE_CONFIG on
      // syntactic errors and on wrong-type fields (name/type). Both
      // CJS and ESM raise the error since nodejs/node#48606.
    ...
    getPackageType(url) {
      ...
      if (found.tuple !== undefined) {
        // Position 2 of the tuple is `type` per the C++ binding
        // shape (name, main, type, imports, exports, filePath).
