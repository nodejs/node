...
const providers = [
  ...
  },
  {
    ...
    create(resolvedPath, stats, options) {
      ...
      return new RealFSProvider(resolvedPath, options);
    },
  },
];
 ...
 * @param {(resolvedPath: string, stats: object, options: object) => object}
 *   entry.create Returns the VirtualProvider backing `resolvedPath`.
 *   `options` carries the `name` given on the command line, if any.
 */
...
function selectProvider(resolvedPath, stats, options) {
  for (let i = 0; i < providers.length; i++) {
    if (providers[i].canHandle(resolvedPath, stats)) {
      return providers[i].create(resolvedPath, stats, options);
    }
  }
  return null;
}
