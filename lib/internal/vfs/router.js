 ...
 * spelled the way a number is, and so is reserved for layer ids rather
 * than available as a mount name. `07` is not: no layer id is spelled so.
 * @param {string} segment
 * @returns {boolean}
 */
function isLayerId(segment) {
  return `${MathRound(+segment)}` === segment;
}
