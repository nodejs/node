...
class SEAProvider extends VirtualProvider {
  ...
  constructor(options = {}) {
    ...
    // Build asset map, size cache, and derive directory structure
    this[kAssets] = new SafeMap();
    this[kDirectories] = new SafeMap();
    // Cache of asset sizes (path -> number) to avoid loading full content for stat
    this._assetSizes = new SafeMap();
  ...
  openSync(path, flags, mode) {
    // Normalize numeric flags (e.g. O_RDONLY = 0) to string
    ...
    const content = this.#getAssetContent(normalized);
  ...
  statSync(path, options) {
    ...
    if (this.#isFile(normalized)) {
      let size = this._assetSizes.get(normalized);
      if (size === undefined) {
        const content = this.#getAssetContent(normalized);
        size = content.length;
        this._assetSizes.set(normalized, size);
      }
      return createFileStats(size, { mode: 0o444 });
    }
  ...
  readdirSync(path, options) {
    ...
    const names = ArrayFrom(children);
  ...
  #readdirRecursive(dirPath, withFileTypes) {
    ...
    const walk = (currentPath, relativePath) => {
      const children = this[kDirectories].get(currentPath);
      if (!children) return;
