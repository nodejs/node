...
class MemoryProvider extends VirtualProvider {
   ...
   * @param {string} [options.name] A name identifying this provider
   */
  constructor(options) {
    super(options);
    // Root directory
  ...
  #createStats(entry, size, bigint) {
    ...
    if (entry.isFile()) {
      ...
      if (fileSize === undefined) {
        fileSize = entry.isDynamic() ?
          entry.getContentSync().length :
  ...
  renameSync(oldPath, newPath) {
    ...
    if (entry.isDirectory()) {
      // Resolve both paths so symlinked parents cannot hide a descendant.
      const { resolvedPath: resolvedOld } = this.#lookupEntry(normalizedOld, false);
      const { resolvedPath: resolvedParent } = this.#lookupEntry(pathPosix.dirname(normalizedNew));
      if (resolvedParent === resolvedOld ||
          StringPrototypeStartsWith(resolvedParent, pathPosix.join(resolvedOld, '/'))) {
