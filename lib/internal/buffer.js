...
function _asciiWrite(string, offset = 0, length = string.length) {
  if (length <= 32) {
    // TODO: What if this is not a Buffer?
    // TODO: Errors are not exactly the same as for asciiWrite?
    ...
    if (typeof offset !== 'number') {
      throw new ERR_INVALID_ARG_TYPE('offset', 'number', string);
    }

    if (offset > this.length || offset < 0) {
      throw new ERR_BUFFER_OUT_OF_BOUNDS("\"offset\" is outside of buffer bounds");
    }

    if (typeof length !== 'number') {
      throw new ERR_INVALID_ARG_TYPE('length', 'number', string);
    }

    if (length > this.length - offset || length < 0) {
      throw new ERR_BUFFER_OUT_OF_BOUNDS("\"length\" is outside of buffer bounds");
    }

    for (let n = 0; n < length; n++) {
      this[offset + n] = string.charCodeAt(n);
    }
  } else {
