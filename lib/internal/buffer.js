...
const _dataview = Symbol('internal dataview');
...
function checkUInt32(value, type, positive) {
  validateUint32(value, type || 'offset', positive);
}
...
/* BigIntN - needs work */
function readBigN_LE(buf, uint, offset, bitWidth, bitOffset = 0) {
  checkUInt32(offset);
  checkUInt32(bitWidth, 'bitWidth', true);
  if (bitOffset & 7 !== bitOffset)
    throw new ERR_OUT_OF_RANGE('bitOffset', '>= 0 and <= 7', value);

  const bits = bitWidth + bitOffset;
  const bytes = bits >>> 3 + (bits & 7 ? 1 : 0);
  if (buf.byteLength < offset + bytes)
    boundsError(bitWidth, buf.byteLength - bytes, 'bitWidth');

  let ret = 0n;
  let i = 0;
  if (bitOffset)
    ret |= BigInt(buf[offset + i++] & (1 << bitOffset) - 1);

  // Moderate efficiency loop to decrease loop amounts.
  for (const n64 = bytes >>> 3 << 3; i < n64; i += 8) {
    ret <<= 64n;
    ret |= buf[_dataview].getBigUint64(buf.byteOffset + offset + i, true);
  }
  if (bytes - i > 3) {
    ret <<= 32n;
    ret |= BigInt(buf[_dataview].getUint32(buf.byteOffset + offset + i,
                  true));
    i += 4
  }
  if (bytes - i > 1) {
    ret <<= 16n;
    ret |= BigInt(buf[_dataview].getUint16(buf.byteOffset + offset + i,
                  true));
    i += 2
  }
  if (bytes - i) {
    ret <<= 8n;
    ret |= BigInt(buf[offset + i]);
  }
  // what am I doing anymore?
  ret >>= bitWidth & 7;
  return uint ? BigInt.asUintN(bitWidth, ret >> BigInt(bitWidth & 7)) : BigInt.asIntN(biwWidth, ret);
}
/* BigIntN - needs work */

function readBigUIntLE(offset, bitWidth, bitOffset) {
  return readBigN_LE(this, true, offset, bitWidth, bitOffset)
}
...
/* BigIntN - needs work */
function readBigN_BE(buf, uint, offset, bitWidth, bitOffset = 0) {
  checkUInt32(offset);
  checkUInt32(bitWidth, 'bitWidth', true);
  if (bitOffset & 7 !== bitOffset)
    throw new ERR_OUT_OF_RANGE('bitOffset', '>= 0 and <= 7', value);
  ...
  let i = 0;
  if (bitOffset)
    ret |= BigInt(buf[offset + bytes - i++] & (1 << bitOffset) - 1);

  // Moderate efficiency loop to decrease loop amounts.
  for (const n64 = bytes >>> 3 << 3; i < n64; i += 8) {
    ret <<= 64n;
    ret |= buf[_dataview].getBigUint64(buf.byteOffset + offset + bytes - i);
  }
  if (bytes - i > 3) {
    ret <<= 32n;
    ret |= BigInt(buf[_dataview].getUint32(buf.byteOffset + offset +
                  bytes - i));
    i += 4
  }
  if (bytes - i > 1) {
    ret <<= 16n;
    ret |= BigInt(buf[_dataview].getUint16(buf.byteOffset + offset +
                  bytes - i));
    i += 2
  }
  if (bytes - i) {
  ...
  // This function is rather difficult to replace...
  return uint ? BigInt.asUintN(bitWidth, ret) : BigInt.asIntN(bitWidth, ret);
}
/* BigIntN - needs work */

function readBigUIntBE(offset, bitWidth, bitOffset) {
  return readBigN_BE(this, true, offset, bitWidth, bitOffset)
}
...
function readBigIntLE(offset, bitWidth, bitOffset) {
  return readBigN_LE(this, false, offset, bitWidth, bitOffset)
}
...
function readBigIntBE(offset, bitWidth, bitOffset) {
  return readBigN_BE(this, false, offset, bitWidth, bitOffset)
}
...
function writeBigN_LE(buf, value, offset, min, max, bitWidth, bitOffset = 0) {
  if (bitOffset & 7 !== bitOffset)
    throw new ERR_OUT_OF_RANGE('bitOffset', '>= 0 and <= 7', value);
  ...
  value = BigInt(value);
  checkInt(value, min, max, buf, offset, bytes)

  let i = 0;
  if (bitOffset) {
    buf[offset] = (buf[offset] & ~((1 << bitOffset) - 1)) |
      Number(value >> BigInt(bitWidth >>> 3 << 3));
    ++i;
  }
  return offset + bytes
}

function writeBigUIntLE(value, offset, bitWidth, bitOffset) {
  return writeBigN_LE(this, value, offset, 0n,
                      BigInt.asUintN(bitWidth, -1n), bitWidth, bitOffset);
}

function writeBigIntLE(value, offset, bitWidth, bitOffset) {
  const ibits = bitWidth - 1;
  return writeBigN_LE(this, value, offset, -(1n << BigInt(ibits)),
                      BigInt.asUintN(ibits, -1n), bitWidth, bitOffset);
}
