...
const triggerAsyncIdSymbol = Symbol('triggerAsyncId');
const destroyedSymbol = Symbol('destroyed');

function wrapPromise(promise, parent, silent) {
  const asyncId = getOrSetAsyncId(promise);

  if (parent) {
    promise[triggerAsyncIdSymbol] = getOrSetAsyncId(parent);
    promise.isChainedPromise = true;
  } else {
    promise[triggerAsyncIdSymbol] = getDefaultTriggerAsyncId();
    promise.isChainedPromise = false;
  }

  if (!silent && initHooksExist()) {
    emitInitScript(asyncId, 'PROMISE', promise[triggerAsyncIdSymbol], promise);
  }
...
function promiseHook(type, promise, parent) {
  if (type === 'init' || !promise[async_id_symbol]) {
    ...
    if (parent instanceof Promise) {
      if (!parent[async_id_symbol]) {
        wrapPromise(parent, null, true);
      }

      wrapPromise(promise, parent, silent);
    } else {
      wrapPromise(promise, null, silent);
    }
  }
  ...
  switch (type) {
    case 'before':
      const triggerId = promise[triggerAsyncIdSymbol];
      emitBeforeScript(getOrSetAsyncId(promise), triggerId, promise);
      break;
    case 'after':
      emitAfterScript(getOrSetAsyncId(promise));
      break;
    case 'promiseResolve':
      emitPromiseResolveNative(getOrSetAsyncId(promise));
      break;
  }
}
