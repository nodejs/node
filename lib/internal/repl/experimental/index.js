...
class ExperimentalREPLServer extends EventEmitter {
  ...
  complete(line) {
    ...
    if (line[line.length - 1] === ' ') return undefined;

    if (line[line.length - 1] === '.' || StringPrototypeSlice(line, -2, -1) === '[') {
    ...
    if (keys) {
      if (filter) {
        keys = ArrayPrototypeMap(
          ArrayPrototypeFilter(keys, (k) => StringPrototypeStartsWith(k, filter) && k !== filter),
