...
function isRecoverableError(e, code) {
  ...
  try {
    RecoverableParser.parse(code, { __proto__: null, ecmaVersion: 'latest' });
...
function setupPreview(repl, contextSymbol, bufferSymbol, active) {
  ...
  if (process.env.TERM === 'dumb' || !active) {
    return { __proto__: null, showPreview() {}, clearPreview() {} };
  }
  ...
  function getPreviewPos() {
    ...
    const cursorPos = repl.line.length !== repl.cursor ?
      repl.getCursorPos() :
      displayPos;
    return { __proto__: null, displayPos, cursorPos };
  }
...
function setupReverseSearch(repl) {
  ...
  if (process.env.TERM === 'dumb') {
    return { __proto__: null, reverseSearch() { return false; } };
  }
  ...
  return { __proto__: null, reverseSearch };
}
