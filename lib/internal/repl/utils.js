...
// TODO(@avivkeller): Amaro exposes `isRecoverableError()` for this purpose,
// but as of now it does not recognise input that ends in the middle of
// an expression (e.g. `foo(` or `const x = {`) as recoverable, so acorn is
// still used here.
...
function isValidSyntax(input) {
  const { isValidSyntax } = require('internal/deps/amaro/dist/nodejs');
  return isValidSyntax(input);
}
