      ...
      raise(pos, message) {
        switch (message) {
          ...
          case 'Unterminated string constant': {
            const token = StringPrototypeSlice(
              this.input,
              this.lastTokStart,
              this.pos
            );
...
function setupPreview(repl, contextSymbol, bufferSymbol, active) {
  ...
  function getPreviewPos() {
    ...
    const cursorPos =
      repl.line.length !== repl.cursor ? repl.getCursorPos() : displayPos;
  ...
  function isCursorAtInputEnd() {
    ...
    return (
      cursorPos.rows === displayPos.rows && cursorPos.cols === displayPos.cols
    );
  }
  ...
  function isInStrictMode(repl) {
    return (
      ...
      ArrayPrototypeIncludes(
        ArrayPrototypeMap(process.execArgv, (e) =>
          StringPrototypeReplace(StringPrototypeToLowerCase(e), /_/g, '-')
        ),
        '--use-strict'
      )
    );
  ...
  function getInputPreview(input, callback) {
    ...
    if (
      StringPrototypeStartsWith(input, '{') &&
  ...
  repl._moveCursor = (dx) => {
    ...
    if (
      currentCursor + dx > repl.line.length &&
