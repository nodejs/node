...
// Wrap falsy thrown values so the evaluator callback cannot mistake them for
// successful completion while retaining the exact value for error display.
class ThrownValue {
  constructor(value) {
    this.value = value;
  }
...
function createReplEval(repl) {
  ...
  async function inspectorEval(rawCode, context, file, cb) {
    ...
    try {
      ...
      return cb(err ? err : new ThrownValue(err));
    }
