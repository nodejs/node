...
// Give each REPL its own object group. Multiple REPLs can evaluate
// concurrently, and releasing a shared group from one channel could otherwise
// collect a promise that another channel is still awaiting.
let nextObjectGroupId = 0;
...
const kStashFn = `function() {
  globalThis[${JSONStringify(kStashKey)}].value = this;
}`;
...
function prepareContext(context) {
  if (!ReflectHas(context, kStashKey)) {
    ObjectDefineProperty(context, kStashKey, {
      __proto__: null,
...
class ReplInspectorChannel {
  constructor(repl) {
    ...
    this.objectGroup = `node-repl-${nextObjectGroupId++}`;
  }
  ...
  postInterruptible(method, params, breakOnSigint) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Let the terminal driver turn Ctrl+C into SIGINT while synchronous
      // inspector dispatch is blocking the main thread.
      const previouslyInRawMode = this.repl._setRawMode(false);
      let interrupted;
      try {
        interrupted = runInterruptible(() => {
          this.getSession().post(method, params, handle);
        });
      } finally {
        this.repl._setRawMode(previouslyInRawMode);
      }
      if (interrupted) {
  ...
  async toLiveValue(remote, context) {
    if (remote.objectId !== undefined) {
      const response = await this.post('Runtime.callFunctionOn', {
      ...
      const value = context[kStashKey].value;
      context[kStashKey].value = undefined;
