...
function awaitImport(specifier) {
  return `await ${kDynamicImportName}(${JSONStringify(specifier)})`;
}
...
function importDeclarationToScript(node) {
  ...
  if (namespaceName !== undefined) {
    let out = `const ${namespaceName} = ${awaitImport(specifier)};`;
  ...
  return `const { ${ArrayPrototypeJoin(named, ', ')} } = ${awaitImport(specifier)};`;
}
