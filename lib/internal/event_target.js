...
const kPreprocessEvent = Symbol('kPreprocessEvent');
  ...
  dispatchEvent(event) {
    ...
    while (handler !== undefined &&
      ...
      if (handler.once) {
        ...
        this[kRemoveListener](root.size, event.type, listener, capture);
      }

      try {
        const arg = handler.isNodeStyleListener ?
          this[kPreprocessEvent](event) : event;
