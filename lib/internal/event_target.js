...
function weakListeners() {
  ...
  return { __proto__: null, registry: weakListenersState, map: objectToWeakListenerMap };
}
...
class EventTarget {
  ...
  [kNewListener](size, type, listener, once, capture, passive, weak) {
    if (this[kMaxEventTargetListeners] > 0 &&
      ...
      const w = new Error('Possible EventTarget memory leak detected. ' +
                          `${size} ${type} listeners ` +
                          `added to ${inspect(this, { __proto__: null, depth: -1 })}. Use ` +
  ...
  addEventListener(type, listener, options = kEmptyObject) {
    ...
    if (root === undefined) {
      root = { __proto__: null, size: 1, next: undefined, resistStopPropagation: Boolean(resistStopPropagation) };
  ...
  [kCreateEvent](nodeValue, type) {
    return new NodeCustomEvent(type, { __proto__: null, detail: nodeValue });
  }
  [customInspectSymbol](depth, options) {
    if (!isEventTarget(this))
      throw new ERR_INVALID_THIS('EventTarget');
    ...
    return `${name} ${inspect({ __proto__: null }, opts)}`;
  }
}
...
class NodeEventTarget extends EventTarget {
  ...
  on(type, listener) {
    if (!isNodeEventTarget(this))
      throw new ERR_INVALID_THIS('NodeEventTarget');
    this.addEventListener(type, listener, { __proto__: null, [kIsNodeStyleListener]: true });
    return this;
  }
  ...
  once(type, listener) {
    ...
    this.addEventListener(type, listener,
                          { __proto__: null, once: true, [kIsNodeStyleListener]: true });
    return this;
  }
...
function validateEventListenerOptions(options) {
  if (typeof options === 'boolean')
    return { __proto__: null, capture: options };

  if (options === null)
    return kEmptyObject;
  validateObject(options, 'options', {
    __proto__: null, allowArray: true, allowFunction: true,
  });
  return {
    __proto__: null,
