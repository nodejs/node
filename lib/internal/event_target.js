...
class Listener {
  ...
  constructor(listener, once, capture, passive) {
...
class EventTarget {
  ...
  addEventListener(type, listener, options = {}) {
    ...
    const existingListener = this[kEvents].get(type);

    if (existingListener === undefined) {
      this[kNewListener](1, type, listener, once, capture, passive);
      this[kEvents].set(type, new Listener(listener, once, capture, passive));
      return;
    }

    if (Array.isArray(existingListener)) {
      for (const handler of existingListener) {
        if (handler.same(listener, capture)) { // Duplicate! Ignore
          return;
        }
      }
      existingListener.push(new Listener(listener, once, capture, passive));
      this[kNewListener](
        existingListener.length + 1,
        type, listener, once, capture, passive
      );
    } else {
      this[kEvents].set(type, [
        existingListener,
        new Listener(listener, once, capture, passive)
      ]);
      this[kNewListener](2, type, listener, once, capture, passive);
    }
  }
  ...
  removeEventListener(type, listener, options = {}) {
    ...
    const handler = this[kEvents].get(type);
    if (handler === undefined)
      return;

    if (Array.isArray(handler)) {
      for (let i = 0; i < handler.length; i++) {
        if (handler[i].same(listener, capture)) {
          handler.splice(i, 1);
          break;
        }
      }
      ...
      if (handler.length === 1) {
        this[kEvents].set(type, handler[0]); // Optimize for singular listener
      }

      this[kRemoveListener](handler.length, type, listener, capture);
      return;
    }

    if (handler.same(listener, capture)) {
      this[kEvents].delete(type);
      this[kRemoveListener](0, type, listener, capture);
    }
  }
  ...
  dispatchEvent(event) {
    ...
    const handler = this[kEvents].get(event.type);
    if (handler === undefined) {
      return true;
    }
...
class NodeEventTarget extends EventTarget {
  ...
  listenerCount(type) {
    const handlers = this[kEvents].get(String(type));
