...
// Returns: string
message.withkey = function(key, ...args) {
  const message = messages.msg(key);
  ...
  if (!debugs[set]) {
    if (new RegExp(`\\b${set}\\b`, 'i').test(debugEnviron)) {
      ...
      debugs[set] = function(key, ...args) {
        const message = messages.msg(key);
        const msg = template(message.msg, args, message.id);
