...
function ChildProcess() {
  ...
  this._handle.onexit = (exitCode, signalCode) => {
    ...
    // Do it on queueMicrotask so that the user has one last chance
    // to consume the output, if for example they only want to
    // start reading the data once the process exits.
    queueMicrotask(() => flushStdio(this));
...
ChildProcess.prototype.spawn = function(options) {
  ...
  if (err === UV_EACCES ||
      ...
      err === UV_ENFILE ||
      err === UV_ENOENT) {
    queueMicrotask(() => onErrorNT(this, err));
...
function setupChannel(target, channel, serializationMode) {
  ...
  target.send = function(message, handle, options, callback) {
    ...
    if (typeof callback === 'function') {
      ...
      queueMicrotask(() => this.emit('error', ex));
    }
    return false;
  };
  ...
  function handleMessage(message, handle, internal) {
    ...
    queueMicrotask(() => emit(eventName, message, handle));
  }
