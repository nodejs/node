...
function setupChannel(target, channel) {
  ...
  target.send = function(message, handle, callback) {
    ...
    const ex = new I18N.Error(I18N.CHILD_PROCESS_CHANNEL_CLOSED);
  ...
  target._send = function(message, handle, swallowErrors, callback) {
    ...
    if (message === undefined)
      throw new I18N.TypeError(I18N.REQUIRED_ARGUMENT, 'message');
    ...
    if (handle) {
      ...
      if (handle instanceof net.Socket) {
        ...
        throw new I18N.TypeError(I18N.CHILD_PROCESS_BAD_HANDLE_TYPE);
      }
  ...
  target.disconnect = function() {
    if (!this.connected) {
      this.emit('error', new I18N.Error(I18N.CHILD_PROCESS_IPC_DISCONNECTED));
      return;
    }
...
function _validateStdio(stdio, sync) {
  ...
  if (typeof stdio === 'string') {
    switch (stdio) {
      ...
      default:
        throw new I18N.TypeError(I18N.CHILD_PROCESS_INCORRECT_STDIO, stdio);
    }
  } else if (!Array.isArray(stdio)) {
    throw new I18N.TypeError(I18N.CHILD_PROCESS_INCORRECT_STDIO,
                         util.inspect(stdio));
  }
  ...
  stdio = stdio.reduce(function(acc, stdio, i) {
    ...
    if (stdio === null || stdio === 'ignore') {
      ...
      if (sync || ipc !== undefined) {
        ...
        cleanup();
        if (!sync)
          throw new I18N.Error(I18N.CHILD_PROCESS_ONE_IPC);
        else
          throw new I18N.Error(I18N.CHILD_PROCESS_NOIPC_SYNC);
      }
      ...
      if (!sync) {
        cleanup();
        throw new I18N.TypeError(I18N.CHILD_PROCESS_BUFFER_NOT_SUPPORTED,
                             util.inspect(stdio));
      }
    } else {
      // Cleanup
