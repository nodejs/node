...
function ChildProcess() {
  ...
  this._handle.onexit = (exitCode, signalCode) => {
    ...
    process._deferTick(flushStdio, this);
...
ChildProcess.prototype.spawn = function(options) {
  ...
  if (err === UV_EACCES ||
      ...
      err === UV_ENFILE ||
      err === UV_ENOENT) {
    process._deferTick(onErrorNT, this, err);
    ...
    process._deferTick(onSpawnNT, this);
  }
...
function setupChannel(target, channel, serializationMode) {
  ...
  target.send = function(message, handle, options, callback) {
    ...
    if (typeof callback === 'function') {
      ...
      process._deferTick(() => this.emit('error', ex));
    }
    return false;
  };
  ...
  function handleMessage(message, handle, internal) {
    ...
    process._deferTick(emit, eventName, message, handle);
  }
