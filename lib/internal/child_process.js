...
ChildProcess.prototype.kill = function(sig) {
  ...
  if (signal === undefined) {
    throw new errors.Error('UNKNOWNSIGNAL', sig);
  }
...
function setupChannel(target, channel) {
  ...
  target.send = function(message, handle, options, callback) {
    ...
    const ex = new errors.Error('CHANNELCLOSED');
  ...
  target._send = function(message, handle, options, callback) {
    ...
    if (message === undefined)
      throw new errors.TypeError('REQUIREDARG', 'message');
    ...
    if (handle) {
      ...
      if (handle instanceof net.Socket) {
        ...
        throw new errors.TypeError('IPCBADHANDLE');
      }
  ...
  target.disconnect = function() {
    if (!this.connected) {
      this.emit('error', new errors.Error('IPCDISCONNECTED'));
      return;
    }
...
function _validateStdio(stdio, sync) {
  ...
  if (typeof stdio === 'string') {
    switch (stdio) {
      ...
      default: throw new errors.TypeError('INVALIDOPTVALUE', 'stdio', stdio);
    }
  } else if (!Array.isArray(stdio)) {
    throw new errors.TypeError('INVALIDOPTVALUE',
                               'stdio',
                               util.inspect(stdio));
  }
  ...
  stdio = stdio.reduce(function(acc, stdio, i) {
    ...
    if (stdio === null || stdio === 'ignore') {
      ...
      if (sync || ipc !== undefined) {
        ...
        cleanup();
        if (!sync)
          throw new errors.Error('IPCONEIPC');
        else
          throw new errors.Error('IPCNOSYNCFORK');
      }
      ...
      if (!sync) {
        cleanup();
        throw new errors.TypeError('IPCBUFFERASYNCFORK', util.inspect(stdio));
      }
    } else {
      // Cleanup
      cleanup();
