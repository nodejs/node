...
function stdioStringToArray(stdio, channel) {
  ...
  switch (stdio) {
    ...
    case 'pty': ArrayPrototypePush(options, 'pipe', 'pty', 'pty'); break;
...
function getPtyDimensions(pty) {
  if (pty == null || pty === false) return null;
  let cols;
  let rows;
  let name;
  if (pty === true) {
    ...
    validateObject(pty, 'options.pty');
    cols = pty.cols ?? process.stdout.columns ?? 80;
    rows = pty.rows ?? process.stdout.rows ?? 24;
    name = pty.name;
  }
  validateUint32(cols, 'options.pty.cols', true);
  validateUint32(rows, 'options.pty.rows', true);
  if (name != null) {
    validateString(name, 'options.pty.name');
  }
  return { cols, rows, name };
}

function setEnvPair(envPairs, key, value) {
  const prefix = `${key}=`;
  // Windows env keys are case-insensitive; match like normalizeSpawnArguments.
  const matchKey = process.platform === 'win32' ?
    StringPrototypeToUpperCase(key) : null;
  for (let i = 0; i < envPairs.length; i++) {
    ...
    if (matchKey !== null) {
      ...
      if (eq === -1)
        continue;
      if (StringPrototypeToUpperCase(StringPrototypeSlice(pair, 0, eq)) ===
          matchKey) {
        envPairs[i] = `${prefix}${value}`;
        return;
      }
    } else if (StringPrototypeStartsWith(pair, prefix)) {
      envPairs[i] = `${prefix}${value}`;
      return;
    }
  }
  ArrayPrototypePush(envPairs, `${prefix}${value}`);
}

function stdioRequestsPty(stdio) {
  if (stdio === 'pty') return true;
  if (!ArrayIsArray(stdio)) return false;
  for (let i = 0; i < stdio.length && i < 3; i++) {
    if (stdio[i] === 'pty') return true;
  }
  return false;
}

function resolvePtyOption(options) {
  if (options.pty === false) {
    if (stdioRequestsPty(options.stdio)) {
      throw new ERR_INVALID_ARG_VALUE(
        'options.pty',
        false,
        "cannot be false when stdio requests 'pty'");
    }
    return null;
  }
  if (options.pty != null) return options.pty;
  if (stdioRequestsPty(options.stdio)) return true;
  return null;
}

// When PTY mode is enabled, remap fds 0-2 to ['pipe', 'pty', 'pty'].
// getValidStdio then maps fd 2 'pty' to ignore for the parent while the
// child still sees a TTY on stderr. Libuv contract: stdio[0]=PTY in (pipe),
// stdio[1]=PTY out, stdio[2]=ignore.
function applyPtyStdio(stdio) {
  let extra = [];
  if (typeof stdio === 'string') {
    if (stdio !== 'pipe' && stdio !== 'pty') {
      throw new ERR_INVALID_ARG_VALUE(
        'stdio',
        stdio,
        "must be 'pipe' or 'pty' when options.pty is set");
    }
    ...
    for (let i = 0; i < stdio.length && i < 3; i++) {
      ...
      if (i === 0 && s === 'pty') {
        throw new ERR_INVALID_ARG_VALUE(
          'stdio[0]',
          s,
          "cannot be 'pty' (use 'pipe' for PTY input)");
      }
      if (s == null || s === 'pipe' || s === 'pty') continue;
      if (i === 2 && s === 'ignore') continue;
      throw new ERR_INVALID_ARG_VALUE(
        `stdio[${i}]`,
        s,
        "must be 'pipe' or 'pty' when options.pty is set");
    }
    extra = ArrayPrototypeSlice(stdio, 3);
    for (let i = 0; i < extra.length; i++) {
      if (extra[i] === 'ipc') {
        throw new ERR_INVALID_ARG_VALUE(
          `stdio[${i + 3}]`,
          extra[i],
          'cannot be used with options.pty');
      }
    }
  } else if (stdio != null) {
    throw new ERR_INVALID_ARG_VALUE('stdio', stdio);
  }
  const result = ['pipe', 'pty', 'pty'];
  ArrayPrototypePushApply(result, extra);
  return result;
}

function assertPtyCompatibleOptions(options) {
  ...
  if (options.detached && process.platform === 'win32') {
    throw new ERR_INVALID_ARG_VALUE(
      'options.detached',
      options.detached,
      'cannot be used with options.pty on Windows');
  }
}
...
ChildProcess.prototype.spawn = function spawn(options) {
  ...
  this._pty = false;
  const ptyDimensions = getPtyDimensions(resolvePtyOption(options));
  if (ptyDimensions != null) {
    assertPtyCompatibleOptions(options);
    this._pty = true;
    if (options.envPairs === undefined)
      options.envPairs = [];
    else
      validateArray(options.envPairs, 'options.envPairs');
    if (ptyDimensions.name) {
      setEnvPair(options.envPairs, 'TERM', ptyDimensions.name);
    }
  }

  stdio = getValidStdio(stdio, false, ptyDimensions != null);
  ...
  if (ptyDimensions != null)
    spawnFlags |= processConstants.kProcessFlagPty;
  ...
  const err = this._handle.spawn(
    ...
    options.uid,
    options.gid,
    ptyDimensions?.cols ?? 0,
    ptyDimensions?.rows ?? 0,
  );
...
/**
 * Resizes the pseudo-terminal for a child spawned with `options.pty`.
 * @param {number} cols
 * @param {number} rows
 * @returns {number} `0` on success, or a libuv error code.
 */
ChildProcess.prototype.resize = function resize(cols, rows) {
  validateUint32(cols, 'cols', true);
  validateUint32(rows, 'rows', true);
  if (!this._pty) {
    throw new ERR_INVALID_STATE(
      'Child process was not spawned with options.pty');
  }
  if (!this._handle) {
    throw new ERR_INVALID_STATE(
      'Child process handle is already closed');
  }
  return this._handle.resize(cols, rows);
};
...
function getValidStdio(stdio, sync, usePty = false) {
  let ipc;
  let ipcFd;
  ...
  stdio = ArrayPrototypeReduce(stdio, (acc, stdio, i) => {
    ...
    if (stdio === 'ignore') {
      ...
      if (i === 0) {
        cleanup();
        throw new ERR_INVALID_ARG_VALUE('stdio', stdio,
                                      'pty cannot be used for stdin');
      }
      if (i === 1 || (i === 2 && hasPtyOutput)) {
        if (i === 2) {
          ArrayPrototypePush(acc, { type: 'ignore' });
        } else {
          const a = {
...
function spawnSync(options) {
  // Prefer dims already attached by lib/child_process.js (stdio may already
  // be getValidStdio object form, so resolvePtyOption via stdio can miss).
  let ptyDimensions = null;
  if (options.ptyCols != null && options.ptyRows != null) {
    ptyDimensions = { cols: options.ptyCols, rows: options.ptyRows };
  } else {
    ptyDimensions = getPtyDimensions(resolvePtyOption(options));
    if (ptyDimensions != null) {
      assertPtyCompatibleOptions(options);
      options.ptyCols = ptyDimensions.cols;
      options.ptyRows = ptyDimensions.rows;
      if (ptyDimensions.name) {
        if (options.envPairs === undefined)
          options.envPairs = [];
        setEnvPair(options.envPairs, 'TERM', ptyDimensions.name);
  ...
  result.stderr = ptyDimensions != null ? null : result.output?.[2];
