      ...
      if (handle && !options.keepOpen) {
        if (target) {
          ...
          target._pendingMessage =
              { __proto__: null, callback, message, handle, options, retransmissions: 0 };
        } else {
          handle.close();
        }
      }
...
function createSocket(pipe, readable) {
  return net.Socket({ __proto__: null, handle: pipe, readable });
}
...
function setupChannel(target, channel, serializationMode) {
  ...
  channel.sockets = { __proto__: null, got: { __proto__: null }, send: { __proto__: null } };
  ...
  target.on('internalMessage', function(message, handle) {
    ...
    if (!handle)
      return target._send({ __proto__: null, cmd: 'NODE_HANDLE_NACK' }, null, true);

    // Acknowledge handle receival. Don't emit error events (for example if
    // the other side has disconnected) because this call to send() is not
    // initiated by the user and it shouldn't be fatal to be unable to ACK
    // a message.
    target._send({ __proto__: null, cmd: 'NODE_HANDLE_ACK' }, null, true);
  ...
  target.send = function(message, handle, options, callback) {
    ...
    options = { __proto__: null, swallowErrors: false, ...options };
  ...
  target._send = function(message, handle, options, callback) {
    ...
    if (typeof options === 'boolean') {
      options = { __proto__: null, swallowErrors: options };
    }
...
function getValidStdio(stdio, sync) {
  ...
  stdio = ArrayPrototypeReduce(stdio, (acc, stdio, i) => {
    ...
    if (stdio === 'ignore') {
      ArrayPrototypePush(acc, { __proto__: null, type: 'ignore' });
  ...
  return { __proto__: null, stdio, ipc, ipcFd };
}
