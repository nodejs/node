  ...
  'net.Socket': {
    send: function(message, socket, options) {
      ...
      if (socket.server) {
        ...
        const firstTime = !this._channel.sockets.send[message.key];
    ...
    got: function(message, handle, emit) {
      const socket = new net.Socket({handle: handle});
...
function ChildProcess() {
  ...
  this._handle.onexit = function(exitCode, signalCode) {
    ...
    //
    const syscall = self.spawnfile ? 'spawn ' + self.spawnfile : 'spawn';
    const err = (exitCode < 0) ? errnoException(exitCode, syscall) : null;
...
function createSocket(pipe, readable) {
  const s = new net.Socket({ handle: pipe });
...
function getSocketList(type, slave, key) {
  const sockets = slave._channel.sockets[type];
