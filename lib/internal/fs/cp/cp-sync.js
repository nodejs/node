...
function getStatsSync(src, dest, opts) {
  ...
  const statFunc = opts.dereference ?
    (file) => statSync(file, { __proto__: null, bigint: true }) :
    (file) => lstatSync(file, { __proto__: null, bigint: true });
  const srcStat = statFunc(src);
  try {
    destStat = statFunc(dest);
  } catch (err) {
    if (err.code === 'ENOENT') return { __proto__: null, srcStat, destStat: null };
    throw err;
  }
...
function checkParentPathsSync(src, srcStat, dest) {
  ...
  let destStat;
  try {
    destStat = statSync(destParent, { __proto__: null, bigint: true });
  } catch (err) {
...
function checkParentDir(destStat, src, dest, opts) {
  ...
  if (!existsSync(destParent)) mkdirSync(destParent, { __proto__: null, recursive: true });
