...
// It works for all file types including symlinks since it checks the src and
// dest inodes. It starts from the deepest parent and stops once it reaches the
// src parent or the root path.
async function checkParentPaths(src, srcStat, dest) {
  const srcParent = resolvePath(dirnamePath(src));
  const destParent = resolvePath(dirnamePath(dest));
  if (pathEquals(destParent, srcParent) || isPathRoot(destParent)) return;
...
async function copyDir(src, dest, opts) {
  const dir = await opendir(src, { encoding: 'buffer' });
...
async function onLink(destStat, src, dest, opts) {
  let resolvedSrc = maybeDecodePath(
    await readlink(src, { encoding: 'buffer' }),
    typeof src === 'string',
  );
  if (!opts.verbatimSymlinks) {
  ...
  try {
    resolvedDest = maybeDecodePath(
      await readlink(dest, { encoding: 'buffer' }),
      typeof dest === 'string',
    );
  } catch (err) {
  ...
  const srcStat = typeof src === 'string' ? null : await stat(src);
  const srcIsDir = srcStat ?
    srcStat.isDirectory() :
  ...
  // Do not copy if src is a subdir of dest since unlinking dest in this case
  // would result in removing src contents and therefore a broken symlink.
  const followedSrcStat = srcStat || await stat(src);
  if (followedSrcStat.isDirectory() && isSrcSubdir(resolvedDest, resolvedSrc)) {
