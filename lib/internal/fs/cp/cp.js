...
function getStats(src, dest, opts) {
  const statFunc = opts.dereference ?
    (file) => stat(file, { __proto__: null, bigint: true }) :
    (file) => lstat(file, { __proto__: null, bigint: true });
...
async function checkParentDir(destStat, src, dest, opts) {
  ...
  await mkdir(destParent, { __proto__: null, recursive: true });
...
async function checkParentPaths(src, srcStat, dest) {
  ...
  let destStat;
  try {
    destStat = await stat(destParent, { __proto__: null, bigint: true });
  } catch (err) {
