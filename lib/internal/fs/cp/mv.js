...
// This file is based on the fs-extra's move method.
...
async function mvFn(src, dest, opts) {
...
async function checkPaths(src, dest, opts) {
  ...
  if (destStat) {
    ...
    if (srcStat.isDirectory() && !destStat.isDirectory()) {
      throw new ERR_FS_MV_DIR_TO_NON_DIR({
    ...
    if (!srcStat.isDirectory() && destStat.isDirectory()) {
      throw new ERR_FS_MV_NON_DIR_TO_DIR({
  ...
  if (srcStat.isDirectory() && isSrcSubdir(src, dest)) {
    throw new ERR_FS_MV_EINVAL({
      message: `cannot move ${src} to a subdirectory of self ${dest}`,
      path: dest,
      syscall: 'mv',
      errno: EINVAL,
      code: 'EINVAL',
...
async function onLink(destStat, src, dest) {
  ...
  if (isSrcSubdir(resolvedSrc, resolvedDest)) {
    throw new ERR_FS_CP_EINVAL({
      message: `cannot move ${resolvedSrc} to a subdirectory of self ` +
            `${resolvedDest}`,
      path: dest,
      syscall: 'mv',
      errno: EINVAL,
  ...
  // Do not move if src is a subdir of dest since unlinking
