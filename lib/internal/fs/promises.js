...
if (getOptionValue('--experimental-stream-iter')) {
  ...
  FileHandle.prototype.writer = function writer(options = kNullPrototo) {
    ...
    function releaseAfterFailure() {
      if (!errored || asyncPending !== 0 || released) return;
      released = true;
      handle[kLocked] = false;
      handle[kUnref]();
      if (autoClose) {
    ...
    function finishEnd() {
      if (!closing || errored || asyncPending !== 0 || endingCleanup) return;
      endingCleanup = true;
      PromisePrototypeThen(cleanup(), () => {
        ...
        closing = false;
        closed = true;
        pendingEnd.resolve(totalBytesWritten);
        pendingEnd = null;
      }, (error) => {
        endingCleanup = false;
        if (errored) return;
    ...
    function trackOperation(operation) {
      ...
      asyncPending++;
      PromisePrototypeThen(operation, (value) => {
        const active = pendingWrites.delete(pending);
        asyncPending--;
        releaseAfterFailure();
        if (active) resolve(value);
        finishEnd();
      }, (error) => {
        const active = pendingWrites.delete(pending);
        asyncPending--;
    ...
    return {
      ...
      write(chunk, options = kNullPrototo) {
        ...
        if (closing) {
          return PromiseReject(
            new ERR_INVALID_STATE.TypeError('The writer is closing'));
        }
        if (signal?.aborted) {
      ...
      writeSync(chunk) {
        chunk = newStreamsToWriterUint8Array(chunk);
        if (errored || closed || closing || asyncPending) return false;
