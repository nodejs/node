...
async function write(handle, buffer, offsetOrOptions, length, position) {
  ...
  if (isArrayBufferView(bufferView)) {
    ...
    if (typeof position !== 'number')
      position = null;
    validateOffsetLengthWrite(offset, length, bufferView.byteLength);
    const bytesWritten =
      (await binding.writeBuffer(handle.fd, bufferView, offset,
...
async function writev(handle, buffers, position) {
  ...
  const bytesWritten = (await binding.writeBuffers(handle.fd, buffersToWrite, position,
                                                   kUsePromises)) || 0;
