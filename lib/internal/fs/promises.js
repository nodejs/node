...
async function readdirRecursive(originalPath, options) {
  originalPath = pathModule.resolve(originalPath);
  const result = [];
  const queue = [
    [
      originalPath,
  ...
  if (options.withFileTypes) {
    while (queue.length > 0) {
      ...
      for (const dirent of getDirents(path, readdir)) {
        ...
        if (
          dirent.isDirectory() &&
          (typeof options.recursive !== 'function' || await options.recursive(dirent, depth))
        ) {
    ...
    while (queue.length > 0) {
      ...
      for (const ent of readdir) {
        ...
        if (
          stat === 1 &&
          (typeof options.recursive !== 'function' || await options.recursive(ent, depth))
        ) {
          ArrayPrototypePush(queue, [
            direntPath,
            await binding.readdir(
