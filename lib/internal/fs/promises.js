...
async function access(path, mode = F_OK) {
  ...
  mode = mode | 0;
  return binding.access(toNamespacedPath(path), mode,
                        kUsePromises);
}
...
async function copyFile(src, dest, flags) {
  ...
  flags = flags | 0;
  return binding.copyFile(toNamespacedPath(src),
                          toNamespacedPath(dest),
                          flags, kUsePromises);
}
...
async function open(path, flags, mode) {
  ...
  return new FileHandle(
    await binding.openFileHandle(toNamespacedPath(path),
...
async function rename(oldPath, newPath) {
  ...
  return binding.rename(toNamespacedPath(oldPath),
...
async function rmdir(path) {
  path = toPathIfFileURL(path);
  validatePath(path);
  return binding.rmdir(toNamespacedPath(path), kUsePromises);
}
...
async function mkdir(path, options) {
  ...
  return binding.mkdir(toNamespacedPath(path),
...
async function readdir(path, options) {
  ...
  path = toPathIfFileURL(path);
  validatePath(path);
  const result = await binding.readdir(toNamespacedPath(path),
                                       options.encoding,
...
async function readlink(path, options) {
  ...
  return binding.readlink(toNamespacedPath(path),
...
async function lstat(path, options = { bigint: false }) {
  path = toPathIfFileURL(path);
  validatePath(path);
  const result = await binding.lstat(toNamespacedPath(path),
...
async function stat(path, options = { bigint: false }) {
  path = toPathIfFileURL(path);
  validatePath(path);
  const result = await binding.stat(toNamespacedPath(path),
                                    options.bigint, kUsePromises);
  return getStatsFromBinding(result);
}
...
async function link(existingPath, newPath) {
  ...
  validatePath(newPath, 'newPath');
  return binding.link(toNamespacedPath(existingPath),
                      toNamespacedPath(newPath),
                      kUsePromises);
}
...
async function unlink(path) {
  path = toPathIfFileURL(path);
  validatePath(path);
  return binding.unlink(toNamespacedPath(path), kUsePromises);
}
...
async function chmod(path, mode) {
  ...
  validatePath(path);
  mode = validateMode(mode, 'mode');
  return binding.chmod(toNamespacedPath(path), mode, kUsePromises);
}
...
async function lchown(path, uid, gid) {
  ...
  return binding.lchown(toNamespacedPath(path),
...
async function chown(path, uid, gid) {
  ...
  validateUint32(uid, 'uid');
  validateUint32(gid, 'gid');
  return binding.chown(toNamespacedPath(path),
                       uid, gid, kUsePromises);
}
...
async function utimes(path, atime, mtime) {
  path = toPathIfFileURL(path);
  validatePath(path);
  return binding.utimes(toNamespacedPath(path),
