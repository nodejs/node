...
async function access(path, mode = F_OK) {
  return binding.access(getValidatedPath(path), mode, kUsePromises);
}
...
async function read(handle, bufferOrParams, offset, length, position) {
  ...
  let bytesRead = await binding.read(handle.fd, buffer, offset, length, position, kUsePromises);
  bytesRead ||= 0;
...
async function readv(handle, buffers, position) {
  ...
  let bytesRead = await binding.readBuffers(handle.fd, buffers, position, kUsePromises);
  bytesRead ||= 0;
...
async function write(handle, buffer, offsetOrOptions, length, position) {
  ...
  if (isArrayBufferView(buffer)) {
    ...
    let bytesWritten = await binding.writeBuffer(handle.fd, buffer, offset, length, position, kUsePromises);
    bytesWritten ||= 0;
...
async function writev(handle, buffers, position) {
  ...
  let bytesWritten = await binding.writeBuffers(handle.fd, buffers, position, kUsePromises);
  bytesWritten ||= 0;
...
// No need to make this function async, since binding returns a promise already.
...
async function readlink(path, options) {
  ...
  return binding.readlink(path, options.encoding, kUsePromises);
}
...
async function statfs(path, options = { bigint: false }) {
  const result = await binding.statfs(getValidatedPath(path), options.bigint, kUsePromises);
...
function unlink(path) {
  return binding.unlink(getValidatedPath(path), kUsePromises);
}
...
async function chmod(path, mode) {
  path = getValidatedPath(path);
  mode = parseFileMode(mode, 'mode');
  return binding.chmod(path, mode, kUsePromises);
}
...
async function lchown(path, uid, gid) {
  ...
  return binding.lchown(path, uid, gid, kUsePromises);
}
...
async function chown(path, uid, gid) {
  ...
  validateInteger(uid, 'uid', -1, kMaxUserId);
  validateInteger(gid, 'gid', -1, kMaxUserId);
  return binding.chown(path, uid, gid, kUsePromises);
}

async function utimes(path, atime, mtime) {
  return binding.utimes(
...
async function realpath(path, options) {
  options = getOptions(options);
  return binding.realpath(getValidatedPath(path), options.encoding, kUsePromises);
}
