...
const { F_OK, O_SYMLINK, O_WRONLY, S_IFMT, S_IFREG } =
...
class FileHandle extends EventEmitterMixin(JSTransferable) {
  ...
  readableWebStream() {
    ...
    const readable = newReadableStreamFromStreamBase(this[kHandle], undefined, {
      ondone: () => this[kUnref](),
    });
...
async function readFileHandle(filehandle, options) {
  ...
  if (size > 0) {
    ...
    result =
      buffers.length === 1 ? buffers[0] : Buffer.concat(buffers, totalRead);
  }
...
async function read(handle, bufferOrOptions, offset, length, position) {
  ...
  if (length === 0) return { bytesRead: length, buffer };
...
async function lstat(path, options = { bigint: false }) {
  path = getValidatedPath(path);
  const result = await binding.lstat(
    pathModule.toNamespacedPath(path),
    options.bigint,
    kUsePromises
  );
...
async function stat(path, options = { bigint: false }) {
  path = getValidatedPath(path);
  const result = await binding.stat(
    pathModule.toNamespacedPath(path),
    options.bigint,
    kUsePromises
  );
