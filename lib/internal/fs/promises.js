...
class FileHandle extends EventEmitterMixin(JSTransferable) {
  ...
  readableWebStream(options = kEmptyObject) {
    ...
    if (options.type !== 'bytes') {
      ...
      readable = newReadableStreamFromStreamBase(
        this[kHandle],
        undefined,
        { __proto__: null, ondone: () => this[kUnref]() });
  ...
  createReadStream(options = undefined) {
    ...
    return new ReadStream(undefined, { __proto__: null, ...options, fd: this });
  }
  ...
  createWriteStream(options = undefined) {
    ...
    return new WriteStream(undefined, { __proto__: null, ...options, fd: this });
  }
...
async function read(handle, bufferOrParams, offset, length, position) {
  ...
  if (!isArrayBufferView(buffer)) {
    ...
    if (bufferOrParams !== undefined) {
      validateObject(bufferOrParams, 'options', { __proto__: null, nullable: true });
    }
    ({
...
async function mkdir(path, options) {
  if (typeof options === 'number' || typeof options === 'string') {
    options = { __proto__: null, mode: options };
  }
  const {
    recursive = false,
    mode = 0o777,
...
async function stat(path, options = { __proto__: null, bigint: false }) {
  path = getValidatedPath(path);
