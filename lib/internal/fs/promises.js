...
async function access(path, mode = F_OK) {
  ...
  if (h !== null) {
    const vfsResult = await h.access(path, mode);
...
async function copyFile(src, dest, mode) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.copyFile(src, dest, mode);
    if (vfsResult !== undefined) return;
  }
  return await PromisePrototypeThen(
    binding.copyFile(
...
async function rename(oldPath, newPath) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.rename(oldPath, newPath);
    if (vfsResult !== undefined) return;
  }
...
async function truncate(path, len = 0) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.truncate(path, len);
    if (vfsResult !== undefined) return;
  }
...
async function rm(path, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.rm(path, options);
    if (vfsResult !== undefined) return;
  }
  path = getValidatedPath(path);
...
async function rmdir(path, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.rmdir(path);
    if (vfsResult !== undefined) return;
  }
  path = getValidatedPath(path);
...
async function mkdir(path, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.mkdir(path, options);
...
async function readlink(path, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.readlink(path, options);
    if (vfsResult !== undefined) return vfsResult;
  }
  options = getOptions(options);
...
async function symlink(target, path, type) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.symlink(target, path, type);
    if (vfsResult !== undefined) return;
  }
...
async function stat(path, options = { bigint: false, throwIfNoEntry: true }) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.stat(path, options);
    if (vfsResult !== undefined) return vfsResult;
  }
  const result = await PromisePrototypeThen(
...
async function link(existingPath, newPath) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.link(existingPath, newPath);
    if (vfsResult !== undefined) return;
  }
...
async function unlink(path) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.unlink(path);
    if (vfsResult !== undefined) return;
  }
  return await PromisePrototypeThen(
...
async function realpath(path, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.realpath(path, options);
    if (vfsResult !== undefined) return vfsResult;
  }
  options = getOptions(options);
  return await PromisePrototypeThen(
...
async function mkdtemp(prefix, options) {
  const h = vfsState.handlers;
  if (h !== null) {
    const vfsResult = await h.mkdtemp(prefix);
    if (vfsResult !== undefined) return vfsResult;
  }
