...
function lazyNewStreams() {
  if (newStreamsPull === undefined) {
    newStreamsPull = require('internal/streams/new/pull').pull;
    newStreamsParsePullArgs =
      require('internal/streams/new/utils').parsePullArgs;
  }
}
...
class FileHandle extends EventEmitter {
  ...
  pull(...args) {
    ...
    lazyNewStreams();
    const { transforms, options } = newStreamsParsePullArgs(args);
        ...
        try {
          if (signal) {
            ...
            while (true) {
              ...
              // Allocate a fresh buffer each iteration. At 64 KiB this
              // bypasses the slab pool, so there is no reuse benefit.
              // Yielding the buffer directly avoids the per-chunk copy
              // that was needed when a single buffer was reused.
              const buf = Buffer.allocUnsafe(readSize);
              let bytesRead;
              try {
                bytesRead =
                  (await binding.read(fd, buf, 0,
                                      readSize, -1, kUsePromises)) || 0;
              } catch (err) {
  ...
  writer(options) {
    ...
    async function writeAll(buf, offset, length, position) {
      let retries = 0;
      while (length > 0) {
    ...
    async function writevAll(buffers, position) {
      let totalSize = 0;
