...
const { fileURLToPathBuffer, isURL, toPathIfFileURL } = require('internal/url');
const {
  validateAbortSignal,
  validateBoolean,
  validateBuffer,
...
const getValidatedCpPath = hideStackFrames((fileURLOrPath, propName = 'path') => {
  if (!isURL(fileURLOrPath)) {
    validatePath(fileURLOrPath, propName);
    return typeof fileURLOrPath === 'string' || Buffer.isBuffer(fileURLOrPath) ?
      fileURLOrPath : Buffer.from(fileURLOrPath);
  }

  const pathBuffer = fileURLToPathBuffer(fileURLOrPath);
  const path = pathBuffer.toString();
  return Buffer.from(path).equals(pathBuffer) ? path : pathBuffer;
});
