...
class DirectoryEntry {
  ...
  isUnknown() {
    return this[kType] === UV_DIRENT_UNKNOWN;
  }
}

class DirectoryEntryFromStats extends DirectoryEntry {
...
for (const name of Reflect.ownKeys(DirectoryEntry.prototype)) {
  if (name === 'isUnknown' || name === 'constructor') {
    continue;
  }
  DirectoryEntryFromStats.prototype[name] = function() {
...
function getDirectoryEntries(path, [names, types], callback) {
  var i;
  if (typeof callback == 'function') {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (type === UV_DIRENT_UNKNOWN) {
        ...
        lazyLoadFs().stat(pathModule.resolve(path, name), (err, stats) => {
          ...
          names[idx] = new DirectoryEntryFromStats(name, stats);
          if (--toFinish === 0) {
            callback(null, names);
          }
        });
    ...
    for (i = 0; i < len; i++) {
      ...
      if (type === UV_DIRENT_UNKNOWN) {
        ...
        names[i] = new DirectoryEntryFromStats(name, stats);
      } else {
        names[i] = new DirectoryEntry(names[i], types[i]);
      }
    }
