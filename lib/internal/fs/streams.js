...
function FileSystemStream(path, options) {
  ...
  if (options.fd) {
    if (typeof options.fd === 'number') {
      ...
      this[kHandle] = options.fd;
      this.fd = options.fd.fd;
      this[kFs] = FileHandleOperations(this[kHandle]);
      this[kHandle][kRef]();
      options.fd.on('close', FunctionPrototypeBind(this.close, this));
    } else
...
FileSystemStream.prototype[kClose] = function close(err, cb) {
  if (this.fd) {
    this[kFs].close(this.fd, (er) => {
      this.closed = true;
      cb(er || err);
    });
    this.fd = null;
...
FileSystemStream.prototype._construct = function _construct(callback) {
  if (typeof this.fd === 'number') {
    callback();
    return;
  }

  if (this.open !== openWriteFs && this.open !== openReadFs) {
...
FileSystemStream.prototype._destroy = function _destroy(err, cb) {
  ...
  if (this[kIsPerformingIO]) {
    this.once(kIoDone, (er) => this[kClose](err || er, cb));
  } else {
    this[kClose](err, cb);
  }
};

ObjectDefineProperty(FileSystemStream.prototype, 'pending', {
...
function ReadStream(path, options) {
  ...
  FunctionPrototypeCall(FileSystemStream, this, path, options);
...
ReadStream.prototype._construct = FileSystemStream.prototype._construct;
...
ReadStream.prototype._destroy = FileSystemStream.prototype._destroy;
ReadStream.prototype[kClose] = FileSystemStream.prototype[kClose];
...
ObjectDefineProperty(ReadStream.prototype, 'pending',
                     ObjectGetOwnPropertyDescriptor(FileSystemStream.prototype,
                                                    'pending'));
...
WriteStream.prototype._construct = FileSystemStream.prototype._construct;
...
WriteStream.prototype._destroy = FileSystemStream.prototype._destroy;
WriteStream.prototype[kClose] = FileSystemStream.prototype[kClose];
