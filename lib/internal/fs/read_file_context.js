...
class ReadFileContext {
  constructor(callback, encoding, kPerRead) {
    this.fd = undefined;
  ...
  read() {
    ...
    if (this.size === 0) {
      ...
      if (this.kPerRead === 0 && length > kReadFileBufferLength) {
        length = kReadFileBufferLength;
      } else if (this.kPerRead > 0 && length > this.kPerRead) {
        length = this.kPerRead * 1024;
      }
    }
