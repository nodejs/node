...
class ReadFileContext {
  constructor(callback, encoding, bPerRead) {
    this.fd = undefined;
  ...
  read() {
    ...
    if (this.size === 0) {
      ...
      if (this.bPerRead === 0 && length > kReadFileBufferLength) {
        length = kReadFileBufferLength;
      } else if (this.bPerRead > 0 && length > this.bPerRead) {
        length = this.bPerRead;
      }
    }
