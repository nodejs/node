...
const {
  codes: { ERR_DIR_CLOSED, ERR_DIR_CONCURRENT_OPERATION, ERR_MISSING_ARGS },
...
const { validateCallback, validateUint32 } = require('internal/validators');
...
class Dir {
  constructor(handle, path, options) {
    ...
    this[kDirReadPromisified] = FunctionPrototypeBind(
      internalUtil.promisify(this[kDirReadImpl]),
      this,
      false
    );
    this[kDirClosePromisified] = FunctionPrototypeBind(
      internalUtil.promisify(this.close),
      this
    );
  }
  ...
  [kDirReadImpl](maybeSync, callback) {
    ...
    if (this[kDirBufferedEntries].length > 0) {
      const { 0: name, 1: type } = ArrayPrototypeSplice(
        this[kDirBufferedEntries],
        0,
        2
      );
