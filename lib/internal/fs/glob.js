...
let picomatch;
function lazyPicomatch() {
  picomatch ??= require('internal/deps/picomatch/index');
  return picomatch;
}
...
class Pattern {
  ...
  isLast(isDirectory) {
    return this.indexes.has(this.last) ||
      ...
      this.indexes.has(this.last - 1) && this.at(-2) === lazyPicomatch().GLOBSTAR);
  }
  isFirst() {
  ...
  test(index, path) {
    ...
    if (pattern === lazyPicomatch().GLOBSTAR) {
      return true;
    }
...
class Glob {
  ...
  constructor(pattern, options = kEmptyObject) {
    ...
    this.matchers = ArrayPrototypeMap(patterns, (pattern) => new (lazyPicomatch().Picomatch)(pattern, {
      __proto__: null,
  ...
  #addSubpatterns(path, pattern) {
    ...
    } else if (isLast && pattern.at(-1) === lazyPicomatch().GLOBSTAR &&
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      for (const index of pattern.indexes) {
        ...
        if (current === lazyPicomatch().GLOBSTAR) {
