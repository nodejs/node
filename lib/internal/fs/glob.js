...
class Glob {
  ...
  constructor(patterns, options = kEmptyObject) {
    ...
    const { exclude, cwd, signal } = options;
    if (exclude != null) {
  ...
  *#addSubpatternsSync(path, pattern) {
    ...
    if (pattern.isFirst()) {
      const shouldFinish = this.addPatternsOnFirst(pattern);
    ...
    if (typeof firstPattern === 'string') {
      ...
      if (stat) {
        ...
        yield* this.addFromChildren(stat, path, fullpath, pattern, last, isLast);
      }
    ...
    for (let i = 0; i < children.length; i++) {
      const childrenResults = this.addFromChildren(children[i], path, fullpath, pattern, last, isLast);
  ...
  * addFromChildren(entry, path, fullpath, pattern, last, isLast) {
  ...
  addPatternsOnFirst(pattern) {
    if (isWindows && typeof pattern.at(0) === 'string' && StringPrototypeEndsWith(pattern.at(0), ':')) {
  ...
  *[SymbolIterator]() {
    ...
    while (this.#queue.length > 0) {
      ...
      for (let i = 0; i < item.patterns.length; i++) {
        yield *this.#addSubpatternsSync(item.path, item.patterns[i]);
      }
      this.#subpatterns
  ...
  async *[SymbolAsyncIterator]() {
    ...
    while (this.#queue.length > 0) {
      ...
      for (let i = 0; i < item.patterns.length; i++) {
        ...
        yield* await this.#addSubpatterns(item.path, item.patterns[i]);
      }
      this.#subpatterns
        .forEach((patterns, path) => ArrayPrototypePush(this.#queue, { __proto__: null, path, patterns }));
      this.#subpatterns.clear();
