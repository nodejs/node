...
class Pattern {
  ...
  cacheKey(index) {
    return this.#globStrings.slice(index).join('/');
  }
}
...
class Glob {
  ...
  constructor(pattern, options = kEmptyObject) {
    ...
    const patterns = ArrayIsArray(pattern) ? pattern : [pattern];
  ...
  globSync() {
    ...
    while (this.#queue.length) {
      const { path, patterns } = ArrayPrototypePop(this.#queue);
      for (const pattern of patterns) {
        this.#addSubpatterns(path, pattern);
      }
      this.#subpatterns
  ...
  async* glob() {
    ...
    while (this.#queue.length > 0) {
      const { path, patterns } = ArrayPrototypePop(this.#queue);
      for (const pattern of patterns) {
        yield* this.#iterateSubpatterns(path, pattern);
      }
      this.#subpatterns
        .forEach((patterns, path) => ArrayPrototypePush(this.#queue, { __proto__: null, path, patterns }));
      this.#subpatterns.clear();
