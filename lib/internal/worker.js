...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    if (options.execArgv) {
      ...
      execArgv = ArrayPrototypeMap(options.execArgv, String);
    }
    ...
    if (isInternal) {
      ...
      doEval = 'module';
      execArgv ??= [];
      ArrayPrototypePush(execArgv, '--expose-internals');
      filename = Worker.#wrapUrlModule(filename);
    } else {
      doEval = false;
      if (isURL(filename)) {
        url = filename;
  ...
  static #wrapUrlModule(url) {
    return (
      `import loader from 'node:internal/modules/esm/loader';
      import util from 'node:internal/util';

      const cascadedLoader = loader.getOrInitializeCascadedLoader();
      await cascadedLoader.import(${JSONStringify(url)}, undefined, util.kEmptyObject, undefined, true);`
    );
  }
