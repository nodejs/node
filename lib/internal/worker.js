...
class Worker extends EventEmitter {
  ...
  [kOnMessage](message) {
    if (message.type === messageTypes.UP_AND_RUNNING) {
      return this.emit('online');
    }
    if (message.type === messageTypes.COULD_NOT_SERIALIZE_ERROR) {
      return this[kOnCouldNotSerializeErr]();
    }
    if (message.type === messageTypes.ERROR_MESSAGE) {
      return this[kOnErrorMessage](message.error);
    }
    if (message.type === messageTypes.STDIO_PAYLOAD) {
      const { stream, chunk, encoding } = message;
      return this[kParentSideStdio][stream].push(chunk, encoding);
    }
    assert(
      message.type === messageTypes.STDIO_WANTS_MORE_DATA,
