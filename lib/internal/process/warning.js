...
// Lazy Loading
const fs = getLazy(() => require('internal/fs'));
const warningFile = getLazy(() => {
  const diagnosticDir = getOptionValue('--diagnostic-dir');
  const redirectWarnings = getOptionValue('--redirect-warnings');
  return redirectWarnings !== '' ? redirectWarnings : diagnosticDir !== '' ? diagnosticDir : '';
});
const path = getLazy(() => require('path'));
const error = getLazy(() => require('internal/console/global').error);
