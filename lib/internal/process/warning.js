...
function lazyOption() {
  ...
  if (warningFile === undefined) {
    const diagnosticDir = options.getOptionValue('--diagnostic-dir');
    const redirectWarnings = options.getOptionValue('--redirect-warnings');

    warningFile = redirectWarnings !== '' ? redirectWarnings : diagnosticDir !== '' ? diagnosticDir : '';
  }
  return warningFile;
}
