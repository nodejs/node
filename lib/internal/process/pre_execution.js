...
function initializePermission() {
  ...
  if (permission) {
    ...
    for (let i = 0; i < warnFlags.length; i++) {
      const flag = warnFlags[i];
    ...
    for (let i = 0; i < warnCommaFlags.length; i++) {
      const flag = warnCommaFlags[i];
    ...
    for (let i = 0; i < experimentalWarnFlags.length; i++) {
      const flag = experimentalWarnFlags[i];
      if (getOptionValue(flag)) {
        process.emitWarning(
