...
function setupWarningHandler() {
  ...
  if (getOptionValue('--warnings') &&
    process[env_private_symbol].NODE_NO_WARNINGS !== '1') {
...
function setupCodeCoverage() {
  ...
  // handle coverage.
  if (process[env_private_symbol].NODE_V8_COVERAGE &&
      !getOptionValue('--experimental-test-coverage')) {
    process[env_private_symbol].NODE_V8_COVERAGE =
      setupCoverageHooks(process[env_private_symbol].NODE_V8_COVERAGE);
  }
}
...
function setupDebugEnv() {
  require('internal/util/debuglog').initializeDebugEnv(process[env_private_symbol].NODE_DEBUG);
...
function setupChildProcessIpcChannel() {
  if (process[env_private_symbol].NODE_CHANNEL_FD) {
    ...
    const fd = NumberParseInt(process[env_private_symbol].NODE_CHANNEL_FD, 10);
    assert(fd >= 0);

    // Make sure it's not accidentally inherited by child processes.
    delete process[env_private_symbol].NODE_CHANNEL_FD;

    const serializationMode =
      process[env_private_symbol].NODE_CHANNEL_SERIALIZATION_MODE || 'json';
    delete process[env_private_symbol].NODE_CHANNEL_SERIALIZATION_MODE;
...
function initializeClusterIPC() {
  if (process.argv[1] && process[env_private_symbol].NODE_UNIQUE_ID) {
    const cluster = require('cluster');
    cluster._setupWorker();
    // Make sure it's not accidentally inherited by child processes.
    delete process[env_private_symbol].NODE_UNIQUE_ID;
  }
}
