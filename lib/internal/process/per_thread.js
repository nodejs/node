...
function wrapProcessMethods(binding) {
  ...
  function kill(pid, sig) {
    ...
    if (err) throw errnoException(err, 'kill');
...
function buildAllowedFlags() {
  ...
  class NodeEnvironmentFlagsSet extends Set {
    ...
    entries() {
      this[kInternal].set ??= new Set(
        new SafeArrayIterator(this[kInternal].array)
      );
      return SetPrototypeEntries(this[kInternal].set);
    }

    forEach(callback, thisArg = undefined) {
      ArrayPrototypeForEach(this[kInternal].array, (v) =>
  ...
  NodeEnvironmentFlagsSet.prototype.keys = NodeEnvironmentFlagsSet.prototype[
    SymbolIterator
