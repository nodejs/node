...
function wrapProcessMethods(binding) {
   ...
   */
  let lazyOnExitFreeRegistry = null;

  const onExitFreeRefs = {
    exit: [],
    beforeExit: [],
  };

  const onExitFunctions = {
    exit: onExitFree,
    beforeExit: onBeforeExitFree,
  };

  function onExitFreeObjects(ref) {
    for (const event of ['exit', 'beforeExit']) {
      const index = onExitFreeRefs[event].indexOf(ref);
      onExitFreeRefs[event].splice(index, index + 1);
      uninstallOnExit(event);
    }
  }

  function callRefsToFree(event) {
    process._rawDebug(`Calling callRefsToFree for ${event} event`);

    for (const ref of onExitFreeRefs[event]) {
      const obj = ref.deref();
      const fn = ref.fn;

      process._rawDebug(`Calling ${fn.toString()} for ${obj} on ${event} event`);
      ...
      if (obj !== undefined) {
        ...
        process._rawDebug(`Unlikely case happened, object was GCed before ${event} event`);
      }
    }
    onExitFreeRefs[event] = [];
  }
  ...
  function onBeforeExitFree() {
    callRefsToFree('beforeExit');
  }

  function installOnExitFreeEvent(event) {
    if (onExitFreeRefs[event].length > 0) {
      return;
    }

    process.on(event, onExitFunctions[event]);
  }

  function uninstallOnExit(event) {
    if (onExitFreeRefs[event].length > 0) {
      return;
    }

    process.removeListener(event, onExitFunctions[event]);

    if (onExitFreeRefs.exit.length === 0 && onExitFreeRefs.beforeExit.length === 0) {
      lazyOnExitFreeRegistry = null;
    }
  }

  function ensureOnExitFreeRegistry() {
    if (!lazyOnExitFreeRegistry) {
      process._rawDebug('Creating FinalizationRegistry for onExitFreeObjects');
      lazyOnExitFreeRegistry = new FinalizationRegistry(onExitFreeObjects);
    }
  }

  function _registerOnEventExit(event, obj, fn) {
    if (obj === undefined) {
      throw new ERR_INVALID_ARG_TYPE('obj', 'not undefined', obj);
    }

    installOnExitFreeEvent(event);

    const ref = new WeakRef(obj);
    ref.fn = fn;

    ensureOnExitFreeRegistry();

    process._rawDebug(`Registering object ${obj} for event ${event} with function ${fn.toString()}`);

    lazyOnExitFreeRegistry.register(obj, ref);
    onExitFreeRefs[event].push(ref);
  }
   ...
   * @param {Function} fn
   */
  function registerFreeOnExit(obj, fn) {
    _registerOnEventExit('exit', obj, fn);
  }
   ...
   * @param {Function} fn
   */
  function registerFreeOnBeforeExit(obj, fn) {
    _registerOnEventExit('beforeExit', obj, fn);
  }
   ...
   * @param {object} obj
   */
  function unregisterFree(obj) {
    if (!lazyOnExitFreeRegistry) {
      return;
    }
    lazyOnExitFreeRegistry.unregister(obj);
    for (const event of ['exit', 'beforeExit']) {
      onExitFreeRefs[event] = onExitFreeRefs[event].filter((ref) => {
