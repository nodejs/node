...
ObjectDefineProperty(
  ...
  kCreateEvent,
  {
    ...
    value: function(data, type) {
      ...
      return new MessageEvent(type, { __proto__: null, data, ports });
    },
    configurable: false,
    writable: false,
    enumerable: false,
...
class WritableWorkerStdio extends Writable {
  constructor(port, name) {
    super({ __proto__: null, decodeStrings: false });
    this[kPort] = port;
    this[kName] = name;
  ...
  _writev(chunks, cb) {
    this[kPort].postMessage({
      ...
      chunks: ArrayPrototypeMap(chunks,
                                ({ chunk, encoding }) => ({ __proto__: null, chunk, encoding })),
    });
  ...
  _final(cb) {
    this[kPort].postMessage({
      ...
      type: messageTypes.STDIO_PAYLOAD,
      stream: this[kName],
      chunks: [ { __proto__: null, chunk: null, encoding: '' } ],
    });
    cb();
  }
...
function onMessageEvent(type, data) {
  this.dispatchEvent(new MessageEvent(type, { __proto__: null, data }));
}
