...
const {
  codes: { ERR_INVALID_ARG_TYPE, ERR_INVALID_THIS, ERR_MISSING_ARGS },
...
ObjectDefineProperties(MessageEvent.prototype, {
  data: {
    get() {
      if (!isMessageEvent(this)) throw new ERR_INVALID_THIS('MessageEvent');
      return this[kData];
    },
    enumerable: true,
    configurable: true,
...
ObjectDefineProperty(MessagePort.prototype, inspect.custom, {
  ...
  value: function inspect() {
    ...
    return ObjectAssign(
      ObjectCreate(MessagePort.prototype),
      ref === undefined
        ? {
            active: false,
          }
...
class WritableWorkerStdio extends Writable {
  ...
  _writev(chunks, cb) {
    this[kPort].postMessage({
      ...
      stream: this[kName],
      chunks: ArrayPrototypeMap(chunks, ({ chunk, encoding }) => ({
        chunk,
        encoding,
      })),
    });
...
class BroadcastChannel extends EventTarget {
  ...
  constructor(name) {
    ...
    this[kOnMessageError] = FunctionPrototypeBind(
      onMessageEvent,
      this,
      'messageerror'
    );
