...
class LockInternals {
  ...
  request(
    ...
    if (steal) {
      ...
      for (let i = 0; i < this.state.held.length;) {
        ...
        ArrayPrototypePush(this.state.stolen, this.state.held[i]);
        // Remove lock from held.
        ArrayPrototypeSplice(this.state.held, i, 1);
      }
      // Prepend request in queue.
      ArrayPrototypeUnshift(this.state.pending, request);
    } else {
      ...
      ArrayPrototypePush(this.state.pending, request);
    }
      ...
      abort: () => {
        ...
        for (let i = 0; i < this.state.pending.length; i++) {
          if (this.state.pending[i].id === request.id) {
            ArrayPrototypeSplice(this.state.pending, i, 1);
            break;
          }
        }
  ...
  processLockRequestQueue() {
    ...
    for (let i = 0; i < this.state.pending.length;) {
      ...
      ArrayPrototypeSplice(this.state.pending, i, 1);
      ArrayPrototypePush(this.state.held, request);
      this.availableMap.get(request.id).resolve();
      this.availableMap.delete(request.id);
      didModify = true;
    }
    for (let i = 0; i < this.state.stolen.length;) {
      ...
      this.resolveMap.delete(request.id);
      ArrayPrototypeSplice(this.state.stolen, i, 1);
      didModify = true;
    }
  ...
  snapshot() {
    ...
    return {
      held: ArrayPrototypeMap(this.state.held, printLock),
      pending: ArrayPrototypeMap(this.state.pending, printLock),
    };
  }
}
let locks;
