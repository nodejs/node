...
const ENCODINGS = {
  ...
  BASE64URL: 5,
  HEX: 6,
  // This case is only supported for crypto usage.
  BUFFER: 7,
};
 ...
 * @param {unknown} enc
 * @returns {number | undefined} Returns undefined if there is no match.
 */
function normalizeEncoding(enc) {
  if (enc == null || enc === 'utf8' || enc === 'utf-8') return ENCODINGS.UTF8;
...
function slowCases(enc) {
  switch (enc.length) {
    case 4:
      if (enc === 'UTF8') return ENCODINGS.UTF8;
      if (enc === 'ucs2' || enc === 'UCS2') return ENCODINGS.UTF16LE;
      enc = StringPrototypeToLowerCase(enc);
      if (enc === 'utf8') return ENCODINGS.UTF8;
      if (enc === 'ucs2') return ENCODINGS.UTF16LE;
      break;
    case 3:
      if (enc === 'hex' || enc === 'HEX' ||
      StringPrototypeToLowerCase(enc) === 'hex')
        return ENCODINGS.HEX;
      break;
    case 5:
      if (enc === 'ascii') return ENCODINGS.ASCII;
      if (enc === 'ucs-2') return ENCODINGS.UTF16LE;
      if (enc === 'UTF-8') return ENCODINGS.UTF8;
      if (enc === 'ASCII') return ENCODINGS.ASCII;
      if (enc === 'UCS-2') return ENCODINGS.UTF16LE;
      enc = StringPrototypeToLowerCase(enc);
      if (enc === 'utf-8') return ENCODINGS.UTF8;
      ...
      break;
    case 6:
      if (enc === 'base64') return ENCODINGS.BASE64;
      if (enc === 'latin1' || enc === 'binary') return ENCODINGS.LATIN1;
      if (enc === 'BASE64') return ENCODINGS.BASE64;
      if (enc === 'LATIN1' || enc === 'BINARY') return ENCODINGS.LATIN1;
      enc = StringPrototypeToLowerCase(enc);
      if (enc === 'base64') return ENCODINGS.BASE64;
    ...
    case 9:
      ...
      StringPrototypeToLowerCase(enc) === 'base64url')
        return ENCODINGS.BASE64URL;
      break;
    default:
      if (enc === '') return ENCODINGS.UTF8;
  }
}
