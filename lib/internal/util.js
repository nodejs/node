...
function isInsideNodeModules() {
  if (getStructuredStack === undefined) {
    ...
    })()`, { __proto__: null, overrideStackTrace }, { __proto__: null, filename: 'structured-stack' });
  }
...
function createDeferredPromise() {
  ...
  return { __proto__: null, promise, resolve, reject };
}
...
function getInternalGlobal() {
  if (internalGlobal == null) {
    // Lazy-load to avoid a circular dependency.
    const { runInNewContext } = require('vm');
    internalGlobal = runInNewContext('this', undefined, { __proto__: null, contextName: 'internal' });
  }
