...
function stopTest(timeout, signal) {
  ...
  if (timeout === kDefaultTimeout) {
    ...
    setOwnProperty(
      deferred,
      'promise',
      PromisePrototypeThen(deferred.promise, () => {
...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    // Validate and process flaky option
    if (flaky != null && flaky !== false) {
      this.flakyRetries = flaky === true ? kDefaultFlakyRetries : flaky;
      if (!NumberIsInteger(this.flakyRetries) || this.flakyRetries < 1) {
        throw new ERR_INVALID_ARG_TYPE(
          'options.flaky',
    ...
    } else if (flaky === undefined && this.parent?.flakyRetries > 0) {
      // Inherit flaky from parent suite
      this.flakyRetries = this.parent.flakyRetries;
    } else {
      this.flakyRetries = 0;
    }
    this.retriesTaken = 0;
    ...
    if (this.loc != null && this.root.harness.previousRuns != null) {
      ...
      const previousAttempt =
        this.root.harness.previousRuns[this.attempt - 1]?.[testIdentifier];
  ...
  addReadySubtest(subtest) {
    ...
    if (
      this.unfinishedSubtests.delete(subtest) &&
        ...
        for (let i = 0; i < coverages.length; i++) {
          ...
          if (actual < threshold) {
            ...
            reporter.diagnostic(
              nesting,
              loc,
              `Error: ${NumberPrototypeToFixed(actual, 2)}% ${name} coverage does not meet threshold of ${threshold}%.`,
              'error',
            );
          }
        }
  ...
  getReportDetails() {
    ...
    } else if (this.flakyRetries > 0 && this.retriesTaken > 0 && this.passed) {
