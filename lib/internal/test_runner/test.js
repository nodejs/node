...
class Test extends AsyncResource {
  ...
  async processPendingSubtests() {
    while (this.pendingSubtests.length > 0 && this.hasConcurrency()) {
      ...
      test.reporter.dequeue(test.nesting, test.loc, test.name, type);
      await test.run();
      deferred.resolve();
    }
  }
  ...
  start() {
    ...
    const type = this.reportedType ?? 'test';
    this.reporter.enqueue(this.nesting, this.loc, this.name, type);
    ...
    this.reporter.dequeue(this.nesting, this.loc, this.name, type);
    return this.run();
  }
