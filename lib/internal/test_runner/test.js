...
class TestContext {
  ...
  attempt(name, options, fn) {
    ...
    const subtest = this.#test.createSubtest(
      // eslint-disable-next-line no-use-before-define
      RetryableTest, name, options, fn, overrides,
    );
...
class RetryableTest extends Test {
  #retries; // The maximum number of retries allowed.
  #failedCount; // The number of failed retries.

  constructor(options) {
    super(options);

    const { retries } = options;
    validateUint32(retries, 'options.retries', true);

    this.#retries = options.retries;
    this.#failedCount = 0;
    this.fn = this.#retryableFn(this.fn);
  }
  ...
  #retryableFn(fn) {
    const retryWrapped = async (...args) => {
      try {
        ...
        if (this.#failedCount < this.#retries) {
          this.#failedCount++;
          this.diagnostic(`Retrying test, attempt ${this.#failedCount}`);
