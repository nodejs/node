...
const kHookNames = ObjectSeal(['before', 'after', 'beforeEach', 'afterEach', 'beforeEachSuite', 'afterEachSuite']);
...
class SuiteContext {
  ...
  beforeEachSuite(fn, options) {
    this.#suite.createHook('beforeEachSuite', fn, options);
  }

  afterEachSuite(fn, options) {
    this.#suite.createHook('afterEachSuite', fn, options);
  }
}
  ...
  constructor(options) {
    ...
    if (parent === null) {
      ...
      this.hooks = {
        ...
        beforeEachSuite: ArrayPrototypeSlice(parent.hooks.beforeEachSuite),
        afterEachSuite: ArrayPrototypeSlice(parent.hooks.afterEachSuite),
      };
    }
...
class Suite extends Test {
  ...
  async run() {
    ...
    const afterEachSuite = runOnce(async () => {
      if (this.parent?.hooks.afterEachSuite.length > 0) {
        await this.parent.runHook('afterEachSuite', this.parent.getRunArgs());
      }
    });
    ...
    try {
      ...
      if (this.parent.hooks.beforeEachSuite.length > 0) {
        await this.parent.runHook('beforeEachSuite', this.parent.getRunArgs());
      }
      ...
      await afterEachSuite();
      await after();

      this.pass();
    } catch (err) {
      try { await afterEachSuite(); } catch { /* suite is already failing */ }
