...
function stopTest(timeout, signal) {
  ...
  if (timeout === kDefaultTimeout) {
    deferred[SymbolDispose] = addAbortListener(signal, deferred.resolve)[SymbolDispose];
    return deferred;
  }
    ...
    try {
      ...
      if (this.fn.length === runArgs.length - 1) {
        ...
        if (isPromise(ret)) {
          ...
          await SafePromiseRace([ret, stopPromise.promise]);
        } else {
          await SafePromiseRace([PromiseResolve(promise), stopPromise.promise]);
        }
      } else {
...
class Suite extends Test {
  ...
  async run() {
    ...
    try {
      ...
      await SafePromiseRace([promise, stopPromise.promise]);
