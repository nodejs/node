...
class TestContext {
  ...
  retries(number) {
    validateNumber(number, 'number', 0, Number.MAX_SAFE_INTEGER);
    this.#test.retries = number;
  }

  get retryCount() {
    return this.#test.retries;
  }

  get currentAttempt() {
    return this.#test.currentAttempt;
  }
...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    let { fn, name, parent, retries } = options;
    if (retries === undefined) retries = 0;
    const { concurrency, loc, only, timeout, todo, skip, signal } = options;
    validateNumber(retries, 'options.retries', 0, Number.MAX_SAFE_INTEGER)

    this.retries = retries;
    this.currentAttempt = 0;
    fn = this.#retryableFn(fn);
  ...
  #retryableFn(fn) {
    const retryWrapped = async (...args) => {
      try {
        return await fn(...args);
      } catch (err) {
        if (this.currentAttempt < this.retries) {
          await this.runHook('afterEach', this.getRunArgs());
          this.currentAttempt++;
          this.diagnostic(`Retrying test, attempt ${this.currentAttempt}`);
          await this.runHook('beforeEach', this.getRunArgs());
