...
class TestContext {
  ...
  get assert() {
    if (this.#assert === undefined) {
      ...
      // choice is resolved once, not re-evaluated per assertion.
      const capturedPlan = test.plan;
      const countPlan = test.isFlaky ?
        () => test.plan?.count() :
        () => capturedPlan?.count();
...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    if (flaky == null) {
      ...
      validateInteger(flaky, 'options.flaky', 1);
      this.flakyRetries = flaky;
    } else {
      throw new ERR_INVALID_ARG_TYPE(
        'options.flaky', ['boolean', 'number'], flaky);
    }
    this.isFlaky = this.flakyRetries > 0;
    this.retryCount = 0;
  ...
  #resetForRetry(ctx, baseDiagnosticsCount, baseHookCounts) {
    ...
    // `subtests`, which crashed report() (see the guard there).
    this.subtests = [];
    this.outputSubtestCount = 0;
    this.activeSubtests = 0;
    this.pendingSubtests = [];
    ...
    this.subtestsPromise = null;
    this.waitingOn = 0;
    // reportOrder keys readySubtests and is matched against waitingOn, so it
    // must restart together with `subtests`.
    this.nextReportOrder = 1;
    // Counts parked by the discarded attempt's subtests (see #countCompleted).
    this.pendingCountedSubtests = undefined;
    ...
    // The retry decision for that attempt has already been made, so this cannot
    // be misread as an external abort. A fresh controller then backs the next
    // attempt, preventing the previous abort from being observed as an external
    // abort by the retry decision below.
    this.abortController.abort();
    this.abortController = new AbortController();
    this.signal = this.abortController.signal;
  }
  ...
  async run() {
    ...
    // `afterEach` is recreated for each attempt so that it runs again on every
    // retry of a flaky test (including after a failed attempt). `before`/`after`
    // remain suite-once: their `hook.run` is wrapped with runOnce in createHook(),
    // so re-entering them across attempts is a no-op. One factory keeps the
    // initial creation and the per-attempt recreation in sync.
    const makeAfterEach = () => runOnce(async () => {
    ...
    // A test marked `flaky: N` runs once and is then retried up to N more times
    // until it passes, for a maximum of N + 1 attempts ("inclusive"). When the
    // test is not flaky, `flakyRetries` is 0 so this is 1 and exactly one
    // attempt runs, keeping the behavior byte-identical to a non-flaky test.
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      if (attempt > 1) {
        this.#resetForRetry(ctx, baseDiagnosticsCount, baseHookCounts);
        afterEach = makeAfterEach();
      }
      // Set by the catch path when it defers the final-attempt duties
      // (error publish, `after`) to a retry; see the loop bottom.
      let willRetry = false;
      let attemptError;
      ...
      try {
        ...
        // A fresh timeout timer is created for each attempt so that a slow
        // attempt does not consume the budget of subsequent retries.
        ...
        if (isTestFailureError(err)) {
          if (
            ...
            // Timeouts are cancelled rather than failed in two cases: a
            // non-flaky test keeps its existing cancellation semantics, and an
            // expectFailure test would otherwise have fail() count the timeout
            // as its expected failure (passed = true), masking a hang as a
            // pass. Flaky non-expectFailure timeouts fall through to fail()
            // below so they can be retried.
            this.#cancel(err);
          } else {
            // A flaky timeout must not flow through #cancel: #cancel aborts the
            // abort controller, which the retry decision would misread as an
            // external abort and stop retrying. Record it as a normal failure so
            // it can be retried, and so that on exhaustion it is counted as a
        ...
        // publishes, as before.
        willRetry = this.#retriesRemaining(attempt, maxAttempts) && !this.passed;
      ...
      // retries. An external abort stops retrying immediately; an expectFailure
      // test that fails as expected stops because its verdict is already a pass.
  ...
  postRun(pendingSubtestsError) {
    ...
    if (this.parent !== null) {
      ...
      if (harness.counters.flaky > 0) {
        reporter.diagnostic(nesting, loc, `flaky ${harness.counters.flaky}`);
      }
  ...
  report() {
    if (this.pendingCountedSubtests !== undefined) {
      const pending = this.pendingCountedSubtests;
      this.pendingCountedSubtests = undefined;
      for (let i = 0; i < pending.length; i++) {
        pending[i].#countCompleted();
      }
    }
    this.#countCompleted();
