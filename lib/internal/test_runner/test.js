...
class TestContext {
  ...
  get assert() {
    if (this.#assert === undefined) {
      ...
      // A flaky test recreates its plan on each retry, so its assertions must
      // read `test.plan` live. A non-flaky test captures the plan reference once
...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    const { concurrency, entryFile, expectFailure, flaky, loc, only, plan, signal, skip, timeout, todo } = options;
  ...
  async run() {
    ...
    // so re-entering them across attempts is a no-op.
    let afterEach = runOnce(async () => {
    ...
    // until it passes, for a maximum of N + 1 attempts ("inclusive"). When
    // the test is not flaky, `flakyRetries` is 0 and exactly one attempt runs,
    // keeping the behavior byte-identical to a non-flaky test.
    const maxAttempts = this.flakyRetries > 0 ? this.flakyRetries + 1 : 1;
    // Diagnostics emitted before the run (e.g. during construction) must survive
    // retries; only diagnostics produced by a previous attempt are discarded.
    const baseDiagnosticsCount = this.diagnostics.length;

    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      if (attempt > 1) {
        // Reset per-attempt state before retrying a flaky test. Anything
        // that would otherwise leak between attempts is cleared here so that the
        // retry starts from a clean slate. `before`/`after` (suite-once) and the
        // accumulated parent hooks are intentionally left untouched.
        this.error = null;
        this.passed = false;
        this.cancelled = false;
        ...
        // otherwise it accumulates across attempts while `subtests` holds only
        // the final attempt's children, inflating the plan count and - when the
        ...
        this.subtestsPromise = null;
        this.waitingOn = 0;
        // Snapshot numbering lives on the shared per-file SnapshotFile and
  ...
  postRun(pendingSubtestsError) {
    ...
    if (this.parent !== null) {
      ...
      // Only surface the flaky counter when there were flaky test-cases, so the
      // summary is unchanged for runs that do not use the feature (mirrors the
      // conditional `coverage` summary line).
  ...
  getReportDetails() {
    ...
    } else if (this.flakyRetries > 0) {
      // Emit the flaky directive for every flaky-marked test (retryCount may be
      // 0 when it passed on the first attempt). Reporters only render `# FLAKY`
      // when retryCount > 0, but always surfacing retryCount lets the
      // run() process-isolation parent recognise and count flaky tests and
      // classify an exhausted retried timeout as a failure, not cancelled.
      directive = this.reporter.getFlaky(this.retriesTaken);
    }
