...
class Test extends AsyncResource {
  ...
  #resetForRetry(ctx, baseDiagnosticsCount) {
    this.error = null;
    this.passed = false;
    this.cancelled = false;
  ...
  async run() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      if (attempt > 1) {
        this.#resetForRetry(ctx, baseDiagnosticsCount);
      ...
      try {
        ...
        if (isTestFailureError(err)) {
          if (err.failureType === kTestTimeoutFailure && !this.isFlaky) {
            this.#cancel(err);
          } else {
        ...
        // publishes, as before.
        const willRetry = this.#retriesRemaining(attempt, maxAttempts) && !this.passed;
