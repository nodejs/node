...
class BaseContext {
  ...
  get signal() {
    return this[kContext].signal;
  }

  get name() {
    return this[kContext].name;
  }
}

class TestContext extends BaseContext {
  diagnostic(message) {
    this[kContext].diagnostic(message);
  }

  get mock() {
    this[kContext].mock ??= new MockTracker();
    return this[kContext].mock;
  }

  runOnly(value) {
    this[kContext].runOnlySubtests = !!value;
  }

  skip(message) {
    this[kContext].skip(message);
  }

  todo(message) {
    this[kContext].todo(message);
  }

  test(name, options, fn) {
    // eslint-disable-next-line no-use-before-define
    const subtest = this[kContext].createSubtest(Test, name, options, fn);

    return subtest.start();
  }

  beforeEach(fn, options) {
    this[kContext].createHook('beforeEach', fn, options);
  }

  afterEach(fn, options) {
    this[kContext].createHook('afterEach', fn, options);
  }
}

class SuiteContext extends BaseContext {
}
