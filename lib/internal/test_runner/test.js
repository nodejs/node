...
class TestContext extends BaseContext {
  diagnostic(message) {
    this._context.diagnostic(message);
  }

  get mock() {
    this._context.mock ??= new MockTracker();
    return this._context.mock;
  }

  runOnly(value) {
    this._context.runOnlySubtests = !!value;
  }

  skip(message) {
    this._context.skip(message);
  }

  todo(message) {
    this._context.todo(message);
  }

  test(name, options, fn) {
    // eslint-disable-next-line no-use-before-define
    const subtest = this._context.createSubtest(Test, name, options, fn);

    return subtest.start();
  }

  beforeEach(fn, options) {
    this._context.createHook('beforeEach', fn, options);
  }

  afterEach(fn, options) {
    this._context.createHook('afterEach', fn, options);
  }
}
