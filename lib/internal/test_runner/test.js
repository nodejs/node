...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    if (this.loc != null && this.config.previousRuns != null) {
      let testIdentifier = `${relative(process.cwd(), this.loc.file)}:${this.loc.line}:${this.loc.column}`;
      ...
      const previousAttempt = this.config.previousRuns[this.attempt - 1]?.[testIdentifier]?.passed_on_attempt;
