...
function runInParentContext(Factory) {
  function validateTestOptions(options) {
    const validOptions = ['concurrency', 'only', 'signal', 'skip', 'todo', 'timeout'];
    ArrayPrototypeForEach(ObjectKeys(options), (key) => {
      if (ArrayPrototypeFindIndex(validOptions, (v) => v === key) === -1) {
        throw new ERR_TEST_FAILURE(
          `Invalid option "${key}" passed to test function`,
          kInvalidTestOptions,
        );
      }
    });
