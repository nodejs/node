'use strict';
const { FunctionPrototypeBind, ArrayPrototypeForEach, ReflectApply, SafeMap } = primordials;
const {
  createHook,
  executionAsyncId,
} = require('async_hooks');
...
function runInParentContext(factory) {
  function cb(name, options, fn, overrides) {
    const parent = testResources.get(executionAsyncId()) || setup(root);
    const subtest = ReflectApply(parent[factory], parent, [name, options, fn, overrides]);
    if (parent === root) {
      subtest.run();
    }
    return subtest;
  ...
  ArrayPrototypeForEach(['skip', 'todo', 'only'], (keyword) => {
    cb[keyword] = (name, options, fn) => {
      return cb(name, options, fn, { [keyword]: true });
    };
  });
  return (name, options, fn) => cb(name, options, fn);
}

module.exports = {
  test: FunctionPrototypeBind(test, root),
  describe: runInParentContext('createSubSuite'),
  it: runInParentContext('createSubtest'),
};
