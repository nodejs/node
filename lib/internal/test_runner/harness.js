...
const { kCancelledByParent, kInvalidTestOptions, Test, Suite } = require('internal/test_runner/test');
const {
  parseCommandLine,
  reporterScope,
  setupTestReporters,
...
// Set up valid options schema
const validOptions = new SafeMap();
validOptions.set('concurrency', ['number', 'boolean']);
validOptions.set('only', ['boolean']);
validOptions.set('signal', ['object']);
validOptions.set('skip', ['boolean', 'string']);
validOptions.set('todo', ['boolean', 'string']);
validOptions.set('timeout', ['number']);
...
function validateTestOptions(options) {
  ArrayPrototypeForEach(ObjectKeys(options), (optionKey) => {
    if (!validOptions.has(optionKey)) {
      throw new ERR_TEST_FAILURE(
        `Invalid option "${optionKey}" passed to test function`,
        kInvalidTestOptions,
      );
    }

    const expectedTypes = validOptions.get(optionKey);

    if (!ArrayPrototypeIncludes(expectedTypes, typeof options[optionKey])) {
    ...
    if (
      ...
      optionKey === 'signal' &&
      !(options[optionKey] instanceof AbortSignal)
    ) {
      throw new ERR_INVALID_ARG_TYPE(
        `options.${optionKey}`,
        ['AbortSignal'],
...
function runInParentContext(Factory) {
  ...
  const test = (name, options, fn) => {
    // Check for invalid options in `options` argument
    if (Factory === Test) {
