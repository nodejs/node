...
function createTestTree(rootTestOptions, globalOptions) {
  ...
  const globalSetupDeferred = PromiseWithResolvers();
  ...
  const globalSetupContext = { __proto__: null };
  const harness = {
    __proto__: null,
    ...
    globalSetupPromise: globalSetupDeferred.promise,
    watching: false,
    ...
    isFilteringByName,
    isFilteringByOnly,
    async waitForGlobalSetup() {
      if (globalSetupExecuted) {
        globalSetupDeferred.resolve();
        return;
      }

      const globalSetupFunctions =
      await setupGlobalSetupTeardownFunctions(
        globalOptions.globalSetupPath,
        globalOptions.cwd,
      );
      globalSetupFunction = globalSetupFunctions.globalSetupFunction;
      globalTeardownFunction = globalSetupFunctions.globalTeardownFunction;
      harness.globalTeardownFunction = globalTeardownFunction;
  ...
  if (!getOptionValue('--test')) {
    harness.waitForGlobalSetup();
  }
  globalRoot = new Test({
    __proto__: null,
    ...rootTestOptions,
  ...
  const exitHandler = async (kill) => {
    ...
    if (root.harness.globalTeardownFunction) {
      await root.harness.globalTeardownFunction({
        __proto__: null,
        context: root.harness.globalSetupContext,
      });
...
async function startSubtestAfterBootstrap(subtest) {
  if (subtest.root.harness.buildPromise) {
    if (subtest.root.harness.globalSetupPromise) {
      if (getOptionValue('--test')) {
        queueMicrotask(() => {
          subtest.root.harness.waitForGlobalSetup();
        });
      }
      await subtest.root.harness.globalSetupPromise;
      subtest.root.harness.globalSetupPromise = null;
    }
