...
const moduleImporters = new SafeMap();

/**
 * @param {string} url Resolved URL of the module to start from.
 * @returns {SafeSet} The set of importer URLs.
 */
function collectMockDependents(url) {
  const dependents = new SafeSet();
  const queue = [url];

  while (queue.length > 0) {
    const current = ArrayPrototypePop(queue);
    const importers = moduleImporters.get(current);
    ...
    for (const importer of importers) {
      if (!dependents.has(importer)) {
        ...
        ArrayPrototypePush(queue, importer);
      }
    }
  }
...
function resolve(specifier, context, nextResolve) {
  ...
  if (context.parentURL !== undefined) {
    const importers = moduleImporters.get(mockSpecifier);
    if (importers === undefined) {
      moduleImporters.set(mockSpecifier, new SafeSet([context.parentURL]));
    } else {
      importers.add(context.parentURL);
    }
  }
