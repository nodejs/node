...
function run(options = kEmptyObject) {
  ...
  const runChain = async () => {
    if (root.harness?.waitForGlobalSetup) {
      await root.harness.waitForGlobalSetup({ __proto__: null, context: root.harness.globalSetupContext });
      root.harness.globalSetupFunction = null;
    }
    ...
    await runFiles();
    postRun?.();
    if (root.harness?.waitForGlobalTeardown) {
      await root.harness.waitForGlobalTeardown({ __proto__: null, context: root.harness.globalSetupContext });
