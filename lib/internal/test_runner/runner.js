...
class FileTest extends Test {
  ...
  #skipReporting() {
    const hasError = this.error && this.error.failureType !== kSubtestsFailed;
    if (hasError) {
      return false;
    }
...
function run(options = kEmptyObject) {
  ...
  root.harness.isFilteringByName ||= testNamePatterns != null || testSkipPatterns != null ||
    getOptionValue('--test-name-pattern').length > 0 ||
    getOptionValue('--test-skip-pattern').length > 0;
