...
function runTestFile(path, filesWatcher, opts) {
  ...
  const subtest = opts.root.createSubtest(FileTest, testPath, testOpts, async (t) => {
    ...
    const env = { __proto__: null, ...(opts.env || process.env) };
    ...
    // Encode the test-runner context and colorize flag in NODE_TEST_CONTEXT as
    // a JSON string. Using a dedicated field avoids setting FORCE_COLOR, which
    // would bleed into user code and override explicit stream.isTTY=false checks
    // in util.styleText(). See https://github.com/nodejs/node/issues/57921.
    env.NODE_TEST_CONTEXT = JSONStringify({
      __proto__: null,
      context: 'child-v8',
      colorize: opts.root.harness.shouldColorizeTestFiles,
    });
