...
class FileTest extends Test {
  ...
  #processRawBuffer() {
    ...
    while (bufferHead) {
      // Emit any leading bytes that are not the start of a frame as stdout,
      // until a v8 header sits at the front of the buffer.
      ...
      while (bufferHead?.length >= kSerializedSizeHeader) {
        ...
        // bytes only mimic a frame, so stop deserializing and fall through to
        // the resync below. A genuine frame that fails to deserialize is left
        // to throw, so real report-protocol regressions are not hidden.
      ...
      if (!mimicsFrame) {
        // The head is either exhausted or an incomplete frame we must wait for.
        break;
      }

      // The head only mimics a frame. Emit its first byte as stdout and restart
      // the header search, so a real frame that follows is reported right away
      // instead of waiting for #drainRawBuffer at shutdown. This mirrors the no
      // progress path in #drainRawBuffer, so the stdout bytes come out the same.
      this.addToReport({
        __proto__: null,
        type: 'test:stdout',
