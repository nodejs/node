...
function createTestFileList(patterns, cwd) {
  ...
  // We need to resolve paths as this function's cwd may differ from the process cwd
  const results = ArrayPrototypeMap(glob.globSync(), (result) => relative(process.cwd(), resolve(cwd, result)));

  // If the cwd is not the same as the process.cwd(), we need to resolve the paths
  if (cwd !== process.cwd()) {
    ArrayPrototypeForEach(results, (result, i) => {
      results[i] = resolve(cwd, result);
    });
  }
