...
async function setupGlobalSetupTeardownFunctions(globalSetupPath, cwd) {
  ...
  if (globalSetupPath) {
    ...
    const globalSetupModule = await esmLoader.getOrInitializeCascadedLoader()
...
function run(options = kEmptyObject) {
  ...
  const runChain = async () => {
    ...
    let globalTeardownFunction;
    if (globalSetupPath) {
      const setupFuncs = await setupGlobalSetupTeardownFunctions(globalSetupPath, cwd);
      globalSetupFunction = setupFuncs.globalSetupFunction;
      globalTeardownFunction = setupFuncs.globalTeardownFunction;
    }
    if (typeof setup === 'function') {
      await setup(root.reporter);
    }
    if (globalSetupFunction) {
      await globalSetupFunction({
        __proto__: null,
        context,
      });
    }
    ...
    postRun?.();
    teardown?.();
    if (globalTeardownFunction) {
      await globalTeardownFunction({
        __proto__: null,
        context,
      });
    }
