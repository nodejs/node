  ...
  for await (const { type, data } of source) {
    switch (type) {
      ...
      case 'test:stderr':
        ...
        break;
      case 'test:stdout':
        yield reportStdout(data.message);
        break;
...
function reportStdout(message) {
  const lines = StringPrototypeSplit(message, kLineBreakRegExp);

  const formattedLines = [];
  for (let i = 0; i < lines.length; i++) {
    ArrayPrototypePush(formattedLines, `# ${lines[i]}`);
  }

  return `${ArrayPrototypeJoin(formattedLines, '\n')}\n`;
}
