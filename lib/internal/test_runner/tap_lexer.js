...
class InputStream {
  ...
  error(message, token) {
    ...
    const errorDetails = `, got "${token.value}" at line ${line}, column ${column} (start ${start}, end ${end})`;
...
class TapLexer {
  ...
  *scan() {
  ...
  scanHash(char) {
    ...
    if (this.isEOFSymbol(lastCharacter)) {
      this.isComment = true;
      return new Token({
    ...
    if (this.isWhitespaceSymbol(lastCharacter) && !this.isHashSymbol(nextToken)) {
      return new Token({
        kind: TokenKind.HASH,
        value: char,
        stream: this.source,
