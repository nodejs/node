...
class TapLexer {
  ...
  *scan() {
    while (!this.source.eof()) {
      ...
      // remenber the last scanned token (except for whitespace)
  ...
  *scanAll() {
  ...
  scanSpecialCharacter(char) {
    ...
    // for example:
    //    # is a special token
    //    \# is a literal and outputs as #
    //    \\# is also a literal and outputs as \#
    if (this.hasTheCurrentCharacterBeenEscaped(char)) {
      // consume the 1st backslash character so we don't output it
      // consume the 2nd backslash character as a literal
      return this.scanLiteral(char);
    }
  ...
  scanHash(char) {
    if (this.isComment || this.hasTheCurrentCharacterBeenEscaped(char)) {
      return new Token({
        type: TokenKind.LITERAL,
        value: char,
        stream: this.source,
    ...
    // if last token is whitespace or EOL, we consume as a comment
    if (
  ...
  isEscapeSymbol(char) {
    // NOTE(@manekinekko): we need to escape the escape character!
    // so, \\ means '\' in the source stream
    return char === '\\';
  }
