...
function buildFileTree(summary) {
  ...
  ArrayPrototypeForEach(summary.files, (file) => {
    let longestPart = 0;
    const parts = StringPrototypeSplit(relative(summary.workingDirectory, file.path), ('/'));
    let current = tree;
...
function getCoverageReport(pad, summary, symbol, color, table) {
  ...
  function writeReportLine({ file, depth = 0, coveragesColumns, fileCoverage, uncoveredLines }) {
    const fileColumn = `${prefix}${StringPrototypeRepeat(' ', depth)}${getCell(file, filePadLength - depth, StringPrototypePadEnd, false, fileCoverage)}`;
