...
function parseCommandLine() {
  ...
  let contextName;
  try {
    contextName = JSONParse(process.env.NODE_TEST_CONTEXT).contextName;
  } catch {
    contextName = process.env.NODE_TEST_CONTEXT;
  }
  const isChildProcess = contextName === 'child';
  const isChildProcessV8 = contextName === 'child-v8';
  let globalSetupPath;
  let concurrency;
