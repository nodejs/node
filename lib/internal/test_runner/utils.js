...
function parseCommandLine() {
  ...
  const rerunFilePath = getOptionValue('--test-rerun-failures');
  ...
  let previousRuns;
  if (rerunFilePath) {
    validatePath(rerunFilePath, '--test-rerun-failures');
    previousRuns = parsePreviousRuns(rerunFilePath);
    if (previousRuns === null) {
      throw new ERR_INVALID_ARG_VALUE('--test-rerun-failures', rerunFilePath, 'is not a valid rerun file');
    }
  }
