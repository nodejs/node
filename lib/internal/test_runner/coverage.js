...
class TestCoverage {
  ...
  cleanup() {
    ...
    delete process.env.NODE_TEST_COVERAGE_FILTER_DIR;
    delete process.env.NODE_TEST_COVERAGE_EXCLUDE_NODE_MODULES;
...
function setupCoverage(options) {
  ...
  if (originalCoverageDirectory === undefined) {
    process.env.NODE_TEST_COVERAGE_FILTER_DIR = enabled;

    if ((options.coverageIncludeGlobs?.length ?? 0) === 0) {
      process.env.NODE_TEST_COVERAGE_EXCLUDE_NODE_MODULES = '1';
    }
  }
