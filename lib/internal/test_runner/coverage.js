...
const { isWindows, setupCoverageHooks } = require('internal/util');
const { tmpdir } = require('os');
const { join, resolve, relative } = require('path');
const { fileURLToPath, pathToFileURL, URL } = require('internal/url');
const { kFirstEntry, SourceMap } = require('internal/source_map/source_map');
const {
  codes: {
...
// Match the relative path with `dot` enabled so globs cover dotfiles within
// the project (e.g. `test/.foo.js`), while keeping plain matching for the
// absolute path to avoid misinterpreting dot segments in the filesystem path
// (e.g. tmp dirs like `test/.tmp.0`).
function matchesCoverageGlob(relativePath, absolutePath, pattern) {
  return matchGlobPattern(relativePath, pattern, isWindows, true) ||
         matchGlobPattern(absolutePath, pattern);
}
...
class TestCoverage {
      ...
      for (let j = 0; j < functions.length; ++j) {
        ...
        for (let k = 0; k < ranges.length; ++k) {
          ...
          if (!startEntry.originalSource && endEntry.originalSource &&
            ...
            const { originalSource, originalLine, originalColumn } = sourceMap[kFirstEntry];
  ...
  #computeShouldSkipFileCoverage(url) {
    ...
    for (let i = 0; i < excludeGlobs.length; ++i) {
      if (matchesCoverageGlob(relativePath, absolutePath, excludeGlobs[i])) {
        return true;
      }
    }
    ...
    if (includeGlobs.length > 0) {
      for (let i = 0; i < includeGlobs.length; ++i) {
        if (matchesCoverageGlob(relativePath, absolutePath, includeGlobs[i])) {
          return false;
        }
      }
      return true;
