...
class TapParser extends Transform {
  ...
  parse(chunkAsString = '', callback = null) {
    ...
    // This is fine because we don't want to parser to reparse the previous chunks
    ...
    try {
      this.#tokens = this.#lexer
        .scan()
        .filter((chunk) => chunk.length > 0 && chunk[0].kind !== TokenKind.EOL);
  ...
  check() {
    ...
    // TODO(@manekinekko): add validation support for async parsing.
  ...
  #Comment() {
    ...
    if (subtestKeyword) {
      ...
      if (this.#isSyncParsingEnabled && commentLiteral !== '') {
