...
class TapParser extends Transform {
    ...
    if (firstToken) {
      ...
      switch (kind) {
        ...
        case TokenKind.LITERAL:
          ...
          if (RegExpPrototypeTest(/^Bail\s+out!/i, tokenChunksAsString)) {
            return this.#Bailout();
  ...
  #TestPoint() {
    ...
    if (todoOrSkipToken && todoOrSkipToken.kind === TokenKind.LITERAL) {
      if (RegExpPrototypeTest(/todo/i, todoOrSkipToken.value)) {
        todoOrSkipToken = 'todo';
        this.#next(); // skip token
      } else if (RegExpPrototypeTest(/skip/i, todoOrSkipToken.value)) {
  ...
  #Pragma() {
    ...
    while (
      ...
      pragmaKey = StringPrototypeReplace(pragmaKey, /,/g, '');
