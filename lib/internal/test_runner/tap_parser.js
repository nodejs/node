...
class TapParser extends Transform {
  ...
  #getCurrentIndentationLevel(chunk) {
    ...
    return (whitespaceCount / indentationFactor) | 0;
  }
  ...
  #Version() {
    ...
    // TODO: only run in sync mode: this.#visitVersion(version);
  ...
  #Plan() {
    ...
    // TODO: only run in sync mode: this.#visitPlan(plan);
  ...
  #TestPoint() {
    ...
    // TODO: only run in sync mode: this.#visitTestPoint(test);
  ...
  #Bailout() {
    ...
    const reason = this.#readNextLiterals().trim();
    // TODO: only run in sync mode: this.#visitBailout(reason);
  ...
  #Comment() {
    ...
    if (subtestKeyword) {
      if (/^Subtest:/i.test(subtestKeyword.value)) {
        ...
        // TODO: only run in sync mode: this.#visitSubtestName(subtestNameLiteral);
        return {
      ...
      // TODO: only run in sync mode: this.#visitComment(commentLiteral);
      return {
  ...
  #YAMLBlock() {
    ...
    if (yamlBlockSymbol.kind === TokenKind.TAP_YAML_START) {
      ...
      // TODO: only run in sync mode: this.#visitYAMLBlock(diagnostics);
  ...
  #Pragma() {
    ...
    // TODO: only run in sync mode: this.#visitPragma(pragmas);
  }
}
