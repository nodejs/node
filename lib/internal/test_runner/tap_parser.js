...
class TapParser extends Transform {
  ...
  parseAsync(chunkAsString, callback = null) {
    this.#isAsyncParsingEnabled = true;
    this.#currentTokenChunk = 0;
    this.#currentTokenIndex = 0;
    // Warning: we are overwriting the input on each stream call
  ...
  _transform(chunk, _encoding, next) {
    ...
    this.parseAsync(kChunkAsString, (ast, error) => {
      if (error) {
  ...
  #parseChunk(chunk, callback = null) {
    ...
    // then it's a sub-test

    // Remove the indentation block from the current chunk
    // Point the current token index to the first token in the sub chunk:
    // root level           --> #currentTokenIndex = 0
    // sub-test level 1     --> #currentTokenIndex = 4
    // sub-test level 2     --> #currentTokenIndex = 8
    // ...
    this.#currentTokenIndex = this.#subTestNestingLevel * this.#kSubtestBlockIndentationFactor;
    if (this.#isYAMLBlock) {
      if (this.#subTestNestingLevel >= this.#kYamlIndentationFactor) {
        // when we are in a YAML block, we need to take into account the extra 2 spaces
        // that are added to the indentation level
        this.#currentTokenIndex -= this.#kYamlIndentationFactor ;
      }
    }
  ...
  #visitTestPoint(value) {
    this.#visit(this.#documents, this.#subTestNestingLevel, (node) => {
      ...
      if (this.#subtestsStack.length > 0) {
        ...
        if (
          level === this.#subTestNestingLevel + 1 &&
    ...
    this.#visit(
      ...
      (node) => {
        ...
        this.#subtestsStack.push({
          name: value,
          nesting: this.#subTestNestingLevel + 1,
        });
      },
  ...
  #YAMLBlock() {
    ...
    if (yamlBlockSymbol.kind === TokenKind.TAP_YAML_START) {
      if (this.#isYAMLBlock) {
        ...
        this.#error('Unexpected YAML start marker', yamlBlockSymbol);
      }
      ...
      if (!this.#isYAMLBlock) {
        ...
        this.#error('Unexpected YAML end marker', yamlBlockSymbol);
      }
