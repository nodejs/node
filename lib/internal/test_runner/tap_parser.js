...
class TapParser {
  ...
  skipEOL() {
    ...
    if (token && token.type === TokenKind.EOL) {
  ...
  readNextLiterals() {
    const literals = [];
    while (this.peek(false) && this.peek(false).type !== TokenKind.EOL && this.peek(false).type !== TokenKind.EOF) {
  ...
  Body() {
    ...
    while (this.peek().type !== TokenKind.EOF) {
