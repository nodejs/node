...
class TapParser {
  ...
  peek(shouldSkipBlankTokens = true) {
    if (shouldSkipBlankTokens) {
      this.skip(TokenKind.EOL, TokenKind.WHITESPACE);
    }
  ...
  readNextLiterals() {
    ...
    // scan for EOL and Whitespace as well
    let nextToken = this.peek(false);

    // read all literals (and numerics) until we hit a non-literal
    while (
      nextToken &&
      ...
      literals.push(this.next(false).value);
