  ...
  parse() {
    ...
    this.tokens.forEach((chunk) => {
      ...
      if (type === TokenKind.TAP) {
        ...
        this.Comment();
      } else if (type === TokenKind.TAP_PRAGMA) {
        this.pragma = {
          ...this.pragma,
          ...this.Pragma(),
        };
      } else if (type === TokenKind.WHITESPACE) {
        // check if current token and next one are both whitespace tokens
        // this means we are reading a YAML block
        if (/^\s{2}/i.test(chunkAsString)) {
          const yamlBlock = this.YAMLBlock();
          if (yamlBlock) {
            // append YAML block to last test
            this.output.tests.at(-1).yaml = yamlBlock;
          }
        }

        // check if the 1st four tokens are whitespace tokens
        // this means we are reading a subtest block
        else if (/^\s{4}/i.test(chunkAsString)) {
          // TODO(@manekinekko): handle subtest blocks
        }
      } else if (type === TokenKind.LITERAL) {
        // check for "Bail out!" literal
        if (/^Bail\s+out!/i.test(chunkAsString)) {
          this.output.bailout = this.Bailout();
        } else {
