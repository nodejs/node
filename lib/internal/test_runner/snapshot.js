...
class SnapshotManager {
  ...
  serialize(input, inline = false, serializers = serializerFns) {
    try {
      let value = input;
  ...
  createAssert() {
    ...
    return function snapshotAssertion(actual, options = kEmptyObject) {
      ...
      const value = manager.serialize(actual, false, serializers);
  ...
  createInlineAssert() {
    ...
    return function inlineSnapshotAssertion(actual, expected, options = kEmptyObject) {
      emitExperimentalWarning(kExperimentalWarning);
      validateObject(options, 'options');
      validateString(expected, 'expected');
      const { serializers = serializerFns } = options;
      validateFunctionArray(serializers, 'options.serializers');
      const value = manager.serialize(actual, true, serializers);
