...
const kSourceMappingURLMagicComment = /(\/[*/]#\s+sourceMappingURL=)(?<sourceMappingURL>[^\s]+)(?![\s\S]*\1)/;
const kSourceURLMagicComment = /(\/[*/]#\s+sourceURL=)(?<sourceURL>[^\s]+)(?![\s\S]*\1)/;
...
function extractSourceURLMagicComment(content) {
  const match = RegExpPrototypeExec(kSourceURLMagicComment, content);
  if (match == null || !match.groups) {
    return null;
  }
  let sourceURL = match.groups.sourceURL;
