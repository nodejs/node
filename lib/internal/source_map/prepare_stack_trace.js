    ...
    try {
      ...
      if (sm) {
        ...
        if (originalSource && originalLine !== undefined &&
          ...
          const originalSourceNoScheme = StringPrototypeReplace(
            originalSource,
             /^file:\/\//,
              ''
            );
...
function getErrorSource(payload, originalSource, firstLine, firstColumn) {
  let exceptionLine = '';
  const originalSourceNoScheme = StringPrototypeReplace(originalSource,
                                                        /^file:\/\//, '');

  let source;
  const sourceContentIndex = ArrayPrototypeIndexOf(payload.sources, originalSource);
