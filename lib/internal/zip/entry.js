  ...
  *#rawChunksSync() {
  ...
  contentIterator(options) {
    return decodeMemberStream(this.#rawSource(), {
      ...
      // Apply the same default ceiling as content(): without it, streaming is
      // bounded only by the entry's declared (attacker-controlled) size.
      maxSize: options?.maxSize ?? getMaxZipContentSize(),
    });
  }
