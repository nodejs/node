...
// Given the fixed 30-byte local header read at `offset`, return where the
// member's data ends. The data starts after the full local header (fixed +
// file name + extra field), whose length lives only in the local header, not
// the central directory - so the exact end can only be measured by reading it.
// A malformed local header (bad signature) is not rejected here: fall back to
// the 30-byte minimum so the archive stays openable (a read of that entry will
// surface the error) while the member is still bounded by a safe lower bound.
// Well-formed headers - the case an overlap attack must use to be readable -
// are measured exactly, matching the read path and the in-memory reader.
function localHeaderEnd(fixed, offset, compressedSize) {
  if (fixed.readUInt32LE(0) !== SIG_LOCAL_FILE_HEADER) {
    return offset + 30 + compressedSize;
  }
  return offset + LocalFileHeader.length(fixed, 0) + compressedSize;
}

// Reject any member whose actual compressed bytes cannot lie inside the file,
// and any pair of members whose data ranges overlap each other or the central
// directory. The buffered read paths (`ZipEntry.prototype.content()` and
// friends) allocate `compressedSize` bytes before reading, so without the
// bounds check a tiny file whose central directory lies about a member's size
// could force an allocation of up to `kMaxLength` bytes. The overlap check
// counters the "quoted overlap" zip-bomb shape (CVE-2024-0450 in other
// implementations): N records quoting the same data region turn one small file
// into N full-size extractions, while real archives lay members out
// disjointly. The exact data range depends on the local header's length, which
// is read here (one small read per member) so the check matches the read path
// and the in-memory reader (`readArchiveEntries()`) rather than trusting a
// looser lower bound.
async function validateMemberBounds(fd, headers, prefix, size, centralDirectoryOffset) {
  const members = [];
  for (let i = 0; i < headers.length; i++) {
    const header = headers[i];
    const offset = header.localFileHeaderOffset + prefix;
    const fixed = Buffer.allocUnsafe(30);
    await readFdFully(fd, fixed, offset);
    const dataEnd = localHeaderEnd(fixed, offset, header.compressedSize);
    checkMemberRange(members, header.fileName, offset, dataEnd, size);
  }
  checkMemberOverlap(members, centralDirectoryOffset);
}

// Sync counterpart of validateMemberBounds().
function validateMemberBoundsSync(fd, headers, prefix, size, centralDirectoryOffset) {
  const members = [];
  for (let i = 0; i < headers.length; i++) {
    const header = headers[i];
    const offset = header.localFileHeaderOffset + prefix;
    const fixed = Buffer.allocUnsafe(30);
    readFdFullySync(fd, fixed, offset);
    const dataEnd = localHeaderEnd(fixed, offset, header.compressedSize);
    checkMemberRange(members, header.fileName, offset, dataEnd, size);
  }
  checkMemberOverlap(members, centralDirectoryOffset);
...
// Record a member's [offset, dataEnd) range, rejecting one that runs past the
// end of the file.
function checkMemberRange(members, fileName, offset, dataEnd, size) {
  if (dataEnd > size) {
    throw new ERR_ZIP_INVALID_ARCHIVE(
      `entry ${JSONStringify(fileName)} data is out of bounds`);
  }
  ArrayPrototypePush(members, { fileName, offset, dataEnd });
}

// Sorted by local-header start, reject any member whose data runs into the
// next member's local header or into the central directory.
function checkMemberOverlap(members, centralDirectoryOffset) {
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    if (members[i].dataEnd > bound) {
      throw new ERR_ZIP_INVALID_ARCHIVE(
        `entry ${JSONStringify(members[i].fileName)} overlaps the next ` +
...
class ZipFile {
  ...
  // Guard: reject any operation once close() has been initiated, so a
  // request never reaches a closed (or reused) descriptor. Set synchronously
  // by close()/closeSync() so an add() issued after them cannot slip through.
  #assertOpen() {
    if (this.#closing) {
      throw new ERR_INVALID_STATE('the ZipFile has been closed');
    }
  }
  ...
  #handleFor(info) {
    info.entry ??= new ZipEntry(info.central, null, null, this.#handle, info.localOffset);
    return info.entry;
  }
  /**
  ...
  async add(filename, data, options) {
    this.#assertWritable();
    this.#assertOpen();
    // Reserve the mutation synchronously (before the async ZipEntry.create()),
    // so a close()/closeSync() issued right after cannot slip in front of it
    // and tear down the descriptor this write depends on.
  ...
  async #doAdd(entry) {
    ...
    try {
      for await (const chunk of entry) {
        await writeFdFully(this.#handle.fd, chunk, localOffset + written);
  ...
  addEntrySync(entry) {
    ...
    try {
      for (const chunk of entry) {
        writeFdFullySync(this.#handle.fd, chunk, localOffset + written);
        written += chunk.length;
      }
    } catch (err) {
  ...
  async #rewriteCentralDirectory() {
    ...
    for (let i = 0; i < chunks.length; i++) {
      await writeFdFully(this.#handle.fd, chunks[i], pos);
      pos += chunks[i].length;
    }
    await fsFtruncateAsync(this.#handle.fd, pos);
  ...
  #rewriteCentralDirectorySync() {
    ...
    let pos = this.#centralDirectoryOffset;
    for (let i = 0; i < chunks.length; i++) {
      writeFdFullySync(this.#handle.fd, chunks[i], pos);
      pos += chunks[i].length;
    }
    fs.ftruncateSync(this.#handle.fd, pos);
    this.#adoptRewrittenCentralDirectory(names, centralHeaders, records);
  }
  ...
  keys() { this.#assertOpen(); return MapPrototypeKeys(this.#entries); }
  *values() {
  ...
  *entriesSync() {
  ...
  get size() { this.#assertOpen(); return MapPrototypeGetSize(this.#entries); }
  ...
  // Idempotent: a second close() is a safe no-op, never a double-close of a
  // (possibly reused) descriptor. `#closing` is set synchronously so any
  // operation issued after this point is rejected rather than racing the fd.
  close() {
   ...
   * synchronous methods. Idempotent.
   */
  closeSync() {
  ...
  static async open(filename, options) {
    ...
    try {
      ...
      await validateMemberBounds(fd, headers, end.prefix, size, end.centralDirectoryOffset);
  ...
  static openSync(filename, options) {
    ...
    try {
      ...
      const headers = readCentralDirectory(directory, end.totalRecords);
      validateMemberBoundsSync(fd, headers, end.prefix, size, end.centralDirectoryOffset);
      return new ZipFile(fd, headers, end.prefix, end.centralDirectoryOffset, end.comment, writable);
    } catch (err) {
      try {
        fs.closeSync(fd);
