...
class BuiltinModule {
  ...
  static getAllBuiltinModuleIds() {
    ...
    ArrayPrototypePushApply(
      ...
      ArrayPrototypeMap(
        ArrayPrototypeFilter(
          ArrayFrom(schemelessBlockList),
          (id) => canBeRequiredByUsersList.has(id),
        ),
        (id) => 'node:' + id,
      ),
    );
  ...
  compileForInternalLoader() {
    ...
    try {
      const requireFn = StringPrototypeStartsWith(id, 'internal/deps/') ?
        (request) => requireWithFallbackInDeps(request, id) : requireBuiltin;
...
// Resolve a relative request (e.g. `./transform.js` or `../lib/wasm.js`) made
// by a builtin under `internal/deps/` to the id of the builtin it refers to.
...
function requireWithFallbackInDeps(request, parentId) {
  if (StringPrototypeStartsWith(request, 'node:')) {
    request = StringPrototypeSlice(request, 5);
  } else if (StringPrototypeStartsWith(request, './') ||
             StringPrototypeStartsWith(request, '../')) {
    request = resolveRelativeInDeps(request, parentId);
