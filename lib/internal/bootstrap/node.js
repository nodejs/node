...
(function bootstrapNodeJSCore(process,
  ...
  function startup() {
    ...
    if (internalBinding('worker').getEnvMessagePort() !== undefined) {
      ...
      if (process._eval != null && !process._forceRepl) {
        ...
        // preserve trailing slash on main.
        const trailingSlash = hasTrailingSlash(process.argv[1], path);
        process.argv[1] = path.resolve(process.argv[1]);
        if (trailingSlash) process.argv[1] += path.sep;
  ...
  // When a path is provided to the CLI the trailing slash must be preserved.
  // Otherwise cjs can load an incorrect file.
  function hasTrailingSlash(filename, path) {
