...
function startup() {
  ...
  if (process.env.NODE_V8_COVERAGE) {
    ...
    // Resolve the coverage directory to an absolute path.
    const coverageDireoctry = resolve(cwd, process.env.NODE_V8_COVERAGE);
    process.env.NODE_V8_COVERAGE = coverageDireoctry;
    const {
      writeCoverage,
      setCoverageDirectory
    } = NativeModule.require('internal/coverage-gen/with_profiler');
    setCoverageDirectory(coverageDireoctry);
    process.on('exit', writeCoverage);
