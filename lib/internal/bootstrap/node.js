...
const configBinding = internalBinding('config');
const workerBinding = internalBinding('worker');
const internalUtil = NativeModule.require('internal/util');
const util = NativeModule.require('util');

const exceptionHandlerState = { captureFn: null };
const isMainThread = workerBinding.threadId === 0;
...
function startup() {
  ...
  const { deprecate } = internalUtil;
  {
    ...
    const { pendingDeprecation } = configBinding;
    ...
    for (const name of [
      ...
      utilBinding[name] = pendingDeprecation ?
        deprecate(types[name],
                  'Accessing native typechecking bindings of Node.js ' +
...
// There are various modes that Node.js can run in. The most common two
...
function startExecution() {
  ...
  if (workerBinding.getEnvMessagePort() !== undefined) {
    NativeModule.require('internal/worker').setupChild(evalScript);
    return;
  }

  // To allow people to extend Node.js in different ways, this hook allows
  // one to drop a file lib/_third_party_main.js into the build
  // directory which will be executed instead of Node.js' normal loading.
...
function executeUserCode() {
  // User passed `-e` or `--eval` arguments to Node.js without `-i` or
  // `--interactive`.
  // Note that the name `_forceRepl` is merely an alias of `interactive`
  // in code.
  // TODO(joyeecheung): Use internal/options instead of
  ...
  if (argv1 && argv1 !== '-') {
    ...
    process.argv[1] = path.resolve(argv1);

    const CJSModule = NativeModule.require('internal/modules/cjs/loader');

    // If user passed `-c` or `--check` arguments to Node.js, check its syntax
    ...
    // Note: this actually tries to run the module as an ESM first if
  ...
  // Create the REPL if `-i` or `--interactive` is passed, or if stdin is a TTY.
  // Note that the name `_forceRepl` is merely an alias of `interactive`
  // in code.
...
function setupTraceCategoryState() {
  ...
  function toggleTraceCategoryState() {
    ...
    if (asyncHooksEnabled) {
      // Lazily load internal/trace_events_async_hooks only if the async_hooks
...
function setupGlobalConsole() {
  ...
  const consoleFromNode = NativeModule.require('internal/console/global');
  ...
  if (process.config.variables.v8_enable_inspector) {
    const inspector = NativeModule.require('internal/console/inspector');
...
function setupProcessFatal() {
  ...
  process._fatalException = (er) => {
    ...
    if (exceptionHandlerState.captureFn !== null) {
      ...
      try {
        const { kExpandStackSymbol } = internalUtil;
...
function setupProcessICUVersions() {
  if (!configBinding.hasIntl) return;  // No Intl/ICU: nothing to add here.
  const icu = internalBinding('icu');
  // With no argument, getVersion() returns a comma separated list
  // of possible types.
  const versionTypes = icu.getVersion().split(',');

  for (const name of versionTypes) {
...
function evalScript(name, body) {
  const CJSModule = NativeModule.require('internal/modules/cjs/loader');
  const path = NativeModule.require('path');
  const cwd = internalUtil.tryGetCwd(path);
