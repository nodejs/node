  ...
  if (pendingDeprecation) {
    ...
    process.env = new Proxy(process.env, {
      set: function(obj, prop, value) {
        const nonValidType = (typeof value !== 'boolean' ||
                              typeof value !== 'string' ||
                              isNaN(value));
        if (nonValidType) {
          const msg = 'Assigning any value other than a string, number, ' +
            'or boolean to a process.env property is deprecated. Please make ' +
            'sure to convert the value to a string before setting' +
            ' process.env with it.';
          process.emitWarning(msg, 'DeprecationWarning', 'DEP0104');
        }
