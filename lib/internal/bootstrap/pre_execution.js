...
function lateCodeInjectorHandler() {
  let resolvedAgent;

  try {
    resolvedAgent = require.resolve(process.env.NODE_LATE_ATTACH_AGENT);
  } catch (err) {
    return err; // to somehow use err
  }

  require(resolvedAgent).execute(function() {
    delete require.cache[resolvedAgent];
  });
}

function setupLateCodeInjector() {
  const signal = process.env.NODE_LATE_ATTACH_SIGNAL;
