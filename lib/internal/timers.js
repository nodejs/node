...
const { validateCallback, validateNumber } = require('internal/validators');
...
function initAsyncResource(resource, type) {
  ...
  const triggerAsyncId = (resource[trigger_async_id_symbol] =
    getDefaultTriggerAsyncId());
...
function getTimerCallbacks(runNextTicks) {
  ...
  function processImmediate() {
    const queue =
      outstandingQueue.head !== null ? outstandingQueue : immediateQueue;
    ...
    while (immediate !== null) {
      ...
      try {
        ...
        if (!argv) immediate._onImmediate();
        else immediate._onImmediate(...argv);
      } finally {
  ...
  function listOnTimeout(list, now) {
    ...
    while ((timer = L.peek(list)) != null) {
      ...
      try {
        ...
        else ReflectApply(timer._onTimeout, timer, args);
      } finally {
