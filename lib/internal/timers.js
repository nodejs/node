...
// The PriorityQueue — an adaptive implementation that uses different strategies
// based on queue size. For small queues (n<10, the typical case), it uses an
// unsorted array with cached minimum for O(1) peek and insert. For larger
// queues (n≥10), it automatically transitions to a 4-ary heap. This design
// optimizes for the common case where the queue holds TimersList objects
// (one per unique timeout duration, NOT per timer), resulting in queue sizes
// of n=3-20 even in large applications. The adaptive approach delivers 60-75%
// better performance than a traditional binary heap for typical workloads while
// gracefully handling pathological cases — which manages the order of expiring
// Timeout lists, and the object map lookup of a specific list by the duration
// of timers within (or creation of a new list). However, these operations
// combined have shown to be trivial in comparison to other timers architectures.
