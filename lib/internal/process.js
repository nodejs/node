...
const assert = process.assert = function(x, msg) {
  if (!x) {
    ...
    throw new errors.Error('ASSERTIONERROR', msg || 'assertion error');
  }
};
  ...
  process.cpuUsage = function cpuUsage(prevValue) {
    ...
    if (prevValue) {
      if (!previousValueIsValid(prevValue.user)) {
        const errors = lazyErrors();
        throw errors.TypeError('INVALIDARGVALUE', 'prevValue.user');
      }

      if (!previousValueIsValid(prevValue.system)) {
        const errors = lazyErrors();
        throw errors.TypeError('INVALIDARGVALUE', 'prevValue.system');
      }
    }
    ...
    if (errmsg) {
      const errors = lazyErrors();
      throw errors.Error('NOCPUUSAGE', errmsg);
    }
...
function setup_hrtime() {
  ...
  process.hrtime = function hrtime(ar) {
    ...
    if (typeof ar !== 'undefined') {
      ...
      const errors = lazyErrors();
      throw errors.TypeError('INVALIDARG', 'prevValue', 'Array');
    }
...
function setupKillAndExit() {
  ...
  process.kill = function(pid, sig) {
    ...
    if (pid != (pid | 0)) {
      const errors = lazyErrors();
      throw errors.TypeError('INVALIDPID');
    }
    ...
    if (0 === sig) {
      ...
      if (lazyConstants()[sig] &&
        ...
        const errors = lazyErrors();
        throw errors.Error('UNKNOWNSIGNAL', sig);
      }
    }
