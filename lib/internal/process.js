...
function setup_cpuUsage() {
  ...
  process.cpuUsage = function cpuUsage(prevValue) {
    ...
    if (!Number.isFinite(prevValue.user)) {
      throw new Error('previous value "user" property is not a number');
    }

    if (!Number.isFinite(prevValue.system)) {
      throw new Error('previous value "system" property is not a number');
    }

    // Return the diff of current value from previous value.
    return {
