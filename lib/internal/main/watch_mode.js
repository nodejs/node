...
const argsWithoutWatchOptions = ArrayPrototypeFilter(process.execArgv, (v, i) => {
  ...
  if (i > 0 && v[0] !== '-') {
    let prevArg = process.execArgv[i - 1]
...
function start() {
  ...
  if (!kRun) {
    ...
    if (kCommand.length !== 0) {
      throw new Error('cannot provide positional argument to both --run and --watch')
    }
    // Always get new data in case it changed
    const pkgJSON = getNearestParentPackageJSON(join(process.cwd(), 'abc'), false)
    if (!pkgJSON) {
      throw new Error(`unable to find package.json from directory ${process.cwd()}`)
    }
    let rawJSON
    try {
      rawJSON = JSON.parse(readFileSync(pkgJSON.data.pjsonPath, {
          encoding: 'utf8'
      }))
    } catch (e) {
      console.error(e)
    ...
    if (rawJSON) {
      if (!('scripts' in rawJSON)) {
        throw new Error(`package.json from directory ${process.cwd()} (${pkgJSON.data.pjsonPath}) has no "scripts" field`)
      }
      if (!(kRun in rawJSON.scripts)) {
        throw new Error(`package.json from directory ${process.cwd()} (${pkgJSON.data.pjsonPath}) has no entry in "scripts": ${JSON.stringify(kRun)}`)
      }
