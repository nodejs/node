...
function start() {
  ...
  child.once('exit', (code) => {
    ...
    if (code === 0) {
      process.stderr.write(`${blue}Completed running ${kCommandStr}. ${waitingForChanges}${white}\n`);
    } else {
      process.stderr.write(`${red}Failed running ${kCommandStr}. ${waitingForChanges}${white}\n`);
    }
  });
  return child;
}
...
function reportGracefulTermination() {
  ...
  graceTimer = setTimeout(() => {
    reported = true;
    process.stderr.write(`${blue}Waiting for graceful termination...${white}\n`);
  }, 500).unref();
  return () => {
    clearTimeout(graceTimer);
    if (reported) {
      ...
      process.stderr.write(`${green}Gracefully restarted ${kCommandStr}${white}\n`);
    }
  };
}
...
async function restart(child) {
  ...
  restarting = true;
  try {
    if (!kPreserveOutput) process.stderr.write(clear);
    process.stderr.write(`${green}Restarting ${kCommandStr}${white}\n`);
    await stop(child);
    return start();
  } finally {
    restarting = false;
