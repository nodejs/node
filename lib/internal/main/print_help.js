...
const envVars = new SafeMap(ArrayPrototypeConcat([
  ['FORCE_COLOR', { __proto__: null, helpText: "when set to 'true', 1, 2, 3, or an empty " +
   'string causes NO_COLOR and NODE_DISABLE_COLORS to be ignored.' }],
  ['NO_COLOR', { __proto__: null, helpText: 'Alias for NODE_DISABLE_COLORS' }],
  ['NODE_DEBUG', { __proto__: null, helpText: "','-separated list of core modules that " +
    'should print debug information' }],
  ['NODE_DEBUG_NATIVE', { __proto__: null, helpText: "','-separated list of C++ core debug " +
    'categories that should print debug output' }],
  ['NODE_DISABLE_COLORS', { __proto__: null, helpText: 'set to 1 to disable colors in ' +
    'the REPL' }],
  ['NODE_EXTRA_CA_CERTS', { __proto__: null, helpText: 'path to additional CA certificates ' +
    'file. Only read once during process startup.' }],
  ['NODE_NO_WARNINGS', { __proto__: null, helpText: 'set to 1 to silence process warnings' }],
  ['NODE_PATH', { __proto__: null, helpText: `'${require('path').delimiter}'-separated list ` +
    'of directories prefixed to the module search path' }],
  ['NODE_PENDING_DEPRECATION', { __proto__: null, helpText: 'set to 1 to emit pending ' +
    'deprecation warnings' }],
  ['NODE_PENDING_PIPE_INSTANCES', { __proto__: null, helpText: 'set the number of pending ' +
    'pipe instance handles on Windows' }],
  ['NODE_PRESERVE_SYMLINKS', { __proto__: null, helpText: 'set to 1 to preserve symbolic ' +
    'links when resolving and caching modules' }],
  ['NODE_REDIRECT_WARNINGS', { __proto__: null, helpText: 'write warnings to path instead ' +
    'of stderr' }],
  ['NODE_REPL_HISTORY', { __proto__: null, helpText: 'path to the persistent REPL ' +
    'history file' }],
  ['NODE_REPL_EXTERNAL_MODULE', { __proto__: null, helpText: 'path to a Node.js module ' +
    'which will be loaded in place of the built-in REPL' }],
  ['NODE_SKIP_PLATFORM_CHECK', { __proto__: null, helpText: 'set to 1 to skip ' +
    'the check for a supported platform during Node.js startup' }],
  ['NODE_TLS_REJECT_UNAUTHORIZED', { __proto__: null, helpText: 'set to 0 to disable TLS ' +
    'certificate validation' }],
  ['NODE_V8_COVERAGE', { __proto__: null, helpText: 'directory to output v8 coverage JSON ' +
    'to' }],
  ['TZ', { __proto__: null, helpText: 'specify the timezone configuration' }],
  ['UV_THREADPOOL_SIZE', { __proto__: null, helpText: 'sets the number of threads used in ' +
    'libuv\'s threadpool' }],
], hasIntl ? [
  ['NODE_ICU_DATA', { __proto__: null, helpText: 'data path for ICU (Intl object) data' +
    hasSmallICU ? '' : ' (will extend linked-in data)' }],
] : []), (hasNodeOptions ? [
  ['NODE_OPTIONS', { __proto__: null, helpText: 'set CLI options in the environment via a ' +
    'space-separated list' }],
] : []), hasCrypto ? [
  ['OPENSSL_CONF', { __proto__: null, helpText: 'load OpenSSL configuration from file' }],
  ['SSL_CERT_DIR', { __proto__: null, helpText: 'sets OpenSSL\'s directory of trusted ' +
    'certificates when used in conjunction with --use-openssl-ca' }],
  ['SSL_CERT_FILE', { __proto__: null, helpText: 'sets OpenSSL\'s trusted certificate file ' +
...
function print(stream) {
  ...
  options.set('-', { __proto__: null, helpText: 'script read from stdin ' +
                               '(default if no file name is provided, ' +
                               'interactive mode if a tty)' });
  options.set('--', { __proto__: null, helpText: 'indicate the end of node options' });
  stream.write(
    'Usage: node [options] [ script.js ] [arguments]\n' +
    '       node inspect [options] [ script.js | host:port ] [arguments]\n\n' +
    'Options:\n');
  stream.write(indent(format({
    __proto__: null, options, aliases, firstColumn, secondColumn,
  }), 2));

  stream.write('\nEnvironment variables:\n');

  stream.write(format({
    __proto__: null, options: envVars, firstColumn, secondColumn,
  }));
