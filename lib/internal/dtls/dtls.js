...
class DTLSEndpoint {
  ...
  connect(context, host, port, servername, verifyHost, verifyIsIp) {
    // servername (SNI) and verifyHost (expected peer identity) are applied to
    // the client SSL inside the binding, before the handshake's ClientHello is
    // emitted. They cannot be set afterwards from JS.
    const sessionHandle = this.#handle.connect(
      context, host, port, servername, verifyHost, verifyIsIp);
...
function connect(host, port, options = kEmptyObject) {
  ...
  // SNI is never sent for IP literals (also matching Node.js TLS).
  let servername = options.servername !== undefined ?
    (options.servername || undefined) :
    host;
  if (servername !== undefined && isIP(servername) !== 0) {
    servername = undefined;
  }

  // Peer identity to verify. Unless rejectUnauthorized is explicitly false,
  // the server certificate must match this name (or IP address) in addition
  // to chaining to a trusted CA. Defaults to the requested servername,
  // falling back to the connect host.
  let verifyHost;
  let verifyIsIp = false;
  if (options.rejectUnauthorized !== false) {
    verifyHost = options.servername || host;
    verifyIsIp = isIP(verifyHost) !== 0;
  }
