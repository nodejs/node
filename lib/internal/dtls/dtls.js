...
class DTLSEndpoint {
  ...
  connect(context, host, port, servername, session) {
...
function createContext(options = kEmptyObject) {
  ...
  if (isServer) {
    ...
    context.setSessionIdContext(
      options.sessionIdContext ?? defaultSessionIdContext());
...
function unwrapSession(session, verifyHost) {
  if (!Buffer.isBuffer(session) ||
      session.length < kSessionPrefix.length + 2 ||
...
const kContextOptions = [
  ...
  'rejectUnauthorized', 'requestCert', 'sessionIdContext', 'srtp',
  'ticketKeys',
];
...
function applySNIContexts(context, sni) {
  ...
  for (const hostname of hostnames) {
    const value = sni[hostname];

    let secureContext;
    if (value instanceof DTLSSecureContext) {
      if (!value.isServer) {
        throw new ERR_INVALID_ARG_VALUE(
          `options.sni['${hostname}']`, value,
          'was created for a client; SNI contexts need isServer: true');
      }
      secureContext = value;
    } else {
      validateObject(value, `options.sni['${hostname}']`);
      secureContext = createSecureContext({ ...value, isServer: true });
    }

    flat.push(hostname, secureContext[kHandle]);
  }

  context.setSNIContexts(flat);
}
