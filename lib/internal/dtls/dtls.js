...
class DTLSEndpoint {
  ...
  constructor(options = kEmptyObject) {
    ...
    // Socket options are applied when the handle binds, so they are read here
    // and held rather than passed to bind().
    if (options.ipv6Only !== undefined) {
      validateBoolean(options.ipv6Only, 'options.ipv6Only');
    }
    if (options.reusePort !== undefined) {
      validateBoolean(options.reusePort, 'options.reusePort');
    }
    // TTL is 1-255; the buffer sizes have no meaningful ceiling here, since
    // the kernel clamps to its own maximum and reports what it settled on.
    if (options.udpReceiveBufferSize !== undefined) {
      validateInteger(options.udpReceiveBufferSize,
                      'options.udpReceiveBufferSize', 1, 0xffffffff);
    }
    if (options.udpSendBufferSize !== undefined) {
      validateInteger(options.udpSendBufferSize,
                      'options.udpSendBufferSize', 1, 0xffffffff);
    }
    if (options.udpTTL !== undefined) {
      validateInteger(options.udpTTL, 'options.udpTTL', 1, 255);
    }
    this.#handle.setSocketOptions(options.ipv6Only === true,
                                  options.reusePort === true,
                                  options.udpReceiveBufferSize ?? 0,
                                  options.udpSendBufferSize ?? 0,
                                  options.udpTTL ?? 0);
 ...
 * @param {boolean} [options.ipv6Only] Serve IPv6 only on an IPv6 endpoint.
 *   Defaults to false, so binding '::' also accepts IPv4 peers.
 * @param {boolean} [options.reusePort] Set SO_REUSEPORT.
 * @param {number} [options.udpReceiveBufferSize] Socket receive buffer bytes.
 * @param {number} [options.udpSendBufferSize] Socket send buffer bytes.
 * @param {number} [options.udpTTL] Outgoing datagram TTL, 1-255.
