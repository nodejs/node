...
//   uint8_t has_keylog_listener;  // offset 5
class DTLSSessionState {
  ...
  get hasKeylogListener() {
    if (!isAlive(this.#handle)) return false;
    return DataViewPrototypeGetUint8(
      this.#handle, IDX_SESSION_STATE_HAS_KEYLOG_LISTENER) === 1;
  }

  set hasKeylogListener(val) {
    if (!isAlive(this.#handle)) return;
    DataViewPrototypeSetUint8(
      this.#handle, IDX_SESSION_STATE_HAS_KEYLOG_LISTENER, val ? 1 : 0);
  }
}
