...
const ArrayPrototype = ObjectGetPrototypeOf([]);
...
//
// We additionally require at least one side to have a *non-default* prototype
// (anything other than `Object.prototype`, `Array.prototype`, or `null`)
// so that this branch only fires when the mismatch is meaningfully driven by
// a custom class. That keeps the diagnostic narrowly scoped to the cases the
// originating issue describes - chiefly anonymous-class instances whose
// inspect output is `{}` on both sides - and leaves comparisons of two plain
...
function hasTopLevelPrototypeMismatch(actual, expected) {
  ...
  const protoActual = ObjectGetPrototypeOf(actual);
  const protoExpected = ObjectGetPrototypeOf(expected);
  if (protoActual === protoExpected) {
    return false;
  }
  return isCustomPrototype(protoActual) || isCustomPrototype(protoExpected);
}

// True for any prototype that does NOT match the default object/array/null
// prototypes - i.e. an object whose constructor is a user-defined class
// (anonymous or named).
function isCustomPrototype(proto) {
  if (proto === null || proto === ObjectPrototype || proto === ArrayPrototype) {
    return false;
  }
  return true;
}
