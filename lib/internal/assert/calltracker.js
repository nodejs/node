...
function checkMismatchError(
  { actual, expected, stack, callFrames, mode, operator }) {
...
function verifyTrackers() {
  ...
  for (const context of callChecks) {
    ...
    if (error) {
      ...
      combineErrorStacks(context.stack, error);
      errors.push(error);
...
function trackCalls(fn, expected) {
  ...
  if (verifyOnExit === true) {
    ...
    // TODO: Check if it's the correct function name.
    callTracker.add = callTracker.verify;
  } else {
    const checks = [verify];
    callTracker.verify = () => {
      const errors = [];
      ...
      const actual = ' mismatched the expected number of function calls';
      const error = new AssertionError({
        message: `Functions${actual}. Check the "errors" property for details.`,
        actual: `${errors.length} functions${actual}`,
        expected: 'All functions to be called the correct number of times',
        operator: 'verify'
      });
      error.generatedMessage = true;
    ...
    callTracker.add = (...args) => {
      const fn = trackCalls(...args);
      checks.push(fn.verify);
      delete fn.verify;
      return fn;
    };
    callTracker.func = () => callTracker();
  }
