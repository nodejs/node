  ...
  const source = getSource();
    ...
    snapshotValue = new SafeMap(
      ArrayPrototypeMap(
        ...
        (item) => {
          const keyDelimiter = StringPrototypeIndexOf(item, kKeyDelimiter);
          return [StringPrototypeSlice(item, 0, keyDelimiter), StringPrototypeSlice(item, keyDelimiter + kKeyDelimiter.length)];
        }
      ));
  } catch (e) {
    if (e.code === 'ENOENT') {
