 ...
 * @param {*} value
 * @param {string} name
 * @returns {asserts value is a parsable URL string}
 */
const validateURLString = hideStackFrames((value, name) => {
  const { URL } = require('internal/url'); // Can't be at the top
  if (!URL.canParse(value)) throw new ERR_INVALID_ARG_TYPE(name, 'a URL string', value);
});
