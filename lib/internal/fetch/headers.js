...
class Headers {
  ...
  #append(name, value) {
    ...
    } else if (this.#guard === 'request' && forbiddenHeaderNames.has(lowerName)) {
      return;
    } else if (this.#guard === 'request-no-cors') {
    ...
    } else if (this.#guard === 'response' && forbiddenResponseHeaderNames.has(name)) {
      return;
    }
    ...
    if (this.#guard === 'request-no-cors') {
      this.#removePrivilegedNoCORSRequestHeaders();
    }
  }
  ...
  #removePrivilegedNoCORSRequestHeaders() {
    for (const headerName of privilegedNoCORSRequestHeaderNames) {
      this.#headerList.delete(headerName);
    }
  }
  ...
  delete(name) {
    ...
    if (this.#guard === 'request' && forbiddenHeaderNames.has(lowerName)) {
      return;
    }
    if (this.#guard === 'request-no-cors' &&
    ...
    if (!this.#headerList.contains(lowerName)) {
      return;
    }
    this.#headerList.delete(lowerName);
    if (this.#guard === 'request-no-cors') {
      this.#removePrivilegedNoCORSRequestHeaders();
    }
  }
  ...
  *entries() {
    const headers = this.#headerList.sortAndCombine();
    yield* headers;
  }
