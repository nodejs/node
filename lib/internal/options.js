...
function generateConfigJsonSchema() {
  ...
  const envOptionsArray = [...envOptionsMap];
  for (let i = 0; i < envOptionsArray.length; i++) {
    const { 0: key, 1: type } = envOptionsArray[i];
  ...
  const namespaceOptionsArray = [...namespaceOptionsMap];
  for (let i = 0; i < namespaceOptionsArray.length; i++) {
    const { 0: namespace, 1: optionsMap } = namespaceOptionsArray[i];
    ...
    const optionsArray = [...optionsMap];
    for (let j = 0; j < optionsArray.length; j++) {
      const { 0: optionName, 1: optionType } = optionsArray[j];
