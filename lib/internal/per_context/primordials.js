...
const makeSafe = (unsafe, safe, next, {
  ...
  if (next) {
    ...
    ArrayPrototypeForEach(ReflectOwnKeys(unsafe.prototype), (key) => {
      if (!ReflectGetOwnPropertyDescriptor(safe.prototype, key)) {
        ...
        if (iteratorMethods !== null ? ArrayPrototypeIncludes(iteratorMethods, key) : (
  ...
  if (prototypeProperties !== null) {
    ObjectDefineProperties(safe, prototypeProperties);
  }

  copyProps(unsafe, safe);
  if (constructorProperties !== null) {
    ObjectDefineProperties(safe, constructorProperties);
  }
...
primordials.SafeArray = makeSafe(
  ArrayConstructor,
  class SafeArray extends ArrayConstructor {
