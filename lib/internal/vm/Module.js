...
const { extras: {
  createPrivateSymbol: PrivateSymbol,
} } = internalBinding('extras');
...
const kWrap = PrivateSymbol('Wrap');
const kDependencyCache = PrivateSymbol('DependencyCache');
const kLinkingStatus = PrivateSymbol('LinkingStatus');
...
class Module {
  constructor(src, options = {}) {
    ...
    // don't put these in defineProperties because
    // defineProperties won't know they exist.
    this[kWrap] = wrap;
    this[kLinkingStatus] = 'unlinked';
    this[kDependencyCache] = undefined;
  }
  ...
  get dependencySpecifiers() {
    let deps = this[kDependencyCache];
    if (deps !== undefined)
      return deps;

    deps = this[kWrap].getStaticDependencySpecifiers();
    Object.freeze(deps);
    this[kDependencyCache] = deps;
    return deps;
  }
  ...
  async link(linker) {
    ...
    if (this[kLinkingStatus] !== 'unlinked')
    ...
    wrap.link((specifier) => {
      const p = (async () => {
        ...
        const childLinkingStatus = m[kLinkingStatus];
