...
const DNS_DEFAULTS = {
  ...
  options: {
    ndots: 1,
    timeout: 5000, // ms
    attempts: 4, // how many tries before giving up
    rotate: false, // round-robin nameservers instead of always trying first one
    inet6: false, // query AAAA before A
...
exports.lookup = function lookup(hostname, options, callback) {
  ...
  resolve(hostname, type, flags, function resolvecb(err, rep) {
    ...
    if (!isCustomType) {
      var needv4Mapped = (v4mapped && family === 6 && triedIPv4);
      var addr;
      if (family === 0)
        family = (triedIPv4 ? 4 : 6);
      if (all) {
        var addrs = new Array(answers.length);
      ...
      if (all) {
        var vals = new Array(answers.length);
        for (var i = 0; i < answers.length; ++i)
          vals[i] = answers[i].data;
        callback(null, vals);
      } else {
...
function resolve(name, type, flags, cb) {
  var nameservers = dnsConfig.nameserver;
  var nsIdx = -1;
  var TCPOnly = (dnsConfig.options.use_vc || (flags & FLAG_RESOLVE_USEVC) > 0);
  ...
  function udpMsgHandler(msg, rinfo) {
    if (rinfo.address !== nameservers[nsIdx][1] || rinfo.port !== 53)
      return;
    clearTimeout(timer);
  ...
  function nextLookup(retry) {
    ...
    switch (lookupMethod) {
      case 'dns':
        ...
        var ns = nameservers[nsIdx];
        if (!ns) {
          // No more nameservers left, try next lookup method
          ++lookupIdx;
        ...
        if (retryingOnTCP) {
          ...
          socket.connect(53, ns[1]);
        } else {
          socket = new UDPSocket(ns[0] === 4 ? 'udp4' : 'udp6');
      ...
      if (m) {
        ...
        switch (m[1]) {
          ...
          case 'sortlist':
            ...
            break;
          case 'options':
            fillIn(ret.options, parseOptions(val.split(' ')));
            break;
          case 'lookup':
            val = val.split(' ');
...
function parseNsswitchConf() {
  ...
  try {
    ...
    if (m) {
      var order = m[1].replace(REGEX_COMMENT, '').trim().split(/[ \f\t\v,]+/g);
      for (var i = 0, loc; i < order.length; ++i) {
        loc = order[i].toLowerCase();
        if ((loc === 'dns' || loc === 'files') && ret.indexOf(loc) === -1) {
          if (!ret)
            ret = [loc];
          else
            ret.push(loc);
...
function parseHostConf() {
  ...
  try {
    ...
    if (m) {
      ...
      for (var i = 0, loc; i < order.length; ++i) {
        loc = order[i].toLowerCase();
        if (loc === 'bind' && ret.indexOf('dns') === -1) {
          if (!ret)
            ret = ['dns'];
          else
            ret.push('dns');
        } else if (loc === 'hosts' && ret.indexOf('files') === -1) {
          if (!ret)
            ret = ['files'];
          else
            ret.push('files');
...
(function initDNS() {
  ...
  if (process.platform === 'win32') {
    ...
    if (process.env.RES_OPTIONS)
      fillIn(dnsConfig.options, parseOptions(process.env.RES_OPTIONS));
