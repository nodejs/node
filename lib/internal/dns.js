...
// TODO(mscdex): move isIP C++ implementation from cares_wrap
...
// `exclusive` is needed in case we are querying from a cluster worker (and
// especially from Windows where UDP socket sharing is not supported)
const UDP_OPTIONS = { port: 0, exclusive: true };
...
exports.lookup = function lookup(hostname, options, callback) {
  ...
  if (!isCustomType) {
    ...
    if (hostname) {
      ...
      if (matchedFamily) {
        // TODO(mscdex): Return IPv4-mapped IPv6 address if `v4mapped === true`
        //               and `matchedFamily === 4` ?
        if (all) {
...
function resolver(type) {
  ...
  var flags = (type !== 'PTR' ? FLAG_RESOLVE_NSONLY : 0);
  return function query(name, callback) {
    ...
    resolve(name, type, flags, function resolvecb(err, rep) {
      if (err)
        return callback(err);
...
exports.lookupService = function(addr, port, callback) {
  ...
  if (isIP(addr) === 0)
    throw new TypeError('"addr" argument needs to be a valid IP address');
  ...
  exports.lookup(addr, { type: 'PTR' }, function(err, hostname) {
    ...
    if (process.platform === 'win32') {
      ...
      // TODO(mscdex): check /etc/nsswitch.conf for additional/alternate
      //               services file path
...
function resolve(name, type, flags, cb) {
  ...
  function doCb(err, ret, async) {
    ...
    if (!err && ret && ret.rcode !== 'NOERROR') {
      ...
      err = errnoException(ret.rcode, origName);
      ret = undefined;
    }
    ...
    if (rep === false) {
      ...
      if (rep.rcode === 'NOTIMP' || rep.rcode === 'SERVFAIL' ||
        ...
        if (reqIsPtr || !nextName()) {
          ...
          // TODO(mscdex): revert back to UDP if we are on TCP because of a
          //               truncated response? Either way we must use TCP if
          //               `TCPOnly === true`
          triedTCP = false;
  ...
  function nextLookup(retry) {
    ...
    if (!lookupMethod)
      return doCb(errnoException('ENOTFOUND', origName), undefined, false);
            ...
            if (/*(status === 'success' &&
              ...
              if (action === 'return') {
                if (lastStatus === NSS_REQ_ERR_NOTFOUND)
                  doCb(errnoException('ENOTFOUND', origName), undefined, false);
                else if (lastStatus === NSS_REQ_ERR_UNAVAIL)
                  doCb(errnoException('ESERVFAIL', origName), undefined, false);
              ...
              if (lastStatus === NSS_REQ_ERR_SUCCESS) {
                if (reqIsPtr) {
                  results = [{
                    name: name,
                    type: type,
                    ttl: 0,  // TODO(mscdex): fill in when available
                    data: results
                  }];
                } else {
...
function reqToBuf(qtype, name, origName) {
  var origName = name;
  ...
  for (var i = 0; i < name.length; ++i) {
    if (name[i].length > 63)
      return errnoException(exports.NOTFOUND, name);
...
function win32_wmiDNSInfo() {
  // TODO(mscdex): check AppendToMultiLabelName registry value before setting
  //               `DNSDomainSuffixSearchOrder`
  //               http://blogs.technet.com/b/networking/archive/2009/04/16/dns-
  //               client-name-resolution-behavior-in-windows-vista-vs-windows-
  //               xp.aspx
  var ret = false;
  var cmd = [
    'wmic',
    ...
    if (dnsinfo) {
      ...
      ret = {
        ...
        options: {
          ...
          ndots: 1,
          // TODO(mscdex): support `timeout` via DNSQueryTimeouts registry
          //               value?
...
function win32_getRegNameServers() {
  ...
  // TODO(mscdex): make this smarter/better rather than just using first
  //               interface?
  path += '\\Interfaces';
...
function parseResolvOptions(options) {
  ...
  for (var i = 0; i < options.length; ++i) {
    var m = REGEX_OPTS.exec(options[i]);
    if (m) {
      var val = m[2];
      switch (m[1]) {
        case 'attempts':
...
function parseNsswitchConf() {
  // TODO(mscdex): search for explicitly defined services file path too
  var ret = false;
  var seen = [];
...
(function initDNS() {
  ...
  if (process.platform === 'win32') {
    ...
    // TODO(mscdex): create fallbacks for win32_dbPath if it's not set in
    //               registry for some reason (e.g. try
    //               "%SystemRoot%\system32\drivers\etc" then
