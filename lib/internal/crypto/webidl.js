  ...
  'CShakeParams', [
    ...
    [
      ...
      },
      {
        ...
        converter: converters.BufferSource,
        validator(V, dict) {
          let { 0: major } = process.versions.openssl.split('.');
          major = NumberParseInt(major, 10);
          if (major >= 4) {
            // OpenSSL 4.0+ cSHAKE itself enforces that a non-empty function-name
            // is one of "KMAC", "TupleHash", or "ParallelHash" (NIST SP 800-185
            // Section 3.4); any other value fails the operation at digest time.
            this.validator = undefined;
          } else {
            this.validator = validateZeroLength('CShakeParams.functionName');
      ...
      },
      {
        ...
        validator(V, dict) {
          ...
          if (major >= 4) {
            ...
            this.validator = validateZeroLength('CShakeParams.customization');
            this.validator(V, dict);
          }
        },
      },
