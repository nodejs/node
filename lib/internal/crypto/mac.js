...
async function hmacGenerateKey(algorithm, extractable, keyUsages) {
  ...
  const key = await generateKey('hmac', { __proto__: null, length }).catch((err) => {
    throw lazyDOMException(
  ...
  return new InternalCryptoKey(
    key,
    { __proto__: null, name, length, hash: { __proto__: null, name: hash.name } },
    ArrayFrom(usageSet),
    extractable);
}
...
async function hmacImportKey(
  ...
  const { length } = keyObject[kHandle].keyDetail({ __proto__: null });
