...
const onNewHashChannel = dc.channel('crypto.hash.new');
const onCopyHashChannel = dc.channel('crypto.hash.copy');
const onNewHmacChannel = dc.channel('crypto.hmac.new');
...
function Hash(algorithm, options) {
  ...
  if (isCopy && onCopyHashChannel.hasSubscribers) {
    onCopyHashChannel.publish({ options });
  } else if (!isCopy && onNewHashChannel.hasSubscribers) {
    onNewHashChannel.publish({ algorithm, options });
  }
}
...
function Hmac(hmac, key, options) {
  ...
  ReflectApply(LazyTransform, this, [options]);
  if (onNewHmacChannel.hasSubscribers) {
    onNewHmacChannel.publish({
      algorithm: hmac,
      key, options
    });
  }
