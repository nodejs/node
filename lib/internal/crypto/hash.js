...
const kSignature = Symbol('kSignature');
...
// Implementation for Crypto Streams
...
class DigestSink {
  constructor(algorithm) {
    ...
    const name = normalizeHashName(algorithm.name);
    this[kHandle] = new _Hash(name, algorithm.length);
  ...
  async close() {
    ...
    const ret = this[kHandle].digest('buffer');
...
class HmacSink {
  constructor(hash, key, signature) {
    this[kHandle] = new _Hmac();
    this[kHandle].init(hash, key);
  ...
  async close() {
    ...
    if (this[kSignature] === undefined) {
      this[kState].resolve(ret.buffer);
    } else {
      const sig = this[kSignature].buffer || this[kSignature];
      this[kState].resolve(Buffer.compare(ret, new Uint8Array(sig)) === 0);
    }
  }
