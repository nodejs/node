...
async function aesGenerateKey(algorithm, extractable, keyUsages) {
  ...
  const key = await generateKey('aes', { __proto__: null, length }).catch((err) => {
    throw lazyDOMException(
...
async function aesImportKey(
  ...
  switch (format) {
    ...
    case 'jwk': {
      ...
      ({ length } = handle.keyDetail({ __proto__: null }));
  ...
  if (length === undefined) {
    ({ length } = keyObject[kHandle].keyDetail({ __proto__: null }));
    validateKeyLength(length);
  }
