...
const onNewSignChannel = dc.channel('crypto.sign.new');
const onNewVerifyChannel = dc.channel('crypto.verify.new');
...
function Sign(algorithm, options) {
  ...
  if (onNewSignChannel.hasSubscribers) {
    onNewSignChannel.publish({
      algorithm, options
    });
  }
}
...
function signOneShot(algorithm, data, key, callback) {
  ...
  if (onNewSignChannel.hasSubscribers) {
    onNewSignChannel.publish({ algorithm });
  }
...
function Verify(algorithm, options) {
  ...
  ReflectApply(Writable, this, [options]);
  if (onNewVerifyChannel.hasSubscribers) {
    onNewVerifyChannel.publish({ algorithm, options });
  }
}
...
function verifyOneShot(algorithm, data, key, signature, callback) {
  ...
  if (onNewVerifyChannel.hasSubscribers) {
    onNewVerifyChannel.publish({ algorithm });
  }
