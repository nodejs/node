...
const { Promise, SafeSet } = primordials;
...
const {
  codes: { ERR_INVALID_ARG_TYPE, ERR_MISSING_OPTION },
...
async function dsaGenerateKey(algorithm, extractable, keyUsages) {
  const { name, modulusLength, divisorLength, hash } = algorithm;
...
function dsaExportKey(key, format) {
  return jobPromise(
    new DSAKeyExportJob(kCryptoJobAsync, format, key[kKeyObject][kHandle])
  );
}
...
async function dsaImportKey(
    ...
    case 'jwk': {
      ...
      if (keyData.alg !== undefined) {
        ...
        const hash = normalizeHashName(
          keyData.alg,
          normalizeHashName.kContextWebCrypto
        );
  ...
  const { modulusLength, divisorLength } = keyObject[kHandle].keyDetail({});
