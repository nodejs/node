...
function argon2(password, salt, keylen, options, callback) {
  ...
  const { algorithm, secret, ad, iterations, parallelism, memory } = options;
...
function check(password, salt, keylen, options) {
  ...
  const { algorithm, iterations, parallelism, memory } = options;

  validateString(algorithm, 'options.algorithm');
  validateOneOf(algorithm, 'options.algorithm', ['argon2d', 'argon2i', 'argon2id']);
  ...
  validateUint32(iterations, 'options.iterations', true);
  validateInteger(parallelism, 'options.parallelism', 1, (2 ** 24) - 1);
  validateInteger(memory, 'options.memory', 8 * parallelism, MAX_POSITIVE_UINT_32);

  return { password, salt, algorithm, secret, ad, keylen, iterations, parallelism, memory };
}
