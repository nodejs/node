 ...
 * @param {number} [options.threads=4]
...
function argon2(password, salt, keylen, options, callback = defaults) {
  ...
  const { algorithm, secret, ad, iter, threads, lanes, memcost } = options;
  ({ password, salt, keylen } = options);

  validateFunction(callback, 'callback');

  const job = new Argon2Job(
    kCryptoJobAsync, password, salt, algorithm, secret, ad, iter, threads, lanes, memcost, keylen);
...
function argon2Sync(password, salt, keylen, options = defaults) {
  ...
  ({ password, salt, keylen } = options);
  const job = new Argon2Job(
    kCryptoJobSync, password, salt, algorithm, secret, ad, iter, threads, lanes, memcost, keylen);
...
function check(password, salt, keylen, options) {
  ...
  let { algorithm, secret, ad, iter, threads, lanes, memcost } = defaults;
  if (options && options !== defaults) {
    ...
    const has_threads = options.threads !== undefined;
    if (has_threads) {
      threads = options.threads;
      validateInteger(threads, 'threads', 1, (2 ** 24) - 1);
    }
    const has_lanes = options.lanes !== undefined;
    if (has_lanes) {
      lanes = options.lanes;
      validateInteger(lanes, 'lanes', threads, (2 ** 24) - 1);
    }
  ...
  return { password, salt, algorithm, secret, ad, keylen, iter, threads, lanes, memcost };
}
