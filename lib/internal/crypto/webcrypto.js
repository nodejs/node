...
function check(op, alg, length) {
  ...
  if (op === 'importKey') {
    if (
      ...
      (normalizedAlgorithm.length <= 0 || normalizedAlgorithm.length % 8)
    ) {
      return false;
    }
  ...
  if (op === 'encrypt' || op === 'decrypt') {
    if (normalizedAlgorithm.name === 'AES-CTR') {
      if (normalizedAlgorithm.length <= 0 || normalizedAlgorithm.length > 128) return false;
      try {
        validateByteLength(normalizedAlgorithm.counter, 16);
      } catch {
        return false;
      }
    }

    if (normalizedAlgorithm.name === 'AES-CBC') {
      try {
        validateByteLength(normalizedAlgorithm.iv, 16);
      } catch {
        return false;
      }
    }

    if (normalizedAlgorithm.name === 'AES-GCM') {
      try {
        validateMaxBufferLength(normalizedAlgorithm.iv);
      } catch {
        return false;
      }

      if (normalizedAlgorithm.tagLength !== undefined) {
        if (!require('internal/crypto/aes').kTagLengths.includes(normalizedAlgorithm.tagLength)) {
          return false;
        }
      }
      ...
      if (normalizedAlgorithm.additionalData !== undefined) {
        try {
          validateMaxBufferLength(normalizedAlgorithm.additionalData);
        } catch {
          return false;
        }
      }
  ...
  if (op === 'sign' || op === 'verify') {
    if (normalizedAlgorithm.name === 'Ed448' && normalizedAlgorithm.context?.byteLength) {
      return false;
    }
    return true;
  }

  if (op === 'digest') return true;

  if (op === 'generateKey') {
    if (normalizedAlgorithm.publicExponent) {
      if (bigIntArrayToUnsignedInt(normalizedAlgorithm.publicExponent) === undefined) {
        return false;
      }
    }
    ...
    if (normalizedAlgorithm.name === 'HMAC' && normalizedAlgorithm.length !== undefined) {
      if (normalizedAlgorithm.length === 0) return false;

      try {
        validateBitLength(normalizedAlgorithm.length, 'algorithm.length', true);
      } catch {
        return false;
      }
    }
    ...
    if (normalizedAlgorithm.name.startsWith('AES')) {
      if (!ArrayPrototypeIncludes(kAesKeyLengths, normalizedAlgorithm.length)) {
        return false;
      }
    }
  ...
  if (op === 'deriveBits') {
    ...
    if (normalizedAlgorithm.name === 'PBKDF2') {
      if (normalizedAlgorithm.iterations === 0) return false;
      if (length === null) return false;
      if (length % 8) return false;
    }

    if (
      normalizedAlgorithm.name === 'ECDH' ||
      normalizedAlgorithm.name === 'X25519' ||
      normalizedAlgorithm.name === 'X448'
    ) {
      if (normalizedAlgorithm.public.type !== 'public') return false;
      if (normalizedAlgorithm.name !== normalizedAlgorithm.public.algorithm.name) return false;
    }

    return true;
  }

  if (op === 'get key length') {
    if (normalizedAlgorithm.name.startsWith('AES')) {
      if (!ArrayPrototypeIncludes(kAesKeyLengths, normalizedAlgorithm.length)) {
        return false;
      }
