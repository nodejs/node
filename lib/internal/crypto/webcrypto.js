...
async function generateKey(algorithm, extractable, keyUsages) {
  ...
  switch (algorithm.name) {
    ...
    // Fall through
    case 'RSA-OAEP':
      return lazyRequire('internal/crypto/rsa').rsaKeyGenerate(
        algorithm,
        extractable,
        keyUsages
      );
    ...
    // Fall through
    case 'ECDH':
      return lazyRequire('internal/crypto/ec').ecGenerateKey(
        algorithm,
        extractable,
        keyUsages
      );
    case 'HMAC':
      return lazyRequire('internal/crypto/mac').hmacGenerateKey(
        algorithm,
        extractable,
        keyUsages
      );
    ...
    // Fall through
    case 'AES-KW':
      return lazyRequire('internal/crypto/aes').aesGenerateKey(
        algorithm,
        extractable,
        keyUsages
      );
    ...
    // with the `NODE-`
    case 'NODE-DSA':
      return lazyRequire('internal/crypto/dsa').dsaGenerateKey(
        algorithm,
        extractable,
        keyUsages
      );
    case 'NODE-DH':
      return lazyRequire('internal/crypto/diffiehellman').dhGenerateKey(
        algorithm,
        extractable,
        keyUsages
      );
...
async function deriveBits(algorithm, baseKey, length) {
  ...
  switch (algorithm.name) {
    case 'ECDH':
      return lazyRequire('internal/crypto/diffiehellman').asyncDeriveBitsECDH(
        algorithm,
        baseKey,
        length
      );
    case 'HKDF':
      return lazyRequire('internal/crypto/hkdf').hkdfDeriveBits(
        algorithm,
        baseKey,
        length
      );
    case 'PBKDF2':
      return lazyRequire('internal/crypto/pbkdf2').pbkdf2DeriveBits(
        algorithm,
        baseKey,
        length
      );
    case 'NODE-SCRYPT':
      return lazyRequire('internal/crypto/scrypt').scryptDeriveBits(
        algorithm,
        baseKey,
        length
      );
    case 'NODE-DH':
      return lazyRequire('internal/crypto/diffiehellman').asyncDeriveBitsDH(
        algorithm,
        baseKey,
        length
      );
...
async function deriveKey(
  ...
  switch (algorithm.name) {
    case 'ECDH':
      bits = await lazyRequire(
        'internal/crypto/diffiehellman'
      ).asyncDeriveBitsECDH(algorithm, baseKey, length);
      break;
    case 'HKDF':
      bits = await lazyRequire('internal/crypto/hkdf').hkdfDeriveBits(
        algorithm,
        baseKey,
        length
      );
      break;
    case 'PBKDF2':
      bits = await lazyRequire('internal/crypto/pbkdf2').pbkdf2DeriveBits(
        algorithm,
        baseKey,
        length
      );
      break;
    case 'NODE-SCRYPT':
      bits = await lazyRequire('internal/crypto/scrypt').scryptDeriveBits(
        algorithm,
        baseKey,
        length
      );
...
async function exportKeySpki(key) {
  switch (key.algorithm.name) {
    ...
    case 'RSA-OAEP':
      if (key.type === 'public') {
        return lazyRequire('internal/crypto/rsa').rsaExportKey(
          key,
          kWebCryptoKeyFormatSPKI
        );
      }
    ...
    case 'ECDH':
      if (key.type === 'public') {
        return lazyRequire('internal/crypto/ec').ecExportKey(
          key,
          kWebCryptoKeyFormatSPKI
        );
      }
    ...
    case 'NODE-DSA':
      if (key.type === 'public') {
        return lazyRequire('internal/crypto/dsa').dsaExportKey(
          key,
          kWebCryptoKeyFormatSPKI
        );
      }
    ...
    case 'NODE-DH':
      if (key.type === 'public') {
        return lazyRequire('internal/crypto/diffiehellman').dhExportKey(
          key,
          kWebCryptoKeyFormatSPKI
        );
      }
...
async function exportKeyJWK(key) {
  ...
  switch (key.algorithm.name) {
    ...
    // Fall through
    case 'AES-KW':
      jwk.alg = lazyRequire('internal/crypto/aes').getAlgorithmName(
        key.algorithm.name,
...
async function importKey(format, keyData, algorithm, extractable, keyUsages) {
  ...
  switch (algorithm.name) {
    ...
    // Fall through
    case 'RSA-OAEP':
      return lazyRequire('internal/crypto/rsa').rsaImportKey(
        format,
        keyData,
        algorithm,
        extractable,
    ...
    // Fall through
    case 'ECDH':
      return lazyRequire('internal/crypto/ec').ecImportKey(
        format,
        keyData,
        algorithm,
        extractable,
    ...
    // Fall through
    case 'AES-KW':
      return lazyRequire('internal/crypto/aes').aesImportKey(
        algorithm,
        format,
        keyData,
        extractable,
    ...
    case 'NODE-DSA':
      return lazyRequire('internal/crypto/dsa').dsaImportKey(
        format,
        keyData,
        algorithm,
        extractable,
    ...
    case 'NODE-DH':
      return lazyRequire('internal/crypto/diffiehellman').dhImportKey(
        format,
        keyData,
        algorithm,
        extractable,
...
function signVerify(algorithm, key, data, signature) {
  ...
  if (
    !ArrayPrototypeIncludes(key.usages, usage) ||
  ...
  switch (algorithm.name) {
    ...
    // Fall through
    case 'RSASSA-PKCS1-v1_5':
      return lazyRequire('internal/crypto/rsa').rsaSignVerify(
        key,
        data,
        algorithm,
        signature
    ...
    // Fall through
    case 'ECDSA':
      return lazyRequire('internal/crypto/ec').ecdsaSignVerify(
        key,
        data,
        algorithm,
        signature
    ...
    case 'HMAC':
      return lazyRequire('internal/crypto/mac').hmacSignVerify(
        key,
        data,
        algorithm,
        signature
    ...
    // the `NODE-` prefix
    case 'NODE-DSA':
      return lazyRequire('internal/crypto/dsa').dsaSignVerify(
        key,
        data,
        algorithm,
        signature
...
async function cipherOrWrap(mode, algorithm, key, data, op) {
  ...
  switch (algorithm.name) {
    case 'RSA-OAEP':
      return lazyRequire('internal/crypto/rsa').rsaCipher(
        mode,
        key,
        data,
        algorithm
    ...
    // Fall through
    case 'AES-GCM':
      return lazyRequire('internal/crypto/aes').aesCipher(
        mode,
        key,
        data,
        algorithm
