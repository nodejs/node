...
function convertSubtleArgument(prefix, converter, value, index) {
  // Mirrors makeOptions() in internal/webidl, including why it stays an
  // ordinary literal: the converters read every member below, and an absent
  // one would resolve through %Object.prototype%.
  return webidl.converters[converter](value, {
    prefix,
    context: kArgumentContexts[index],
    code: undefined,
    enforceRange: undefined,
    clamp: undefined,
  ...
  switch (normalizedAlgorithm.name) {
    ...
    case 'MLKEM1024-P384':
      return require('internal/crypto/kem_hybrids')
        .kemHybridGenerateKey(normalizedAlgorithm, extractable, usages);
    case 'KMAC128':
      // Fall through
    case 'KMAC256':
      return require('internal/crypto/mac')
...
function importKeySync(format, keyData, algorithm, extractable, usages) {
  ...
  switch (algorithm.name) {
    ...
    case 'MLKEM768-X25519':
      // Fall through
    case 'MLKEM1024-P384':
      result = require('internal/crypto/kem_hybrids')
        .kemHybridImportKey(format, keyData, algorithm, extractable, usages);
      break;
  }
...
function getPublicKeyImpl(key, keyUsages) {
  ...
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    case 'MLKEM1024-P384':
      return require('internal/crypto/kem_hybrids')
        .kemHybridGetPublicKey(key, usages);
  }
...
function encapsulateBitsImpl(encapsulationAlgorithm, encapsulationKey) {
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM1024-P384':
      return require('internal/crypto/kem_hybrids')
        .kemHybridEncaps(encapsulationKey);
    /* c8 ignore start */
    default: {
      const assert = require('internal/assert');
      assert.fail('Unreachable code');
...
function encapsulateKeyImpl(
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM768-X25519':
      // Fall through
    case 'MLKEM1024-P384':
      encapsulatedBits = require('internal/crypto/kem_hybrids')
        .kemHybridEncaps(encapsulationKey);
      break;
    /* c8 ignore start */
    default: {
...
function decapsulateBitsImpl(decapsulationAlgorithm, decapsulationKey, ciphertext) {
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM1024-P384':
      return require('internal/crypto/kem_hybrids')
        .kemHybridDecaps(decapsulationKey, ciphertext);
    /* c8 ignore start */
    default: {
      const assert = require('internal/assert');
      assert.fail('Unreachable code');
...
function decapsulateKeyImpl(
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM768-X25519':
      // Fall through
    case 'MLKEM1024-P384':
      decapsulatedBits = require('internal/crypto/kem_hybrids')
        .kemHybridDecaps(decapsulationKey, ciphertext);
      break;
    /* c8 ignore start */
    default: {
...
function getRandomValues(array) {
  ...
  array = convertSubtleArgument(prefix, 'ArrayBufferView', array, 0);
