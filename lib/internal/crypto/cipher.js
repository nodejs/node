...
const onNewCipherivChannel = dc.channel('crypto.cipheriv.new');
const onNewDecipherivChannel = dc.channel('crypto.decipheriv.new');
...
function Cipher(cipher, password, options) {
  ...
  if (onNewCipherChannel.hasSubscribers) {
    onNewCipherChannel.publish({
      algorithm: cipher,
      password, options
    });
  }
...
function Cipheriv(cipher, key, iv, options) {
  ...
  if (onNewCipherivChannel.hasSubscribers) {
    onNewCipherivChannel.publish({
      algorithm: cipher,
      key, iv, options
    });
  }
...
function Decipher(cipher, password, options) {
  ...
  if (onNewDecipherChannel.hasSubscribers) {
    onNewDecipherChannel.publish({
      algorithm: cipher,
      password, options
    });
  }
...
function Decipheriv(cipher, key, iv, options) {
  ...
  if (onNewDecipherivChannel.hasSubscribers) {
    onNewDecipherivChannel.publish({
      algorithm: cipher,
      key, iv, options
    });
  }
