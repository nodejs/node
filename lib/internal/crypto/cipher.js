...
function invalidArrayBufferView(name, value, ...other) {
  return new ERR_INVALID_ARG_TYPE(
    name,
    ['string', 'Buffer', 'TypedArray', 'DataView', ...other],
    value
  );
}
...
function createCipherWithIV(cipher, key, options, decipher, iv) {
  ...
  if (typeof key !== 'string' && !isArrayBufferView(key)) {
    if (isKeyObject(key)) {
      if (key.getType() !== 'secret')
        throw new ERR_CRYPTO_INVALID_KEY_OBJECT_TYPE(key.getType(), 'secret');
      key = key[kHandle];
    } else {
      throw invalidArrayBufferView('key', key, 'KeyObject');
    }
  }
