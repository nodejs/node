...
function dhGenerateKeys(encoding) {
  let keys = this._handle.generateKeys();
...
function dhComputeSecret(key, inEnc, outEnc) {
  ...
  let ret = this._handle.computeSecret(toBuf(key, inEnc));
...
function dhGetGenerator(encoding) {
  let generator = this._handle.getGenerator();
  encoding = encoding || getDefaultEncoding();
  if (encoding && encoding !== 'buffer')
...
function dhGetPublicKey(encoding) {
  let key = this._handle.getPublicKey();
  encoding = encoding || getDefaultEncoding();
  if (encoding && encoding !== 'buffer')
...
function dhGetPrivateKey(encoding) {
  let key = this._handle.getPrivateKey();
  encoding = encoding || getDefaultEncoding();
  if (encoding && encoding !== 'buffer')
    key = key.toString(encoding);
  return key;
...
ECDH.prototype.getPublicKey = function getPublicKey(encoding, format) {
  ...
  let key = this._handle.getPublicKey(f);
  encoding = encoding || getDefaultEncoding();
  if (encoding && encoding !== 'buffer')
    key = key.toString(encoding);
  return key;
