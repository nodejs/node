...
const { generateKeyPair } = require('internal/crypto/keygen');
...
async function dhGenerateKey(algorithm, extractable, keyUsages) {
  ...
  const { name, primeLength, generator, group } = algorithm;
  let { prime } = algorithm;

  if (prime !== undefined) prime = getArrayBufferOrView(prime);
...
async function asyncDeriveBitsECDH(algorithm, baseKey, length) {
  ...
  if (length !== null) validateUint32(length, 'length');
  if (!isCryptoKey(key))
...
function dhExportKey(key, format) {
  return jobPromise(
    new DHKeyExportJob(kCryptoJobAsync, format, key[kKeyObject][kHandle])
  );
}
...
async function dhImportKey(format, keyData, algorithm, extractable, keyUsages) {
  ...
  const { prime, primeLength, generator, group } = keyObject[kHandle].keyDetail(
    {}
  );
