...
function lazyRequire(name) {
  ...
  if (ret === undefined) ret = lazyRequireCache[name] = require(name);
  return ret;
}
...
const kExportFormats = ['raw', 'pkcs8', 'spki', 'jwk', 'node.keyObject'];
...
const validateByteSource = hideStackFrames((val, name) => {
  ...
  if (isAnyArrayBuffer(val) || isArrayBufferView(val)) return;
