...
const {
    ...
    export(options) {
      if (options !== undefined) {
        ...
        if (options.format === 'jwk') {
          return this[kHandle].exportJwk({ __proto__: null }, false);
        }
      }
  ...
  function normalizeKeyDetails(details = { __proto__: null }) {
        ...
        case 'ec':
          return this[kAsymmetricKeyDetails] ||
             (this[kAsymmetricKeyDetails] = normalizeKeyDetails(
               this[kHandle].keyDetail({ __proto__: null }),
             ));
        default:
...
function parseKeyFormatAndType(enc, keyType, isPublic, objName) {
  ...
  const type = parseKeyType(typeStr,
                            ...
                            isPublic,
                            option('type', objName));
  return { __proto__: null, format, type };
}
...
function parseKeyEncoding(enc, keyType, isPublic, objName) {
  ...
  return { __proto__: null, format, type, cipher, passphrase };
}
...
function prepareAsymmetricKey(key, ctx) {
  if (isKeyObject(key)) {
    ...
    return { __proto__: null, data: getKeyObjectHandle(key, ctx) };
  } else if (isCryptoKey(key)) {
    return { __proto__: null, data: getKeyObjectHandle(key[kKeyObject], ctx) };
  } else if (isStringOrBuffer(key)) {
    // Expect PEM by default, mostly for backward compatibility.
    return { __proto__: null, format: kKeyFormatPEM, data: getArrayBufferOrView(key, 'key') };
    ...
    if (isKeyObject(data))
      return { __proto__: null, data: getKeyObjectHandle(data, ctx) };
    else if (isCryptoKey(data))
      return { __proto__: null, data: getKeyObjectHandle(data[kKeyObject], ctx) };
    else if (format === 'jwk') {
      validateObject(data, 'key.key');
      return { __proto__: null, data: getKeyObjectHandleFromJwk(data, ctx), format: 'jwk' };
    }
