...
class PublicKeyObject extends AsymmetricKeyObject {
  ...
  export(encoding) {
    const {
      format,
      type
    } = parsePublicKeyEncoding(encoding, this.getAsymmetricKeyType());
...
class PrivateKeyObject extends AsymmetricKeyObject {
  ...
  export(encoding) {
    const {
      ...
      cipher,
      passphrase
    } = parsePrivateKeyEncoding(encoding, this.getAsymmetricKeyType());
...
function getKeyObjectHandle(key, isPublic, allowKeyObject) {
  ...
  if (isPublic != null) {
    ...
    if (key.getType() !== expectedType)
      throw new ERR_CRYPTO_INVALID_KEY_OBJECT_TYPE(key.getType(), expectedType);
  }
  return key[kHandle];
}
