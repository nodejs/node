...
function getKeyObjectHandleFromJwk(key, ctx) {
  ...
  if (key.kty === 'AKP') {
    ...
    if (isPublic) {
      const keyData = Buffer.from(key.pub, 'base64url');
      if (keyData.byteLength !== mlDsaPubLen(key.alg)) {
        throw new ERR_CRYPTO_INVALID_JWK();
      }
      const handle = new KeyObjectHandle();

      if (!handle.initMlDsaPublicRaw(key.alg, keyData)) {
        throw new ERR_CRYPTO_INVALID_JWK();
      }
    ...
    const keyData = Buffer.from(key.priv, 'base64url');
    if (keyData.byteLength !== 32) {
      throw new ERR_CRYPTO_INVALID_JWK();
    }

    const bytes = [48, 52, 2, 1, 0, 48, 11, 6, 9, ...oids[key.alg], 4, 34, 128, 32, ...keyData];

    const handle = new KeyObjectHandle();
    try {
      handle.init(kKeyTypePrivate, new Uint8Array(bytes), kKeyFormatDER, kKeyEncodingPKCS8, null);
    } catch {
      throw new ERR_CRYPTO_INVALID_JWK();
    }
    return handle;
