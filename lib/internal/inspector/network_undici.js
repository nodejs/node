 ...
 * @param {{ request: undici.Request, fetchRequest?: object }} event
 */
function onClientRequestStart({ request, fetchRequest }) {
  const url = `${request.origin}${request.path}`;
  request[kInspectorRequestId] = getNextRequestId();
  if (fetchRequest != null) {
    fetchRequest[kInspectorRequestId] = request[kInspectorRequestId];
  }
...
/**
 * When an SSE message is parsed, emit Network.eventSourceMessageReceived event.
 * @param {{ request: object, eventName: string, eventId: string, data: string }} event
 */
function onEventSourceMessage({ request, eventName, eventId, data }) {
...
function onClientResponseHeaders({ request, response }) {
  ...
  const type = mimeType === 'text/event-stream' ?
    kResourceType.EventSource :
    kResourceType.Fetch;
...
module.exports = registerDiagnosticChannels([
  ...
  ['undici:fetch:eventsource:message', onEventSourceMessage],
