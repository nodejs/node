...
class Harness {
  ...
  #diagnostic(error, loc, level = 'info', node = undefined) {
    this.success = false;
  ...
  async #executeSuite(suite) {
    if (suite.buildError !== null) {
      this.#diagnostic(suite.buildError, suite.loc, 'error', suite);
  ...
  async #executeBench(benchmark, forcedError = undefined) {
    ...
    const deadline = benchmark.timeout === Infinity ?
      null : hrtime() + BigInt(MathCeil(benchmark.timeout * 1e6));
    const checkDeadline = () => {
      if (deadline !== null && hrtime() >= deadline) {
