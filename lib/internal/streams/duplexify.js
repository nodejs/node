  ...
  if (isDuplexNodeStream(body)) {
    ...
    const readable = body?.readable ?
      isReadableNodeStream(body?.readable) ? body?.readable :
        duplexify(body.readable, body.options ?? body.readableOptions) :
      undefined;

    const writable = body?.writable ?
      isWritableNodeStream(body?.writable) ? body?.writable :
        duplexify(body.writable, body.options ?? body.writableOptions) :
      undefined;
...
function _duplexify(pair) {
  const r = typeof pair.readable.read !== 'function' ?
