...
async function pumpToNode(iterable, writable, finish, { end, ac }) {
  let error;
  let onresolve = null;
...
function pipelineImpl(streams, callback, opts) {
  ...
  function registerStream(stream, { end, writing, reading, isLastStream }) {
    if (end) {
  ...
  for (let i = 0; i < streams.length; i++) {
    ...
    if (isNodeStream(stream)) {
      registerStream(stream, { end, writing, reading, isLastStream });
    }
    ...
    if (i === 0) {
      ...
      if (reading) {
        ...
        registerStream(pt, { end, writing, reading, isLastStream });
        ...
        if (typeof then === 'function') {
          ...
          finishCount++;
          pumpToNode(ret, pt, finish, { end, ac });
        } else if (isReadableStream(ret) || isTransformStream(ret)) {
          const toRead = ret.readable || ret;
          finishCount++;
          pumpToNode(toRead, pt, finish, { end, ac });
        } else {
          throw new ERR_INVALID_RETURN_VALUE(
      ...
      if (isReadableNodeStream(ret)) {
        ...
        const toRead = ret.readable || ret;
        finishCount++;
        pumpToNode(toRead, stream, finish, { end, ac });
      } else if (isIterable(ret)) {
        finishCount++;
        pumpToNode(ret, stream, finish, { end, ac });
      } else {
        throw new ERR_INVALID_ARG_TYPE(
