...
function createMappedStream(source, fn, options, concurrency, highWaterMark) {
  ...
  const signal = AbortSignal.any([ac.signal, options?.signal].filter(Boolean));
  ...
  function enqueueMappedChunk(chunk) {
    ...
    const mappedPromise = getThenablePromise(mapped);
    if (mappedPromise !== undefined) {
      PromisePrototypeThen(mappedPromise, mapperFinished, mapperRejected);
      queue.push(mappedPromise);
    } else {
  ...
  async function pumpSource() {
    ...
    signal.addEventListener('abort', sourceReadable.notify, { once: true });
    ...
    try {
      ...
      signal.removeEventListener('abort', sourceReadable.notify);
    }
  }
  ...
  // This is lazy to avoid a circular dependency during stream initialization.
