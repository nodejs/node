...
function map(fn, options) {
  ...
  return async function* map() {
    ...
    async function pump() {
      try {
        const streamIterator = stream[SymbolAsyncIterator]();
        let streamResult = await streamIterator.next();
        while (!streamResult.done) {
          let val = streamResult.value;
          if (done) {
            return;
          }
...
async function some(fn, options = undefined) {
  const filterIterator = filter.call(this, fn, options)[SymbolAsyncIterator]();
  const filterResult = await filterIterator.next();
  while (!filterResult.done) {
    return true;
  }
  return false;
}
...
async function forEach(fn, options) {
  ...
  const mapIterator = map.call(this, forEachFn, options)[SymbolAsyncIterator]();
  let mapResult = await mapIterator.next();
  while (!mapResult.done) {
    mapResult = await mapIterator.next();
  }
}
...
async function reduce(reducer, initialValue, options) {
  ...
  try {
    const iterator = this[SymbolAsyncIterator]();
...
function flatMap(fn, options) {
  ...
  return async function* flatMap() {
    const valuesIterator = values[SymbolAsyncIterator]();
    let valuesResult = await valuesIterator.next();
    while (!valuesResult.done) {
      const val = valuesResult.value;
      yield* val;
      valuesResult = await valuesIterator.next();
    }
  }.call(this);
}
