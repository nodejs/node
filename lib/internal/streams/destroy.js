...
function _destroy(self, err, cb) {
  ...
  function onDestroy(err) {
    ...
    if (err) {
      process._deferTick(emitErrorCloseNT, self, err);
    } else {
      process._deferTick(emitCloseNT, self);
    }
  }
  try {
...
function errorOrDestroy(stream, err, sync) {
  ...
  if (
    ...
    if (sync) {
      process._deferTick(emitErrorNT, stream, err);
    } else {
...
function construct(stream, cb) {
  ...
  process._deferTick(constructNT, stream);
}
...
function constructNT(stream) {
  ...
  function onConstruct(err) {
    ...
    if (s.destroyed) {
      ...
      process._deferTick(emitConstructNT, stream);
    }
  }

  try {
    stream._construct((err) => {
      process._deferTick(onConstruct, err);
    });
  } catch (err) {
    process._deferTick(onConstruct, err);
  }
...
function emitErrorCloseLegacy(stream, err) {
  ...
  process._deferTick(emitCloseLegacy, stream);
}
...
function destroyer(stream, err) {
  ...
  if (isServerRequest(stream)) {
    ...
    process._deferTick(emitErrorCloseLegacy, stream, err);
  } else {
    process._deferTick(emitCloseLegacy, stream);
  }
