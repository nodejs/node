...
function eos(stream, options, callback) {
  ...
  const onrequest = () => {
    disposableStack.use(stream.req.use('finish', onfinish));
  };

  if (isRequest(stream)) {
    disposableStack.use(stream.use('complete', onfinish));
    if (!willEmitClose) {
      disposableStack.use(stream.use('abort', onclose));
    }
    if (stream.req) {
      onrequest();
    } else {
      disposableStack.use(stream.use('request', onrequest));
    }
  } else if (writable && !wState) { // legacy streams
    disposableStack.use(stream.use('end', onlegacyfinish));
    disposableStack.use(stream.use('close', onlegacyfinish));
  }

  // Not all streams will emit 'close' after 'aborted'.
  if (!willEmitClose && typeof stream.aborted === 'boolean') {
    disposableStack.use(stream.use('aborted', onclose));
  }

  disposableStack.use(stream.use('end', onend));
  disposableStack.use(stream.use('finish', onfinish));
  if (options.error !== false) {
    disposableStack.use(stream.use('error', onerror));
  }
  disposableStack.use(stream.use('close', onclose));
