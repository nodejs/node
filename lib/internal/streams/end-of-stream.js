...
function eos(stream, options, callback) {
  ...
  if (isRequest(stream)) {
    ...
    stream.onEnd(onlegacyfinish);
    stream.onClose(onlegacyfinish);
  }
  ...
  const cleanup = () => {
    ...
    if (stream.req) stream.req.offFinish(onfinish);
    stream.offEnd(onlegacyfinish);
    stream.offClose(onlegacyfinish);
    stream.offFinish(onfinish);
    stream.offEnd(onend);
    stream.removeListener('error', onerror);
    stream.offClose(onclose);
  };
