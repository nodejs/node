...
function eos(stream, options, callback) {
  ...
  if (options.signal && !closed) {
    const abort = () => {
      ...
      endCallback.call(
        stream,
        new AbortError(undefined, { __proto__: null, cause: options.signal.reason }));
    };
...
function eosWeb(stream, options, callback) {
  ...
  if (options.signal) {
    abort = () => {
      isAborted = true;
      callback.call(stream, new AbortError(undefined, { __proto__: null, cause: options.signal.reason }));
    };
    if (options.signal.aborted) {
      process.nextTick(abort);
    } else {
