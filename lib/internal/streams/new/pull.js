 ...
 * @yields {Uint8Array[]}
 */
async function* applyStatelessAsyncTransform(source, transform) {
 ...
 * @yields {Uint8Array[]}
 */
async function* applyStatefulAsyncTransform(source, transform) {
  const output = transform(source);
...
async function* createAsyncPipeline(source, transforms, signal) {
  ...
  // Track stateful transforms for abort handling
  const statefulTransforms = [];
  ...
  try {
    ...
    for (let i = 0; i < transforms.length; i++) {
      const transform = transforms[i];
      if (isTransformObject(transform)) {
        ArrayPrototypePush(statefulTransforms, transform);
    ...
    for (let i = 0; i < statefulTransforms.length; i++) {
      const transformObj = statefulTransforms[i];
      if (transformObj.abort) {
        try {
          await transformObj.abort(
...
function pipeToSync(source, ...args) {
  ...
  try {
    ...
    if (!options?.preventAbort) {
      writer.abort(error instanceof Error ? error : new ERR_OPERATION_FAILED(String(error)));
    }
    throw error;
  }
