...
function collectHTTPQuotedString(str, marker) {
  ...
  while (true) {
    ...
    const collectedPoints = nextDelimiter === -1 ? StringPrototypeSlice(str, index) : StringPrototypeSlice(str, index, nextDelimiter);
    ...
    let quoteOrBackslash = str[index];
    let quoteOrBackslashAsCode = quoteOrBackslash == null ? 0 : StringPrototypeCharCodeAt(quoteOrBackslash, 0);
  ...
  static parseParametersString(str, position, params) {
    ...
    while (marker.position < endOfSource) {
      ...
      if (nextChar === QUOTE_CHARCODE) {
        ...
        let remaining = StringPrototypeIndexOf(str, SEMICOLON, marker.position);
