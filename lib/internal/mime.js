...
function stringifyMIMETypeParams(data) {
  let result = '';
  for (const { 0: key, 1: value } of data) {
...
function moveUntil(str, predicate, marker, { toASCII, removeBackslashes } = { toASCII: false, removeBackslashes: false }) {
...
class MIMEParams {
  ...
  *values() {
  ...
  toString() {
    return stringifyMIMETypeParams(this.#data);
    // let ret = '';
    ...
    // console.log('called mimeparams', ret)
    // console.log('called mimeparams from', new Error())
    // return ret;
  }
...
class MIMEType {
  ...
  toString() {
    ...
    const paramStr = stringifyMIMETypeParams(this.#parameters.entries());
