...
function parseTypeAndSubtype(str) {
  ...
  if (typeEnd === -1 || mimetype.length === 0 || SafeStringPrototypeSearch(
    mimetype, NOT_HTTP_TOKEN_CODE_POINT) !== -1) {
  ...
  if (trimmedSubtype.length === 0 ||
    SafeStringPrototypeSearch(trimmedSubtype,
  ...
  *values() {
  ...
  static parseParametersString(str, position, params) {
    ...
    while (marker.position < endOfSource) {
      ...
      const endOfWhitespace = SafeStringPrototypeSearch(str, WHITESPACE_REGEX);

      marker.position = endOfWhitespace > 0 ? endOfWhitespace + 1 : marker.position;
