...
function collectHTTPQuotedString(str, marker) {
  ...
  while (true) {
    ...
    const collectedPoints = nextDelimiter === -1 ? StringPrototypeSlice(str, index) :
      StringPrototypeSlice(str, index, nextDelimiter);
    ...
    const quoteOrBackslash = StringPrototypeCharCodeAt(str, index);

    // #5-4 Advance position by 1.
    index++;

    // #5-5 If quoteOrBackslash is U+005C (\), then:
    if (quoteOrBackslash === BACKSLASH_CHARCODE) {
    ...
    // #5-6-2 Break
    if (quoteOrBackslash === QUOTE_CHARCODE) break;
  }
  ...
  *values() {
  ...
  static parseParametersString(str, position, params) {
    ...
    while (marker.position < endOfSource) {
      ...
      marker.position += endOfWhitespace + 1;
      ...
      if (nextChar === QUOTE_CHARCODE) {
        ...
        // #11-9-1 Set parameterValue to the result of collecting a sequence of code points
        // that are not U+003B (;) from input, given position.
