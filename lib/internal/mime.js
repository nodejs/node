...
// Index at which the run of trailing HTTP whitespace begins, or str.length
// when there is none. Equivalent to the result of searching for
// /[\r\n\t ]*$/, but a backward scan avoids the quadratic backtracking that an
// unanchored search performs on a long internal whitespace run.
function startOfTrailingWhitespace(str) {
  let i = str.length;
  while (i > 0) {
    const c = StringPrototypeCharCodeAt(str, i - 1);
...
function parseTypeAndSubtype(str) {
  ...
  const trimmedSubtype = StringPrototypeSlice(
    rawSubtype,
    0,
    startOfTrailingWhitespace(rawSubtype));
...
class MIMEParams {
  ...
  *values() {
  ...
  #parse() {
    ...
    const endOfSource = startOfTrailingWhitespace(
      StringPrototypeSlice(str, position),
    ) + position;
    while (position < endOfSource) {
      ...
      if (char === '"') {
        ...
        const trimmedValue = StringPrototypeSlice(
          rawValue,
          0,
          startOfTrailingWhitespace(rawValue),
        );
        // Ignore parameters without values
