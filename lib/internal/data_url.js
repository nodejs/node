...
function dataURLProcessor(dataURL) {
  ...
  if (StringPrototypeStartsWith(mimeType, ';')) {
...
function URLSerializer(url, excludeFragment = false) {
  ...
  const serialized = hashLength === 0 ? href : StringPrototypeSubstring(href, 0, href.length - hashLength);

  if (!hashLength && StringPrototypeEndsWith(href, '#')) {
...
function parseMIMEType(input) {
  ...
  if (type.length === 0 || RegExpPrototypeExec(HTTP_TOKEN_CODEPOINTS, type) === null) {
    return 'failure';
  }
  ...
  // contain HTTP token code points, then return failure.
  if (subtype.length === 0 || RegExpPrototypeExec(HTTP_TOKEN_CODEPOINTS, subtype) === null) {
    return 'failure';
  }

  const typeLowercase = StringPrototypeToLowerCase(type);
  const subtypeLowercase = StringPrototypeToLowerCase(subtype);
  ...
  while (position.position < input.length) {
    ...
    collectASequenceOfCodePoints(
      ...
      (char) => RegExpPrototypeExec(HTTP_WHITESPACE_REGEX, char) !== null,
      input,
      position,
    );
    ...
    if (
      ...
      RegExpPrototypeExec(HTTP_TOKEN_CODEPOINTS, parameterName) !== null &&
      (parameterValue.length === 0 || RegExpPrototypeExec(HTTP_QUOTED_STRING_TOKENS, parameterValue) !== null) &&
...
function forgivingBase64(data) {
  ...
  if (dataLength % 4 === 0) {
    ...
    if (StringPrototypeCharCodeAt(data, dataLength - 1) === 0x003D) {
      --dataLength;
      if (StringPrototypeCharCodeAt(data, dataLength - 1) === 0x003D) {
        --dataLength;
      }
  ...
  //  ASCII alphanumeric
  // then return failure.
  if (RegExpPrototypeExec(/[^+/0-9A-Za-z]/, data.length === dataLength ? data : StringPrototypeSubstring(data, 0, dataLength)) !== null) {
    return 'failure';
  }
...
function collectAnHTTPQuotedString(input, position, extractValue) {
  ...
  return StringPrototypeSlice(input, positionStart, position.position);
}
