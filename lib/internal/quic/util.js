...
function GetMaxHeaderLength() {
  getOptionValue('--max-http-header-size') | 0;
}
...
function validateTransportParams(params) {
  const {
    ...
    h3: {
      ...
      maxPushes,
      maxHeaderPairs,
      maxHeaderLength = GetMaxHeaderLength(),
    },
  } = { h3: {}, ...params };
...
function validateQuicSocketOptions(options = {}) {
  ...
  validateQuicEndpointOptions(endpoint);

  if (endpoint == null || typeof endpoint !== 'object')
    throw new ERR_INVALID_ARG_TYPE('options.endpoint', 'Object', endpoint);

  if (client == null || typeof client !== 'object')
    throw new ERR_INVALID_ARG_TYPE('options.client', 'Object', client);

  if (server == null || typeof server !== 'object')
    throw new ERR_INVALID_ARG_TYPE('options.server', 'Object', server);
