...
const { kInspect } = require('internal/quic/symbols');
...
function checkAttachable(session, state) {
  if (session.destroyed ||
    ...
    getQuicSessionHandle(session) === undefined) {
...
class Http3Session {
  ...
  constructor(session, options = kEmptyObject) {
    ...
    if (ongoaway !== undefined) {
      ...
      session.ongoaway = this.#bind(ongoaway);
    }
    if (onorigin !== undefined) {
      this.#onorigin = onorigin;
      session.onorigin = this.#bind(onorigin);
    }
    if (onsettings !== undefined) {
      this.#onsettings = onsettings;
      session.onapplication = this.#bind(onsettings);
    }
  }
