 ...
 * @property {OnWTSessionCloseCallback} [onwtsessionclose] Default perstream initial callback
 *   for incoming close capsules
    ...
    if (webtransportSession) {
      if (webtransport && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.webtransport',
                                       'webtransport can not be set for creating a webtransport stream' +
                                       'associated with a webtransport session');
      }

      if (headers && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.headers',
                                       'headers can not be set for creating a webtransport stream' +
                                       ' associated with a webtransport session');
      }
