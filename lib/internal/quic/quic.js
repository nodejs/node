...
let isQuicStream;
let isQuicSession;
let getQuicSessionHandle;
let createApplicationStream;
let isQuicEndpoint;
...
class QuicSession {
  ...
  static {
    ...
    getQuicSessionHandle = (session) => {
      assertIsQuicSession(session);
      return session.#handle;
    };

    createApplicationStream = (session, direction, options) => {
      assertIsQuicSession(session);
      return session.#createStream(direction, options);
    };
  ...
  // If a non-default application type is used, this session does not own the
