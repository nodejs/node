 ...
 * @property {boolean} [webtransport] Indicates that the headers should send signal
 ...
 * @property {'error'|'ignore'} [truncatedReads] Truncated read policy
 ...
 * @property {OnWTSessionCloseCallback} [onwtsessionclose] Default perstream
 *   initial callback for incoming close capsules
...
function updateWebtransportInterest(handle, inner) {
  if (handle === undefined) return;
  setSessionIdInterest(
    handle,
...
class QuicStream {
   ...
   * @param {'error'|'ignore'} truncatedReads
   */
  ...
  async *[SymbolAsyncIterator]() {
    ...
    // No reader means either an outbound-only unidirectional stream, or a
  ...
  set onsessionid(fn) {
    ...
    updateWebtransportInterest(this.#handle, inner);
  }
  ...
  closeWebtransportSessionStream(code, msg) {
    ...
    if (code !== undefined) {
      validateInteger(code);
    }
    inner.destroying = true;
    const handle = this.#handle;
    this[kCloseWebtransportSessionStream](code, msg);
  ...
  destroy(error, options = kEmptyObject) {
    ...
    this.#handle.destroy();
    this[kFinishClose](error); // A no-op, unless destroy() failed
  }
  ...
  [kMakeWebtransportStream](session) {
    ...
    return makeWebtransportStream(this.#handle, session.#handle);
  }
  ...
  [kCloseWebtransportSessionStream](code, msg) {
    ...
    return closeWebtransportSessionStream(this.#handle, code, msg);
  }
  ...
  [kFinishClose](error) {
    ...
    if (error?.errorCode === 0x170D7B68n &&  inner.wtSessionCloseError) {
        error = inner.wtSessionCloseError;
        inner.wtSessionCloseError = undefined;
    }
    if (error !== undefined) {
  ...
  [kWTSessionClose](errorcode, errormessage) {
    ...
    inner.wtSessionCloseError = makeQuicError(
          'ERR_QUIC_APPLICATION_ERROR',
          'Webtransport error',
          'application',
          errorcode ?? 0,
...
class QuicSession {
   ...
   * @param {{ truncatedReads?: 'error'|'ignore' }} [options]
   */
  ...
  async #createStream(direction, options = kEmptyObject) {
    ...
    if (webtransportSession) {
      if (webtransport && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.webtransport',
                                       'webtransport can not be set for ' +
                                       'creating a webtransport stream associated with a webtransport session');
      }

      if (headers && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.headers',
                                       'headers can not be set for ' +
                                       'creating a webtransport stream associated with a webtransport session');
      }
      if (!isQuicStream(webtransportSession)) {
        throw new ERR_INVALID_ARG_TYPE('options.webtransportSession',
                                       'webtransportSession was to be of type QuicStream');
      }
    }
