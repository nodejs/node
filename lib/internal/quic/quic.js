 ...
 * @property {number} [sendOrder] A signed integer representing the send order,
 *   aligned with the W3C WebTransport `sendOrder` concept. Any integer is
 *   accepted; values are clamped to the QUIC urgency range [0, 7]. Note that
 *   QUIC urgency semantics are inverted relative to W3C: a lower urgency value
 *   means *higher* priority. When specified, overrides `priority`.
 *   Full W3C `sendOrder` semantics (including `sendGroups`) require upstream
 *   ngtcp2 support and are not yet available.
 ...
 * @property {number} sendOrder The QUIC urgency value in [0, 7] used by
 *   this stream. Lower values mean higher priority. This corresponds to the
 *   clamped input from W3C WebTransport `sendOrder`.
 * @property {boolean} incremental Whether to interleave data with same-priority streams.
 */
...
class QuicStream {
  ...
  get priority() {
    ...
    // `sendOrder` exposes the raw QUIC urgency value (0-7), aligned with
    // the W3C WebTransport sendOrder concept (lower value = higher priority).
    return { level, sendOrder: urgency, incremental };
  }
  ...
  setPriority(options = kEmptyObject) {
    ...
    if (sendOrder !== undefined) {
      // sendOrder is aligned with the W3C WebTransport sendOrder concept.
      // W3C accepts any long long; we validate it is a finite integer and
      // clamp to the QUIC urgency range [0, 7]. Note: QUIC urgency has
      // inverted priority direction vs W3C sendOrder. Full W3C semantics
      // (sendGroups, full range) require upstream ngtcp2 support.
      validateInteger(sendOrder, 'options.sendOrder');
      urgency = MathMax(0, MathMin(7, sendOrder));
    } else {
      validateOneOf(level, 'options.level', ['default', 'low', 'high']);
      urgency = level === 'high' ? 0 : level === 'low' ? 7 : 3;
    }
  ...
  async #createStream(direction, options = kEmptyObject) {
    ...
    if (inner.state.isPrioritySupported) {
      ...
      if (sendOrder !== undefined) {
        ...
        urgency = priority === 'high' ? 0 : priority === 'low' ? 7 : 3;
      }
