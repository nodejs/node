...
setCallbacks({
  ...
  onEndpointClose(context, status) {
    debug('endpoint close callback', this[kOwner], status);
  ...
  onSessionClose(errorType, code, reason) {
    debug('session close callback', this[kOwner], errorType, code, reason);
  ...
  onStreamHeaders(headers, kind) {
    ...
    debug('stream headers callback', this[kOwner], headers, kind);
...
class QuicStream {
  ...
  constructor(privateSymbol, config, handle, session, direction) {
    ...
    debug(`stream ${this.id} created [${this.direction}]`);
  }
  ...
  sendHeaders(headers) {
    ...
    debug('stream sending headers', headers);
...
class QuicSession {
  ...
  [kFinishClose](errorType, code, reason) {
    ...
    debug('finishing closig the session with an error', errorType, code, reason);
...
class QuicEndpoint {
  ...
  close() {
    if (!this.#isClosedOrClosing) {
      ...
      debug('endpoint closing gracefully');
