...
class QuicSessionState {
  ...
  get headersSupported() {
    const handle = this.#handle;
    if (handle === undefined || !this.hasApplication) return undefined;
  ...
  /**
   * The application the session is running, as an `Application::Type` value,
   * or 0 for none. The `QUIC_APPLICATION_PENDING` flag will be set if the
   * application attach has been requested but not yet completed.
  ...
  set applicationType(val) {
    const handle = this.#handle;
    if (handle === undefined) return;
    DataViewPrototypeSetUint8(handle, this.#offset + IDX_STATE_SESSION_APPLICATION_TYPE, val);
  }
  ...
  get hasApplication() {
    ...
    const type = DataViewPrototypeGetUint8(
      handle, this.#offset + IDX_STATE_SESSION_APPLICATION_TYPE);
    return type !== 0 && (type & QUIC_APPLICATION_PENDING) === 0;
  }
  ...
  get isServer() {
    ...
    if (handle === undefined) return undefined;
    return DataViewPrototypeGetUint8(
      handle, this.#offset + IDX_STATE_SESSION_IS_SERVER) !== 0;
  }
...
class QuicStreamState {
  ...
  [kInspect](depth, options) {
    if (this.#disconnected || this.#handle === undefined ||
  ...
  [kFinishClose]() {
    // Copy the final values out of the shared buffer (zeroed when the
    // underlying stream goes away) so getters keep reporting the final
    // state, the same way QuicStreamStats does.
    const handle = this.#handle;
    if (handle === undefined ||
        DataViewPrototypeGetByteLength(handle) <
          this.#offset + IDX_STATE_STREAM_SIZE) {
      this.#handle = undefined;
      this.#disconnected = true;
      return;
    }
    const copy = TypedArrayPrototypeSlice(new Uint8Array(
      DataViewPrototypeGetBuffer(handle),
      DataViewPrototypeGetByteOffset(handle) + this.#offset,
      IDX_STATE_STREAM_SIZE));
    this.#handle = new DataView(TypedArrayPrototypeGetBuffer(copy));
    this.#offset = 0;
    this.#disconnected = true;
  }
}
