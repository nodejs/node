...
assert(IDX_STATE_STREAM_WANTS_SESSIONID !== undefined);
...
class QuicStreamState {
  ...
  get wantsSessionId() {
    const handle = this.#handle;
    if (handle === undefined) return undefined;
    return DataViewPrototypeGetUint8(handle, this.#offset + IDX_STATE_STREAM_WANTS_SESSIONID) !== 0;
  }

  /** @type {boolean} */
  set wantsSessionId(val) {
    const handle = this.#handle;
    if (handle === undefined) return;
