...
const {
  ...
  IDX_STATE_SESSION_DATAGRAM_STATUS,
...
assert(IDX_STATE_SESSION_DATAGRAM_STATUS !== undefined);
...
class QuicSessionState {
  ...
  get hasDatagramStatusListener() {
    if (this.#handle.byteLength === 0) return undefined;
    return !!DataViewPrototypeGetUint8(this.#handle, IDX_STATE_SESSION_DATAGRAM_STATUS);
  }

  /** @type {boolean} */
  set hasDatagramStatusListener(val) {
    if (this.#handle.byteLength === 0) return;
    DataViewPrototypeSetUint8(this.#handle, IDX_STATE_SESSION_DATAGRAM_STATUS, val ? 1 : 0);
  }
  ...
  get maxDatagramSize() {
    if (this.#handle.byteLength === 0) return undefined;
    return DataViewPrototypeGetBigUint64(this.#handle, IDX_STATE_SESSION_MAX_DATAGRAM_SIZE);
  }
