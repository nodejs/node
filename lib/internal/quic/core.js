...
class QuicEndpoint {
  ...
  constructor(socket, options) {
    ...
    if (typeof options[kUDPHandleForTesting] === 'object') {
      FunctionPrototypeBind(state.udpSocket, options[kUDPHandleForTesting]);
  ...
  async [kBind](options) {
    ...
    try {
      ...
      const ret = FunctionPrototypeBind(udpHandle, ip, state.port, flags);
      if (ret)
...
class QuicSocket extends EventEmitter {
  ...
  async [kBind](options) {
    ...
    for (const endpoint of state.endpoints)
      binds.push(FunctionPrototypeBind(endpoint, { signal }));
