...
// `hasLoaderOverrides` is a single-load fast-path gate so wrappers can
// skip the Map lookup entirely when no VFS is mounted - the common case.
...
const loaderReadFile = wrapLoaderMethod(nativeLoaderMethods.readFile);
const toRealPath = wrapLoaderMethod(nativeLoaderMethods.realpath);
...
function setLoaderOverride(key, override) {
  ...
  if (override) loaderOverrides.set(originalFn, override);
  else loaderOverrides.delete(originalFn);
...
function setLoaderFsOverrides(overrides = kEmptyObject) {
  setLoaderOverride('stat', overrides.stat);
  setLoaderOverride('readFile', overrides.readFile);
  setLoaderOverride('realpath', overrides.realpath);
...
// Candidate suffixes tried by legacy CommonJS `main` resolution. The
// indexes returned by the C++ binding (and the VFS override in
