...
let getLayerForPathOverride = null;
...
function setLoaderFsOverrides(overrides = kEmptyObject) {
  ...
  getLayerForPathOverride = overrides.getLayerForPath ?? null;
}
...
function loaderGetLayerForPath(filename) {
  if (getLayerForPathOverride !== null) {
    return getLayerForPathOverride(filename);
  }
  return undefined;
}
...
/**
 * Clear the realpath cache used by toRealPath's fallback path.
 * Called when VFS instances are unmounted so that paths that overlap a
 * removed mount are re-resolved against the real filesystem next time.
 */
