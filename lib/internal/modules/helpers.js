...
function initializeCjsConditions() {
  ...
  cjsConditions.add('require');
  if (getOptionValue('--experimental-require-module')) {
    // If require() exists, it will be taken. Otherwise if --experimental-require-module
    // is enabled, use "import".
    // TODO(joyeecheung): test this.
    cjsConditions.add('import');
  }
  cjsConditions.add('node');
  if (!getOptionValue('--no-addons')) {
    cjsConditions.add('node-addons');
  }
  const userConditions = getOptionValue('--conditions');
  if (userConditions.length > 0) {
    for (let i = 0; i < userConditions.length; ++i) {
      cjsConditions.add(userConditions[i]);
    }
  }
}
