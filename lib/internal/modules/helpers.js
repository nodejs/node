 ...
 * Entries for unrelated paths are untouched. VFS mount points live in
 * a reserved namespace that cannot collide with real paths, so a
 * prefix scan on the keys is exact. Called by internal/vfs/setup.js.
...
function purgeRealpathCacheForPrefix(mountPoint) {
  ...
  MapPrototypeForEach(realpathCache, (realPath, requestPath) => {
    if (requestPath === mountPoint ||
        StringPrototypeStartsWith(requestPath, prefix)) {
      realpathCache.delete(requestPath);
    }
  });
}
