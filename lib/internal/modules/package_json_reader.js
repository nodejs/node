 ...
 * @param {URL['href'] | string | URL} parentLocation The location of the current module (ex file://tmp/foo.js).
 */
function findPackageJSON(specifier, parentLocation, getPackageScope = true) {
  try {
    specifier = `${specifier}`;
  } catch {
  ...
  if (typeof parentLocation === 'symbol') {
    validateString(parentLocation, 'parentLocation');
  }
  const parentURL = URL.parse(parentLocation) || pathToFileURL(parentLocation);
