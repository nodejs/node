 ...
 * Real-fs entries and other-VFS entries are untouched. VFS mount
 * points live in a reserved namespace that cannot collide with real
 * paths, so a prefix scan is exact. Called by internal/vfs/setup.js.
...
function purgePackageJSONCacheForPrefix(mountPoint) {
  ...
  const owns = (p) => typeof p === 'string' &&
    (p === mountPoint || StringPrototypeStartsWith(p, prefix));
  // Keyed by module path; the value is the owning package.json path.
  MapPrototypeForEach(moduleToParentPackageJSONCache, (pjsonPath, modPath) => {
    if (owns(modPath) || owns(pjsonPath)) {
      moduleToParentPackageJSONCache.delete(modPath);
    }
  });
  // Keyed by package.json path.
  MapPrototypeForEach(deserializedPackageJSONCache, (config, pjsonPath) => {
    if (owns(pjsonPath)) {
      deserializedPackageJSONCache.delete(pjsonPath);
    }
  });
}
