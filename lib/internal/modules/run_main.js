...
function runMainESM(mainPath) {
  ...
  handleMainPromise(loadESM(async (esmLoader) => {
    ...
    if (_importMapSpecifier) {
      ...
      const importMapSpecifier = await esmLoader.resolve(_importMapSpecifier, parentURL, importAttributes);
      const job = await esmLoader.getJobFromResolveResult(importMapSpecifier, parentURL, importAttributes);
      const { module: importMapModule } = await job.run();
      esmLoader.importMap = new ImportMap(importMapModule.getNamespace().default, new URL(importMapSpecifier.url, parentURL))
    }
