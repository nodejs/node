...
class ESMLoader {
  ...
  static pluckHooks({
    ...
    if (obsoleteHooks.length) process.emitWarning(
      `Obsolete loader hook(s) supplied: ${
  ...
  async addCustomLoaders(
    ...
    for (let i = 0, count = customLoaders.length; i < count; i++) {
  ...
  async import(specifiers, parentURL) {
    ...
    const namespaces = new Array(count);
    ...
    for (let i = 0; i < count; i++) {
      ...
      namespaces[i] = module.getNamespace();
    }
  ...
  /**
   * Provide the ready-to-use source (output is something node understands).
   *
  ...
  async load(url, context = {}) {
    const { 0: defaultLoader, 1: customLoader } = this.#loaders;
    ...
    if (
      ...
      'string, SharedArrayBuffer, or Uint8Array',
      'loader load',
      'source',
      source
    );
  ...
  async resolve(originalSpecifier, parentURL) {
    ...
    const { 0: defaultResolver, 1: customResolver } = this.#resolvers;
