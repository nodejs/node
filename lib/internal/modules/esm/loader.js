...
class ESMLoader {
   ...
   * hooks starts at the top and each call to `next()` moves down 1 step until
   * it reaches the bottom or short-circuits.
   *
  ...
  async load(url, context = {}) {
    ...
    const next = async (nextUrl, ctx = context) => {
      --hookIndex; // `next` has been called, so decrement our pointer
      ...
      const output = await loader(nextUrl, ctx, next);
  ...
  async resolve(
    ...
    const next = async (suppliedSpecifier, ctx = context) => {
      --hookIndex; // `next` has been called, so decrement our pointer
      ...
      const output = await resolver(suppliedSpecifier, ctx, next);
