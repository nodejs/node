...
class CustomizedModuleLoader extends DefaultModuleLoader {
   ...
   * @returns {{ format: string, url: URL['href'] }}
   */
  register(originalSpecifier) {
    return hooksProxy.makeSyncRequest('register', originalSpecifier);
  }
...
function register(specifier) {
  ...
  if (getOptionValue('--experimental-loader').length < 1) {
    const { ERR_ESM_LOADER_REGISTRATION_UNAVAILABLE } = require('internal/errors').codes;
