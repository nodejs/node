...
class ESMLoader {
  ...
  static pluckHooks({
    ...
    if (getFormat) ArrayPrototypePush(obsoleteHooks, 'getFormat');
    if (getSource) ArrayPrototypePush(obsoleteHooks, 'getSource');
    if (transformSource) ArrayPrototypePush(obsoleteHooks, 'transformSource');

    if (obsoleteHooks.length)
      process.emitWarning(
        `Obsolete loader hook(s) supplied and will be ignored: ${ArrayPrototypeJoin(
          obsoleteHooks,
          ', '
        )}`,
        'DeprecationWarning'
    ...
    if (globalPreload) {
      acceptedHooks.globalPreloader = FunctionPrototypeBind(
        globalPreload,
        null
      );
    }
  ...
  async addCustomLoaders(customLoaders = []) {
    ...
    for (let i = 0; i < customLoaders.length; i++) {
      ...
      const { globalPreloader, resolver, loader } =
  ...
  #createModuleJob(url, importAssertions, parentURL, format) {
    const moduleProvider = async (url, isMain) => {
      const { format: finalFormat, source } = await this.load(url, {
        format,
        importAssertions,
      });
    ...
    const inspectBrk =
      parentURL === undefined && getOptionValue('--inspect-brk');
  ...
  async load(url, context = {}) {
    ...
    const loader =
      this.#loaders.length === 1 ? defaultLoader : this.#loaders[1];
  ...
  preload() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const { port1: insidePreload, port2: insideLoader } = channel;
  ...
  async resolve(
    ...
    const resolver =
      this.#resolvers.length === 1 ? defaultResolver : this.#resolvers[1];
