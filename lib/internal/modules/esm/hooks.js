...
class Hooks {
  ...
  addCustomLoader(url, exports) {
    ...
    if (resolve) {
      ArrayPrototypePush(this.#chains.resolve, { __proto__: null, fn: resolve, url });
    }
    if (load) {
      ArrayPrototypePush(this.#chains.load, { __proto__: null, fn: load, url });
    }
  }
  ...
  async resolve(
    ...
    const nextResolve = nextHookFactory(chain, meta, { __proto__: null, validateArgs, validateOutput });
   ...
   */
  async load(url, context = { __proto__: null }) {
    ...
    const nextLoad = nextHookFactory(chain, meta, { __proto__: null, validateArgs, validateOutput });
...
class HooksProxy {
  ...
  async makeAsyncRequest(method, ...args) {
    ...
    debug('post async message to worker', { __proto__: null, method, args });
    this.#worker.postMessage({ __proto__: null, method, args, port: asyncCommChannel.port2 }, [asyncCommChannel.port2]);
    ...
    let response;
    do {
      debug('wait for async response from worker', { __proto__: null, method, args });
    ...
    debug('got async response from worker', { __proto__: null, method, args }, this.#lock);
  ...
  makeSyncRequest(method, ...args) {
    ...
    // Pass work to the worker.
    debug('post sync message to worker', { __proto__: null, method, args });
    this.#worker.postMessage({ __proto__: null, method, args });

    let response;
    do {
      debug('wait for sync response from worker', { __proto__: null, method, args });
    ...
    debug('got sync response from worker', { __proto__: null, method, args });
...
function nextHookFactory(chain, meta, { validateArgs, validateOutput }) {
  ...
  if (meta.hookIndex > 0) {
    ...
    nextNextHook = nextHookFactory(chain, meta, { __proto__: null, validateArgs, validateOutput });
  } else {
