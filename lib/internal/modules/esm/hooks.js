    ...
    if (!this.#isReady) {
      ...
      if (!this.worker[kIsOnline]) {
        ...
        if (response.type === 'error') {
          // An exception was thrown during initialization of the worker thread; re-throw to crash the process.
          const { triggerUncaughtException } = internalBinding('errors');
          triggerUncaughtException(response.value);
          throw response.value;
        } else {
          const { preloadScripts } = response.value;
