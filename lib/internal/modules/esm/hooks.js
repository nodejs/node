...
class HooksProxy {
  ...
  constructor() {
    ...
    const lock = new SharedArrayBuffer(8); // Signal to tell the other thread to sleep or wake
    this.#lock = new Int32Array(lock);
    const syncCommChannel = new MessageChannel();
    const errorCommChannel = new MessageChannel();
    ...
    const worker = this.worker = new InternalWorker('internal/modules/esm/worker', {
      ...
      workerData: {
        ...
        errorCommPort: errorCommChannel.port2,
      },
      transferList: [syncCommChannel.port2, errorCommChannel.port2],
    });
    worker.unref(); // ! Allows the process to eventually exit.

    PromisePrototypeThen(AtomicsWaitAsync(this.#lock, 1, 0).value, () => {
      debug('uncaughtExceptionFromWorker');
      const { message: { exception, origin } } = receiveMessageOnPort(errorCommChannel.port1);
      onGlobalUncaughtException(exception, origin === 'unhandledRejection');
    });
  }
  ...
  #waitForWorker() {
    if (!this.#isReady) {
      ...
      if (!this.worker[kIsOnline]) {
        ...
        AtomicsWait(this.#lock, 0, 0); // ! Block the main thread until the worker is ready.
  ...
  async makeAsyncRequest(method, ...args) {
    ...
    if (this.#pendingAsyncMessages++ === 0) this.worker.ref();

    let response;
    debug('start waiting for async response from worker', { method, args });
    while (response == null) {
      AtomicsStore(this.#lock, 0, 0); // Reset lock.
      await AtomicsWaitAsync(this.#lock, 0, 0).value;

      response = receiveMessageOnPort(asyncCommChannel.port1);
    }
    debug('got async response from worker', { method, args });

    if (--this.#pendingAsyncMessages === 0) this.worker.unref();
  ...
  makeSyncRequest(method, ...args) {
    ...
    while (response == null) {
      AtomicsStore(this.#lock, 0, 0); // Reset lock.
      AtomicsWait(this.#lock, 0, 0); // Sleep until worker responds.
  ...
  #unwrapMessage(response) {
    if (response.message.status === 'error') {
      if (typeof response.message.body !== 'object' || response.message.body === null) {
        throw response.message.body;
      }

      const { exception } = response.message.body;
      const descriptors = { __proto__: null };
      const { ownProperties, values, enumerable } = response.message.body;
      for (let i = 0; i < ownProperties.length; i++) {
        const key = ownProperties[i];
        descriptors[key] = {
          __proto__: null,
          configurable: true,
          enumerable: enumerable[i],
          writable: true,
          value: values[i],
        };
      }
      ObjectDefineProperties(exception, descriptors);
      throw exception;
    } else {
