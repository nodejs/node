...
class HooksProxy {
  ...
  async makeAsyncRequest(method, ...args) {
    ...
    while (response == null) {
      ...
      await AtomicsWaitAsync(this.#lock, 0, 0);
  ...
  #unwrapMessage(response) {
    if (response.message.status === 'error') {
      if (!response.message.body?.isSerializedError) throw response.message.body;

      let exceptionConstructor;
      switch (response.message.body.errorType) {
        case 'AggregateError':
          exceptionConstructor = AggregateError;
          break;
        case 'EvalError':
          exceptionConstructor = EvalError;
          break;
        case 'RangeError':
          exceptionConstructor = RangeError;
          break;
        case 'ReferenceError':
          exceptionConstructor = ReferenceError;
          break;
        case 'SyntaxError':
          exceptionConstructor = SyntaxError;
          break;
        case 'TypeError':
          exceptionConstructor = TypeError;
          break;
        default:
          exceptionConstructor = Error;
      }
      const exception = ReflectConstruct(exceptionConstructor, response.message.body.errorConstructorArguments);
      if (exception != null) {
