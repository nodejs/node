...
class HooksProxy {
  ...
  constructor() {
    ...
    const lock = new SharedArrayBuffer(12); // Signal to tell the other thread to sleep or wake
  ...
  #beforeExitTellMyChildILoveHer = () => {
    const value = AtomicsOr(this.#lock, 1, 0b10);
    debug('beforeExit main thread', value, this.#lock);
    if (value & 0b01 && AtomicsLoad(this.#lock, 2) === 0) {
  ...
  async makeAsyncRequest(method, ...args) {
    ...
    AtomicsAdd(this.#lock, 2, 1);
    AtomicsNotify(this.#lock, 2);
    ...
    AtomicsAnd(this.#lock, 1, ~0b10); // If the main thread was bored before, it's no longer the case.
    if (--this.#pendingAsyncMessageResponses === 0) {
