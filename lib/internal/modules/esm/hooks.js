...
function debug(...args) {
  appendFileSync('/dev/fd/1',
    'Hooks: ' + args.map((arg) => inspect(arg)).join(' ') + '\n'
  );
}
...
class HooksProxy {
  ...
  constructor(customLoaders) {
    ...
    debug('setting initial data for worker', customLoaders);
    this.#setRequest('init', customLoaders);
  }

  #setRequest(type, ...args) {
    const { serialize } = require('v8');

    TypedArrayPrototypeFill(this.#requestResponseData, 0); // Erase handled request/response data

    const request = serialize({ type, args });
    debug(
      `sending ${type} request to worker (and then sleeping) with`,
      ...(args.length ? args : ['no args']),
    );
    TypedArrayPrototypeSet(this.#requestResponseData, request);
  }

  makeRequest(type, ...args) {
    if (!this.#isReady) {
      const { kIsOnline } = require('internal/worker');
      if (!this.worker[kIsOnline]) {
        debug('blocking main thread until the loader thread is ready');
        Atomics.wait(this.#lock, 0, 0); // ! Block this module until the worker is ready.
        debug('main thread awakened');
      }

      this.#isReady = true;
    }

    this.#setRequest(type, ...args);

    Atomics.store(this.#lock, 0, 0); // Send request to worker
    Atomics.notify(this.#lock, 0); // Notify worker of new request
    debug('notified worker of request; sleeping main.');

    Atomics.wait(this.#lock, 0, 0); // Sleep until worker responds

    const { deserialize } = require('v8');

    debug(`received response from worker for ${type} request`, this.#requestResponseData);
    return deserialize(this.#requestResponseData);
  }
}
