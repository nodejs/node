...
class HooksProxy {
  ...
  #beforeExitTellMyChildILoveHer = () => {
    switch (AtomicsOr(this.#lock, 1, 0b10)) {
      case 0b00:
      case 0b10:
      ...
      case 0b11:
        ...
        this.worker.unref();
        AtomicsNotify(this.#lock, 1); // Wake up worker so it can die.
        break;
    }
  };
  ...
  async makeAsyncRequest(method, ...args) {
    ...
    if (this.#pendingAsyncMessages++ === 0) {
      AtomicsStore(this.#lock, 1, 0); // Reset signal
      process.on('beforeExit', this.#beforeExitTellMyChildILoveHer);
    } else if (this.#lock[1] & 0b01) {
      // The worker locked itself waiting for a message from us, let's unlock it
      // so it can process the message just sent.
      AtomicsStore(this.#lock, 1, 0); // Reset signal
      debug('wake up worker');
      AtomicsNotify(this.#lock, 1);
    }
    ...
    if (--this.#pendingAsyncMessages === 0) {
      this.worker.unref();
      process.off('beforeExit', this.#beforeExitTellMyChildILoveHer);
    }
