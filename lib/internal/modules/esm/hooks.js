...
class HooksProxy {
  ...
  constructor() {
    ...
    const worker = this.worker = new InternalWorker('internal/main/loader_thread', {
      stderr: false,
      stdin: false,
      stdout: false,
  ...
  #beforeExitHandler = () => {
    const value = AtomicsOr(this.#lock, IDLENESS_OF_THREADS, MAIN_THREAD_IS_IDLE);
  ...
  async makeAsyncRequest(method, ...args) {
    AtomicsAdd(this.#lock, NUMBER_OF_MESSAGES_IN_TRANSIT, 1);
    this.#waitForWorker();
    ...
    if (AtomicsAdd(this.#lock, NUMBER_OF_INCOMING_ASYNC_RESPONSES, 1) === 0) {
      // On the next lines, the main thread will await a response from the
      // worker thread that might come after the last task in the event
      // loop has run its course and there would be nothing left keeping the
      // thread alive (and once the main thread dies, the whole process stops).
      // However we want to keep the process alive until the worker thread
      // responds (or until the event loop of the worker thread is also empty).
      // So we add the beforeExit handler whose mission is to lock the main
      // thread until we hear back from the worker thread.
      // The if condition is there so we only add the event handler once (if
      // there are already pending async responses, the previous calls have
