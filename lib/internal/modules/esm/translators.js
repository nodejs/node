...
function createCJSModuleWrap(url, source, isMain, format, loadCJS = loadCJSModule) {
  ...
  return new ModuleWrap(url, undefined, wrapperNames, function() {
    ...
    const exportNamesArray = ArrayIsArray(exportNames) ? exportNames : [...exportNames];
    for (let i = 0; i < exportNamesArray.length; i++) {
      const exportName = exportNamesArray[i];
  ...
  if (reexports.length) {
    ...
    for (let i = 0; i < reexports.length; i++) {
      ...
      if (format === 'commonjs' ||
        ...
        const reexportNamesArray = ArrayIsArray(reexportNames) ? reexportNames : [...reexportNames];
        for (let i = 0; i < reexportNamesArray.length; i++) {
          const name = reexportNamesArray[i];
...
translators.set('wasm', async function(url, source) {
  ...
  const { module } = createDynamicModule([...importsList], [...exportsList], url, (reflect) => {
    ...
    const importsArray = [...importsList];
    for (let i = 0; i < importsArray.length; i++) {
      const impt = importsArray[i];
      ...
      if (wasmInstance) {
        ...
        for (let j = 0; j < wasmGlobalImports.length; j++) {
          const { module, name } = wasmGlobalImports[j];
          if (module !== impt) {
            continue;
          }
    ...
    for (let i = 0; i < exportsArray.length; i++) {
      const expt = exportsArray[i];
