...
translators.set('wasm', async function(url, source) {
  ...
  const importNames = ArrayFrom(
    new SafeSet(
      ArrayPrototypeMap(WebAssembly.Module.imports(compiled),
                        ({ module }) => module)));
  const importNamespaces = await asyncESM.esmLoader.import(
    importNames,
  ...
  const ret = new ModuleWrap(url, undefined, exports, function() {
    const imports = ObjectCreate(null);
    for (let i = 0; i < importNames.length; i++) {
      imports[importNames[i]] = importNamespaces[i];
    }
    const { exports } = new WebAssembly.Instance(compiled, imports);
    for (const expt of ObjectKeys(exports))
      this.setExport(expt, exports[expt]);
  });
  return ret;
});
