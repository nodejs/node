...
const { dirname, extname, isAbsolute, join } = require('path');
const {
  hasEsmSyntax,
  loadBuiltinModule,
  stripBOM,
...
function createCJSModuleWrap(url, source) {
  ...
  return new ModuleWrap(url, undefined, namesWithDefault, function() {
    ...
    const requireFn = function require(spec) {
      ...
      const job = asyncESM.esmLoader.getModuleJob(spec, url, kEmptyObject, true);
      job.runSync();
...
function cjsPreparseModuleExports(filename, source) {
  ...
  try {
    // TODO: use a TextDecoder, or add support for BufferView in `cjsParse`
    const sourceString = typeof source === 'string' ? source : `${source}`;
    ({ exports, reexports } = cjsParse(sourceString || ''));
  } catch {
    exports = [];
    reexports = [];
  }
