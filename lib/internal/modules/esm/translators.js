  ...
  if (reexports.length) {
    ...
    for (let i = 0; i < reexports.length; i++) {
      ...
      try {
        ...
        // Doing so would mean dropping support for CJS in the loader thread, as this call needs to be sync
        // from the perspective of the main thread, which we can do via HooksProxy and Atomics, but we can't
        // do within the loaders thread. Until this is done, `resolve` hooks will not be run for `require` input.
