...
/**
 * Loads a CJS module without using the monkey-patchable CJS loader. Modules
 * loaded with this method have a special `require` function that calls the ESM
 * loader.
 */
function loadCJSModule(module, source, url, filename) {
  ...
  const requireFn = function require(spec) {
    ...
    if (!StringPrototypeStartsWith(spec, 'node:')) {
      ...
      const path = CJSModule._resolveFilename(spec, module);
      if (spec !== path) {
        switch (extname(path)) {
          case '.json':
            ...
            break;
          case '.node':
            return CJSModule._load(spec, module);
          default:
        }
        spec = `${pathToFileURL(path)}`;
      }
    }
    const job = asyncESM.esmLoader.getModuleJobSync(spec, url, importAttributes);
    job.runSync();
  ...
  setOwnProperty(requireFn, 'resolve', function resolve(spec) {
    ...
    const { url: resolvedURL } = asyncESM.esmLoader.resolveSync(spec, url, kEmptyObject);
...
translators.set('commonjs', async function commonjsStrategy(url, source,
  ...
  // commonjs source associated with a file: URL. In this case, the source is
...
function cjsPreparseModuleExports(filename, source) {
  ...
  debug(`IsLoaded CJSModule ${filename}? ${loaded}`);
  if (!loaded) {
  ...
  if (reexports.length) {
    ...
    for (let i = 0; i < reexports.length; i++) {
      ...
      if ((ext === '.js' || ext === '.cjs' || !CJSModule._extensions[ext]) &&
          isAbsolute(resolved)) {
        const { exportNames: reexportNames } = cjsPreparseModuleExports(resolved, readFileSync(resolved, 'utf-8'));
