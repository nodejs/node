...
// Use a custom loader getSource hook if one is provided.
function getSource(loader, url, format) {
  if (loader._getSource) {
    return loader._getSource({ url, format }, defaultGetSource);
  } else {
    return defaultGetSource(url);
  }
}

// Use a custom loader transformSource hook if one is provided.
function transformSource(loader, url, format, source) {
  if (loader._transformSource) {
    return loader._transformSource({ url, format }, source);
  } else {
    return source;
  }
}
...
translators.set('module', async function moduleStrategy(url) {
  let source = `${await getSource(this, url, 'module')}`;
  source = await transformSource(this, url, 'module', source);
...
translators.set('json', async function jsonStrategy(url) {
  ...
  let content = `${await getSource(this, url, 'json')}`;
  content = await transformSource(this, url, 'json', content);
  if (pathname) {
...
translators.set('wasm', async function(url) {
  ...
  let buffer = await getSource(this, url, 'wasm');
  buffer = await transformSource(this, url, 'wasm', buffer);
