...
function defaultGetSource({ url, format }, defaultGetSource, loader) {
...
function defaultTransformSource({ url, format, source },
                                defaultTransformSource, loader) {
  return source;
}
...
translators.set('module', async function moduleStrategy(url) {
  let source = `${await this._getSource(
    { url, format: 'module' }, defaultGetSource, this)}`;
  source = await this._transformSource(
    { url, format: 'module', source }, defaultTransformSource, this);
...
translators.set('json', async function jsonStrategy(url) {
  ...
  let source = `${await this._getSource(
    { url, format: 'json' }, defaultGetSource, this)}`;
  source = await this._transformSource(
    { url, format: 'json', source }, defaultTransformSource, this);
  if (pathname) {
...
translators.set('wasm', async function(url) {
  ...
  let source = await this._getSource(
    { url, format: 'wasm' }, defaultGetSource, this);
  source = await this._transformSource(
    { url, format: 'wasm', source }, defaultTransformSource, this);
