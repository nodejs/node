...
const { internalModuleParseCJS } = internalBinding('fs');
...
translators.set('commonjs', async function commonjsStrategy(url, isMain) {
  ...
  return new ModuleWrap(url, undefined, namesWithDefault, function() {
    ...
    // Verify that it really does set __esModule, not just statically.
    const esModule = ObjectPrototypeHasOwnProperty(exports, '__esModule') &&
        exports.__esModule;
    if (esModule) {
...
const strictReserved = new SafeSet([
...
function cjsPreparseModuleExports(filename) {
  ...
  const [exports, reexports] =
    source ? internalModuleParseCJS(source) : [[], []];

  const esModule = ArrayPrototypeIncludes(exports, '__esModule');
  const exportNames = new SafeSet(
    esModule ? exports.filter((expt) => !strictReserved.has(expt)) : []
  );
  ...
  for (const reexport of reexports) {
    let resolved;
    try {
      // Reexports are still raw specifier strings, so use JSON.parse.
    ...
    if (ext === '.js' || ext === '.cjs' || !CJSModule._extensions[ext]) {
      ...
      if (reexportNames.has('__esModule')) {
