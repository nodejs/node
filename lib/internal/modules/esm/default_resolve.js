...
function defaultGetFormat({ url, isMain }, defaultGetFormat, loader) {
...
function defaultResolve({ specifier, parentURL }, defaultResolve, loader) {
  const isMain = parentURL === undefined;
  let parsed;
  try {
    parsed = new URL(specifier);
    if (parsed.protocol === 'data:') {
      const { format } = loader._getFormat(
        { url: specifier, isMain }, defaultGetFormat, loader);
      return {
        url: specifier,
        format
      };
  ...
  const { format } = loader._getFormat(
    { url, isMain }, defaultGetFormat, loader);
