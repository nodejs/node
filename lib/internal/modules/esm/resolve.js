    ...
    if (
      ...
      if (shouldBeTreatedAsRelativeOrAbsolutePath(specifier)) {
        ...
        if (
          ...
          throw new ERR_UNSUPPORTED_ESM_NETWORK_IMPORT_SPECIFIER(
            specifier,
            parentURL,
            'cannot import local location from remote location,' +
            'load a it via a remote location.');
        } else {
...
if (policy) {
  ...
  module.exports.defaultResolve = async function defaultResolve(specifier, context) {
    const ret = await $defaultResolve(specifier, context, $defaultResolve);
    if (policy.manifest.mightAllow(ret.url)) {
      return ret;
    }
    
    throw new ERR_MANIFEST_DEPENDENCY_MISSING(context.url, specifier, context.conditions);
  };
}
