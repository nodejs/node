...
function finalizeResolution(resolved, base) {
  ...
  if (resolved.pathname.endsWith('/')) return resolved;
...
function resolveExportsTargetString(
  ...
  if (!resolvedPath.startsWith(packagePath) ||
      resolvedPath.includes('/node_modules/', packagePath.length - 1)) {
    throwExportsInvalid(match, target, packageJSONUrl, base);
  }
  ...
  if (!subpathResolvedPath.startsWith(resolvedPath) ||
      subpathResolvedPath.includes('/node_modules/', packagePath.length - 1)) {
...
function isArrayIndex(key) {
  const keyStr = `${key}`;
  if (`${+keyStr}` !== keyStr) return false;
  key >>>= 0;
  if (ObjectIs(key, 0)) return true;
  return key > 0 && key < 0xFFFF_FFFF;
}
...
function packageExportsResolve(
  ...
  for (const key of keys) {
    ...
    if (packageSubpath.startsWith(key) && key.length > bestMatch.length)
      bestMatch = key;
  }

  if (bestMatch) {
    const target = exports[bestMatch];
    const subpath = packageSubpath.substr(bestMatch.length);
    const resolved = resolveExportsTarget(
      packageJSONUrl, target, subpath, packageSubpath, base);
    return finalizeResolution(resolved, base);
  }
...
function packageResolve(specifier /* string */, base /* URL */) { /* -> URL */
  ...
  const packageName = separatorIndex === -1 ?
    specifier : specifier.substr(0, separatorIndex);
  ...
  const packageSubpath = separatorIndex === -1 ?
    '' : '.' + specifier.substr(separatorIndex);
  ...
  do {
    const stat = tryStatSync(
      packageJSONPath.substr(0, packageJSONPath.length - 13));
    if (!stat.isDirectory()) {
