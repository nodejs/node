...
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  if (stats === 1) {
    ...
    if (process.env.WATCH_REPORT_DEPENDENCIES && process.send) {
      process.send({ '__proto__': null, 'watch:require': [path || resolved.pathname] });
    }
...
function packageExportsResolve(
  ...
  if (isConditionalExportsMainSugar(exports, packageJSONUrl, base))
    exports = { '__proto__': null, '.': exports };
...
function parsePackageName(specifier, base) {
  ...
  return { __proto__: null, packageName, packageSubpath, isScoped };
}
...
function defaultResolve(specifier, context = { __proto__: null }) {
