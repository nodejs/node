...
function emitInvalidSegmentDeprecation(target, request, match, pjsonUrl, base) {
  ...
  const pjsonPath = fileURLToPath(pjsonUrl);
  const double = RegExpPrototypeTest(doubleSlashRegEx, target);
  process.emitWarning(
...
function resolvePackageTargetString(target, subpath, match, packageJSONUrl,
  ...
  if (RegExpPrototypeTest(invalidSegmentRegEx,
                          StringPrototypeSlice(target, 2))) {
    if (!RegExpPrototypeTest(deprecatedInvalidSegmentRegEx,
                             StringPrototypeSlice(target, 2))) {
      if (!isPathMap) {
  ...
  if (RegExpPrototypeTest(invalidSegmentRegEx, subpath)) {
    const request = pattern ?
        StringPrototypeReplace(match, '*', () => subpath) : match + subpath;
    if (!RegExpPrototypeTest(deprecatedInvalidSegmentRegEx, subpath)) {
      if (!isPathMap) {
        const resolvedTarget = pattern
          ? RegExpPrototypeSymbolReplace(patternRegEx, target, () => subpath)
          : target;
...
function parsePackageName(specifier, base) {
  ...
  // \\ separators.
  if (RegExpPrototypeTest(invalidPackageNameRegEx, packageName))
    validPackageName = false;
