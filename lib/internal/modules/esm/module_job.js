    ...
    if (status === kInstantiated || status === kErrored) {
      ...
      if (status === kInstantiated) {
        ...
        if (this.inspectBrk) {
          ...
          const deps = isPromise(this.linked) ? [] : this.linked;
          for (const dep of deps) {
            dep.module.evaluateSync(urlToFilename(dep.url), filename);
          }
          const { callAndPauseOnStart } = internalBinding('inspector');
          namespace = callAndPauseOnStart(this.module.evaluateSync, this.module,
  ...
  async run(isEntryPoint = false) {
    ...
    try {
      if (this.inspectBrk) {
        // Pre-evaluate all direct dependencies so that V8 will skip them when we evaluate the entry point.
        // This ensures `callAndPauseOnStart` pauses at the entry point's first line rather than a dependency's.
        const deps = isPromise(this.linked) ? await this.linked : this.linked;
        for (const dep of deps) {
          await dep.module.evaluate(timeout, breakOnSigint);
        }
        const { callAndPauseOnStart } = internalBinding('inspector');
        await callAndPauseOnStart(this.module.evaluate, this.module, timeout, breakOnSigint);
      } else {
