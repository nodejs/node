...
class ModuleJob extends ModuleJobBase {
  ...
  async run(isEntryPoint = false) {
    ...
    const breakOnSigint = false;
    setHasStartedUserESMExecution();
    // module deliberately evaluated outside try / catch so that V8 doesn't mark top-level promises as handled
    const evaluatePromise = this.module.evaluate(timeout, breakOnSigint);
    try {
