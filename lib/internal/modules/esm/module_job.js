 ...
 * @returns {void}
 */
function decorateModuleNotFoundError(error, parentURL, request) {
  if (error?.code !== 'ERR_MODULE_NOT_FOUND' ||
      !StringPrototypeStartsWith(parentURL, 'file://') ||
      typeof request.lineNumber !== 'number' ||
      typeof request.columnNumber !== 'number') {
    return;
  }
  ...
  const sourceLine = lines[request.lineNumber];
  if (sourceLine === undefined) {
    return;
  }

  let column = StringPrototypeIndexOf(sourceLine, request.specifier, request.columnNumber);
  if (column === -1) {
    column = StringPrototypeIndexOf(sourceLine, request.specifier);
  }
  if (column === -1) {
  ...
  syncLink(requestType) {
    ...
    try {
      for (let idx = 0; idx < moduleRequests.length; idx++) {
        ...
        let job;
        try {
          job = this.loader.getOrCreateModuleJob(this.url, request, requestType);
        } catch (e) {
          decorateModuleNotFoundError(e, this.url, request);
          throw e;
        }
