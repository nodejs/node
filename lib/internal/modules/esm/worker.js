...
(async function customizedModuleWorker() {
  ...
  function beforeExitTellMyMomILoveHer() {
    switch (AtomicsOr(lock, 1, 0b01)) {
      case 0b00:
      case 0b01:
        // The main thread is still in good health, let's pause the worker
        // thread until a new message comes in.
        if (AtomicsWait(lock, 1, 0) === 'ok') {
          // If the value is 0, it means the main thread just sent a message for
          // us to process.
          // Any other value means the main thread is also dying.
          tickCount = 0;
          checkForMessages();
        }
        break;
      ...
      case 0b10:
      case 0b11:
        // The main thread has already reported waiting for us to die together.
  ...
  async function handleMessage({ method, args, port }) {
    ...
    try {
      ...
      if (port != null && --immediateCount === 0) {
        ...
        syncCommPort.on('message', handleMessage);
        AtomicsStore(lock, 1, 0); // Reset signal
      }
    } catch (exception) {
      hasError = true;
      response = exception;
