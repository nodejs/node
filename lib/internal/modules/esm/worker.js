...
(async function setupESMWorker() {
  ...
  while (true) {
    ...
    // This next line needs to be outside of the previous try/catch block in order to return any above errors to the main thread;
    // but since this call itself might throw an exception, we need to repeat the pattern to send any exceptions from this call
    // to the main thread in the same way.
    try {
