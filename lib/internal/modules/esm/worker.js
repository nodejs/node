...
function wrapMessage(status, body) {
  ...
  const ownProperties = ObjectGetOwnPropertyNames(body);
  return {
    status,
    body: {
      isSerializedError: true,
      errorType: body.constructor?.name,
      errorConstructorArguments: [body.message],
      ownProperties,
      values: ArrayPrototypeMap(ownProperties, (key) => {
        try {
