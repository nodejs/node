...
(async function customizedModuleWorker() {
  ...
  process.on('beforeExit', () => {
    ...
    if (value & MAIN_THREAD_IS_IDLE && AtomicsLoad(lock, NUMBER_OF_INCOMING_MESSAGES) === 0) {
    ...
    // The pending messages won't come back at this point, we can reset the counters.
  ...
  async function handleMessage({ method, args, port }) {
    ...
    if (port != null && pendingAsyncResponses++ === 0) {
      // We are about to yield the execution with the await ReflectApply below.
