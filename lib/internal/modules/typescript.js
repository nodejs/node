...
function stripTypeScriptModuleTypes(source, filename, emitWarning = true) {
  ...
  if (isUnderNodeModules(filename)) {
    const stripUnderNodeModules = getOptionValue('--experimental-strip-node-modules');
    if (!stripUnderNodeModules) {
      throw new ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING(filename);
    }
    emitExperimentalWarning('--experimental-strip-node-modules');
  }
