...
class FilesWatcher extends EventEmitter {
  ...
  filterFile(file, owner, options = kEmptyObject) {
    ...
    if (owner) {
      ...
      const dependencies = this.#ownerDependencies.get(owner) ?? new SafeSet();
      owners.add(owner);
  ...
  unfilterFilesOwnedBy(owners) {
    owners.forEach((owner) => {
      this.#ownerDependencies.get(owner)?.forEach((dependency) => {
        ...
        const dependencyOwners = this.#dependencyOwners.get(dependency);
        if (dependencyOwners !== undefined) {
          dependencyOwners.delete(owner);
