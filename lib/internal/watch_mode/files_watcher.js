...
class FilesWatcher extends EventEmitter {
  ...
  #onChange(trigger) {
    ...
    this.emit('changed', { __proto__: null, owners });
  ...
  watchPath(path, recursive = true) {
    ...
    const watcher = watch(path, { __proto__: null, recursive, signal: this.#signal });
    watcher.on('change', (eventType, fileName) => this
      .#onChange(recursive ? resolve(path, fileName) : path));
    this.#watchers.set(path, { __proto__: null, handle: watcher, recursive });
    if (recursive) {
