...
function onlookup(err, addresses) {
  ...
  this.resolve({ __proto__: null, address: addresses[0], family });
...
function createLookupPromise(family, hostname, all, hints, verbatim) {
  return new Promise((resolve, reject) => {
    if (!hostname) {
      ...
      resolve(all ? [] : { __proto__: null, address: null, family: family === 6 ? 6 : 4 });
      return;
    }
    ...
    if (matchedFamily !== 0) {
      const result = { __proto__: null, address: hostname, family: matchedFamily };
...
function onlookupservice(err, hostname, service) {
  ...
  this.resolve({ __proto__: null, hostname, service });
