...
function validateTimeout(options) {
  const { timeout = -1 } = { __proto__: null, ...options };
  validateInt32(timeout, 'options.timeout', -1);
  return timeout;
}

function validateTries(options) {
  const { tries = 4 } = { __proto__: null, ...options };
...
class ResolverBase {
  constructor(options = undefined) {
    ...
    if (isBuildingSnapshot()) {
      this[kSnapshotStates] = { __proto__: null, timeout, tries };
    }
  ...
  setLocalAddress(ipv4, ipv6) {
    ...
    if (isBuildingSnapshot()) {
      this[kSnapshotStates].localAddress = { __proto__: null, ipv4, ipv6 };
    }
  }
