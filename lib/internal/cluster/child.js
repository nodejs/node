...
cluster._setupWorker = function() {
  ...
  send({ __proto__: null, act: 'online' });
...
cluster._getServer = function(obj, options, cb) {
  ...
  if (indexSet === undefined) {
    indexSet = { __proto__: null, nextIndex: 0, set: new SafeSet() };
  ...
  send(message, (reply, handle) => {
    ...
    if (handle) {
      ...
      shared(reply, { __proto__: null, handle, indexesKey, index }, cb);
    } else {
      // Round-robin.
      rr(reply, { __proto__: null, indexesKey, index }, cb);
    }
  });
...
function shared(message, { handle, indexesKey, index }, cb) {
  ...
  handle.close = function() {
    send({ __proto__: null, act: 'close', key });
    handles.delete(key);
...
function rr(message, { indexesKey, index }, cb) {
  ...
  const handle = { __proto__: null, close, listen, ref, unref };
  handle.ref();
  if (message.sockname) {
...
function onconnection(message, handle) {
  ...
  send({ __proto__: null, ack: message.seq, accepted });
  ...
  function checkWaitingCount() {
    ...
    if (waitingCount === 0) {
      ...
      if (primaryInitiated) {
        ...
        send({ __proto__: null, act: 'exitedAfterDisconnect' }, () => process.disconnect());
      }
    }
  }
