...
cluster.setupPrimary = function(options) {
  ...
  if (initialized === true)
    return queueMicrotask(() => setupSettingsNT(settings));
  ...
  queueMicrotask(() => setupSettingsNT(settings));
...
cluster.fork = function(env) {
  ...
  queueMicrotask(() => emitForkNT(worker));
...
cluster.disconnect = function(cb) {
  ...
  if (workers.length === 0) {
    queueMicrotask(() => intercom.emit('disconnect'));
  } else {
    for (const worker of ObjectValues(cluster.workers)) {
      if (worker.isConnected()) {
        worker.disconnect();
