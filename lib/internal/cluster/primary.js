...
cluster.setupPrimary = function(options) {
  ...
  if (initialized === true)
    return process._deferTick(setupSettingsNT, settings);
  ...
  process._deferTick(setupSettingsNT, settings);
...
cluster.fork = function(env) {
  ...
  process._deferTick(emitForkNT, worker);
...
cluster.disconnect = function(cb) {
  ...
  if (workers.length === 0) {
    process._deferTick(() => intercom.emit('disconnect'));
  } else {
    for (const worker of ObjectValues(cluster.workers)) {
      if (worker.isConnected()) {
        worker.disconnect();
