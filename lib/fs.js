...
fs.mkdirtemp = function(template, callback) {
  callback = makeCallback(callback);
  if (!nullCheck(template, callback)) return;
  var req = new FSReqWrap();
  req.oncomplete = callback;
  binding.mkdirtemp(pathModule._makeLong(template), req);
};

fs.mkdirtempSync = function(template) {
  nullCheck(template);
  return binding.mkdirtemp(pathModule._makeLong(template));
};
