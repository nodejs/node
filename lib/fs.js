...
fs.access = promisify(function(path, mode, callback) {
...
fs.readFile = promisify(function(path, options, callback_) {
...
fs.close = promisify(function(fd, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
fs.rename = promisify(function(oldPath, newPath, callback) {
...
fs.ftruncate = promisify(function(fd, len, callback) {
  if (typeof len === 'function') {
    callback = len;
    len = 0;
  } else if (len === undefined) {
...
fs.fdatasync = promisify(function(fd, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
fs.fsync = promisify(function(fd, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
fs.mkdir = promisify(function(path, mode, callback) {
...
fs.fstat = promisify(function(fd, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
  binding.fstat(fd, req);
});
...
fs.readlink = promisify(function(path, callback) {
  callback = makeCallback(callback);
  if (!nullCheck(path, callback)) return;
  var req = new FSReqWrap();
  req.oncomplete = callback;
...
fs.symlink = promisify(function(target, path, type_, callback_) {
...
fs.link = promisify(function(srcpath, dstpath, callback) {
  callback = makeCallback(callback);
...
fs.fchmod = promisify(function(fd, mode, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
if (constants.hasOwnProperty('O_SYMLINK')) {
  fs.lchmod = promisify(function(path, mode, callback) {
    callback = maybeCallback(callback);
...
fs.chmod = promisify(function(path, mode, callback) {
  callback = makeCallback(callback);
  if (!nullCheck(path, callback)) return;
  var req = new FSReqWrap();
  req.oncomplete = callback;
...
if (constants.hasOwnProperty('O_SYMLINK')) {
  fs.lchown = promisify(function(path, uid, gid, callback) {
    callback = maybeCallback(callback);
    fs.open(path, constants.O_WRONLY | constants.O_SYMLINK, function(err, fd) {
      if (err) {
        callback(err);
...
fs.fchown = promisify(function(fd, uid, gid, callback) {
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
fs.chown = promisify(function(path, uid, gid, callback) {
  callback = makeCallback(callback);
  if (!nullCheck(path, callback)) return;
  var req = new FSReqWrap();
  req.oncomplete = callback;
...
fs.utimes = promisify(function(path, atime, mtime, callback) {
  callback = makeCallback(callback);
  if (!nullCheck(path, callback)) return;
  var req = new FSReqWrap();
  req.oncomplete = callback;
...
fs.futimes = promisify(function(fd, atime, mtime, callback) {
  atime = toUnixTimestamp(atime);
  mtime = toUnixTimestamp(mtime);
  var req = new FSReqWrap();
  req.oncomplete = makeCallback(callback);
...
fs.writeFile = promisify(function(path, data, options, callback_) {
  var callback = maybeCallback(arguments[arguments.length - 1]);
...
fs.appendFile = promisify(function(path, data, options, callback_) {
  var callback = maybeCallback(arguments[arguments.length - 1]);
