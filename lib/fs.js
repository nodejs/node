...
function throwOptionsError(options) {
  throw I18N.TypeError(I18N.ARGUMENT_STRING_OR_OBJECT, 'options');
}
...
function assertEncoding(encoding) {
  if (encoding && !Buffer.isEncoding(encoding)) {
    throw I18N.Error(I18N.UNKNOWN_ENCODING, encoding);
  }
}

function nullCheck(path, callback) {
  if (('' + path).indexOf('\u0000') !== -1) {
    var er = I18N.Error(I18N.PATH_NULL_BYTES, 'path');
    er.code = 'ENOENT';
...
function readFileAfterStat(err, st) {
  ...
  if (size > kMaxLength) {
    err = I18N.RangeError(I18N.FS_SIZE_TOO_LARGE, kMaxLength.toString(16));
    return context.close(err);
  }
...
function stringToFlags(flag) {
  ...
  throw I18N.Error(I18N.FS_UNKNOWN_FLAG, flag);
}
...
function toUnixTimestamp(time) {
  ...
  throw I18N.Error(I18N.FS_MALFORMED_TIME, time);
}
...
fs.watchFile = function(filename, options, listener) {
  ...
  if (typeof listener !== 'function')
    throw I18N.Error(I18N.FUNCTION_REQUIRED, 'listener');
...
function ReadStream(path, options) {
  ...
  if (this.start !== undefined) {
    if (typeof this.start !== 'number') {
      throw I18N.TypeError(I18N.INVALID_OPTION_TYPE, 'start', 'number');
    }
    if (this.end === undefined) {
      this.end = Infinity;
    } else if (typeof this.end !== 'number') {
      throw I18N.TypeError(I18N.INVALID_OPTION_TYPE, 'end', 'number');
    }

    if (this.start > this.end) {
      throw I18N.Error(I18N.FS_OPTION_LESS_THAN, 'start', 'end');
    }
...
function WriteStream(path, options) {
  ...
  if (this.start !== undefined) {
    ...
    if (this.start < 0) {
      throw I18N.Error(I18N.FS_OPTION_MORE_THAN_0, 'start');
    }
...
WriteStream.prototype._write = function(data, encoding, cb) {
  if (!(data instanceof Buffer))
    return this.emit('error', I18N.Error(I18N.FS_INVALID_DATA));
...
SyncWriteStream.prototype.write = function(data, arg1, arg2) {
  ...
  if (arg1) {
    if (typeof arg1 === 'string') {
      ...
      throw I18N.Error(I18N.BAD_ARGUMENTS);
    }
  }
  assertEncoding(encoding);
