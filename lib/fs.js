...
function throwOptionsError(options) {
  throw new errors.TypeError('INVALIDARG', 'options', ['Object', 'string']);
}
...
function makeCallback(cb) {
  ...
  if (typeof cb !== 'function') {
    throw new errors.TypeError('CALLBACKREQUIRED');
  }
...
function readFileAfterStat(err, st) {
  ...
  if (size > kMaxLength) {
    return context.close(new errors.RangeError('FILETOOBIG', kMaxLength));
  }
...
function ReadStream(path, options) {
  ...
  if (this.start !== undefined) {
    if (typeof this.start !== 'number') {
      throw new errors.TypeError('INVALIDOPT', 'start', 'number');
    }
    if (this.end === undefined) {
      this.end = Infinity;
    } else if (typeof this.end !== 'number') {
      throw new errors.TypeError('INVALIDOPT', 'end', 'number');
    }

    if (this.start > this.end) {
      throw new errors.Error('INVALIDOPT', 'start',
                             'number <= the "end" option');
    }
...
function WriteStream(path, options) {
  ...
  if (this.start !== undefined) {
    ...
    if (this.start < 0) {
      throw new errors.Error('INVALIDOPT', 'start', 'number >= zero');
    }
...
fs.mkdtemp = function(prefix, options, callback) {
  if (!prefix || typeof prefix !== 'string')
    throw new errors.TypeError('REQUIREDARG', 'prefix');
