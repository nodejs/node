...
const { currentId, setInitTriggerId } = require('async_hooks');
...
// Used to pull the id and triggerId from getIdsFromFd().
const f64array = new Float64Array(2);
...
function setActualInitTriggerId(self) {
  if (self._asyncId > 0) {
    setInitTriggerId(self._asyncId);
    return;
  }
  binding.getIdsFromFd(self.fd, f64array);
  setInitTriggerId(f64array[0] > 0 ? f64array[0] : currentId());
}
...
fs.readFile = function(path, options, callback) {
  ...
  if (context.isUserFd) {
    // If the fd was opened synchronously then there will be no asyncId.
    binding.getIdsFromFd(path, f64array);
    context._asyncId = f64array[0] > 0 ? f64array[0] : currentId();
    setInitTriggerId(context._asyncId);
  ...
  // No need to setInitTriggerId() here since the triggerId and currentId are
  // the same.
  binding.open(pathModule._makeLong(path),
               stringToFlags(options.flag || 'r'),
               0o666,
               req);
  context._asyncId = req.getAsyncId();
};
...
function ReadFileContext(callback, encoding) {
  ...
  this.err = null;
  // No need to store _triggerId because it won't be passed on to other
  // resources, and the value is stored in the ReqWrap for the call to
  // MakeCallback.
  this._asyncId = -1;
}
...
ReadFileContext.prototype.read = function() {
  ...
  setActualInitTriggerId(this);
  binding.read(this.fd, buffer, offset, length, -1, req);
  // Since a ReadFileContext encapsulates many interactions with a file, update
  // the _asyncId of the instance to the latest interaction so that it properly
  // propagates to the next action's triggerId.
  this._asyncId = req.getAsyncId();
};
...
ReadFileContext.prototype.close = function(err) {
  ...
  // This will be needed for either the nextTick() or .close().
  setActualInitTriggerId(this);
...
function ReadStream(path, options) {
  ...
  this.pos = undefined;
  this.bytesRead = 0;
  // If no fd or asyncId is passed then assume that .open() will be called and
  // that _asyncId will be assigned at that time.
  this._asyncId = -1;
