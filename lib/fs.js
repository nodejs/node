...
fs.readFile = function(path, options, callback_) {
  ...
  const context = new ReadFileContext(callback, encoding);
...
function readFileAfterStat(err, st) {
  ...
  const size = context.size = st.isFile() ? st.size : 0;
...
fs.readFileSync = function(path, options) {
  ...
  const fd = isUserFd ? path : fs.openSync(path, flag, 0o666);
  ...
  const pos = 0;
  const buffer; // single buffer with file data
  const buffers; // list for when size is unknown
...
fs.readSync = function(fd, buffer, offset, length, position) {
  ...
  const r = binding.read(fd, buffer, offset, length, position);
  if (!legacy) {
    return r;
  }

  const str = (r > 0) ? buffer.toString(encoding, 0, r) : '';
  return [str, r];
};
