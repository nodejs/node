...
fs.Stats.prototype._checkModeProperty = function _checkModeProperty(property) {
  return ((this.mode & constants.S_IFMT) === property);
};

fs.Stats.prototype.isDirectory = function isDirectory() {
  return this._checkModeProperty(constants.S_IFDIR);
};

fs.Stats.prototype.isFile = function isFile() {
  return this._checkModeProperty(constants.S_IFREG);
};

fs.Stats.prototype.isBlockDevice = function isBlockDevice() {
  return this._checkModeProperty(constants.S_IFBLK);
};

fs.Stats.prototype.isCharacterDevice = function isCharacterDevice() {
  return this._checkModeProperty(constants.S_IFCHR);
};

fs.Stats.prototype.isSymbolicLink = function isSymbolicLink() {
  return this._checkModeProperty(constants.S_IFLNK);
};

fs.Stats.prototype.isFIFO = function isFIFO() {
  return this._checkModeProperty(constants.S_IFIFO);
};

fs.Stats.prototype.isSocket = function isSocket() {
...
ReadFileContext.prototype.read = function read() {
  var buffer;
  var offset;
  var length;
...
ReadFileContext.prototype.close = function close(err) {
  var req = new FSReqWrap();
...
fs.open = function open(path, flags, mode, callback_) {
...
fs.symlink = function symlink(target, path, type_, callback_) {
...
fs.appendFile = function appendFile(path, data, options, callback) {
  callback = maybeCallback(arguments[arguments.length - 1]);
...
FSWatcher.prototype.start = function start(filename,
                                     persistent,
                                     recursive,
                                     encoding) {
  nullCheck(filename);
...
FSWatcher.prototype.close = function close() {
  this._handle.close();
};
...
StatWatcher.prototype.start = function start(filename, persistent, interval) {
  nullCheck(filename);
  this._handle.start(pathModule._makeLong(filename), persistent, interval);
};


StatWatcher.prototype.stop = function stop() {
  this._handle.stop();
};
...
ReadStream.prototype.destroy = function destroy() {
  if (this.destroyed)
    return;
  this.destroyed = true;
  this.close();
...
WriteStream.prototype._writev = function _writev(data, cb) {
  if (typeof this.fd !== 'number')
    return this.once('open', function() {
