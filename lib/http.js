...
exports.createServerListener = function createServerListener(listener,port,statusCallback)
{
  if(port)
    return new Server(listener).listen(port);
  else if(port && statusCallback)
    return new Server(listener).listen(port,statusCallback);
  else
