...
# fusion opportunities.
    ...
    xor     x13,x13,x30 # XOR in key

    xor     x15,x15,x16
    # T2 = [T3 T15 T11 T7 T2 T14 T10 T6]
    xor     x15,x15,x12 # XOR in key
...
#================================FINAL ROUND====================================
    ...
    xor     x17,x17,x30 # xor in key

    xor     x13,x13,x25
    # T0 = [T3 T2 T1 T0   T15 T14 T13 T12]
    xor     x13,x13,x12 # xor in key
...
AES_set_encrypt_key:
    ...
    sd      x19,8(sp)
    sd      x20,0(sp)
    beqz    x10,1f    # if (!userKey || !key) return -1;
    beqz    x12,1f
    j       2f
1:
    li      a0,-1
...
2:
    ...
    bne     x11,x28,1f
    li      x13,10          # key->rounds = 10 if bits == 128
    j       3f
1:
    bne     x11,x29,2f
    li      x13,12          # key->rounds = 12 if bits == 192
    j       3f
2:
    li      x13,14          # key->rounds = 14 if bits == 256
    beq     x11,x30,3f
...
1:
    ...
    xor     x16,x16,x17
    xor     x14,x14,x16
    add     x15,x9,x19 # rcon[i] (i increments by 4 so it can double as
                                # a word offset)
    lwu     x15,0(x15)
...
2:
    ...
    xor     x28,x28,x29
    xor     x16,x16,x28
    add     x17,x9,x19 # rcon[i] (i increments by 4 so it can double as
                                # a word offset)
    lwu     x17,0(x17)
...
3:
    ...
    xor     x30,x30,x31
    xor     x28,x28,x30
    add     x29,x9,x19 # rcon[i] (i increments by 4 so it can double as
                                # a word offset)
    lwu     x29,0(x29)
