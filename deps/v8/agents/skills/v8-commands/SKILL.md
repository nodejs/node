...
Use this skill to find the correct commands for common development tasks in V8.
This unifies the commands listed in workspace configuration and ensures
consistent usage.
...
- **Sync DEPS Options:** Use `tools/dev/gm.py quiet x64.optdebug.d8 --sync` to
  sync DEPS when switching between branches. Use the slower `--sync=force`
  option if there are still issues with broken DEPS.

*Note: Always pass the `quiet` keyword unless told otherwise to avoid excessive
output.*
...
- `--allow-natives-syntax`: Enables calling of internal V8 functions (e.g.,
  `%OptimizeFunctionOnNextCall(f)`) from JavaScript for testing purposes.
- `--gdbjit`: Allows seeing Maglev graphs or JavaScript code in GDB backtraces.
  To debug issues involving code motion disable gdbjit (as it disables code
  motion).

A comprehensive list of all flags can be found by running
`out/x64.debug/d8 --help`.

## Testing

For detailed instructions on running tests and interpreting failures, see the
dedicated test execution guidelines.
