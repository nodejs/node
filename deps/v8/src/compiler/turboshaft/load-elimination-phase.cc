...
void LoadEliminationPhase::Run(PipelineData* data, Zone* temp_zone) {
  UnparkedScopeIfNeeded unparked_scope(
      data->broker(), v8_flags.turboshaft_trace_load_elimination);

  turboshaft::CopyingPhase<LoopStackCheckElisionReducer,
                           LateLoadEliminationReducer,
                           MachineOptimizationReducer, BranchEliminationReducer,
