...
class ZoneWithName final {
 public:
  ...
  ZoneWithName(Zone* non_owned_zone)  // NOLINT(runtime/explicit)
      : scope_(nullptr, non_owned_zone->name()),
        non_owned_zone_(non_owned_zone) {}

  ZoneWithName(const ZoneWithName&) = delete;
  ZoneWithName(ZoneWithName&& other) V8_NOEXCEPT
      : scope_(std::move(other.scope_)),
        non_owned_zone_(nullptr) {
 ...
 private:
  ...
  // TODO(nicohartmann): Ideally we would want to remove this once all zone
