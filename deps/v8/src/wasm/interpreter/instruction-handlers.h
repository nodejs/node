...
#define GENERATE_MEM_MEM_INSTR_HANDLER(V, name) \
  V(name##_Idx32_Idx64)                         \
  V(name##_Idx64_Idx32)

#define GENERATE_MULTIMEM64_INSTR_HANDLER(V, name) \
  V(name##_MultiMem64)                             \
  V(name##_MultiMem)                               \
  V(name##_Mem64)

// Super-instructions only support memory0, so they only need _Mem64 variants.
#define GENERATE_MEM64_ONLY_INSTR_HANDLER(V, name) V(name##_Mem64)

// Regular load/store handlers that need full MultiMem support.
#define FOREACH_LOAD_STORE_REGULAR_INSTR_HANDLER(V, ...) \
  /* LoadMem */                                          \
...
// Super-instructions: only supported for memory0 (no MultiMem variants).
#define FOREACH_LOAD_STORE_SUPER_INSTR_HANDLER(V, ...)     \
  /* LoadMem_LocalSet */                                   \
...
// All load/store handlers (regular + super-instructions).
#define FOREACH_LOAD_STORE_INSTR_HANDLER(V, ...)                         \
  FOREACH_LOAD_STORE_REGULAR_INSTR_HANDLER(V __VA_OPT__(, ) __VA_ARGS__) \
  FOREACH_LOAD_STORE_SUPER_INSTR_HANDLER(V __VA_OPT__(, ) __VA_ARGS__)

// Multi-memory variants: full for regular, Mem64-only for super-instructions.
#define FOREACH_MULTIMEM64_LOAD_STORE_INSTR_HANDLER(V)                        \
  FOREACH_LOAD_STORE_REGULAR_INSTR_HANDLER(GENERATE_MULTIMEM64_INSTR_HANDLER, \
                                           V)                                 \
  FOREACH_LOAD_STORE_SUPER_INSTR_HANDLER(GENERATE_MEM64_ONLY_INSTR_HANDLER, V)

#define FOREACH_MULTIMEM64_ATOMIC_INSTR_HANDLER(V) \
  FOREACH_ATOMIC_INSTR_HANDLER(GENERATE_MULTIMEM64_INSTR_HANDLER, V)

#define FOREACH_ATOMIC_INSTR_HANDLER(V, ...)                    \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_AtomicNotify)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicWait)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicWait)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAdd)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAdd8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAdd16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicSub)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicSub8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicSub16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAnd)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAnd8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicAnd16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicOr)                 \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicOr8U)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicOr16U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicXor)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicXor8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicXor16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicExchange)           \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicExchange8U)         \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicExchange16U)        \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAdd)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAdd8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAdd16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAdd32U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicSub)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicSub8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicSub16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicSub32U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAnd)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAnd8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAnd16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicAnd32U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicOr)                 \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicOr8U)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicOr16U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicOr32U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicXor)                \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicXor8U)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicXor16U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicXor32U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicExchange)           \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicExchange8U)         \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicExchange16U)        \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicExchange32U)        \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicCompareExchange)    \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicCompareExchange8U)  \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicCompareExchange16U) \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicCompareExchange)    \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicCompareExchange8U)  \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicCompareExchange16U) \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicCompareExchange32U) \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicLoad)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicLoad8U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicLoad16U)            \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicLoad)               \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicLoad8U)             \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicLoad16U)            \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicLoad32U)            \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicStore)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicStore8U)            \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I32AtomicStore16U)           \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicStore)              \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicStore8U)            \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicStore16U)           \
  V(__VA_ARGS__ __VA_OPT__(, ) s2s_I64AtomicStore32U)
...
#define FOREACH_NO_BOUNDSCHECK_INSTR_HANDLER(V)  \
  ...
  V(s2s_DataDrop)                                \
  V(s2s_MemoryCopy)                              \
  V(s2s_MemoryCopy_Mem64)                        \
  V(s2s_MemoryCopy_Idx64_Idx32)                  \
  V(s2s_MemoryCopy_Idx32_Idx64)                  \
  V(s2s_MemoryFill)                              \
  V(s2s_Memory64Fill)                            \
  V(s2s_TableGet)                                \
  V(s2s_Table64Get)                              \
  ...
  V(s2s_SimdS128LoadMem_Mem64)                   \
  V(s2s_SimdS128LoadMem_MultiMem)                \
  V(s2s_SimdS128LoadMem_MultiMem64)              \
  V(s2s_SimdS128StoreMem)                        \
  V(s2s_SimdS128StoreMem_Mem64)                  \
  V(s2s_SimdS128StoreMem_MultiMem)               \
  V(s2s_SimdS128StoreMem_MultiMem64)             \
  V(s2s_SimdI64x2Shl)                            \
  V(s2s_SimdI64x2ShrS)                           \
  V(s2s_SimdI64x2ShrU)                           \
  V(s2s_SimdI32x4Shl)                            \
  ...
  V(s2s_SimdS128Load8Splat_Mem64)                \
  V(s2s_SimdS128Load8Splat_MultiMem)             \
  V(s2s_SimdS128Load8Splat_MultiMem64)           \
  V(s2s_SimdS128Load16Splat)                     \
  V(s2s_SimdS128Load16Splat_Mem64)               \
  V(s2s_SimdS128Load16Splat_MultiMem)            \
  V(s2s_SimdS128Load16Splat_MultiMem64)          \
  V(s2s_SimdS128Load32Splat)                     \
  V(s2s_SimdS128Load32Splat_Mem64)               \
  V(s2s_SimdS128Load32Splat_MultiMem)            \
  V(s2s_SimdS128Load32Splat_MultiMem64)          \
  V(s2s_SimdS128Load64Splat)                     \
  V(s2s_SimdS128Load64Splat_Mem64)               \
  V(s2s_SimdS128Load64Splat_MultiMem)            \
  V(s2s_SimdS128Load64Splat_MultiMem64)          \
  V(s2s_SimdS128Load8x8S)                        \
  V(s2s_SimdS128Load8x8S_Mem64)                  \
  V(s2s_SimdS128Load8x8S_MultiMem)               \
  V(s2s_SimdS128Load8x8S_MultiMem64)             \
  V(s2s_SimdS128Load8x8U)                        \
  V(s2s_SimdS128Load8x8U_Mem64)                  \
  V(s2s_SimdS128Load8x8U_MultiMem)               \
  V(s2s_SimdS128Load8x8U_MultiMem64)             \
  V(s2s_SimdS128Load16x4S)                       \
  V(s2s_SimdS128Load16x4S_Mem64)                 \
  V(s2s_SimdS128Load16x4S_MultiMem)              \
  V(s2s_SimdS128Load16x4S_MultiMem64)            \
  V(s2s_SimdS128Load16x4U)                       \
  V(s2s_SimdS128Load16x4U_Mem64)                 \
  V(s2s_SimdS128Load16x4U_MultiMem)              \
  V(s2s_SimdS128Load16x4U_MultiMem64)            \
  V(s2s_SimdS128Load32x2S)                       \
  V(s2s_SimdS128Load32x2S_Mem64)                 \
  V(s2s_SimdS128Load32x2S_MultiMem)              \
  V(s2s_SimdS128Load32x2S_MultiMem64)            \
  V(s2s_SimdS128Load32x2U)                       \
  V(s2s_SimdS128Load32x2U_Mem64)                 \
  V(s2s_SimdS128Load32x2U_MultiMem)              \
  V(s2s_SimdS128Load32x2U_MultiMem64)            \
  V(s2s_SimdS128Load32Zero)                      \
  V(s2s_SimdS128Load32Zero_Mem64)                \
  V(s2s_SimdS128Load32Zero_MultiMem)             \
  V(s2s_SimdS128Load32Zero_MultiMem64)           \
  V(s2s_SimdS128Load64Zero)                      \
  V(s2s_SimdS128Load64Zero_Mem64)                \
  V(s2s_SimdS128Load64Zero_MultiMem)             \
  V(s2s_SimdS128Load64Zero_MultiMem64)           \
  V(s2s_SimdS128Load8Lane)                       \
  V(s2s_SimdS128Load8Lane_Mem64)                 \
  V(s2s_SimdS128Load8Lane_MultiMem)              \
  V(s2s_SimdS128Load8Lane_MultiMem64)            \
  V(s2s_SimdS128Load16Lane)                      \
  V(s2s_SimdS128Load16Lane_Mem64)                \
  V(s2s_SimdS128Load16Lane_MultiMem)             \
  V(s2s_SimdS128Load16Lane_MultiMem64)           \
  V(s2s_SimdS128Load32Lane)                      \
  V(s2s_SimdS128Load32Lane_Mem64)                \
  V(s2s_SimdS128Load32Lane_MultiMem)             \
  V(s2s_SimdS128Load32Lane_MultiMem64)           \
  V(s2s_SimdS128Load64Lane)                      \
  V(s2s_SimdS128Load64Lane_Mem64)                \
  V(s2s_SimdS128Load64Lane_MultiMem)             \
  V(s2s_SimdS128Load64Lane_MultiMem64)           \
  V(s2s_SimdS128Store8Lane)                      \
  V(s2s_SimdS128Store8Lane_Mem64)                \
  V(s2s_SimdS128Store8Lane_MultiMem)             \
  V(s2s_SimdS128Store8Lane_MultiMem64)           \
  V(s2s_SimdS128Store16Lane)                     \
  V(s2s_SimdS128Store16Lane_Mem64)               \
  V(s2s_SimdS128Store16Lane_MultiMem)            \
  V(s2s_SimdS128Store16Lane_MultiMem64)          \
  V(s2s_SimdS128Store32Lane)                     \
  V(s2s_SimdS128Store32Lane_Mem64)               \
  V(s2s_SimdS128Store32Lane_MultiMem)            \
  V(s2s_SimdS128Store32Lane_MultiMem64)          \
  V(s2s_SimdS128Store64Lane)                     \
  V(s2s_SimdS128Store64Lane_Mem64)               \
  V(s2s_SimdS128Store64Lane_MultiMem)            \
  V(s2s_SimdS128Store64Lane_MultiMem64)          \
  ...
  V(s2s_RefTestFails)                            \
  V(s2s_RefIsNonNull)                            \
  FOREACH_MULTIMEM64_LOAD_STORE_INSTR_HANDLER(V) \
  FOREACH_MULTIMEM64_ATOMIC_INSTR_HANDLER(V)     \
  FOREACH_ATOMIC_INSTR_HANDLER(V)
