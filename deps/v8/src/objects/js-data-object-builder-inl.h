...
class FoldedMutableHeapNumberAllocator {
 public:
  ...
  ~FoldedMutableHeapNumberAllocator() {
    ...
    // We should have calculated the exact correct number of mutable heap
    // numbers for the double fields we've seen, however nested value
    // allocations could have transitioned those double field to tagged.
    Address raw_bytes_end = reinterpret_cast<Address>(raw_bytes_->end());
    if (mutable_double_address_ < raw_bytes_end) {
      isolate_->heap()->CreateFillerObjectAt(
          mutable_double_address_,
          base::checked_cast<int>(raw_bytes_end - mutable_double_address_));
    } else {
      DCHECK_EQ(mutable_double_address_, raw_bytes_end);
    }
...
Handle<JSObject> JSDataObjectBuilder::BuildFromIterator(
    PropertyIterator&& it, MaybeDirectHandle<FixedArrayBase> maybe_elements) {
...
Handle<JSObject> JSDataObjectBuilder::BuildFromIterator(
  ...
  for (; !it.Done(); it.Advance()) {
    if (!TryAddFastPropertyForValue(it.GetKey(), it.GetValue(true))) {
      break;
    }
  }
  ...
  if (!Object::FitsRepresentation(*value, expected_representation)) {
    if (IsUninitializedHole(*value)) {
      if (expected_representation.IsDouble()) {
        // Value is uninitialized, so we'll unconditionally need a new box for
        // double fields.
...
bool JSDataObjectBuilder::IsOnExpectedFinalMapFastPath() const {
