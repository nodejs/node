...
// Note on the --no-turbo-loop-peeling flag: it just makes the repro a bit nicer
// to write. It can be replaced by a large number of pure operations in the loop
// that depends of {i} (which will make the loop be too large to be peeled).
...
let glob = { x : 42, offset_16 : "skipping offset 16", y : "abc", z : "aze" };
...
function foo(x, ...rest) {
  ...
  // We can use %MajorGCForCompilerTesting(), but here we're trying
  // to prove that the bug can happen organically (although it's hard to force
  // a GC to happen in this loop exactly, it could theoretically happen given
  // the right setup).
  for (let i = 0; i < 42; i++) {
    // Storing a Float64 to a AnyTagged field, which will require a HeapNumber
    // allocation.
    glob.z = i + 0.35;

    // We need to replicate the store from after the loop here so that
    // StoreStoreElimination correctly sees that `arr[2]` is Unobservable before
    // the loop.

    // This is a bit limiting: it means that to trigger the bug we need the
    // tagging of `i + 0.35` above to trigger a GC in the first iteration of
    // this loop. Still, might be doable.
    arr[2] = 42;
  }
