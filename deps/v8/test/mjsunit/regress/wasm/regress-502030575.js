...
// header offset and the extra offset for the 2nd i32 load/store we end up with
// a value that exactly equals INT32_MIN (which is the singular value that
// returns false for LoadOp::OffsetIsValid()).
// Note: This logic has been replaced and is now handled in the
// LoadStoreSimplificationReducer exclusively.
...
builder.addFunction("testStore", makeSig([kWasmI32], [kWasmI64]))
  .addLocals(wasmRefNullType(array_type), 1)
  .addBody([
    // Create an array with a single value
    ...wasmI32Const(1),
    kGCPrefix, kExprArrayNewDefault, array_type,
    kExprLocalTee, 1,
    // Write a value at offset 0 by having the index calculation
    // (BIG_CONST - BIG_CONST). The i32.const in the code will be folded into
...
let storeResult = instance.exports.testStore(-BIG_CONST);
assertEquals(testValue, storeResult);
